...
| 95 | ~~**`validate Heartbeat` is 12.3% slower on `main` than it was before phase-2 PR A, and three more session-dictionary cases moved with it.**~~ — **CLOSED 2026-09-23, boot E S1, as a bisect at merge granularity: the whole step is inside PR B.** `target/boot-e-evidence/s1m/summary.txt`, n = 21 per arm, CONTROL `ms`: `validate Heartbeat` `wa` 6fbe851 **166.8** → `b1` e673e8f (PR B merge) **190.8** → `b2` 188.1 → `b3` 188.0 → `w1` 189.0 → `m` 188.7; `validate NewOrderSingle` 950.3 → 1012.9 → 1008.1 → 1010.1 → 1009.2 → 1010.0; `validate TestRequest, w2w bytes` 220.6 → 239.5 → … → 242.5. One step of +14 % / +6.6 % / +8.6 % between `wa` and `b1`, then flat within `max/median` ≤ 1.03. **The commit inside PR B is not named** (PR B is one merge of FIXT 1.1 / FIX 5.0 SP2 dictionary work); the baselines now carry the new level with the ledger, and whether PR B's step is bought back is a plan of its own, not this item. The original finding: **`validate Heartbeat` is 12.3% slower …** `[measured 2026-09-22, boot D step D4]` `wa` `6fbe851` → `w1` `76e53cb`, n = 20 each, same features, one rotation against a shared control on the §9 desk (`pass 16 fail 0 unknown 0`): `validate Heartbeat` 167.2 → 187.8 ns (**+12.32%, outside the 1.10 band**), `validate TestRequest, w2w bytes` +9.39%, `validate NewOrderSingle` +5.75%, `validate NewOrderSingle, w2w bytes` +4.97%. **The engine turn did not move**: −0.6% at 1 busy session and +0.3…+0.9% at 2–64, so this is the session's dictionary pass and not the turn. The commits in that span are PRs B, C, #85 and #86 — none of which promised this band beforehand, which is why the plan's rule opens an item rather than stopping the boot. **No bisect has been run**, and no cause is claimed; ADR-0086's descent is the obvious suspect and is *not* evidence (item 96 is why). **Seen against the recorded baselines rather than against `wa`, this is worse than +12.32% looks** — `validate NewOrderSingle` is only +5.75% on the `wa`→`w1` delta but **×1.160 over its own line** (item 97), so the span merely finished a breach that was already building. [measured-costs.md](docs/reference/measured-costs.md) *Boot D … The band across PRs B, C, #85 and #86*. | ADR-0031, ADR-0085, ADR-0086 |
...
| 99 | **`journal put, 191 bytes, one slot` read 7.4 ns in one `--strict` run and 12.4 ns forty minutes later in the same boot, same pinned binary, and stayed at 12.4 on four re-runs.** `[measured 2026-09-23, boot E]` `target/boot-e-evidence/s3-strict.txt` (21:07 UTC) 7.4, in band; `s4-strict.txt` (21:44 UTC) 12.4 `OVER BASELINE` (band [6.7, 8.1]); then three bare runs of the `main-tree` `journal` binary from `../fb-s9e/MANIFEST.txt` and one under `taskset -c 6`, all 12.4. The two `walking` cases did **not** move (8.8 / 5.3 ns), `check-machine.sh` still `pass 17 fail 0 unknown 0`, THP `never`. Between the two runs the desk ran S2 (`perf record -g` ×5 of the `validate` bench) and S1c (24 rounds of `sbe`). The case is pure L1 (one slot, eight lines) so the suspects are CPU-side state a `perf record` could leave (PMU / NMI-watchdog counter re-armed), not the code. **The baseline line is not moved.** First check next boot: run the journal binary **before** any `perf record`, then after one, and read the case both times. | ADR-0031; `bench.sh --strict` on the desk |
