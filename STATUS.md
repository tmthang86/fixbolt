...
* **`the_sixty_fix50sp2_definitions_pass_through_a_real_socket` is load-dependent, and it is
  pre-existing.** `[measured 2026-09-20]` 0 red in 40 sequential runs on this branch; **11 red in
  50** run as ten concurrent copies; **8 red in 50** the same way on `main` at `64ea6c2`, which
  carries none of this branch's changes. Under contention a timer fires and the engine emits a
  `35=5` the definition never asked for, shifting a positional comparator by one message. The
  engine is not misbehaving and the branch did not cause it; the oracle cannot say "this message
