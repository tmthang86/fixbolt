...
| 35 | **An initiator that loses its connection is on its own.** `[2026-09-02]` criterion 4 is met and it bought **one scenario against one counterparty**: logon, application messages in, an unprompted heartbeat, a `TestRequest`, a `ResendRequest`, a gap this end opens and gap-fills, a logout. **Reconnect, backoff and a session schedule for an initiator are covered by nothing** — not by the 59 acceptance definitions, which are written for an acceptor and never reconnect an initiator; not by the mirrored corpus, which is at 0 / 50; and not by `scripts/interop.sh`, which connects once. ADR-0004 recorded this as debt before the initiator existed and it is still debt. It is also **the engine's work, not the session's**: `fixbolt_engine::connect` opens a socket and nothing decides when to open it again. **Not started, and not blocked on anything** — it needs a plan | An initiator a deployment can leave running overnight |
| 36 | **The mirrored corpus reads 2 / 50 against a ceiling of 45.** `[measured 2026-09-02]` **it moved off zero and, more to the point, it can now fall** — it had asserted `passed == 0` since 2026-08-30, and while it stood there an initiator that answered a `Logon` with a `Logon` went through it. Two things moved it and only one was in the session: `Input::Originate(Intent)` lets the harness play the operator for the 46 files that need a message no state machine can invent — 141 drives, and it was 179 until the harness learned not to speak over a session that had already answered, **at the same score**, which is the argument for asserting the count next to the number — and **the loader was feeding messages no session would accept** — an `E` line carries `52=00000000-00:00:00.000` because the comparator matches that tag by shape, and mirrored it is an *input*, so the session refused the counterparty's Logon and dropped ([expected-output-is-not-valid-input](docs/reference/expected-output-is-not-valid-input.md), `[to testing-skills]`). What identified it was the harness's own drive counter reading `0×1` across fifty files, not the 196 identical failures. **What remains, and it needs its own plan**: the next failures are real protocol disagreements and some need session API that does not exist — a `123=Y` `SequenceReset` an operator orders, for one, where `send_sequence_reset` writes `123=N`. `CLAUDE.md` §1 says stop rather than diverge, so it stopped. The ceiling is **45, not 50**: `1c_InvalidSenderCompID`, `1c_InvalidTargetCompID`, `1d_InvalidLogonBadSendingTime`, `1d_InvalidLogonLengthInvalid` and `1e_NotLogonMessage` ask this end to send a message a correct engine cannot, and recording that needs an ADR. **This gate is secondary and will stay so** — it cannot check its own reading, and `scripts/interop.sh` is what stands in front of it | The mirrored gate at 45 / 50, and the ADR that fixes the ceiling |
| 37 | **`shard::the_same_identity_always_lands_on_the_same_shard` is red on a shared cloud VM and green on `ubuntu-latest`.** `[measured 2026-09-02, Intel Xeon @ 2.80GHz, 4 vCPU shared VM, Linux 6.18.44]` it fails 3 runs of 3, deterministically, at `set.settled().expect("the Logon arrived over loopback")` after its **5-second** deadline — a Logon written to a loopback socket that never settles. **Not caused by anything in the branch that found it**: it fails identically on `origin/main` in a clean worktree on the same box, and the `affinity` CI step is green on `ubuntu-latest` in runs [`33623429649`](https://github.com/tmthang86/fixbolt/actions/runs/33623429649) and [`33624018140`](https://github.com/tmthang86/fixbolt/actions/runs/33624018140). Every other `--features affinity` test passes here, `shard_wire` included at **59 / 59 through one shard and through two**. So it is one test, in one environment, and **it has not been diagnosed** — 5 seconds is a very long time for loopback, so *"the VM is slow"* is a guess and not a finding, and this row says so rather than filing it as a flake. `CLAUDE.md` §10: a cause accepted because a knob moved with it. **Whoever picks this up: reproduce it first, instrument `PendingSet::turn`, and do not raise the deadline** | A gate that means the same thing on both machines, or a defect named |
