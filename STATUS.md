...
Last updated: **2026-09-05** — **the largest candidate for the missing 3.6 µs was measured, and it is a sixth of it.**

## Start here — 2026-09-05, latest: the dictionary pass, timed, and it explains 17%
...
| Plan | Closes |
|---|---|
| **[the-baselines-and-the-pass-nobody-timed](docs/plans/2026-09-02-the-baselines-and-the-pass-nobody-timed.md)** | 41 → 39 → 34 — **approved 2026-09-02, in progress**, and the one plan of this repository that **requires the §9 desktop**: `benches/baselines.tsv` is keyed on the CPU model, so no other machine may speak. **Step 1 (item 41) closed 2026-09-05** — ADR-0049, `scripts/check-bench-alignment.sh`, 24 baselines re-recorded. **Step 2 (item 39) closed the same day** — ADR-0050, `crates/session/benches/validate.rs`, four baselines at n = 21, §8 gains a dictionary-pass row and the 3 898 ns is added back with **~2 804 ns left over as new item 49**; the bench also found item 50 on its first run. **Only step 3 (item 34) is open**, and 3a is already paid for — the three `fixbolt/cost` figures were recorded on 2026-09-05. **Revised four times, every revision written before the code moved**: Sửa 3 (two of the plan's three options for step 2 do not exist) and Sửa 4 (the `parse.rs` fixture) are today's. It was missing from this table entirely until 2026-09-05, which is how a plan *đang làm* stayed invisible to the row above it |
...
| 48 | **A deployment cannot recover its own outbound sequence number, and the naive way looks right.** `[đo 2026-09-05]` every example in this repository derives `Resumed::next_out` as `journal.highest() + 1` — `crates/engine/tests/on_disk.rs:288` does, and so did `tools/interop` until a real `libquickfix` refused the resumed session in English: `MsgSeqNum too low, expecting 4 but received 3`. **`Journal::put` records application messages only** (`crates/session/src/lib.rs:1871`, `:2481`) — the journal is the resend store, and administrative messages are gap-filled, never replayed — so `highest() + 1` is short by exactly the administrative messages sent after the last application one. **A clean logout is enough**: answering the venue's `35=5` spends a number the journal never hears about. The interop gate pins the shortfall at exactly one so the explanation is testable, not just the failure. **And the right number is unreachable**: it lives in the live session, `Observer` can read it, and `connect_and_serve` hands out no `Observer` — so this item cannot be closed before item 47. `MessageLog` does hold both directions and cannot help either: one writer thread, flushed on close, owned by the engine for the whole run. **`Resumed::next_out`'s rustdoc already said *"usually"*** and that was not enough — the only worked example in the tree contradicted it, and `GUIDE.md` §8c warned about the louder mistake next door. Now a fifth point in §8c, [a-journal-holds-messages-not-numbering](docs/reference/a-journal-holds-messages-not-numbering.md), and `scripts/interop.sh`'s `known_gap`, which goes red the day this is fixed | an initiator that can be left running across a clean logout |
| 49 | **~2 804 ns of the application round trip is unattributed, and this is the first time the remainder has a number.** `[measured 2026-09-05]` `--path app` costs **3 898 ns** more than `--path admin` at p50 on the §9 desktop. Committed benchmarks now account for **~1 094 ns, 28%**: inbound parse ~+60, **the dictionary pass +679** (item 39), dispatch +9, the application's own `Validation::NONE` re-parse +114, its template encode +233. Not subtracted, because it has no committed case: the session's own `Heartbeat` serialise, which the application path does not do. **This row exists instead of a guess.** The candidates, none measured and none claimed: two kernel copies of a larger payload each way (149 bytes in and ~200 out against 79 and ~70); the engine's framing and read-buffer management, which no bench isolates; `Journal::put` of the outbound `ExecutionReport` into the in-memory ring, which the admin path never does because a `Heartbeat` is not kept for resend; and the client's blocking `read` returning on a bigger message. **What it needs**: one of those priced, not four of them reasoned about — and item 39 is the warning, since the largest named candidate turned out to be a sixth of the answer | `DESIGN.md` §8's bottom line |
