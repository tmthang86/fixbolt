...
- **Nothing here touches the engine thread under TLS.** `scripts/check-no-kernel-sleep.sh` still
  ...
  touch `codec`, `session`, `transport.rs` or `engine`'s non-test code.
...
- ADR-0005 open questions **2** (kernel/cipher floor) and **6** (a TLS 1.3 key update under
  kTLS) are still open. Question **3** (what asserts which mode is live) is answered as of this
...
- **No §8 TLS number exists.** `DESIGN.md` §8's TLS row is still empty. Every figure produced on
