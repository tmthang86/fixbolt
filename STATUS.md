...
success). PR `<pending>`; CI run for this PR's closing commit `<pending>`. **Docs only** — no line
...
1. **kTLS was slower than userspace `rustls` in every arm, in both procedures, at p50, p99 and
   p99.9.** Over `off`, kTLS added 9.0–9.7 µs at p50; userspace added 1.8–4.7 µs (admin) and
   1.4–1.6 µs (app). Loopback, so no NIC offload exists in either arm. The engine thread's most
   frequent syscalls under kTLS were the plain arm's (`recvfrom`, `accept4`, `sendto`) plus
   `getrandom` ×9 and `setsockopt` ×4, and no sleeper. **Not a cause, and not by
   itself a reversal of ADR-0005 decision 2**, whose reasons are D8, parse-in-place and zero
   allocation — the kTLS arm still counts 0. Item **84**, for the architect.
2. **`hft`/admin/`userspace` moved 15.9% between two identical procedures** (runs 20 639 .. 20 930,
   then 17 333 .. 17 813) while its spread read 1.008; the other seven arms moved 0.2–2.5%. The
   only recorded differences are time since boot and "first procedure after the reboot" —
   correlations. **A per-procedure spread bounds one procedure, not the next**, and the spread is
   max/median, so a fast run cannot move it at all.
...
- **Anything about a NIC, another kernel, or another suite.** The plan's optional suite step
  (§6.7 item 8) did not run; question 2 closed at the plan's fallback level minus the CI
  runner's kernel, which `docs/CONFORMANCE.md` §8 records only as accepting `TCP_ULP`.
- **That any TLS figure is reproduced.** Two procedures in one boot; one arm disagreed with itself.
- **`hft` on an initiator under TLS.** `tools/w2w` is an acceptor; `connect_and_serve_tls` is
  `standard` only.
- **Item 85 has no regression test.** The procedure still publishes from one twenty-run median.
...
- [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md): its B3 (this work) is
  ...
  B2 publishes.
- Machine: plan §6.7 item 10 — `fixbolt-machine off`, and whether the three boot parameters stay,
  which is the owner's call. Whether it was done is not recorded here.
...
`CLAUDE.md` §2 records it. **`hft` under TLS is still checked by nothing**:
`connect_and_serve_tls` is `standard` only.
...
- **`hft` mode under TLS is unmeasured and unchecked.** `connect_and_serve_tls` is `standard` only.
  `[narrowed 2026-09-14]` *unmeasured* no longer holds for the acceptor: `hft` with kTLS through
  `tools/w2w` is in `DESIGN.md` §8. The initiator half stands.
...
`[2026-09-14]` **Walked for the `tls` 6-M/7b pull request (`<pending>`).** `grep` over this file for
...
- **`hft` mode under TLS is unmeasured and unchecked by anything.** `connect_and_serve_tls` is
  ...
  prove* header. `[narrowed 2026-09-14]` *unmeasured* no longer holds for the acceptor: `hft` with
  kTLS through `tools/w2w` is in `DESIGN.md` §8. The initiator half stands.
...
| [tls](docs/plans/2026-09-04-tls.md) | 45, wave B, plan 4 — `[2026-09-14]` **EVERY STEP BUILT: 6-M and 7b on branch `plan/tls-numbers-on-s9`, PR `<pending>`, measured in their own §9 boot rather than inside the-second-linux-desk's; the plan closes when that pull request merges.** First TLS figures in `DESIGN.md` §8; kTLS read slower than userspace `rustls` in every arm, and one arm moved 15.9% between two identical procedures — items **84** and **85**, no cause for either. Before that: **STEPS 1–3, 4a, 4b, 4c, 5a–5c, 6a–6c, 7a CLOSED; 6-M and 7b OPEN and both need the §9 machine.** Session A merged as PR [#66](https://github.com/tmthang86/fixbolt/pull/66), CI green on the closing commit `022ca36`, run [`34739943517`](https://github.com/tmthang86/fixbolt/actions/runs/34739943517), 14 of 14. [ADR-0063](docs/decisions/ADR-0063-a-peers-key-update-is-the-second-named-carve-out-and-a-ticket-is-not-read.md) `Accepted`: a peer's `KeyUpdate` allocates `2 × rekeyed directions` boxes of 184 bytes each and a session ticket allocates nothing, both bound to `Cargo.lock` and to `--locked` in CI. **The pull request's three most valuable findings were none of the ones it was written to produce**: `ktls-core`'s `tls13-key-update` was off, so a counterparty's `KeyUpdate` killed the session in merged code; the initiator dialled 879 times in 1.5 s because `up` was set after the first turn; and a kTLS guard written the same day was a false green whose reversal went red in the wrong place. A third machine check for non-negotiable 4 came out of it — `the_dial_loop_sleeps_rather_than_spins_while_the_handshake_waits`, because both of rule 4's scripts trace `tools/w2w` and `w2w` is an acceptor. **~~No §8 TLS number is published~~ `[struck 2026-09-14]`, `hft` under TLS is checked by nothing, and nothing here has spoken TLS to another engine.** ~~6-M and 7b run inside the §9 sitting of [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md)~~ `[2026-09-14]` they ran in a §9 boot of their own |
...
| 84 | **kTLS was slower than userspace `rustls` on loopback, in every arm and both procedures — and ADR-0005 decision 2 prefers kTLS for `hft`.** `[measured 2026-09-14]` on the §9 desktop, `pass 12 fail 0 unknown 1` (`DESIGN.md` §8 *The round trip under TLS, measured*; `docs/reference/measured-costs.md` *TLS on the wire*). p50 added over `off`, procedure 1 / 2: kTLS **+9 233 / +8 987** (`hft` admin), **+9 302 / +9 503** (`hft` app), **+9 613 / +9 689** ns (`standard` admin); userspace **+4 709 / +1 803** (admin) and **+1 352 / +1 598** ns (app). kTLS minus userspace: admin 4 524 / 7 184, app 7 950 / 7 905 ns, and positive at p99 and p99.9 as well. TLS 1.3 `TLS13_AES_128_GCM_SHA256` at **both** ends of the socket. **No cause is claimed** — nothing was isolated (`CLAUDE.md` §10). **What it does not show:** a NIC — over loopback neither arm had hardware offload to use — another kernel, another suite, or one end's cost. **What it does not change by itself:** decision 2's stated reasons are D8, parse-in-place and the hot-path guarantee, and the kTLS arm still counts `allocs 0` against userspace's 80 000 per 20 000 round trips. **The design question, for the architect:** does decision 2 still hold for `hft`; what would a NIC with TLS offload change; and does ADR-0005's *Alternatives* row, which prices a sidecar against "a sub-microsecond crypto cost", survive a measured round-trip delta of 1.4–9.7 µs with TLS at both ends. A reversed decision is a new ADR (`CLAUDE.md` §5). **Not a defect, and not a cause** | ADR-0005 decision 2 for `hft`; `docs/best-practices-hft.md` §9's recommendation rests on it |
| 85 | **An arm moved 15.9% between two identical procedures, and the per-procedure spread could not see it.** `[measured 2026-09-14]` `hft`/admin/`userspace` read **20 774** ns (runs 20 639 .. 20 930, spread 1.008) in procedure 1 and **17 473** (runs 17 333 .. 17 813, spread 1.019) in procedure 2 — same command, same boot, half an hour apart; the other seven arms moved 0.2–2.5%, all faster. Inside procedure 2, `hft`/app/`off` printed spread **1.008** with five runs at p50 17 323–19 136 against a median of 19 998: the spread is max/median (`scripts/w2w-baseline.sh:222-225`), so a fast run cannot move it. Recorded differences between the two procedures: time since boot (~7 against 38 minutes) and "first procedure after the reboot" — **correlations, not causes**. **Why it matters:** the §8 procedure publishes from **one** twenty-run median; run once, this arm's published figure would have depended on which half hour it ran in. **No guard yet** — no test and no gate; `DESIGN.md` §8 shows both procedures side by side in the meantime. Needs a plan: how a figure is reproduced before it is published, and a dispersion that reads both sides. [a-tight-spread-inside-one-procedure-did-not-reproduce-across-two.md](docs/reference/a-tight-spread-inside-one-procedure-did-not-reproduce-across-two.md) `[to testing-skills]` | how every §8 figure is published, B2 of [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md) included |
