...
- **29 — the application-message resend path is written and has never run.** `Session::replay`
  reads the journal and re-emits the real message; every message the 59 definitions send is
  administrative, so nothing has ever reached it. A **test** gap, not a feature gap — and it was
  nearly filed as the latter, which is why the item says how it was checked.
...
| 29 | **The application-message resend path is implemented and has never run.** `[verified 2026-09-01]` `Session::replay` (`crates/session/src/lib.rs:1030`) reads the journal and re-emits the real message with `43=Y` and `122=` — **the code is there**, which is why this is a test gap rather than a feature gap, and why it was nearly written up as the latter. What is missing is any exercise of it: **every message the 59 definitions send is administrative**, so the branch has never carried a business message. A real counterparty's first `ResendRequest` over real orders is what will run it, and a wrongly answered `ResendRequest` is a protocol violation visible from outside. **Cheapest item on this list and the highest-risk untested path in the repo** — it needs a test, not a decision. `[to testing-skills]` **once written up**: *a path can be fully implemented, fully reviewed, and covered by a 59/59 corpus that structurally cannot reach it* — the corpus's own scope is the blind spot, not its size | Phase-1 deployability; `PRD.md` §4 |
