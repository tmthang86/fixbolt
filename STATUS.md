...
Last updated: **2026-09-09** — **the ADR that gate stands behind is written and awaits approval. `ADR-0057` chooses where sub-millisecond time comes from, and reading the interop oracle's own source to write it found that this repository had been quoting a key spelling from the wrong engine.**
...
- **`[2026-09-08]` Nothing here has seen a real venue send a microsecond `52=`.** Half A of
  ...
  invented message; there is no capture, and the four widths are held by
  `crates/session/tests/skew.rs` and `crates/session/tests/application.rs` alone. **What is
