...
| [the-tls-tests-have-no-ci](docs/plans/2026-09-10-the-tls-tests-have-no-ci.md) | 62 — **Chờ duyệt 2026-09-10**, not started. A `tls` CI job, the kernel requirement asserted where it can fail in its own words, and step 1 is finding out what the runner actually has. **Touches no line of `crates/*/src`** |
...
| 65 | **A TLS test that is green 20 times here is red on GitHub's runner, and PR [#61](https://github.com/tmthang86/fixbolt/pull/61) is blocked on it.** `[measured 2026-09-10]` `crates/engine/tests/tls.rs::a_logon_sent_straight_after_finished_is_not_lost` failed on run [`34502185123`](https://github.com/tmthang86/fixbolt/actions/runs/34502185123): early data read `[]` where `[104, 101, 108, 108, 111]` — `b"hello"` — was wanted, under the message *"application data that arrived before the handover was lost"*. **On this desktop it is 20 passed / 0 failed out of 20.** **This is item 62 paying for itself on the first run of the job it created**: the test shipped in PR [#57](https://github.com/tmthang86/fixbolt/pull/57) on 2026-09-09 and had never run anywhere but one machine. **The environment step passed first** — `verdict: READY`, the runner is `Linux 6.17.0-1022-azure` with `CONFIG_TLS=m` — so the `tls` job's whole reason for existing worked: this is reported as an engine-or-test failure and **not** as a missing kernel module, which is what the same failure would have looked like a day earlier. **Two candidates, neither eliminated, and the shape must not decide it.** (a) *The test's assumption*: it needs the client's `hello` to arrive **interleaved with** the handshake, so `rustls` reports `ReadTraffic` and the bytes land in `Handshake::take_early_data`; on a runner where the client has not written yet when the handshake completes there is simply no early data, and the test asserts there must be. (b) *A real loss*: the bytes arrived and the engine dropped them, which is exactly what the test's name claims to guard. **(a) reads as more likely and that is not evidence** — reasoning from the shape of a feature is what produced the wrong prediction in reversal 1 of step 4a, twelve hours earlier. **The one-variable experiment that settles it**: print how many bytes reached the pre-handover buffer, on both machines. If the runner shows zero arrived, it is (a); if it shows bytes arrived and `take_early_data` returned none, it is (b) and it is a defect in `crates/engine/src/tls.rs`. **Also read from the log, and it matters for the next run**: the job reported **5 tests ran, not 11** — `cargo test` stops at the first failing test binary, so `tls_wire.rs` and `tls_mode.rs` **have still never run on any machine but this desk**. Whatever else 4a and 4b are, they are not yet CI-proven | PR #61, and `crates/engine/tests/tls.rs:236` |
