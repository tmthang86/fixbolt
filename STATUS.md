...
- **The single-logon rule's *discrimination* is invisible to the corpus, and this is the tenth
  ...
  **The corpus catches deletion of the rule; only that test catches the failure to compare.**
- **What a turn costs while somebody *is* watching has never been measured.** `[2026-09-01]`
  ...
  *nanosecond* half is not: [ADR-0032](docs/decisions/ADR-0032-observation-is-a-snapshot-taken-on-request.md)
  ...
  §9 machine and was not run**. Everything built for `observe` was built on a Mac.
...
- **An `Engine` cannot resume a session at all, so no recovery rule of any kind is reachable
  from it.** `[verified 2026-09-02]` both `Engine::add_with_journal` and
  `add_with_prefix_and_config` build `Session::new(cfg)`, which resets, and there is no other
  path: `conns` is private and no method hands a caller a built `Session`. So
  `Session::resume`, `resume_at`, ADR-0010 and ADR-0017 are all real, all tested, and all
  unreachable through the public engine API. `crates/engine/tests/recovery.rs` contains **zero
  occurrences of `Engine`** — it drives a `Session` directly.
  **This bullet replaces a false one written the same day.** It said a restart *keeps
  yesterday's numbers*; a restart in fact **resets unconditionally**, because the engine only
  ever calls `Session::new`. The claim was made from the session layer's API without checking
  which of its constructors the engine actually uses. `STATUS.md` item 31.
...
| 31 | **An `Engine` cannot resume a session, so the whole recovery mechanism is unreachable from it.** `[verified 2026-09-02]` `Engine::add_with_journal` and `Engine::add_with_prefix_and_config` both build `Session::new(cfg)` — which resets — and nothing else constructs a session. `conns` is private; no method returns a `&mut Connection`; `Connection::new` is `pub` but nothing public reaches it. **Everything needed exists and none of it is wired**: `Journal::highest`/`highest_in` read the numbers back (item 16, closed 2026-08-31), `Session::resume` carries them ([ADR-0010](docs/decisions/ADR-0010-a-reconnect-is-not-a-restart.md)), `Session::resume_at` carries them plus the instant a schedule boundary is compared against ([ADR-0033](docs/decisions/ADR-0033-a-schedule-is-utc-arithmetic-and-the-calendar-stays-outside.md)), and `Session::last_active_ms` is what would be persisted. **How it was missed:** item 16 closed truthfully — the journal *is* readable and `Session::resume` *does* work — and `crates/engine/tests/recovery.rs` proves both with **zero occurrences of `Engine` in the file**. A layer was finished and the seam above it was never asked about, by a plan whose exit criteria were all satisfiable one layer down. **Two things this makes false, both corrected 2026-09-02:** a bullet in *Not proven* claiming a restart keeps yesterday's numbers (it resets, always), and `GUIDE.md` §5a's note about `Session::resume` (the engine never calls it). **The schedule reset is only the newest thing this blocks** — the older one is that `Durability::Fsync` still buys nothing an engine can use. Needs its own plan: the registry chooses a `Config` per connection at `Logon`, so recovery has to be asked per identity, which is a design question rather than a wiring job | Phase-1 deployability of recovery AND of schedules; item 16's closure is narrower than it reads |
| 30 | **Not enough exists for running this in production.** `[2026-09-01]` **(b) and (f) are done** — module `observe`, [ADR-0032](docs/decisions/ADR-0032-observation-is-a-snapshot-taken-on-request.md), `GUIDE.md` §8a. `Engine::observer()` hands out a `Send + Sync` handle; `Observer::request()` gets a `Snapshot` carrying, per session, logged-on state, both sequence numbers, whether output is backed up and **the measured clock skew** — recorded whether the message was accepted or refused, because a `max_skew_ms` refusal is silent by protocol. `Snapshot::healthy()` is the probe, a pure function on the same data. On request only: the cost while nobody watches is one relaxed load per turn, and `benches/alloc.rs` cases `observe-idle` and `observe-asked` both read 0. **The four that remain:** **(a) ordered shutdown** — `Logout`, flush the journal, wait for the ack or time out; dropping the engine with a live `WakeHandle` was a `SIGPIPE` kill until 2026-08-30, so the shutdown path is under-designed rather than merely absent. **(c) sequence-number administration on a live engine** — `Session::resume` is the mechanism and it is a *constructor*; the 3 a.m. operation every FIX operator performs has no path. **(d) a structured event stream** — logon, logout, gap detected, resend issued, reject sent, disconnect **with its reason**; `tracing` behind a feature flag is not an audit trail and D8 forbids logging on the hot path anyway. **(e) an offline journal reader** — the store is mmap and nothing outside the process can read it, so *"we never received order X"* cannot be answered. The shared constraint — readable from another thread without touching the hot path — **is now solved once**, in `observe`, and (a), (c) and (d) reuse that mechanism rather than inventing a second. `PRD.md` open decision 9 is answered by [ADR-0027](docs/decisions/ADR-0027-the-engine-owes-a-byte-stream-not-an-archive.md). **Two things this leaves unproven:** the nanosecond cost of a turn that *does* publish (needs the §9 machine — [plan](docs/plans/2026-09-01-operability.md) reversal 2), and ring depth and pending-set occupancy, which (b) asked for and the snapshot does not yet carry | Phase-1 deployability; steps 3–6 of [docs/plans/2026-09-01-operability.md](docs/plans/2026-09-01-operability.md) |
