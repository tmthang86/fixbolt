...
| 89 | **The `hft` engine thread spends 49.9% (admin) / 36.7% (app) of its samples asking an empty listener for a connection.** `[measured 2026-09-15]` boot B step B9, **diagnostic**: `perf record -F 4999 -g -t <engine-tid>` for 4 s of a 400 000-round-trip loopback `hft` run. `fixbolt_engine::Acceptor::accept` holds **49.9%** of engine-thread samples on the administrative path and **36.7%** on the application path; `__x64_sys_accept4` 27.7% / 18.3%, and under `do_accept` the kernel allocates a socket file and inode (`sock_alloc_file` 10.9% / 7.5%, `inode_init_always_gfp`, then `__fput`, `evict`) before answering `EAGAIN`. The thread spins, so these are shares of the spin loop, **not** a per-message cost: what a listener poll adds to the time between a request landing and the engine reading it is not measured. Non-negotiable 4's `hft` gate already lists `accept4` as allowed. For the architect: whether the listener should be polled less often than the sessions, and what that measures as. `docs/reference/measured-costs.md` *Boot B* | `DESIGN.md` §8's floor; D8 |
