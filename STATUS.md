...
| 49 | **~2 770 ns of the application round trip is unattributed, two of the four candidates are dead with numbers, and the survivor holds almost all of it.** `[measured 2026-09-15]` **D_in is measured: 765.5 ns** (difference of the medians, n = 20; paired per-run median 767.7, 734.3 .. 781.3), boot B step B1, `pass 15 fail 0 unknown 0`, HEAD `f43d7e8` — `DESIGN.md` §8 *The 3 898 ns, added back*. Set beside the 2026-09-05 figure, **~3 130 ns of the 3 898 is outside one engine turn**, so the engine's framing and read-buffer management can hold at most part of 765.5 ns; and the separately measured in-process rows add to ~1 104 ns, more than D_in, so they are not independent terms. **Not a cause.** Boot B's B2 re-measured both paths on today's code: `hft` app − admin p50 reads **4 198 ‖ 3 802 ns** (procedures 1 ‖ 2, from arms that did not reproduce), beside the 3 898 ns above. `[2026-09-14, later]` **The bench that splits it exists and has no baseline.** `engine turn, 1 busy, admin` in `crates/engine/benches/density.rs` (`ed96d9a`, PR [#72](https://github.com/tmthang86/fixbolt/pull/72)) — a `TestRequest` in, a `Heartbeat` out, in the same frame as the N = 1 application case — prints `NO BASELINE for AMD Ryzen 7 3700X`. **The in-process difference D_in is not measured**; boot B step B1 of [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md) records it at n = 20 and sets it beside the 3 898 ns. `[measured 2026-09-05]` `--path app` costs **3 898 ns** more than `--path admin` at p50 on the §9 desktop. Committed benchmarks now account for **~1 128 ns, 28.9%**: inbound parse ~+60, **the dictionary pass +679** (item 39), dispatch +9, the application's own `Validation::NONE` re-parse +114, its template encode +233, **the kernel copying a bigger payload +24.5**, **`Journal::put` of the reply +8.9**. **Retired:** the payload term is **0.9%** of what it was nominated for — read off an 8 → 8192 byte lever, because the two real sizes differ by tens of ns inside 12 600 and their direct difference changed sign across three repetitions (−4, +13, +46). `Journal::put` is **8.9 ns**, confirmed in situ by sweeping the ring 4 KiB → 2 MiB for a 1.5% non-monotone spread. **Still open, and now holding almost the whole remainder:** the engine's framing and read-buffer management, which no bench isolates. Also unsubtracted in either direction: the session's own `Heartbeat` serialise, which has no committed case. **Twice now the largest named candidate has not been the answer** — the dictionary pass is 17.4% of the gap and the payload term 0.9%. [ADR-0052](docs/decisions/ADR-0052-two-candidates-are-retired-with-numbers-and-a-baseline-disagrees-with-its-neighbours.md). Case 4.1 + C-49 close it; the remainder is published as an in-situ kernel number. | `DESIGN.md` §8's bottom line |
...
| 91 | **The timing suite reads 3.0–8.4% slower than its 2026-09-05 lines, in band, everywhere B1 looked, except `encode ExecutionReport (template)`, 3.5% faster.** `[measured 2026-09-15]` boot B step B1, 20 clean runs of `serialize`, `density` and `validate`: `density` +3.0–4.7% (`engine turn, 64 busy sessions` +3.0%), `validate` +3.9–8.4% (`validate NewOrderSingle` 882.1 → 955.9), `encode ExecutionReport (template)` 229.3 against 237.6 — **3.5% faster**, and two cases left the band and were re-recorded (`SendingTime from the cache` 4.9 → 5.8, `validate NewOrderSingle, w2w bytes` 897.3 → 994.5). Not isolated: code merged since 2026-09-05, the machine, or both. `benches/baselines.tsv` tail. C-91 decides machine vs code. | ADR-0016, ADR-0031 |
