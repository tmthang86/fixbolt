...
- **A hardware timestamp read.** `enp9s0` has no carrier; cmsg parsing is proven only on absent
...
- **The macOS build of `--connect`** — `cargo check --target x86_64-apple-darwin` only, Mac
  offline; `GENERATOR_SSH` never ran over a real ssh.
- **`DESIGN.md` §8's journal/log row** is still `[unmeasured]`; records written during a
