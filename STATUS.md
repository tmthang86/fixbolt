...
| 112 | **`scripts/check-no-kernel-sleep-by-ctxt.sh` prints "GREEN ok … 0 voluntary" when w2w fails to run**: `read … <<<"$(run_and_read hft)" \|\| exit 1` swallows the function's failure and an empty value compares as 0; the script still exits red through its other half. Found building phase 4 row 3. | trap page `a-shell-read-heredoc-swallows-a-failed-functions-exit-status.md` on `plan/p4-store-sqlite` |
