...
| 105 | **`Shards` has no join, so the shard path's `after_serving` wait is unobservable from outside**: `after_serving.rs`'s shard test does not go red when `after_serving` is emptied (row M). | ADR-0088 decision 5; ADR-0153 |
