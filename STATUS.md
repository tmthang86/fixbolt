...
| # | Item | Blocks |
|---|---|---|
| 1 | **Final name.** `nanofixengine` is a placeholder, and `[checked 2026-08-30]` **it is a near-collision with the project this one is benchmarked against.** `matthart1983/nanofix` is *"Ultra-low-latency FIX protocol engine in Rust"* — the reference project of item 12 and of `CLAUDE.md` §2 rule 7's "276 unwraps" — and `LMAX-Exchange/nanofix` is a Java FIX test client with 40 stars in the same domain. **So `nanofix` is not a candidate and neither is anything built on `nano`**: being mistaken for the slower project this repository defines itself against is worse than a bland name. crates.io alone does not settle this — `nanofix` is *free* there, which is how it nearly got recommended. Verified free on **both** crates.io and GitHub: `fixwire`, `veloxfix`, `luxfix`, `fixbolt`, `sohwire`, `machfix` (only an unrelated `Machfixer`), `tachyonfix` (only a dead `bigtop-tachyonfix`). Taken: `rustfix` (the crate behind `cargo fix`), `fefix`, `hotfix`, `quickfix`. **`fixwire` is the standing suggestion** — zero GitHub hits, resists the "bug fix" misreading that `rustfix` proves is real, and names the one §6 gate that measures what a counterparty experiences | Nothing yet — but renaming after a crates.io publish is expensive |
