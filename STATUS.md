...
| Plan | Closes |
|---|---|
| **[reconnect-interop](docs/plans/2026-09-05-reconnect-interop.md)** | 38 — **written 2026-09-05, *Chờ duyệt*.** The optional step 5 of `acceptor-interop`, as its own plan, because reading the code changed its shape: `tools/interop --role initiator` drives `Session` directly and never calls `connect_and_serve`, so an eighth step on that role would test the tool's loop and not the engine's. A new `--role reconnect` through `connect_and_serve` with a `Recovery` on a `FileJournal`, two scenarios in `scripts/interop.sh` — the C++ acceptor killed with `SIGKILL`, and stopped with `SIGTERM` so it says goodbye first — and five relational assertions read off the **C++ transcript**, never off fixbolt's own word. Touches `tools/interop`, `scripts/interop.sh`, `ci.yml`, docs; **no file under `crates/`**. Names one thing to check on the way: `tests/on_disk.rs:288` derives `next_in` without the `+ 1` the trait's rustdoc requires |
