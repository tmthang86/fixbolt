...
| B9 | — | `perf`: half the `hft` engine thread's samples are an empty-listener `accept4` (item 89) |
| Q15 | `0057678` | `check-machine.sh` row `eee`; `DESIGN.md` §9 row |

### Not proven

- **Any zero-interval loopback figure from boot B as a single number** — all eight arms of B2, B5
  and B8 failed ADR-0068's 5% rule (item 85). The paced arms, nanofix and the Mac's `standard`
  table reproduced.
- **A back-to-back wire figure** — `igb` skipped a TX stamp within 1–4 runs every time (item 40).
- **The 1 s wire figure** — 5.2% and 10.1% apart across procedures, and 39 714 ns an hour later.
...
- ~~**`DESIGN.md` §8's journal/log row** is still `[unmeasured]`~~ `[2026-09-15]` **measured** at boot B5 (tmpfs); was: records written during a
  `--listen` run were not read mid-run; the mode scripts pass no `--journal`/`--log`.
...
| 91 | **The timing suite reads 3.6–8.4% slower than its 2026-09-05 lines, in band, everywhere B1 looked.** `[measured 2026-09-15]` boot B step B1, 20 clean runs of `serialize`, `density` and `validate`: `density` +3.6–4.8%, `validate NewOrderSingle` 882.1 → 955.9, and two cases left the band and were re-recorded (`SendingTime from the cache` 4.9 → 5.8, `validate NewOrderSingle, w2w bytes` 897.3 → 994.5). Not isolated: code merged since 2026-09-05, the machine, or both. `benches/baselines.tsv` tail | ADR-0016, ADR-0031 |
