...
| 60 | **Tests that wait on a wall-clock deadline for an asynchronous state fail under CI load, and it is a defect class rather than one flaky test.** `[amended 2026-09-09, later]` **A second one, in a different crate, with the same signature**: `crates/engine/tests/events.rs::an_operator_learns_why_each_connection_ended` blew a **5 s** deadline waiting for two `Ended` events, and again **two runs of the same job on the same commit disagreed** — [`34366510489`](https://github.com/tmthang86/fixbolt/actions/runs/34366510489) red, [`34366505380`](https://github.com/tmthang86/fixbolt/actions/runs/34366505380) green. **Raising the deadline is refused as a fix, and the measurement is the reason**: that test completes in **0.00 s** locally, including pinned to one core with `taskset`. A gate that finishes five hundred times inside its budget and then consumes all of it and sees nothing did not run slightly slow — a step did not happen. Changing the number would move the score and not the cause, which is `CLAUDE.md` §10's *a cause accepted because a knob moved with it*. **Two hypotheses are dead**: `Observer::events`' drain **appends** rather than replacing, so events seen in different polls do accumulate (read, not assumed); and the change under test is off by default and untouched by the test. A readiness race between the harness connecting and the engine serving **fits and is not demonstrated**. **The cause is not found**, and that is recorded rather than rounded to a plausible diagnosis. Original text follows. **A test in `crates/library` fails under CI load and passes everywhere else, and it is a defect in the test rather than in the engine.** `[measured 2026-09-09]` `end_to_end.rs::an_operator_stops_the_front_door_and_serve_comes_back` asserts a `Logon` comes back and got an **empty read** — not the timeout, which has its own message, so the acceptor closed the socket or returned end-of-file. **Two runs of the same job on the same commit disagreed**, one red and one green within the same minute, which is what classified it: [`34362977166`](https://github.com/tmthang86/fixbolt/actions/runs/34362977166) failed and [`34362986989`](https://github.com/tmthang86/fixbolt/actions/runs/34362986989) passed. Ruled out as a consequence of item 59's change two ways first — the test stamps its `Logon` at 17 bytes, a width whose code path is provably identical before and after, and 8 consecutive local runs were green. **Neither of those could have distinguished a flake from a real race that needs a slower host**; only the second sample of the same job could. Likely a readiness race in the test's own helper: `connect()` retries until `TcpStream::connect` succeeds, and a listening socket accepts connections before the accept loop is running. **Not fixed**, and this row is the record that it is open rather than explained away. [reference](docs/reference/the-same-commit-went-red-and-green-in-the-same-minute.md) | a test whose readiness check is the engine's readiness, not the kernel's |
