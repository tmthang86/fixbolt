...
| Plan | Closes |
|---|---|
| [a-journal-that-knows-the-numbering](docs/plans/2026-09-05-a-journal-that-knows-the-numbering.md) | 48 — **Chờ duyệt 2026-09-05**, not started. `Journal::mark_out` / `highest_out`, `Resumed::from_journal`, a third mark on disk; `known_gap` becomes `continued` and the `SIGTERM` scenario goes back to 5 / 5, plus a `SIGKILL` run with a Heartbeat in the window. Ahead of wave B by item 45 (c), and ahead of item 47 because it is smaller and does not need it |
