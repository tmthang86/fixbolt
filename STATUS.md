...
| 94 | **The alloc bench's message fails its own checksum, and the 2026-09-05 correction reached one file of four.** `[2026-09-19]` `crates/codec/benches/alloc.rs` carries `10=098` where the checksum is 097, so the `parse` and `parse via Encoding` cases measure a walk that ends in `Err(BadCheckSum)` — the whole message is walked either way, so no allocation count moves, and the liveness asserts added in `6f82cf3` now state the outcome rather than depend on it. Correcting the literal to `10=097` is a one-line change to a fixture a §2 gate reads, deliberately not made while the owner was away. `engine/benches/dispatch.rs` and `engine/benches/ring_full.rs` hold the same literal and never parse it. [a-bench-message-that-fails-its-own-checksum](docs/reference/a-bench-message-that-fails-its-own-checksum.md), [a-benchmark-parsed-a-message-the-parser-rejects](docs/reference/a-benchmark-parsed-a-message-the-parser-rejects.md). |
