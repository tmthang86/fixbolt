...
Last updated: **2026-09-12, later** — **items 58 and 23 are closed and on `main`, TLS is
...
- **`[2026-09-12]` Neither new script has been through `shellcheck`.**
  `scripts/check-no-crate-root-allow.sh` and `scripts/check-scratch-fixtures.sh` are proven by
  seven reversals, each red on a FAIL sentence written down before it was run — that is evidence
  about **what they assert**, and none at all about quoting, word splitting, or an unset variable
  on a path no reversal took. `shellcheck` is **not installed on this desk** and **no CI job runs
  it over `scripts/`**; four older scripts carry `# shellcheck` directives, which says somebody
  wrote *for* the tool, not that it has ever run here. The risk is not theoretical in this
  repository: item 58's first reversal was defeated by an unquoted backtick pair inside an `echo`,
  and it produced the same number the real defect produces
  ...
  Naming it here is not a claim that the two new scripts are worse than the other seventeen — it is
  that **no** `scripts/*.sh` in this repository has a machine check on its shell.
