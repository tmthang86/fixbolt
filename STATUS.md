...
Last updated: **2026-09-05** — **the bench gate is green, and it was measuring the wrong thing.**

## Start here — 2026-09-05, latest: a benchmark that measured where the compiler put it
...
| Plan | Closes |
|---|---|
| **[the-baselines-and-the-pass-nobody-timed](docs/plans/2026-09-02-the-baselines-and-the-pass-nobody-timed.md)** | 41 → 39 → 34 — **approved 2026-09-02, in progress**, and the one plan of this repository that **requires the §9 desktop**: `benches/baselines.tsv` is keyed on the CPU model, so no other machine may speak. **Step 1 and 1a–1e are closed 2026-09-05 and item 41 with them** — `bench.sh --strict` exits 0, ADR-0049, `scripts/check-bench-alignment.sh`, 24 baselines re-recorded. **Steps 2 (item 39) and 3 (item 34) are open**, which is why this row is still the topmost. It was missing from this table entirely until 2026-09-05, which is how a plan *đang làm* stayed invisible to the row above it |
