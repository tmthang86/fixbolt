...
Last updated: **2026-09-12, latest** — **a senior review attacked all three gates this plan built,
...
| 57 | **The licence gate cannot see a dev-dependency, and it says `ok` rather than saying so.** `[measured 2026-09-08]` `cargo-deny` 0.20.2 does not judge the dev-dependencies of this workspace's members. An `MPL-2.0` crate in `[dev-dependencies]` of `tools/jrnl`, and separately of `crates/codec`, is resolved by `cargo metadata` (12 packages) and printed by `cargo tree`, while `cargo deny list` shows 11 crates and `cargo deny check` prints all four `ok` lines. `exclude-dev = false` changes nothing; stripping the config to two lines changes nothing; **the same crate as a normal dependency is rejected immediately**, so the checker works and the graph it is handed is short. **It nearly hid itself**: the first reversal used a dev-dependency, came back green, and read as a broken reversal — three explanations were checked before *the tool never looked*. Not fixable from here, so it is made loud: the CI job diffs the crate set `cargo deny list` judged against the set `cargo tree` resolves, 11 and 11 today, and fails naming the crate when they differ. **Still open** because the hole itself is still there | [a-license-gate-that-cannot-see-dev-dependencies](docs/reference/a-license-gate-that-cannot-see-dev-dependencies.md) **`[to testing-skills]`** |
