...
- **A deployment that uses `serve`, `serve_hft` or `serve_sharded_hft` still cannot resume
  anything.** `[2026-09-02]` `Engine::add_resumed` exists and is tested, but those entry points
  accept connections through the pre-session stage and hand them to the engine themselves — the
  embedder never sees a transport to call it with. So recovery is reachable **only** by driving
  `Engine::add*` by hand. `STATUS.md` item 31, step 3.
...
| [an-engine-can-resume](docs/plans/2026-09-02-an-engine-can-resume.md) | **STEPS 1, 2, 4 CLOSED 2026-09-02, merged PR #20, CI [`33588190145`](https://github.com/tmthang86/fixbolt/actions/runs/33588190145) green on `38ca25c`.** Step 3 — a `Recovery` trait for `serve*` — **not started**; item 31 stays open, narrowed |
...
| 31 | **An `Engine` can resume a session now; the automatic path is still missing.** `[2026-09-02]` **`Engine::add_resumed(transport, cfg, journal, next_out, next_in, last_active_ms)`** — steps 1, 2 and 4 of [an-engine-can-resume](docs/plans/2026-09-02-an-engine-can-resume.md). The journal goes in **with** the counts, because correct numbers over an empty journal answer the first `ResendRequest` with a gap fill — legal, and a silent loss of exactly what was asked for; `a_resumed_session_replays_what_it_sent_before_the_restart` and `a_resumed_session_with_an_empty_journal_fills_the_gap_instead` tell the two apart. `last_active_ms` is a separate argument because the numbers cannot imply it, and with it [ADR-0033](docs/decisions/ADR-0033-a-schedule-is-utc-arithmetic-and-the-calendar-stays-outside.md)'s boundary reset is reachable from an engine for the first time. **What is still open is step 3**: `serve`, `serve_hft` and `serve_sharded_hft` accept connections through the pre-session stage and the embedder never sees them, so a deployment using those **still cannot resume anything**. The plan's shape for it is a `Recovery` trait symmetric with `Registry`, asked after the registry picks a `Config` — before that the identity is not known. **And nothing persists `last_active_ms` yet**: `Session::last_active_ms()` is what to save and no journal field holds it, so a caller must keep it themselves | Deployments that use `serve*` rather than driving `Engine::add*` by hand |
