...
| 91 | **The timing suite's 3–8% slowdown is code merged since `0149b26`, not the machine — and it is 7–10% on every engine-turn case, larger than boot B read.** `[measured 2026-09-18]` boot C step C-91, one A/B in one boot, `pass 16 fail 0 unknown 0`: a worktree at `0149b26` (the commit that recorded the 2026-09-05 lines) against `85460c1`'s code, 20 rounds alternating order, three suites each, medians of 20 — `engine turn, 1 busy sessions` 1 660.2 → 1 817.2 (+9.5%), every `engine turn` case +6.7–9.8%, `validate NewOrderSingle` 909.7 → 956.7 (+5.2%), `validate TestRequest, w2w bytes` +6.1%, `encode ExecutionReport (template)` **−3.4%**, `SendingTime from the cache` 4.9 → 5.8. **The old commit in today's boot reads its own 2026-09-05 numbers** (1 660 against the 1 657.7 line), so the kernel `-30 → -31` candidate is dead; the toolchain is pinned at `1.98.0` throughout ([measured-costs](docs/reference/measured-costs.md) *Boot C*, C-91). Two leads, from what the two families of cases do: the validate cases are pure user space and point at `588b350` (`52=` at every precision) and its neighbours; the engine-turn cases add a syscall per turn and point at the TLS branch's changes to `pump`/transport. **Next, and the only thing left here: C-91b**, `git bisect` between `0149b26` and `85460c1` on `engine turn, 1 busy sessions`, bad above 1 743 ns (1 660.2 × 1.05), n = 5 per commit, same boot — [closing-the-open-items](docs/plans/2026-09-18-closing-the-open-items.md) row C-91b. The baselines are **not** re-recorded until the commit is named: a baseline moved to hide a regression is the failure ADR-0016 was written against. Was: *not isolated: code, the machine, or both* | ADR-0016, ADR-0031; the `density` and `validate` baselines |
