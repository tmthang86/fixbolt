...
- **A process restarting across a session boundary still keeps yesterday's numbers.**
  `[2026-09-02]` the session layer can decide a reset — `Session::resume_at` takes the instant
  the numbers were last touched — but **nothing persists that instant**, and `Engine` calls
  `Session::resume`, which by design never resets. So the rule is implemented and the
  deployment path is not. `STATUS.md` item 31.
...
| **[session-schedules](docs/plans/2026-09-02-session-schedules.md)** | **STEPS 1–3 CLOSED 2026-09-02** — `PRD.md`'s Phase-1 gap, named three times and never planned until now. [ADR-0033](docs/decisions/ADR-0033-a-schedule-is-utc-arithmetic-and-the-calendar-stays-outside.md). **Step 4 is item 31** and is not started |
...
| 31 | **A schedule reset cannot survive a restart, because nothing persists when it was last needed.** `[2026-09-02]` the session layer decides a boundary correctly — `Session::resume_at(cfg, out, in, last_active_ms)` compares and resets, and `Session::last_active_ms()` is what an engine would save. **Nothing saves it.** `Engine` builds sessions with `Session::resume`, which carries the numbers and by design asserts nothing about the calendar, so it never resets. A process that stops at 17:00 and starts at 08:00 the next morning therefore carries yesterday's `34=` into a new trading day — the exact dispute a schedule exists to prevent. **The fix is one field in the journal beside the sequence numbers**, and it is step 4 of [session-schedules](docs/plans/2026-09-02-session-schedules.md), deliberately left out of steps 1–3 because it touches the journal format and that is its own change. **Two things are already true and worth not re-deriving:** the comparison is right and tested, and the corpus cannot see any of it | Phase-1 deployability of schedules; `GUIDE.md` §5a says so in the meantime |
