...
* **Item 96's inclusive cost** (above). **Item 99's cause.** **Which commit of PR B** carries
  item 95's step.
* **The desk is left on the §9 grub line** (`grub.fixbolt-desktop-20260922` restores the desktop
  line) and powered off on the owner's instruction; timers return on boot.
...
| 99 | **`journal put, 191 bytes, one slot` read 7.4 ns in one `--strict` run and 12.4 ns twenty-seven minutes later in the same boot, same pinned binary, and stayed at 12.4 on four re-runs.** `[measured 2026-09-23, boot E]` `target/boot-e-evidence/s3-strict.txt` (21:07 UTC) 7.4, in band; `s4-strict.txt` (21:44 UTC) 12.4 `OVER BASELINE` (band [6.7, 8.1]); then three bare runs of the `main-tree` `journal` binary from `../fb-s9e/MANIFEST.txt` and one under `taskset -c 6`, all 12.4. The two `walking` cases did **not** move (8.8 / 5.3 ns), `check-machine.sh` still `pass 17 fail 0 unknown 0`, THP `never`. Between the two runs the desk ran S2 (`perf record -g` ×5 of the `validate` bench) and S1c (24 rounds of `sbe`). The case is pure L1 (one slot, eight lines) so the suspects are CPU-side state a `perf record` could leave (PMU / NMI-watchdog counter re-armed), not the code. **The baseline line is not moved.** First check next boot: run the journal binary **before** any `perf record`, then after one, and read the case both times. | ADR-0031; `bench.sh --strict` on the desk |
| 100 | **`bench.sh --strict` cannot be green on the §9 desk while the `wakeup` bench prints its two p50 cases as `NO BASELINE` by construction.** `[2026-09-23, boot E S3/S4]` `crates/engine/benches/wakeup.rs` lines 59–66 and 153–159: the p50 arms are printed as `NO BASELINE` cases "until boot C records one" and there is no `benches/baselines.tsv` lookup for them, so `bench.sh --strict` counts `2 case(s) had no baseline` on every run — it has been structurally red since item 89 merged (2026-09-18); the last green was boot B step B1 (2026-09-15) and nobody ran `--strict` on the desk in between (item 97 said so). Design question for the architect: give the p50 arms a tsv line (which quantile, which margin, what n — one run is n = 20 000 samples, not 20 runs) or exclude them from `--strict`'s count by decision. Until then a desk `--strict` is read as "red only on `wakeup`" by hand. | ADR-0016, ADR-0031, item 89; `scripts/bench.sh` |
