...
| 11 | **Serialise misses its gate: 93.8 ns against 60 ns** on the M5, and `[measured 2026-08-30]` **177.6–199.4 ns across five runs on Linux x86_64** — a 3.0–3.3× miss there, not 1.6×. (DESIGN §6). **`[measured 2026-08-31]` that cause is real and is about a quarter of the total; the framing "fix the scan and the gate is met" does not survive measurement.** On a 4-core Xeon container (`pass 2 fail 6 unknown 3`, so ratios only): a tag comparison costs **~0.4 ns**, so the 105 of them are worth **~42 ns of ~152**. The cost is **linear in the number of slots, not quadratic in the comparison count**. **~31 ns is spent before a single variable field is written** — prefix, three static fields, body-length render, trailer — which is **51% of the whole 60 ns target on a message carrying nothing**; then **~7 ns per field in `put`**. **`[measured 2026-08-31]` the forward-cursor fix was then written and reverted**: predicted −36 ns, measured **+5.2 ns (+3.4%)** over 30 runs per arm, reproducing at +4.5 and +4.4 within each of the machine's two modes. **That also corrects the ~42 ns above** — the 0.4 ns rate was measured on 78-element scans and does not extrapolate to 14, so the scan's true share is smaller and is not separable by these instruments. What survives is the conclusion: **the scan is not where the target is lost.** Kept from the attempt: `crates/codec/tests/slot_order.rs`, six cases holding that the caller's ordering never reaches the wire — a guard the body path did not have, proven by reversal twice. `checksum` was suspected and **cleared**: 2.3 ns at 43 bytes, 3.2 ns at 169, ~0.9 ns of a 122 ns difference. So a perfect O(1) slot lookup still leaves **~116 ns against a target of 60**. The levers in measured order are the fixed cost, then `put`, then the scan item 11 named. **The first experiment run against this was void** — reversing the caller's slot order leaves the comparison count at 105 either way, so it varied nothing; the working experiment pads unmatched slots in front. Write-up: [measured-costs.md](docs/reference/measured-costs.md). Plan: [2026-08-31-serialise-and-the-60ns-target](docs/plans/2026-08-31-serialise-and-the-60ns-target.md), steps 1-2 closed. **Open now: whether 60 ns is a target this shape can reach at all** — that is an ADR, not a quiet edit. The only red gate that does not need the Linux box. **Start with `scripts/bench.sh --strict`** on the §9 box: the number to optimise against is the one from that machine, not the 93.8 ns or the 177.6-199.4 ns above. | DESIGN §6 serialise row; the `engine` step, where the number is re-measured |
