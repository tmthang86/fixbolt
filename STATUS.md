...
| 92 | **Four `Reachable` mirrored definitions are red, and on today's evidence the harness is the suspect, not the session.** `[measured 2026-09-18]` `crates/conformance/src/mirror.rs` (`a2aa9ce`, [ADR-0076](docs/decisions/ADR-0076-the-mirrored-ceiling-is-a-classified-count-not-an-estimate.md)) puts the mirrored ceiling at **14 of 50**, and the score reads **10 / 14**. The four: `6_SendTestRequest.def`, `8_AdminAndApplicationMessages.def`, `8_OnlyApplicationMessages.def` all fail after the engine emits `Reject 45=2 371=122` (*Required tag missing*, `OrigSendingTime`); `1a_ValidLogonMsgSeqNumTooHigh.def` fails alone. **Hypothesis A — a harness hole**: the mirror harness's `make_receivable` (`crates/conformance/src/script.rs`) turns an `E` line into a `PossDup` (`43=Y`) **without `122=OrigSendingTime`**, so the engine's inbound validation refuses a message the harness built, not one the counterparty would send — FIX 4.4 requires `122` when `43=Y`. The senior developer probed `send_application` twice and saw 14 fields, `34=2` then `34=3`: the session's outbound side is correct. **Decides it**: a unit test on `make_receivable` asserting a `43=Y` output carries `122=`; fix it there, and the three files must go green with no line of `crates/session` touched — if any stays red, hypothesis A is refuted for that file and it becomes a session item. **Hypothesis B — a harness gap**: `1a_ValidLogonMsgSeqNumTooHigh` expects this end to start at `34=5` with the counterparty at 1, i.e. a session resumed at `(5, 1)`, and the harness never calls `Session::resume(cfg, 5, 1)`. **Decides it**: the harness reads the file's opening `I` line's `34=` and resumes at that number; the file goes green with no session change. **Not a session defect on this evidence**, and by ADR-0076 *Consequences* a red `Reachable` row is never moved; the score test names the four in `KNOWN_RED` and fails when the set changes in either direction, so closing this item means emptying that array. Until then `docs/CONFORMANCE.md` publishes *10 / 14* | ADR-0076's ceiling of 14 being met; `docs/CONFORMANCE.md` |
