...
unpinned** — re-run B0 before any figure. The closing CI run id and the review are in the plan's
delivery log.
...
| 90 | **A paced `w2w` run longer than the counterparty's `MaxLatency` is rejected, and nothing refuses to start one.** `[measured 2026-09-15]` boot B step B4: `--interval 1000000` with 125 messages failed at sequence 122 on `35=3 373=10` *SendingTime accuracy problem* — `w2w` renders `52=` before the clock starts, and the session's `MaxLatency` is 120 s. The engine was right; the run was re-done with 105 messages. **No regression test and no guard — an unmet `CLAUDE.md` §4 obligation.** The guard: `tools/w2w` refuses `--interval × (warmup + messages) ≥ 120 s`, with a test. [a-paced-run-outlived-the-sessions-maxlatency](docs/reference/a-paced-run-outlived-the-sessions-maxlatency.md) | `tools/w2w`; every paced §8 figure |
