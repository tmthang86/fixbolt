...
Items **21, 67, 71, 72, 73, 74** close; **55** stays a standing ratchet by decision; items **75–79**
open, all found by the review and all pre-existing. See *Start here* below, which this line
replaces. Before that: **2026-09-13, later** — **CI runs once per commit, and a branch with no pull request
...
[`34739943517`](https://github.com/tmthang86/fixbolt/actions/runs/34739943517), **14 jobs of 14**.
The `tls` plan's steps 5, 6 and 7 are built except the two that need the §9 machine. The three: **a
live defect in merged code** — `ktls-core`'s `tls13-key-update` was off, so a counterparty's
`KeyUpdate` killed the session and no test here had ever sent that message; **a reconnect storm** —
879 dials in 1.5 s, because `up` was set after the first turn rather than at the add; and **a false
green in a guard written the same day**, where the second reader never ran. **No TLS number is
published**: `DESIGN.md` §8's TLS row is still empty and every figure here is from the desktop
configuration. See *Start here* below, which this line replaces. Before that: **2026-09-12, last** — **the four open items that need no §9 machine are closed, and
three of the four decisions that closed them reversed something this repository had in writing.**
Branch `plan/an-obligation-nothing-checks`, tip `845974c`. Items **57**, **64**, **69** and **70**
close; items **71**, **72** and **73** open. The senior review refuted a sentence written on this
branch the same day it was written — for the second pull request running — see *Start here* below,
which this line replaces. Before that: **2026-09-12, latest** — **a senior review attacked all three gates this plan built,
got past all three, and six of what it found are fixed; four are left open by decision, for the
architect.** Branch `plan/gates-that-match-a-meaning`, tip `57b6b3a`, **not yet merged**. Step 6
found a second blind spot in the crate-root gate (the deny-list derivation could not read a
quoted TOML key) and five more ways past the scratch-fixture gate; fixing the first found that
the `tls`-test gate closed two steps earlier was itself wrong twice, for reasons that had nothing
to do with TLS — see *Start here* below, which this line replaces. Before that: **2026-09-12,
later still** — **the two gates the senior review got past now read a
meaning, not a spelling.** Branch `plan/gates-that-match-a-meaning`, tip `7323299`, **not yet
merged**. Steps 1, 2 and 3 close items 66 and 68 outright: `scripts/check-no-crate-root-allow.sh`
reads every inner attribute `tools/attr-scan` finds by lexing a crate root the way `rustc` does
(`ok — 6 crate roots, 10 manifests, 55 inner attributes`), `scripts/check-scratch-fixtures.sh`
reads a `#`-first line as never read, `cp` only in command position, and any assignment prefix as
a seed (`ok — 20 scripts, 1 enter a scratch dir, 1 pins`), and
`scripts/check-feature-gated-tests-ran.sh` asks the `tls` build what tests exist rather than
naming files (`ok — fixbolt-engine --features tls: 38 binaries, 322 listed, 322 accounted for (0
ignored)`). Step 4 narrows item 67 and closes the narrowed part: `doc_table` now drives the
parser from four kinds of `CONFIGURATION.md` §1 cell that are already values, and found no cell
wrong today. **This step's own "not yet run" turned out to matter**: step 6, run afterwards, is
what found the deny-list and gate-B holes above. Before that: **2026-09-12, later** — **items
58 and 23 are closed and on `main`, TLS is
configurable from a file, and the senior review got past both gates this branch built.** PR
[#63](https://github.com/tmthang86/fixbolt/pull/63), merge `a8c9a3c`, CI green on the merge commit
itself, 14 of 14. **The count that proves it is `TLS tests that ran: 55`** — it read 11 before the
branch and 14 in the middle of it, because twice in one day a `--features tls` test existed that no
CI job named, and the second time a list of test *files* could not have caught it. **Three of the
day's findings are reversals that nearly passed**, all the same family, each now a
`docs/reference/` entry that links the other two: a matcher that could not match `clippy::`, a
reversal that removed a guard's `#[cfg]` label and left the guard compiled, and one that never
reached the code it was written for. Before that: **open items 62 and 65 are closed on `plan/tls-4b`, and the job that closed 62 is what found 65.** A TLS test that was green 20 times on this desk was flaky on GitHub's runner; it was diagnosed by measurement rather than by reading the code, and the measurement said the engine was innocent. **The reversal that closed it went red on an assertion the approved plan did not predict, and the predicted one could never have failed.** Before that: **2026-09-09, merged** — **open item 60 is closed and on `main`. It was never a flaky test: reading the event stream destroyed it, and the evidence that said so had been sitting in the first failing CI log for hours.** Before that: **2026-09-09, latest** — **open item 59 is closed, and the survey the owner asked for before approving it changed the answer twice: the QuickFIX family disagrees with itself about this field, and there was a seventh refused width nobody here had counted.** Before that: **wave B plan 3 is closed, merged, and green on the merge commit itself. This engine can send a microsecond `52=`. Its own gate found, on its first run, that the plan which closed the *receive* half the day before had widened one of the field's two readers and left the other — and only somebody else's engine could see it.**
...
**`[2026-09-08]` §9's last box, closed on the commit it asks about.** PR [#51](https://github.com/tmthang86/fixbolt/pull/51) merged as **`6d3a191`**; the merged head `627637e` was **26 of 26** check runs across runs [`34247754396`](https://github.com/tmthang86/fixbolt/actions/runs/34247754396) and [`34247696250`](https://github.com/tmthang86/fixbolt/actions/runs/34247696250), and **the merge commit itself is green**, run [`34252988486`](https://github.com/tmthang86/fixbolt/actions/runs/34252988486), **13 jobs of 13**. `git diff 627637e 6d3a191` is **empty**, so the branch's green transfers to the merge exactly rather than by assumption. The three jobs neither desk can run for itself — `interop` (`Build libquickfix, run both ends, read the output`), `bench` and `deny` — are among the 13.

**And the pass produced a measurement of its own, from its own shell.** `[measured 2026-09-08]` a docs-only change was checked with `cargo test --all 2>&1 | tail -60` and the harness reported **`exit code 0`** — while the sixty lines it printed said `error: failed to run custom build command for fixbolt-dict` and `build failed`. **A pipeline's exit status is its last command's**, so the zero belonged to `tail`; `cargo` was never asked. This is one turn worse than [reading-the-output-you-grepped-for](docs/reference/reading-the-output-you-grepped-for.md) two days ago, where the filter merely could not show the surprise: here the filter **became** the status. Nothing was wrong with the gate — `vendor/` is gitignored by ADR-0001 and a fresh clone has to run `scripts/fetch-quickfix-assets.sh` first, which the message said. What was wrong is that a success was recorded for a suite that never compiled. **And the `tail -60` that revealed it was wide enough by luck** — a `tail -3` would have left `exit code 0` as the only signal. Written up as a fourth transferable rule in that same reference rather than a new file, so the `[to testing-skills]` queue keeps one entry for one lesson.
...
owner's order 2026-09-24: run until phase 4 closes. Built, reviewed and merged; each item's keep/drop verdict is row 7's boot.

### What closed

| Row | PR | Merge | CI (PR head) |
|---|---|---|---|
| 1 metrics exporter + Grafana dashboard (ADR-0170, 0171) | [#108](https://github.com/tmthang86/fixbolt/pull/108) | `fab52fa` | 36023404746, 20/20 |
| 6 Onload/AF_XDP — **dropped at G1** (igb has no RSS key ops before Linux 7.3; ADR-0201 *Result*, ADR-0203) | [#111](https://github.com/tmthang86/fixbolt/pull/111) | `3d5f90f` | 36026410471, 20/20 |
| 3 SQLite store (ADR-0180–0182; engine ADR-0181 handles) | [#109](https://github.com/tmthang86/fixbolt/pull/109) | `ecb081f` | 36027263734, 20/20; main 36028890985 green |
| 5 io_uring transport (ADR-0190–0192) | [#110](https://github.com/tmthang86/fixbolt/pull/110) | `5b4eff8` | 36031961599 (attempt 2), 21/21; main 36035471495 green |
| 7a boot driver `scripts/boot-p4.sh` (ADR-0202, Proposed) | [#113](https://github.com/tmthang86/fixbolt/pull/113) | `1dd99bd` | 36046011847, 21/21 |

Prebuilt for the boot (7a.2): `/home/tmt/Projects/fb-p4-boot/` from `1dd99bd` — `sha256sum -c MANIFEST.txt` 4/4 OK,
both `w2w` carry `cap_net_admin,cap_net_raw=ep`, trees clean, `BUILD-INFO.txt` names the Mac's HEAD `1dd99bd` and its
`w2w` sha256 `2826bbc1…6101`.
...
`check-machine.sh` `pass 17 fail 0 unknown 0`; throw away the first run; `BOOT_ROOT=/home/tmt/Projects/fb-p4-boot
scripts/boot-p4.sh run` and no tool calls until it exits (0 done, 1 finished with FAILED arms, 2 refused, 3 stopped).
Then, in the same boot, **row 2's scrape pair** by hand from the same prebuilt binaries — [metrics plan *Sửa 2*](docs/plans/2026-09-24-p4-metrics-exporter.md)
(~1.5 h: two procedures ≥ 1800 s apart). Then 7b.3 (verdicts, `measured-costs.md`, DESIGN §8 if kept), 7b.4 (restore the
desktop line), rows 8–9 (SIMD), row 10 (close the phase).

### Do not

* Do not rebuild anything in `fb-p4-boot/` after the reboot — a rebuild drops the file capabilities and changes the
  MANIFEST; the driver refuses both ([trap](docs/reference/a-pre-built-w2w-needs-its-file-capabilities-and-a-rebuild-drops-them.md)).
* Do not run a binary from `/tmp` or the scratchpad: tmpfs is `nosuid` and silently drops capabilities.
* Do not treat an ssh failure to the Mac as a key problem: FileVault locks its home until the owner logs in.
* Do not `cargo new`/`cargo add` inside the repo; do not grep cargo output without `--color never`.
...
The owner decided not to publish to crates.io (fixbolt serves their own project): [ADR-0161](docs/decisions/ADR-0161-0-1-0-is-a-git-tag-not-a-crates-io-upload-and-the-stranger-and-the-semver-gate-read-the-tag.md)
replaces ADR-0097 Q5 and exit criteria 7–8; [ADR-0162](docs/decisions/ADR-0162-the-semver-baseline-is-the-newest-release-tag-head-descends-from-and-zero-checks-are-excused-only-by-a-major-bump.md)
makes the semver baseline the newest release tag HEAD descends from.

### What closed

* **main's red CI fixed** — PR [#106](https://github.com/tmthang86/fixbolt/pull/106): dropping `Shards` now joins its shard
  threads (a shard never joins itself; a failed spawn aborts the gate). Merged as `3323e41`.
* **ADR-0161** — PR [#107](https://github.com/tmthang86/fixbolt/pull/107), merged as `890c785`; main CI run
  36011477652 green 19/19 on that commit.
* **Row 7b** — tag `v0.1.0` (annotated, object `8ba45fe`) on `890c785`, pushed; `git ls-remote --tags origin 'v0.1.0^{}'`
  → `890c7850fbc6e564a7854404253382c1e9977664`. GitHub ruleset 23943722 "release tags are immutable (ADR-0161)":
  `refs/tags/v*`, rules deletion/update/non_fast_forward, no bypass, enforcement active.
* **Row 8b / 8b'** — PR [#112](https://github.com/tmthang86/fixbolt/pull/112): `scripts/stranger-check.sh --from git --tag v0.1.0`
  (every `fixbolt*` package resolved from the tag, Logon/Logout answered) and `scripts/check-semver-against-tag.sh`
  (blocking, no argument, baseline v0.1.0, six crates × 196 checks). CI run 36018953370 green 20/20 on `4b1c945`.
  **Phase 3 (ADR-0097 as amended) closes on this PR's merge.**

### Where the work is — phase 4 ([ADR-0098](docs/decisions/ADR-0098-phase-4-is-the-owners-five-items-each-entering-behind-a-measurement-that-can-kill-it.md))

| Row | Branch / PR | State |
|---|---|---|
| 1 metrics exporter + dashboard | `plan/p4-metrics` #108 | steps 1–6, 8 built; senior-review fixes F1 (slow-loris deadline), F3, F4 guards, F2/F5 (plan Sửa 1) in progress; step 7 (real Prometheus + Grafana) running |
| 3 SQLite store | `plan/p4-store-sqlite` #109 | steps 1–7 built (ADR-0181 revisions 1–2); senior review running |
| 5 io_uring transport | `plan/p4-io-uring` #110 | steps 1–4 built (59/59 over io_uring in both modes); ADR-0190 revision 1 being applied, then step 5 |
| 6 Onload bypass | `plan/p4-bypass-boot` #111 | **dropped at G1** (igb has no RSS key ops; ADR-0201 *Result*); drop documented; Onload removed from the desk |
| 7 one §9 boot | #111 | measures io_uring + the store pair only; needs rows 5 and 3 merged, then a reboot = new session |
| 9 SIMD | not started | after row 8's checksum bench |

### Next — the first executable action

Merge #112 (phase 3 closed), then continue the phase-4 rows from their PRs' newest commits; each merges only after its
senior review and green CI.

### Do not

* Do not `cargo new` / `cargo add` in a directory under the repo — cargo appends it to the workspace `members`.
* Do not grep cargo output without `--color never` — CI sets `CARGO_TERM_COLOR=always`
  ([trap](docs/reference/cargo-output-colour-defeats-plain-text-parsing.md)).
* Do not create a tag in a worktree to test tag logic — tags are shared by every worktree; use a clone with `origin` removed.
* Do not trust an ssh failure to the Mac as a key problem: FileVault locks its home until the owner logs in.

### Not proven

* The `semver` job blocks by convention only: `main` has no required status checks.
* Row 1's exporter and row 3's store have no §9 verdict yet (their kill lines are row 2's and row 4's pairs, on the boot).

## Start here — 2026-09-24: phase 2 is closed, phase 3 is built to the owner's `cargo publish`, and the desk is powered off

Session of 2026-09-23 → 24, owner's order: merge #96, run until phases 2 and 3 close, then shut down.
Everything below is on `main`; no pull request is open. The desk is on its **desktop** grub line (no
`isolcpus`); nothing here was measured on the §9 line.

### What closed

* **Phase 2** — PR [#101](https://github.com/tmthang86/fixbolt/pull/101) (plan
  [closing-phase-2](docs/plans/2026-09-23-closing-phase-2.md), ADR-0102, ADR-0103): item **101** is the
  binary's own layout (instructions:u −0.0317 %, same work); item 99's 1.12 residue is single-run
  dispersion (allocation sizes and addresses identical at every file length, bench process only);
  item 95's step is commit `179ab51` (+2 409 instructions/iteration, 130.4 % of the target);
  ADR-0094's reason for "≥" is corrected by ADR-0103. `scripts/bench-instructions.sh` separates a
  layout move from a code move off the desk; `FIXBOLT_BENCH_COUNT_ONLY=1` lets it run on an OVER line.
* **Phase 3** (ADR-0097), rows by PR:
  [#97](https://github.com/tmthang86/fixbolt/pull/97) `Decimal` (ADR-0120);
  [#98](https://github.com/tmthang86/fixbolt/pull/98) secrets stay off disk (ADR-0110);
  [#99](https://github.com/tmthang86/fixbolt/pull/99) the Orchestra diff came out **C** and the owner chose
  QuickFIX's XML with a `NOTICE` (ADR-0101 *Result*, ADR-0104) — `CLAUDE.md` §2 item 9 and §8 changed;
  [#100](https://github.com/tmthang86/fixbolt/pull/100) QuickFIX/J 7/7 in both roles, plaintext and kTLS
  (ADR-0130, ADR-0131);
  [#102](https://github.com/tmthang86/fixbolt/pull/102) FIXP spike against Artio's Binary EntryPoint,
  accepted and refused (ADR-0140, ADR-0141: FIXP not before phase 5, in B3's current dialect only);
  [#104](https://github.com/tmthang86/fixbolt/pull/104) six crates package at 0.1.0, `RELEASING.md`,
  the packaged-sources stranger check (ADR-0160);
  [#103](https://github.com/tmthang86/fixbolt/pull/103) the defects phase 3 found — Async journal record
  > 4096 B stopped the writer (D1), idle writers spun (D5), a dial's reconnect wait ignored shutdown
  (D2), a refused TLS handshake sent no alert (D3), `as_i64` took `+` (D4, **breaking**), the strace
  check judges the serving window (W, ADR-0152), a connection's journal is retired not joined on the
  engine thread (J, ADR-0153), one appender per journal file with parked reconnects (K, ADR-0154,
  ADR-0155), a full ring's loss counted (L). `rust-version` is **1.89**.
* CI run ids for the closing commits are in each PR; the merge of #103 is the last one — its run id is
  in the delivery log of [phase-3-found-defects](docs/plans/2026-09-23-phase-3-found-defects.md).

### Next — the first executable action

**The owner publishes** (ADR-0097 Q5): follow [RELEASING.md](RELEASING.md) from step 0 on a clean
`main`. Then row 8b of [p3-packaging](docs/plans/2026-09-23-p3-packaging-and-first-release.md):
`scripts/stranger-check.sh --from registry` green, and the `semver` CI job made blocking. Phase 3
closes there. Phase 4 ([ADR-0098](docs/decisions/ADR-0098-phase-4-is-the-owners-five-items-each-entering-behind-a-measurement-that-can-kill-it.md))
starts after it.

### Do not

* Do not run `cargo publish` from an agent, and do not `cargo login` — the owner does (ADR-0097 Q5).
* Do not rsync a worktree to make a no-`vendor/` copy: its `.git` pointer comes along and a commit in
  the copy moves the real branch — use `git archive HEAD | tar -x`.
* Do not read the `semver` job's red as a defect before 0.1.0 is on crates.io: it compares against an
  unpublished 0.1.0 on `main` (advisory, `continue-on-error`).
* Do not count voluntary context switches as a test of "the engine does not sleep" under a loaded
  machine — page faults and reclaim count too (plan phase-3-found-defects *Sửa 6/7*).

### Not proven

* crates.io accepting `LicenseRef-QuickFIX-1.0` (fallback `license-file`, RELEASING step 6).
* The two `Decimal` timing cases have no baseline; the harness edit (`FIXBOLT_BENCH_COUNT_ONLY`) may move
  every bench's layout — read the next `--strict` on the §9 line under ADR-0102 decision 2 first.
* No trace of the engine thread while a connection is parked (ADR-0155 rests on an atomic load and a
  deterministic test).
* ~26 % of item 95's step is not attributed to a commit by instruction count.
...
Plan: [2026-09-23-boot-f-closes-the-open-items-and-powers-off](docs/plans/2026-09-23-boot-f-closes-the-open-items-and-powers-off.md),
branch `plan/bench-strict-on-the-desk` (worktree `../fb-strict`), PR [#95](https://github.com/tmthang86/fixbolt/pull/95).
Boot F: 2026-09-23 on the §9 line (`isolcpus=6,7,14,15 rcu_nocbs=6,7,14,15 processor.max_cstate=1`,
...
* **Item 99** — named: the `journal` bench's heap layout, set by the length of `benches/baselines.tsv`
  (old binary 12.4 ns on 24 897 bytes, 7.4 on 24 226; sweep 6.3 … 19.0; after a `perf record` still
  12.4, so not `perf`). Fix `6b2833b`, fixed 1 MiB read buffer. **The post-fix sweep is 7.4 … 8.3,
  max/min 1.12 — not flat by the plan's 1.10 bound**; the ×1.68 step (12.4 / 7.4) is gone and the line was not moved.
* **Item 100** — `Suite::figure`; wakeup lines 5 089.0 / 4 939.0 ns, **n = 19** (one short of 20).
  `--strict` run 1: exit 0, `cases w/o a baseline 0`.
* **Item 85** — P1 16 226, P2 (6 s after a build) 16 256, P3 16 240 ns p50, all three pairs
  `reproduced`; the "after a build" candidate is refuted; the comparator's reversal runs in `gates`.
* **Item 89** — *accept, unnamed* (`8c66123`); ADR-0096 decision 6 adds the sign clause.
* **Item 51** — at the mitigation tier (ADR-0096 decision 5), **and the flush arm ran**: 12 828.7 →
  9 935.4 → 12 818.7 ns/op, **−22.6 %** with tailscaled's ruleset flushed. One machine, one boot, one case.
* **Item 96** — the descent's inclusive price is **≥ 7 021.6 ns** (children 8.49 %, self 2.86 %,
  n = 3, range 6 844.8–8 023.2; 8.87 % of the case), `--call-graph dwarf` on the pinned binary, read
  through a perf/LLD unwind workaround. Boot E's 2.94 % was self only.

### What the boot found

* **Item 101 (new)** — the item 99 fix moved `walk nested group + varData` +10 % (153.6 … 160.3 →
  174.2 … 176.1, A/B same boot; not the mmap threshold). `--strict` run 2 went red on it alone;
  re-recorded with the cause (`5576694`, 159.5 → 176.5); run 3 green. Architect.
* **Item 76 needs nothing.** The plan listed "76-b" as waiting for the owner; row 76 was closed
  2026-09-13 (PR #69) and `docs/CONFIGURATION.md` §1 already reads "non-negative integer … zero means
  no heartbeats at all". The second row numbered 76 is that row's old text, not an open half.
* **A saved iptables-nft ruleset does not load back through `nft -f`** — [reference](docs/reference/a-saved-iptables-nft-ruleset-does-not-load-back-through-nft.md).
* **perf 7.0.14's dwarf unwinder fails on an LLD-linked PIE**, so `--children` reads children ≡ self —
  the "garbage callchains" of boot E; read-time workaround via `--symfs` —
  [reference](docs/reference/perf-dwarf-unwind-fails-on-an-lld-linked-pie.md).
* **Follow-up for the architect (item 96, a note):** ADR-0094's stated reason for "≥" — the
  descent's first level inlined — is wrong; there is one outer call site (`c7046` → `c9680`), and what
  is inlined into `validate_with` is `bad_group_count`'s loop. ADR-0094's substance is not edited.

### Where the desk is

F10's last step, after this commit, CI and the merge: the desktop grub line is restored
(`/etc/default/grub.fixbolt-desktop-20260922`, `update-grub`, `grep CMDLINE` shows no `isolcpus`) and
the desk is powered off; `grub.fixbolt-s9` stays for the next measurement boot; timers return on
boot. If the desk is found on the §9 line, that step did not run.
Items 1 and 14 untouched (no plan by design).

### Next — the first executable action

Verify this handoff (branch merged, closing commit on `main`, CI run above green), then hand
item 101 — and the 1.12 residue of item 99's sweep with it, same class — to the architect in one
brief, with the ADR-0094 "≥" correction as a second, independent note. Neither needs the desk before a decision.

### Do not

* Do not run a timing step with `awk '{print $6}'` on `payload` output: `TCP loopback, 8 in 8 out`
  is six fields before the number; F9's first attempt printed `out` fifteen times and exited 0.
* Do not `nft -f` a saved ruleset that says `managed by iptables-nft`; restart the owning daemon
  (tailscaled here) and diff modulo counters, as `target/boot-f-evidence/f9b.sh` does.
* Do not move `journal put, 191 bytes, one slot` on the strength of the 1.12 sweep: it reads 7.4 / 7.6 / 7.4
  against 7.4 in the three `--strict` runs, inside [6.7, 8.1], and the residue is item 101's question.
* Do not rebuild the bench binaries during a measuring boot except as ADR-0096 decision 3 says
  (once, cores 0–5, named in the plan) — a rebuild is a new layout for every case.
* Do not read `--strict`'s `FAIL:` line as the whole verdict: read the over, under and no-baseline counts.
* Do not read a `--call-graph dwarf` profile of an LLD-linked bench binary without the `--symfs`
  fix: children ≡ self for every symbol, no `_start`, and a report that finishes in seconds is the tell
  ([reference](docs/reference/perf-dwarf-unwind-fails-on-an-lld-linked-pie.md)).

### Not proven

* **Item 99's fix is flat by its own bound**: the post-fix sweep is 1.12, not ≤ 1.10.
* **Item 101's mechanism** (heap position or harness code layout).
* **Item 51's per-mitigation split** — decided, not measured. The flush arm is one machine, one
  boot, one case; whether conntrack still ran with the ruleset flushed was not read.
* **The wakeup lines are n = 19**, not 20.
* **What moved item 85's ten arms on 2026-09-15** — no candidate left.
* **Item 96's "≥"**: how much of `bad_group_count`'s inlined loop belongs to the descent; and
  ADR-0094's stated reason for the bound is wrong (note above).
* **Which commit of PR B** carries item 95's step (unchanged).

---

## Start here — 2026-09-23: phase 2 ran on the §9 line — items 93, 95, 97 closed, the baselines re-recorded once, and three things the boot found that the plan did not foresee
...
* **Item 93** — the fix is confirmed on `main`: `engine turn, 1 busy sessions` 1747.8 ns (n = 21)
  against 1799.1 at boot D, every engine-turn case back inside the 2026-09-05 band.
* **Item 95** — bisected at merge granularity, n = 21 per arm: the whole +14 % step of
  `validate Heartbeat` (166.8 → 190.8) lies between `wa` and `b1`, i.e. inside PR B; `b1…m` flat.
  The commit inside PR B is not named.
* **Item 97** — `--strict` observed red on the old lines, then `benches/baselines.tsv` re-recorded
  from `ms` n = 21 (15 lines moved, +5.2…+8.0 % on the turn, +14…16 % on `validate`; 2 FIXT and
  4 SBE lines added). Ledger in ADR-0095 *Consequences* and [measured-costs](docs/reference/measured-costs.md) *Boot E*.

### What the boot found

* **Item 99 (new)** — `journal put, 191 bytes, one slot` 7.4 → 12.4 ns between two strict runs
  of one pinned binary in one boot; reproducible, its two sibling cases unmoved. Not re-recorded.
* **Item 100 (new)** — `bench.sh --strict` cannot be green on the desk: the two `wakeup` p50
  cases have no baseline mechanism by construction. Red since 2026-09-18; architect decision.
* **Item 96 (still open)** — ADR-0094's experiment ran: the descent's own code is 2 455.6 ns
  (3.0 % of TCR), a lower bound; the inclusive share needs `--call-graph dwarf` with debuginfo.
  Architect.

### Next — the first executable action

Read items 99, 100 and 96 and hand the three to the architect in one brief; none needs the
desk before a decision. If the desk is booted for item 99: run the `main-tree` `journal`
binary from `../fb-s9e/MANIFEST.txt` **before** any `perf record`, then after one.

### Do not

* Do not move `journal put, 191 bytes, one slot` or the `wakeup` lines to make `--strict` green.
* Do not run a rotation while the manager session is active in tool calls: each call put a node
  hook at 30 % for a second and the driver disqualified the arm (4 rounds lost in S1).
* Do not `perf report` without `DEBUGINFOD_URLS=` and `-f` ([reference](docs/reference/perf-report-hangs-on-debuginfod-and-refuses-a-root-owned-record.md)).
* Do not read `--strict`'s `FAIL:` line as the whole verdict: it prints the first fatal count.

### Not proven

* ~~**Item 96's inclusive cost** (above).~~ — ≥ 7 021.6 ns, boot F (*Start here* above). ~~**Item 99's cause.**~~ — named 2026-09-23,
  boot F (*Start here* above). **Which commit of PR B** carries item 95's step.
* ~~**The desk is left on the §9 grub line**~~ — boot F used it; F10 returns the desk to the desktop
  line (`grub.fixbolt-desktop-20260922`) before powering off.
...
* ~~**Four of the five segments of item 93.** Recorded at boot D, not analysed. `perf diff` over
  `target/boot-d-evidence/d2-<sha>-{1,2}.data` still wants running, on any grub line.~~ — **struck 2026-09-22, later:** all four have a verdict (P2 of the closing plan, `f49b8c9`); item 93 row.
...
* ~~**The macOS reversal for ADR-0091 decision 2.** Unchanged, and the Mac mini now owes a second
  ...
  run `scripts/check-machine-verdicts.sh` to see it.~~ — **struck 2026-09-22, later:** ran on the Mac mini, `27`/`12 red in 50` at `bd6be07`, `0 red in 50` on `main` (`fb0230b`, `docs/CONFORMANCE.md` §9); `check-machine-verdicts.sh` on macOS read `pass 62 fail 1` and exposed `find -printf`, fixed in `a2011ca`, now `pass 63 fail 0` on both.
* ~~**The `no timer due` row's `FAIL` and `PASS` shapes have never been seen on a real systemd
  ...
  shapes are proven against a fixed JSON fixture with a fixed `now`, and once against a stub.~~ — **struck 2026-09-22, later:** both shapes seen live on the desk (`f49b8c9`, reference page *Guarded by*).
...
`FIXBOLT_NIC=enp9s0 scripts/check-machine.sh` reading `pass 16 fail 0 unknown 0` and mitigations
IN FORCE. Nothing was compiled during the boot**: every arm is a worktree built before the reboot,
re-hashed against `MANIFEST.txt` before use, and all 17 binaries hashed the same after 20 rounds as
before round 1. Full numbers: [measured-costs.md](docs/reference/measured-costs.md) *Boot D*.

### What closed

* **Phase-2 PR A closes on its own rule — the A-desk band ADR-0082 left empty is measured.**
  `w0` `ece17e7` → `wa` `6fbe851`, **22 cases of 22 in band**, n = 20 per arm. And the price is
  now on the record rather than absent from it: **the engine turn is slower — +4.14…+5.11% by session
  count, +3.56…+4.25% by ring size, +2.16% on the admin turn** — while `parse`/`serialize` move
  **≤ 0.35%** (what ADR-0079 decision 5 asks for; the largest is `encode ExecutionReport
  (template)` at −0.31%, a `serialize.rs` case) and `validate` is **0.5–5.9% faster**. In band is not free; it is in band. `DESIGN.md` D16
  carries one sentence and points at the numbers.
* **Item 52 closes** (boot D step D3). `journal put, 191 bytes, one slot` reads **median 7.4,
  max/median 1.0946** over n = 20 — seventeen runs of 7.4, then 7.5, 8.0, 8.1 — so the ladder's
  floor 1.10 fits and the odd `6.3 / 1.35 / n = 8` row is retired. It could only be measured
  because ADR-0067 made the table read at run time: 8.2, 6.3 and 7.4 are one case on one desk,
  differing only in whether the binary contained its own baseline. Same step recorded the
  **first baseline for `serialize Heartbeat (session)`**: 847.0 / 1.10 / n = 20, where every run
  had printed `NO BASELINE` — neither red nor green.
* **Item 93 segment (3) has a cause and a branch that fixes it.** `w1` → `w2`
  (`ab/parse-utc-fast-path`): `engine turn, 1 busy sessions` **33.1 ns faster**, against a rule
  declared before the boot that ≥ 25 ns confirms and ≤ 10 ns refutes. Both control cases moved
  ≤ 1.1%. The other four segments are **recorded and not analysed**.
* **Item 89 is profiled** (step D5): N = 1 → p50 16 250 ns, N = 16 → 16 351 ns, a gap of
  **101 ns (0.62%)** against boot C's ~1.9%; `voluntary 0` and `allocs 0` on every run.

### What the boot found that nobody was looking for

**Eight medians, over five recorded baseline lines, are breached on `main` on the §9 desk** — open item **97**, and `scripts/bench.sh --strict` would be red there. This is not an
arm-against-arm comparison; it is each arm against `benches/baselines.tsv`. The harness **panicked
on every qualifying round** of both arms carrying `main` `76e53cb`: `w1s`'s `validate` **20 of
20**, `w1`'s `validate` **21 of 21** and `w1`'s `density` **21 of 21**. Medians against their own
recorded lines: `validate NewOrderSingle` **×1.160**, `validate Heartbeat` **×1.154**, `validate
TestRequest, w2w bytes` **×1.136** (`w1s`); `engine turn, 1 busy, ring 4096` **×1.122** and
`ring 64` **×1.114** (`w1`). Nothing read under its floor. The `--summary` tables are blind to this
by construction — two arms can agree with each other while both sit over the line — and the first
write-up of this boot dismissed the harness's own report as a parser artefact. It is not.

### What produced an item instead of a number

* **Item 95** — `validate Heartbeat` is **+12.32%** on `main` against PR A's tree, outside the
  1.10 band, with three more session-dictionary cases at +5.0…+9.4%. **The engine turn did not
  move** across the same span, so this is the dictionary pass. Merged work, no band was promised,
  no bisect run, no cause claimed.
* **Item 96** — the ADR-0086 descent reads **−10.83%** when removed, but **three of four
  group-free control cases moved 2.9–5.8%** where the design required ≤ 2%. The magnitude is
  known; the attribution is not. Designing an experiment that prices a descent without building
  a second binary is the architect's.

### What the boot cost, and the two traps it paid for

* **`sudo` has no `cargo` on its `secure_path`**, so `sudo perf record -- cargo bench` failed in
  one second, wrote a 15 KB header-only `.data`, and **exited 0**. Five runs "succeeded" that way
  before the log was read. The driver now runs the manifest-pinned binary by absolute path —
  which is what "the boot compiles nothing" asked for anyway.
  [perf-record-exits-zero-when-sudo-cannot-find-the-workload](docs/reference/perf-record-exits-zero-when-sudo-cannot-find-the-workload.md).
* **`apt-daily-upgrade.timer` fired at 06:51** and cost rounds 13–20; every arm read busy 10–13%
  and was disqualified, so nothing wrong was published. `check-machine.sh`'s quiet row measures
  one second, **now**, and says nothing about what systemd will start in four hours. The rounds
...
post-boot analysis needs all of it**: `d2-*.data`/`.stat` (item 93's seven bisect points),
...
four unexplained segments of item 93**, on the desktop grub line — it needs no §9 boot, only the
files and the binaries still in `../fb-boot-d/e1`…`e7`. Then `perf annotate` the largest symbol
of each. Segment (3) needs no profile; it needs `ab/parse-utc-fast-path` turned into a PR.

### Do not

* Do not merge `ab/parse-utc-fast-path` or `ab/validate-no-descent` as they stand. The first
  becomes a PR with its own plan and tests; the second is deliberately broken.
...
* **Four of the five segments of item 93.** Recorded, not analysed.
* **What caused item 95's +12.32%.**
* **What the ADR-0086 descent costs** — the experiment is designed (ADR-0094), not run.
* **The macOS reversal for ADR-0091 decision 2.** Unchanged by this boot: the Mac mini's checkout
  is at an old `main` and has no `wire_fixt` binary.
* **Anything about `standard` mode, TLS or the wire.** Boot D was loopback benches and one `w2w`
  arm; ADR-0013 decision 4 applies as ever.
...
**Read this before trusting `373=16` on a message with a nested group, or assuming `35=n` still
draws the `1128` reject.**

[the-group-count-pass-and-is-admin](docs/plans/2026-09-20-the-group-count-pass-and-is-admin.md)
and [ADR-0086](docs/decisions/ADR-0086-a-group-count-is-asked-at-every-depth-admin-is-two-questions-with-two-names-and-xmlnonfix-is-not-asked-the-appl-ver-id-rule.md)
close two items this page carried as *Not proven* since 2026-09-19 — struck through in place
below. Branch `plan/group-count-pass-and-is-admin`, five commits on top of `main`: `585e11c`
(ADR-0086), `c8709a1` (step 1, the two `?` → `let … else { continue; }`), `438228a` (step 3,
`Tables::is_admin` generated from `msgcat`), `8e81aae` (step 2, the descent through nested
groups and `MAX_GROUP_NESTING`), `6e84ef1` (step 4, `ADMIN` renamed `SESSION_OWNED`,
`out_of_family_appl_ver_id` asks `is_admin`). **Not yet merged** — the manager's gate re-run,
the two named reversals and the CI run id for the closing commit are owed at close
(`CLAUDE.md` §12). What each step measured is in its own commit body (`git log`), not repeated
here.

### What closed

* **A lying counter after, or inside, a nested group is rejected again.** `bad_group_count`'s
  two `?` on `Option` — which meant "no fault" on `None` — are `let … else { continue; }`; the
  descent through `GroupEntry::group` asks `373=16` at every level, depth-first, wire order,
  down to `MAX_GROUP_NESTING = 8`. Measured: FIX 4.4 nests 4 deep, the FIXT pair 7 — one level
  of headroom, folded from `GROUP_KEYS` on every run. `docs/SESSION-BEHAVIOUR.md` §3b.
* **`Tables::is_admin` exists**, generated in `build.rs` from `msgcat`, and answers the `1128`
  exemption at `out_of_family_appl_ver_id`. `35=n` is now exempt from ADR-0080 decision 3's
  `1128` rule, matching the transport treatment `is_defined_tag_for` already gave it. The
  seven-entry routing list is renamed `SESSION_OWNED`; its behaviour — `35=n` still reaches
  `Application::on_message` and is still journalled — is unchanged, pinned by
  `xmlnonfix_still_reaches_the_application`. `docs/SESSION-BEHAVIOUR.md` §5b.

### Not proven, added by this work
...
* **`the_sixty_fix50sp2_definitions_pass_through_a_real_socket` was load-dependent on macOS,
  ...
  **0 red, 0 debt events in 122 runs** (`3233032`). The mechanism is ADR-0087's — a `Tick`
  ...
  [ADR-0091](docs/decisions/ADR-0091-the-socket-harness-race-is-the-loopback-stacks-not-the-schedulers-and-its-reversal-runs-on-macos.md).
...
* **`[2026-09-22, boot D step D4]` MEASURED ON THE §9 DESK AND STILL NOT ATTRIBUTABLE — now
  open item 96.** `w1s` `76e53cb` → `w3` `d6f79dc`, both `fix50sp2`, n = 20: **82 071.1 →
  73 182.8 ns, −8 888.3 (−10.83%)**. The design required four group-free control cases to agree
  within 2% before that could be called the descent's cost; three moved 2.9–5.8%, so part of it
  is the other binary's layout. The magnitude on this desk is known; the attribution is not, and
  what experiment would isolate it goes to the architect. Below is what stood before that run.
...
* ~~**`build.rs`'s `die()` on a `<message>` with no `msgcat` was never exercised.** `vendor/` is
  ...
  an observed red.~~ — **CLOSED**, commit `61416be`: `scripts/check-dict-refuses-a-message-without-msgcat.sh`
  damages three copies of the XML under `target/check-msgcat/` and proves all three `die` arms
  fire, by reversal — `None => die(...)` at `build.rs:802` turned into `None => {}` goes red as
  predicted, then is restored byte-identical. It now runs as a step in the `gates` CI job on
  every commit.

---

## Start here — 2026-09-20: the internals set is complete, and the two session defects have an approved plan

PR [#85](https://github.com/tmthang86/fixbolt/pull/85), branch `docs/internals-sbe-and-rule-rows`,
code commit `403ee72`, CI run [`35478647410`](https://github.com/tmthang86/fixbolt/actions/runs/35478647410), **14 jobs of 14**.

### What closed

* **D1b** of [phase-2-fixt-and-sbe](docs/plans/2026-09-19-phase-2-fixt-and-sbe.md):
  `docs/internals/sbe.md`, and six of the seven older pages re-walked. One was wrong rather than
  incomplete — `session.md` still described `Session<Role>`.
* The three `CLAUDE.md` rows recorded under *Not proven* below as fenced, struck there.
* `scripts/check-adr-numbers.sh`, in the `docs` job. Reversal on a copy: a second `ADR-0082` goes
  red naming both files, exit 1.

### In flight

[the-group-count-pass-and-is-admin](https://github.com/tmthang86/fixbolt/pull/86) — approved by
the owner 2026-09-20, PR [#86](https://github.com/tmthang86/fixbolt/pull/86), branch
`plan/group-count-pass-and-is-admin`. It closes the `bad_group_count` and `is_admin` bullets below.
Its delivery log is the state; this section is not.

### Not proven

* `check-adr-numbers.sh` reads one tree. Two **unmerged** branches taking one number is still
  invisible until the second merges; the manual loop in
  [the reference](docs/reference/two-branches-can-take-the-same-adr-number-without-a-conflict.md)
  is the only check for that window.
* `shellcheck` was not run on the new script; the desk that wrote it does not have it.
* **ADR-0080 is still `Proposed`** though built and merged. Found while writing ADR-0086. Its
  status is the owner's to change.

---

## Start here — 2026-09-19, later: PR B is FIXT 1.1 / FIX 5.0 SP2, and **nothing in it was measured for latency**
...
**No latency figure in this PR comes from a §9 machine, because none was taken.** The cloud box
is an `Intel(R) Xeon(R) Processor @ 2.10GHz`; `benches/baselines.tsv` records only a Ryzen, so
every timing case prints `NO BASELINE` and the harness returns **no band verdict at all**. Nothing
was published, and nothing should be inferred from the numbers that appear in commit messages.

Two bands are owed to the desk and neither can be resolved here:

* ~~**A-desk**, PR A's ADR-0031 band — still `**NOT RUN**`, unchanged by this PR.~~ — **RUN 2026-09-22**, boot D step D4: **22 cases of 22 in band**, n = 20 per arm; the engine turn is +4.14…+5.11% by session count, +3.56…+4.25% by ring size and +2.16% on the admin turn, while `parse`/`serialize` move ≤ 0.35%. PR A closes on its own rule.
* **B4b's validate band.** `validate NewOrderSingle` 877.5 → 906.2 ns (+3.3 %) and
  `validate Heartbeat` 140.7 → 166.2 ns (+18.1 %) after the group-member ordering rule. The
  question for the desk is **not** "is the rule too expensive" but "is any of this real": the same
  unchanged code measured 158.4 ns and then 166.2 ns in two batches on this box, and ADR-0049
  records this bench moving 239.1 → 280.4 ns on a commit touching no `crates/*/src` file. One run
  there gives a verdict against the recorded baselines (NewOrderSingle 802.0–970.3, Heartbeat
  154.1–186.5) instead of two medians that cannot be separated.
* **The FIXT table's own cost is unmeasured.** The pair table is 25.6× the generated bytes of
  FIX 4.4's. Whether validating through it costs anything is unknown.

### Not proven

* **Two FIXT boundary behaviours have no oracle** — what a counterparty's `1137` is used for after
  logon, and `1128` on a session-level message. The 180 `.def` files are FIX 4.4 tests replayed
  against FIXT dictionaries; none exercises either. Both are this engine's reading of the
  specification. [SESSION-BEHAVIOUR.md §5b](docs/SESSION-BEHAVIOUR.md).
* **179 / 180, not 180 / 180, and it is not a defect.** `21_RepeatingGroupSpecifierWithValueOfZero.def`
  in the `fix50` corpus cannot pass against an SP2 table: tag 336 carries 0 enumerated values in
  `FIX50.xml` and 7 in `FIX50SP2.xml` (ADR-0084 decision 3). The divergence is asserted by content
  — corpus, file, line, and the engine's actual wire bytes — with **no fixture edited and no
  exclusion list**. The assertion pins the **strict** direction only; a case where the SP2 table is
  more permissive would score green and be invisible.
* ~~**`is_admin` was never built.**~~ **Closed 2026-09-20 by
  ...
  decision 2 and commit `438228a`.** `Tables::is_admin(msg_type: &[u8]) -> bool` is generated in
  `crates/dict/build.rs` from each `<message>`'s `msgcat`, no default method, implemented for
  both `Fix44` and the FIXT pair.
  `crates/dict/tests/fixt.rs::the_transport_files_admin_set_is_the_tables_admin_set` and
  `crates/dict/tests/tables.rs::fix44_calls_xmlnonfix_admin` hold it.
* **A doctest behind a feature still runs nowhere.** `check-feature-gated-tests-ran.sh` is given
  `--tests`. There is no `fix50sp2` doctest today.
* **[closed 2026-09-20, PR #85]** **`CLAUDE.md` §2's *Machine checks* row 3 and §7's "Any session-layer change" row still say 59.**
...
* ~~**No guard against two branches taking the same ADR number.**~~ **Closed 2026-09-20 by `scripts/check-adr-numbers.sh`, PR #85** — for merged trees only; see the newest *Start here*.
...
* ~~**`bad_group_count` abandons the whole `373=16` pass on a nested group, silently.**~~
  **Closed 2026-09-20 by commits `c8709a1` and `8e81aae`, under
  [ADR-0086](docs/decisions/ADR-0086-a-group-count-is-asked-at-every-depth-admin-is-two-questions-with-two-names-and-xmlnonfix-is-not-asked-the-appl-ver-id-rule.md)
  decision 1.** The two `?` on `Option` are `let … else { continue; }`, and every entry a group
  opens is walked down through `GroupEntry::group`, depth-first (wire order), to
...
* ~~**Two answers to "is this a session message?" in one validate pass.**~~ **Closed
  ...
  `SESSION_OWNED`, the same seven entries — answers a different question and is no longer read
  at that call site. `35=n` is now exempt from the `1128` rule; routing is unchanged, pinned by
  `tests/fixt.rs::xmlnonfix_still_reaches_the_application`.
* **[closed 2026-09-20, PR #85]** **`CLAUDE.md` §4's file table has no row for `docs/internals/`**, which ships in this PR. Fenced
...
`grep fix50sp2 .github/workflows/ci.yml` matched **nothing** before it. Five FIXT test binaries
had never executed in CI on any commit. The `gates` job now runs the four crates that declare the
feature and proves through `scripts/check-feature-gated-tests-ran.sh` that the **named tests ran**,
rather than that a command exited 0. Separately, the ADR-0049 alignment read-back was certifying
binaries nobody measured — 17 of 18 matched, one never had — and now matches 18 of 18.

---

## Start here — 2026-09-19, night: PR A is on `main`, and its latency band was never measured

**This is the one thing to read before trusting any timing claim about the tag=value path.**
PR A made `Session`, `Connection` and `Engine` generic over an `Encoding`. That is a hot-path
refactor. The plan gives PR A three closing criteria and **only two were met**:

| PR A's criterion | |
|---|---|
| 59 / 59 in process and over a socket, no fixture edited | met |
| both mode scripts green (non-negotiable 4 re-walked) | met |
| **A-desk: the ADR-0031 band for `parse serialize alloc validate density`, n = 20 each side, two worktrees alternating, on a §9 boot with mitigations in force** | **met 2026-09-22** — boot D step D4, 22 of 22 in band, `pass 16 fail 0 unknown 0`; run as one rotation around a shared control rather than two alternating worktrees (ADR-0090), same shape |

The plan's own words for that third row are *"Lệch band là dừng, không phải ghi chú"* — out of
band is a stop, not a note. It was not run because no session with the §9 desk was available;
the cloud session that merged this has no such machine, and a number from a cloud box quoted as
a desk number is the failure `CLAUDE.md` §10 names by hand.

**The owner was told this before the merge and decided to merge anyway, asking for it to be
written down here.** That is a decision, not an oversight, and this section is where it lives.
What follows from it:

- **No latency claim about the tag=value path may cite anything after `ece17e7` until A-desk
  runs.** The alloc counts are zero (17/17 in `session`, 31/31 in `engine`) and say nothing
  about time.
- **A-desk is still owed, on `main` now rather than on a branch.** If it comes back out of band,
  the fix lands after the fact instead of before, and `DESIGN.md` §8's budget rows are the
  first thing to re-read.
- ADR-0031's band is the comparison; `docs/reference/measured-costs.md` holds what it is
  compared against.

**Where.** `main` carries PR [#79](https://github.com/tmthang86/fixbolt/pull/79). Its last
commit is `5a9ff90`, CI run [`35435156331`](https://github.com/tmthang86/fixbolt/actions/runs/35435156331),
14 jobs of 14.

### What closed

- **ADR-0082 → `Accepted`** (`5a9ff90`), under the owner's delegation of the decision itself
  ("uỷ quyền toàn bộ cho bạn"). Its *Deciders* line records that the owner did not read it and
  who did weigh it. Before accepting, the two load-bearing code claims were checked rather than
  taken from the argument: the five type equalities at `crates/session/src/lib.rs:1487-1493`
  and the seven skeletons `out.rs` builds (msgtypes `0 1 2 3 4 5 A`). The three documents that
  cited it as `Proposed` — `DESIGN.md` D16, `GUIDE.md` 3a, `CHANGELOG.md` — were cleared in the
  same commit.
- PR A's rustdoc defect (`c2df98f`): a link to `serve_with`, which exists only under `standard`.
  It had reddened the cloud branch's `feature-sets` job on six consecutive heads.

### Next — the first executable action

1. **A-desk, on the §9 desk**, exactly as the row specifies. It is now a measurement of `main`,
   not of a branch.
2. The cloud track continues on `feat/phase-2-b` (PR [#82](https://github.com/tmthang86/fixbolt/pull/82)):
   B1, B2, B3 and D1a are delivered and green (run
   [`35430710585`](https://github.com/tmthang86/fixbolt/actions/runs/35430710585), 14 of 14);
   B4 onwards is in flight.

### Do not

- Do not publish, quote or budget from any tag=value latency figure measured after `ece17e7`
  until A-desk has run. This is the whole point of the section.
- Do not treat PR A's green CI as the band. CI proves the gates, not the timing.
- Do not publish any figure from the `mitigations=off` boot; every one is labelled *A/B, not published*.

### Not proven

- **A-desk has not run**, and PR A is now on `main` regardless. This is the only entry here that
  a merge made worse rather than better.
- The wire p99.9 (27–42 % between procedures); the interval where the I211 stops skipping.
- Item 93's five mechanisms; ADR-0069's admin +1.9 %; ADR-0070's superadditivity.


## Start here — 2026-09-19, evening: phase 2 PR A is built and green, and it waits on one approval
...
| Step | Commit | What exists now |
|---|---|---|
| item 51, last arm | merged, PR [#81](https://github.com/tmthang86/fixbolt/pull/81), `main` `55c28d8`, run [`35418404496`](https://github.com/tmthang86/fixbolt/actions/runs/35418404496) | `mitigations=off` takes **58.4 %** off the 8-byte loopback round trip and **55.9 ‖ 55.5 %** off `w2w` `hft` admin p50 — A/B, not published. Suspect 2 holds the order of item 51's whole gap |
| plan, *Sửa* 1 | `3b503d5` | phase 2 runs on three tracks (desk / cloud / Mac); dependencies corrected against the files each step touches; `sbe` + `sbe-gen` are one §7 step, the §1 exception the owner approved |
| **A1** | `44df719` | `codec::encoding`: `trait Encoding`, `SessionFields`, `TagValue<D, N>`, every method `#[inline]`; `parse via Encoding` reads 0 allocations, proven by reversal |
| — | `6f82cf3` | the `parse` alloc case asserts its own path is live; the 2026-09-05 checksum trap is inventoried across all four benches that hold the literal |
| plan, *Sửa* 2 | `08da39f` | the PR A sketch describes the trait as built; `where E::Dict: Tables` moved from the trait to `Session`, where it can be written |
| **A2 + A3** | `9354f58`, `9f0b58c` | `Session<E: Encoding, R, APP>`; `dict::Tables` + `impl for Fix44`; 25 `Fix44::` sites converted; `Connection`/`Engine`/six aliases take a trailing `E` with a default; **`serve*` signatures unchanged**, so `examples/acceptor.rs` is byte-identical |
| ADR-0082 | `d690698` | **Proposed** — the session is generic over tag=value, and the boundary to SBE is the session, not the trait |
| **A4** | `a0d2e23` | `DESIGN.md` D16, §3, §6, §7 step 9; `GUIDE.md` 3a; `CHANGELOG.md` |
| — | `6fb329c`, `c2df98f` | the `interop.sh` report refuted and recorded; the rustdoc link that only resolves with `standard` on |

Gates re-run by the manager on the tree that became each commit: `cargo test --all` and
`cargo test --no-default-features` 107 suites each; **59 / 59 in process and over a socket, both
modes, no fixture edited**; `session` alloc 17 / 17 = 0, `engine` 31 / 31 = 0;
`check-no-kernel-sleep.sh` and `check-standard-gives-the-core-back.sh` exit 0 with their RED
halves tripping; `interop: PASS 7/7 · interop-acceptor: PASS 7/7`; indexing ratchet 181,
unchanged; `check-links.py` clean.

### What the work found

- **`git add -A crates/` is not the diff.** A3 also changed `tools/interop/src/main.rs`; `9354f58`
  was committed without it and **does not build** (`cargo check -p fixbolt-interop`, 3 errors),
  although every gate was green on the tree it was cut from. Stage from `git status`, not from a
  guess about where a step lives.
- **`--all-features` cannot see a feature-gated doc link.** `[`serve_with`]` resolves only with
  `standard` on; CI's powerset job is the only one of the fourteen that documents a crate with its
  features off. All seven combinations of `engine`'s features now document clean locally.
- **A report is a claim.** `scripts/interop.sh` was reported to print `cmake is required` and exit
  0; it has carried `exit 1` since 2026-09-02, and what returned 0 was a pipeline without
  `pipefail` in the caller. Refuted, recorded, nothing fixed.
- **The trait could not carry the session's needs.** `Session` holds five type equalities beyond
  `E::Dict: Tables`, because it walks a view in wire order and builds seven skeletons. That is
  ADR-0082, and it settles row C4: `SbeTables<S>` implements `codec::Dictionary` only.

### Next — the first executable action (a fresh session)

1. **The owner approves or rejects ADR-0082**, and with it the plan's *Sửa* 2 and 3. Nothing else
   in PR A waits on anything. On approval: flip ADR-0082 to `Accepted` and edit the three places
   the plan's delivery log names (D16, `GUIDE.md` 3a, `CHANGELOG.md`), one commit.
2. **A-desk**, the one step of PR A still unbuilt: the ADR-0031 band for `parse serialize alloc
   validate density`, n = 20 each side, two worktrees alternating, **on a §9 boot with mitigations
   in force** — not the desktop line the desk is on now, and not `mitigations=off`. Out of band is
   a stop, not a note.
3. Then PR A merges and the cloud track starts: `feat/phase-2-b` off `plan/phase-2-a`, steps B1
   (minus `impl Tables`), B2, B3 (minus `echo.rs`), D1a. The brief is the plan's *Chạy song song*.

### Do not

- Do not publish any figure from the `mitigations=off` boot; every one is labelled *A/B, not published*.
- Do not flip ADR-0082 to `Accepted` without the owner — three documents cite it as `Proposed`.
- Do not start B4–B7, B1's `impl Tables`, B3's `echo.rs` or C4 before PR A is on `main`.
- Do not delete `fixbolt-direct`: the Mac track and C-desk still need the direct link.

### Not proven

- **A-desk has not run**: nothing says PR A did not cost the tag=value path latency. The alloc
  counts are zero and the timing band is unmeasured.
- The wire p99.9 (27–42 % between procedures); the interval where the I211 stops skipping.
- Item 93's five mechanisms; ADR-0069's admin +1.9 %; ADR-0070's superadditivity.

### The machine

The grub line is back to the desktop backup (`grub.fixbolt-backup-20260914-071641`),
`fixbolt-machine off` has run, `rx-usecs` is 3, and the desk was **shut down** rather than
rebooted, at the owner's instruction — it comes up on the desktop line. `fixbolt-direct` was
deliberately **kept**. `/etc/sudoers.d/fixbolt-all` is still installed; `sudo rm
/etc/sudoers.d/fixbolt-all` ends it whenever the remote stretch does.

## Start here — 2026-09-19, later: the NIC row is met, the slowdown has a first commit, and the desk is about to reboot with `mitigations=off`

**Where.** Branch `fix/missing-stamp-run-is-disqualified`, PR [#80](https://github.com/tmthang86/fixbolt/pull/80),
off `main` at `ece17e7` (PR #78's merge). Code commit `ff2933d`, CI run
[`35416559845`](https://github.com/tmthang86/fixbolt/actions/runs/35416559845). The phase-2 plan
`docs/plans/2026-09-19-phase-2-fixt-and-sbe.md` with ADR-0080/0081 (on that branch only) is on
`plan/phase-2-a`, PR [#79](https://github.com/tmthang86/fixbolt/pull/79), draft, no code yet.

### What closed

| Step | Commit | What exists now |
|---|---|---|
| C-91b | `f81e8a4` | `git bisect`: `588b350` (52= at every precision) is the first commit over the 5 % line, ~+33 ns; the 9 % is five steps of 2–3 % — item 91 closed, **item 93** opened with the five segments; baselines held |
| ADR-0071 rev. | `ecfdd81` | a run over the 0.1 % TX-stamp ceiling is disqualified, not fatal; fewer than half of RUNS qualifying fails the arm |
| C-40 | `ff2933d` | **the NIC-to-NIC row is MET at p50 and p99**, `hft`, back to back: admin 27 050 ‖ 27 114 ns, app 28 894 ‖ 28 878 ns; p99.9 marked; sweep 0–50 µs flat and the I211 keeps skipping stamps; item 40 closed |
| comparator | `ff2933d` | `compare-w2w-procedures.sh` reads the wire rows and keys interval arms |

### What the work found

- **A rebuilt `w2w` has no file capabilities** — `setcap` after every build (hft-playbook §6); C-40's first start died on `AF_PACKET`.
- **A per-run rule must cost one run**: the 0.1 % ceiling as first written aborted three procedures on their fifth run.
- **The comparator compared the wrong rows** on a wire summary — fixed, with a test.
- **`pkill -f <pattern>` kills the shell that typed the pattern.** Anchor with `^`, or kill by pid.

### Next — the first executable action (a fresh session, after the reboot)

The grub line now carries `mitigations=off` (`/etc/default/grub`, `update-grub` run); the owner reboots.
1. `cat /proc/cmdline` must show `mitigations=off`; B0 runtime again: `sudo -n /usr/local/sbin/fixbolt-machine on`;
   `sudo -n ethtool -C enp9s0 rx-usecs 0`; `sudo -n ethtool --set-eee enp9s0 eee off`; `4` into
   `/proc/irq/{85..89}/smp_affinity_list`; `FIXBOLT_NIC=enp9s0 scripts/check-machine.sh` **must FAIL the
   mitigations row** (pass 15 fail 1) — the free reversal of ADR-0023. Throw the first bench run away.
2. **Item 51's last arm, A/B, not published**: `cargo bench -p fixbolt-engine --bench payload` ×5, read
   `TCP loopback, 8 in 8 out` (boot B: 12 594.4 / 12 597.8 ns); then `RUNS=10 ARMS="hft:admin"
   scripts/w2w-baseline.sh` twice (boot B: p50 18 249 / 18 179, bimodal). Both in a branch off `main`,
   recorded in `measured-costs.md` beside ADR-0023 as differences, never in §8.
3. **Boot D**: `sudo -n cp /etc/default/grub.fixbolt-backup-20260914-071641 /etc/default/grub && sudo -n update-grub`,
   reboot, `sudo -n /usr/local/sbin/fixbolt-machine off`, `sudo -n nmcli con delete fixbolt-direct`,
   `sudo -n ethtool -C enp9s0 rx-usecs 3`; say in *Start here* that the desk is back on the desktop line;
   offer `sudo rm /etc/sudoers.d/fixbolt-all`.
4. Then **phase 2, PR A** on `plan/phase-2-a` (#79): step A1 (opus) per the plan's *Chia việc*.
5. Item 93's five perf tasks are desk work for a later boot.

### Do not

- Do not publish any figure taken with `mitigations=off`; label every one *A/B, not published*.
- Do not re-record `density`/`validate` baselines (item 93).
- Do not run a wire measurement without `setcap` on the fresh `w2w`.

### Not proven

- The wire p99.9 (27–42 % between procedures); the interval where the I211 stops skipping (between 50 µs and 1 s).
- Items 93's five mechanisms; ADR-0069's admin +1.9 %; ADR-0070's superadditivity.
- ~~The `mitigations=off` arm (this reboot).~~ `[measured 2026-09-19]` run, A/B not published: −58.4% on the 8-byte loopback round trip, −55.9 ‖ −55.5% at w2w `hft` admin p50 — [measured-costs](docs/reference/measured-costs.md), *Item 51's last arm*.

## Start here — 2026-09-19: boot C measured, fourteen items closed by number or decision, and the timing slowdown is in the code

**Where.** Branch `plan/the-second-linux-desk-c`, PR [#78](https://github.com/tmthang86/fixbolt/pull/78),
off `main` at `7188f1b`. **Code commit `7218615`, CI run
[`35405617390`](https://github.com/tmthang86/fixbolt/actions/runs/35405617390), 14 jobs of 14.** Plans:
[closing-the-open-items](docs/plans/2026-09-18-closing-the-open-items.md) (approved by the manager
2026-09-18 under the owner's mandate: approve plans, merge when green, run every item to done),
[polling-the-listener-less-often-than-the-sessions](docs/plans/2026-09-18-polling-the-listener-less-often-than-the-sessions.md)
step 5, and [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md) boot C. Every
figure: the §9 desktop, `FIXBOLT_NIC=enp9s0 scripts/check-machine.sh` → `pass 16 fail 0 unknown 0`,
loopback, two procedures where ADR-0068 applies; raw logs in gitignored `target/boot-c-evidence/`
and `target/w2w-baseline/boot-c-*`. **The desk is still in the §9 boot line with B0's runtime
settings applied** (knobs on, `rx-usecs 0`, EEE off, IRQs on `cpu4`).

### What closed

| Step | Commit | What exists now |
|---|---|---|
| 89 step 5 | `c8acf18`, `cbfa1cb` | `LISTENER_EVERY ∈ {1,16,256}`, two procedures: hft app **−12.7 ‖ −13.0 %** at 16, admin +1.9 ‖ +1.7 % (unexplained, open in ADR-0069); default **16**, ADR-0069 Accepted |
| plan + ADRs | `8653a36` | ADR-0070–0079 close items 14, 22, 24, 36, 40, 55, 84, 86, 87 and PRD §6 rows 1, 3, 4 by decision; all Accepted at `85460c1` |
| 1.1 | `899c3fc` | every document that stated the old positions points at its ADR; "the fastest" retired (ADR-0077) |
| 2.1 | `d6ef765` | tracer-free gate for non-negotiable 4: `engine-ctxt voluntary/involuntary`, `--assert-no-voluntary-switches`, `scripts/check-no-kernel-sleep-by-ctxt.sh`, CI |
| 2.2, 2.2c | `eb04ab4`, `bb920fb` | ADR-0071: a skipped TX stamp is a missing sample under 0.1 %; `--dump` on both halves and the join in `w2w-baseline.sh` |
| 2.3 | `bcd3954` | `scripts/compare-w2w-procedures.sh` — ADR-0068's verdict as a script, tested |
| 3.1, 3.2, 92 | `a2aa9ce`, `8cd2c2f`, `09effd1`, `02edfdb` | the mirrored corpus's ceiling is a classified count: **14 / 14** (was 10 / "45"); item 92 was a harness hole — the corpus's placeholder timestamp in `122=` and `60=` ([trap](docs/reference/a-corpus-placeholder-timestamp-lives-in-three-tags.md)) |
| 4.1, 4.2 | `246e3e0`, `133ea27` | bench cases `serialize Heartbeat (session)` and `benches/wakeup.rs` |
| C-84 prep | `486bdae` | `--client-tls`: the two ends of a combined run may differ |
| review | `85460c1` | 11 findings fixed: the ctxt sample after `--hold-ms`, the assertion fires in any mode, baseline hft arms gated |
| C-PRD7 | `89b3fc1` | `epoll_wait` wake **4 960 ns** p50 (20 runs), `poll` 4 819; ADR-0025 keeps 4, Accepted |
| C-85 | `89b3fc1` | build slot / thermal is **not** item 85's cause (16 005 ‖ 15 990, reproduced) |
| C-84 | `89b3fc1`, `7218615` | four TLS arms at N=1: the engine's own kTLS share is **+351 ‖ +531 ns**; both-kTLS superadditive (ADR-0070 measured) |
| C-91 | `7218615` | **code, not the machine**: `0149b26` in today's boot reads its 2026-09-05 numbers; today's tree is +7–10 % on every engine-turn case |
| C-49 | `7218615` | kernel share of the application path's larger payload **≤ 50 ns** in situ; item 49 closed by decision |
| w2w default | `8c6e5f1` | `w2w` follows `Limits`' listener default (it had its own 1: C-84/C-85/C-49 ran at N=1, re-based) |

### What the work found

- **A resumed worktree agent edits the main tree.** Two developers lost their worktrees when they
  stopped to ask and were resumed; disjoint files saved it. Brief with absolute paths to untracked files.
- **`strace` inflates voluntary context switches** (1 832 in one run) and **`sched_yield` lands on
  the involuntary column** — ADR-0072 Consequences.
- **The instrument's default diverged from the library's** the moment the library's moved (item 89):
  `w2w --listener-every` defaulted to 1 on its own. ADR-0069 Measured.
- **`cargo fmt` and `RUSTDOCFLAGS=-D warnings cargo doc` are CI gates no step brief named** — the
  first push failed both; every brief now names them.
- **A stale `target/` bakes a dead scratchpad path** into the conformance build script's output
  (`/tmp` is tmpfs): `cargo clean -p fixbolt-conformance` fixes it.

### Next — the first executable action

1. **Merge PR #78** once this handoff's run is green (the manager does it under the mandate), then
   on `main`: nothing else is owed by the desk-free plan.
2. **Owner, on the Mac mini**: `sudo xcodebuild -license accept`, then
   `ssh thangtran@192.168.77.2 'cd Projects/nanofixengine && PATH=$HOME/.cargo/bin:$PATH cargo build --release -p fixbolt-w2w'`
   (source is synced by `rsync`; `git` is blocked there until the licence). Then **C-40**: the
   interval-0 NIC pair with `--dump`/join, two procedures, and the interval sweep — plan row C-40.
3. **C-91b, same boot as C-91 if possible**: `git bisect start 85460c1 0149b26`, judge on
   `engine turn, 1 busy sessions` n=5, bad above 1 743 ns — plan row C-91b. Baselines stay until the
   commit is named.
4. **Owner: the `mitigations=off` reboot** (plan *Boot C*, item 51's last arm), then **boot D** back
   to the desktop line (`/etc/default/grub.fixbolt-backup-20260914-071641`), `fixbolt-machine off`,
   `nmcli con delete fixbolt-direct`, `rx-usecs 3`; offer `sudo rm /etc/sudoers.d/fixbolt-all`.
5. Then phase 2 opens with ADR-0078/0079 (architect writes the phase-2 plan).

### Do not

- Do not re-record `density`/`validate` baselines to today's numbers — C-91 says the code regressed.
- Do not run `check-links.py` with agent worktrees under `.claude/worktrees/` — every failure it
  reports there is noise; remove the worktrees first.
- Do not measure while any `cargo` runs on the desk; C-85's first attempt was disqualified by it.

### Not proven

- Why the hft **admin** path is 1.9 % slower at `ListenerEveryTurns=16` (ADR-0069, open).
- Which commit between `0149b26` and `85460c1` costs the 7–10 % (C-91b).
- The mechanism of both-kTLS superadditivity (ADR-0070).
- C-40's interval-0 NIC pair with the missing-stamp rule and the join — not run (Mac).
- ~~The `mitigations=off` arm of item 51 — not run (reboot).~~ `[measured 2026-09-19]` run, A/B not published; see *Item 51's last arm* in [measured-costs](docs/reference/measured-costs.md).

## Start here — 2026-09-18, later: item 89 is built and unmeasured — the listener on a cadence, default 1

**What closed.** `docs/plans/2026-09-18-polling-the-listener-less-often-than-the-sessions.md`
steps 1–4 and 6, branch `plan/listener-poll-share` (worktree `../fixbolt-wt-89`), PR
[#76](https://github.com/tmthang86/fixbolt/pull/76). **Code commit `55a1549`, CI run
[`35324480648`](https://github.com/tmthang86/fixbolt/actions/runs/35324480648), 14 jobs of 14**, after
run [`35323463898`](https://github.com/tmthang86/fixbolt/actions/runs/35323463898) on `27aa90b` failed
only the feature-set rustdoc job on a link to a private constant.
[ADR-0069](docs/decisions/ADR-0069-the-listener-is-polled-on-a-cadence-in-hft.md) is *Proposed*:
`Limits::listener_every` (settings key `ListenerEveryTurns`, default 1) is how many `hft` spin turns
pass between two polls of the listener; a private `ListenerCadence` in `pump` counts, and every
return from `idle_with` resets it so the `standard` half can never spin on a readable listener.
`tools/w2w --listener-every`, `connect-rtt`/`logon-rtt`/`listener-every:` lines and
`LISTENER_EVERY` in `w2w-baseline.sh` are the instrument. Item **89** is narrowed to *measure at
boot C*; **step 5 is not done and nothing was measured** — every gate ran on the Mac and in CI.
Item 90's merge commit `3c58bdf` on `main` is green, run
[`35319278308`](https://github.com/tmthang86/fixbolt/actions/runs/35319278308), 14 jobs of 14.

**What the work found.** Three things no gate caught, one now guarded:
- **Two streams through one pipe do not keep their order.** `check-links.py 2>&1 | tail -1` showed
  the stdout summary and hid the stderr `FAIL`; the handoff commit `73e8f44` went to CI with a
  dead link and failed run `35317731330`. Rule 5 in
  [reading-the-output-you-grepped-for](docs/reference/reading-the-output-you-grepped-for.md); the
  script now flushes stdout before any `FAIL`. A first explanation (a sibling worktree) was
  refuted with `ls` before it was recorded.
- **A pull request that conflicts with `main` gets no `pull_request` run, silently.** Two pushes
  produced no Actions run at all; `gh pr view --json mergeStateStatus` read `DIRTY`. Rebase
  first, then look for the run. No guard: the tell is a push with no run within a minute.
- **`RUSTDOCFLAGS=-D warnings cargo doc` is a CI gate no step brief names.** Second time
  (2026-09-14 was the first). Add it to the gate line of any step that touches a public item.
- Under the plan: the countdown could not be pinned by a socket test (reversal left all four
  green), so the cadence is a pure struct with unit tests; the 59 defs cannot reach `pump`
  because `serve_*` build their own `SystemClock`; no public seam counts idle returns. All in the
  plan's *Nhật ký giao hàng*.

### Next

- **Boot C** (the 2026-09-15 *Next* below) now carries **two** A/B items: item 89's
  `LISTENER_EVERY ∈ {1, 16, 256}` alternating inside one procedure, two procedures (ADR-0068),
  reading `connect-rtt`/`logon-rtt` and the p50/p99 of the message path; and the `mitigations=off`
  arm already planned. ADR-0069 moves to *Accepted* only with those numbers, and the default is set
  in the commit that publishes them (rule 10).
- Without a desk: items **40**, **84**, **85** remain architect work; nothing else is open on the Mac.

## Start here — 2026-09-18: item 90 — a paced run is refused before it can outlive MaxLatency

**What closed.** [w2w-refuses-a-run-longer-than-maxlatency](docs/plans/2026-09-18-w2w-refuses-a-run-longer-than-maxlatency.md),
branch `plan/w2w-maxlatency-guard` off `fd75a87`, PR [#75](https://github.com/tmthang86/fixbolt/pull/75).
**Code commit `8c5047e`, CI run [`35315426980`](https://github.com/tmthang86/fixbolt/actions/runs/35315426980), 14 jobs of 14.**
`tools/w2w` now refuses, before any engine or socket exists, a paced run whose last `52=` would be
older than the counterparty's `MaxLatency`: `paced_run_fits` bounds
`interval × (warmup + messages − 1) + 1000 ms` under `--max-skew-ms`, default
`fixbolt_session::DEFAULT_MAX_SKEW_MS`, read rather than written a second time. `--listen` refuses
the flag like `--interval`. Item **90** closed; the trap file names the function and its four tests.

**What the work found.** The plan wrote its reversal's FAIL sentence down before running it — on
`paced_run_fits(1_000_000, 5, 120, 120_000)` — and that sentence was wrong: 124 s is 4 s past the
bound and refuses whether the margin is there or not. Loosening the bound by one reddened **only**
the `n = 120` arm of `a_paced_run_inside_maxlatency_is_allowed`; the boundary is pinned by the
119/120 pair, not by the far case. A predicted FAIL that does not come true is the reversal doing
its job on the prediction. Recorded in
[a-paced-run-outlived-the-sessions-maxlatency](docs/reference/a-paced-run-outlived-the-sessions-maxlatency.md).
A second, unplanned fix rode along as `c43ad95`: the rustdoc *"QuickFIX's default `MaxLatency`"*
sat on `DEFAULT_APP_SCRATCH`, and `DEFAULT_MAX_SKEW_MS` had none.

**Not proven.** Nothing on Linux, nothing on the wire: every gate ran on the Mac, and the number
the guard protects (120 s) is the documented default, not a measurement. One stated residue:
with `--max-skew-ms` under 1000 the hint says "at most 1 messages" while 1 is refused too — a
degenerate input, the refusal itself right.
...
| Step | Commit | What exists now |
|---|---|---|
| B0 | `9bee2f2` | knobs on, `rx-usecs 0`, IRQs on `cpu4`; a link bounce narrowed the §9 verdict silently — S3 |
| Sửa 3 | `c49ef08`, `f43d7e8` | four blocking questions answered; ADR-0068 accepted |
| B1 | `547c873` | `bench.sh --strict` ×3 green; five baselines at n = 20; D_in = 765.5 ns |
| S3 | `46811f6` | `check-machine.sh` picks the NIC by physical device; `nic` header line; 5 tests |
| S2 | `d5be4d6` | `w2w-baseline.sh` records commit/tree/uptime/binary, keeps every run, two-sided dispersion |
| S1 | `5ca3889` | `--journal file-async` has `Store`'s 4 096 slots (item 88) |
| B2 B5 B8 B4 | — (measurements) | two procedures each, `DESIGN.md` §8 *Boot B* |
| B7 | — | conntrack on `lo` is ~420 ns of a TCP loopback round trip; a bimodal `w2w` candidate |
| B6 | — | first hardware-stamped wire figures (1 s pacing only); EEE A/B +14.6 µs |
| B9 | — | `perf`: `Acceptor::accept` (cumulative) is 49.9%/36.7% of the `hft` engine thread's samples; `accept4` itself 27.7%/18.3% (item 89) |
| Q15 | `0057678` | `check-machine.sh` row `eee`; `DESIGN.md` §9 row |
| deps | `6074c26` | rustls 0.23.45 for RUSTSEC-2026-0285; `tls_key_update`'s constants held, ADR-0063 revision note |
| B10 | `f516761` | senior review, 18 findings verified: `w2w-baseline.sh` keeps a failing run's output, survives an unreachable generator, reads one verdict, refuses `--journal=`; the documents corrected (an A/B is a difference; ten arms, not eight; the interval-0 diagnostics were failing runs) |

### Not proven

- **Any zero-interval loopback figure from boot B as a single number** — nine of the ten arms of
  B2, B5 and B8 failed ADR-0068's 5% rule (item 85); `standard` app `--journal file-async`
  reproduced. The paced arms, nanofix and the Mac's `standard` table reproduced.
- **A back-to-back wire figure** — `igb` skipped a TX stamp within 1–4 runs every time; only the
  admin arm ever ran at interval 0 (item 40).
- **The 1 s wire figure** — 5.2% and 10.1% apart across procedures measured only 10 minutes apart
  (short of ADR-0068's 30-minute rule), and the EEE-off A/B read 39 714 ns an hour later.
- **Why** procedure 1 was slower, why `w2w` went bimodal after the payload bench, what the
  listener poll costs a request, why the suite reads slower than 2026-09-05 (items 85, 51, 89, 91).
- **A guard for the MaxLatency trap** (item 90) — none; and an EEE row reading UNKNOWN still lets
  `unknown ≤ 1` print *§9 satisfied*.
...
Branch `plan/the-second-linux-desk-a`, off `main` at `25e54dc`, draft PR
[#72](https://github.com/tmthang86/fixbolt/pull/72). Plan
[the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md), window A, steps A1–A8 —
PR 1 of the plan's three. CI run
[`34842390918`](https://github.com/tmthang86/fixbolt/actions/runs/34842390918) on `f9abc1a`,
success, 14 jobs of 14 — **the first run of the two user-namespace steps**: the runner refused
`unshare -Urn`, the step's fallback wrote `0` to
`/proc/sys/kernel/apparmor_restrict_unprivileged_userns` (its log prints the `0`), and both arms
went green. The review fixes `970b612` and `6e716a5` came after that run; **the closing commit is
`0861edf`, CI run [`34849545025`](https://github.com/tmthang86/fixbolt/actions/runs/34849545025),
14 jobs of 14**. The run before it, [`34848285886`](https://github.com/tmthang86/fixbolt/actions/runs/34848285886)
on `2ad9bb0`, failed the rustdoc feature-set job on a doc link the F3 fix added: the manager's
per-step gates ran clippy and tests but never `cargo doc` with `-D warnings` (ADR-0066), so only
CI saw it. A next manager adds that command to every step touching a doc comment. **No figure was produced.** Every gate ran on the Linux desk in the §9
boot line with `fixbolt-machine off` and other work loading it, so a timing printed by any of
them is not a result (`CLAUDE.md` §2 rule 10).

### What closed

| Step | Commit | What exists now | Held by |
|---|---|---|---|
| start | `b2b3a57` | plan re-verified on `25e54dc`; A1's ADR is ADR-0067 and the architect writes it | — |
| A5 | `f48f3ae` | `check-machine.sh` judges a NIC: IRQ affinity against the isolated CPUs, `rx-usecs 0`, `irqbalance`, `busy_read`; with no NIC the output is unchanged | IRQ-onto-an-isolated-CPU and `rx-usecs 3` reversals on `enp9s0`; `scripts/check-machine-verdicts.sh` `pass 27 fail 0` (added in `970b612`) |
| A3a | `658b5c6` | `w2w --listen`, `--connect`, `--interval` | `cargo test -p fixbolt-w2w`; refusal and zero-alloc reversals red; `strace` of `--connect --interval 1000`: 0 `nanosleep`/`futex`; no-flag output identical under a padding-aware mask |
| A1 | `bdd673f` | `benches/baselines.tsv` read at run time — [ADR-0067](docs/decisions/ADR-0067-the-baselines-are-read-at-run-time-not-compiled-in.md) | `parse` bench sha256 unchanged after a line was appended; renamed file → exit 1 naming the path; `crates/codec/tests/bench_baselines.rs`, 6 tests (`970b612`) |
| A7 | `a7d5943` | `matthart1983/nanofix` pinned at `0f79bae` in gitignored `vendor/`, an example acceptor, the patches in prose in `measured-costs.md` | `w2w --connect` on `--path admin` and on `--path app` against it, both exit 0 |
| A6 | `dbeb135` | `w2w-baseline.sh`: interval as `ARMS`' fourth field, a split run (`LISTEN`, `GENERATOR_SSH`), one machine record; `WIRE_NIC` split mode added in `970b612` | `bash -x` command lines identical for today's arms; refusals; `shellcheck -S info` |
| A2 | `ed96d9a` | `engine turn, 1 busy, admin` in `crates/engine/benches/density.rs` | the one-Heartbeat-per-turn setup assertion, reversed red; prints `NO BASELINE` until B1 |
| A4 | `607d40f` | `w2w --journal file-async` and `--log file`, each its own engine type; alloc case `journal-async-busy` | `journal-async-busy 0`, a `to_vec()` reversal read 1; with no flag the engine type is `ed96d9a`'s in five arms |
| Sửa 2 | `e83d06b`, `cce8bd4` | three things the plan got wrong about A3b; owner approved Q8–Q11, *"Duyệt theo đề xuất"* | — |
| A3b | `f9abc1a` | `w2w --wire-timestamps --nic --observer-core`: hardware RX stamps from an `AF_PACKET` tap, hardware TX stamps from the error queue of a `dup` of the accepted socket, paired by TCP byte offset; `--mode standard` refused on a non-loopback NIC | `pair::` tests; both rule-4 scripts with `W2W_EXTRA` inside `unshare -Urn`; `aa-exec -p unconfined` → exit 2; CI run `34842390918` |
| A8 | `970b612`, `6e716a5` | the senior review's findings, fixed | each commit body quotes its gates |

The comm-collation trap of A5 ([comm-compares-by-locale-collation-not-number](docs/reference/comm-compares-by-locale-collation-not-number.md))
is **now guarded by an automated test**, `scripts/check-machine-verdicts.sh`, which CI runs; a
reversal to `comm -12` dropped CPUs 14 and 15.

### What the review found

Sixteen findings — seven defects, nine suggestions — each reproduced or read by the manager
before routing; all fixed except F13. The ones that would have cost boot B:
**F1** `w2w-baseline.sh` could not produce B6 at all (no wire mode); **F3** a Ctrl-C left
`enp9s0`'s timestamping on (`tx on, rx-filter all`, observed) — SIGINT/SIGTERM now restore it;
**F4** the observer could `dup` a descriptor the engine had already closed after its timeout
(rule 8) — the handoff gained a `CLAIMED` state; **F8** both rule-4 scripts passed a build that
ignored `--wire-timestamps`; **F9** `baselines.tsv` accepted `nan`, `inf` and non-positive values;
**F10** `Never` had been changed for `--listen`, which moved the no-flag admin judge's code — `Never`
is `25e54dc`'s again, `--listen` has its own `ListenNever`, and the judge is byte-for-byte
`25e54dc`'s (`objdump` diff empty). **F13 is not fixed** and is item **88**.

### Not proven

- ~~**A hardware timestamp read.**~~ `[2026-09-15]` **proven** — boot B6, `hw-rx-missing 0 hw-tx-missing 0` over the cable to a Mac; see the 2026-09-15 *Start here*. Was: `enp9s0` has no carrier; cmsg parsing is proven only on absent
  stamps (on `lo` every stamp is missing). `w2w-baseline.sh`'s `WIRE_NIC` success branch was
  checked against a fake `w2w` only.
- **The `POLLERR` spin was seen only with a software-stamp build that was deleted.** No gate sees
  it; `w2w`'s refusal (`standard_is_refused_on_a_hardware_nic_and_not_on_loopback`) is the only
  guard ([a-transmit-timestamp-wakes-a-blocking-engine](docs/reference/a-transmit-timestamp-wakes-a-blocking-engine.md)).
- **F4's claim race under real concurrency** — single-threaded state-machine tests plus the CAS
  argument. A signal during a hardware run's combined path; a second signal, `kill -9` or a crash
  restores nothing (`docs/hft-playbook.md` §6 item 4 records the config before B6).
- ~~**The macOS build of `--connect`**~~ `[2026-09-15]` **proven** — built on an arm64 Mac mini at `5ca3889` and driven over a real ssh by every B6 run. Was: `cargo check --target x86_64-apple-darwin` only, Mac
  offline; `GENERATOR_SSH` never ran over a real ssh.
- ~~**`DESIGN.md` §8's journal/log row** is still `[unmeasured]`~~ `[2026-09-15]` **measured** at boot B5 (tmpfs).
- **Records written during a `--listen` run were not read mid-run; the mode scripts pass no
  `--journal`/`--log`.** Still true after boot B5: non-negotiable 4's two mode scripts have still
  never run with a journal writer or a log (`W2W_EXTRA` with `--journal`/`--log`), and no record
  was read mid-run.
- **TLS with wire timestamps has no window** — `--listen` refuses `--tls`.
- **The refused-namespace branch has no automated test**: the `aa-exec` reversal was manual
  ([a-traced-process-gets-no-file-capabilities](docs/reference/a-traced-process-gets-no-file-capabilities.md)).
  [a-masked-diff-compared-the-padding](docs/reference/a-masked-diff-compared-the-padding.md) has
  no automated test either.

### Next

- **Boot B.** `/proc/cmdline` read `isolcpus=6,7,14,15 rcu_nocbs=6,7,14,15
  processor.max_cstate=1` at 2026-09-14T20:09+07:00, so B0's grub half may already be done —
  read `/proc/cmdline` before touching grub. Then `sudo -n /usr/local/sbin/fixbolt-machine on`
  and `status`, the rest of the plan's B0 runbook, and throw away the first bench run.
- **B1 first**: `scripts/bench.sh --strict` three times on the A1 binary (item 52's owed runs),
  `engine turn, 1 busy, admin` recorded at n = 20, D_in for item 49. Then B2 (bound by item 85),
  B4, B5 (**resolve item 88 before it**), B7, B8 (nanofix), B9. **B3 is already done** by PR #71.
- **B6 needs the cable and the Mac** (Q3): `sudo -n setcap cap_net_raw,cap_net_admin+ep
  target/release/w2w` after every build; `ethtool --get-hwtimestamp-cfg enp9s0` before and after;
  `ethtool -S enp9s0 | grep tx_hwtstamp_skipped` before and after each run; wire figures `hft`
  only.
- Items **84** → architect and **85** → a plan, unchanged.

Gates a next manager re-runs on this branch:

```text
cargo fmt --all -- --check
cargo clippy --workspace --all-targets -- -D warnings
cargo clippy --workspace --all-targets --no-default-features -- -D warnings
cargo clippy -p fixbolt-w2w --all-targets --features affinity,tls -- -D warnings
cargo test --all && cargo test --no-default-features
cargo test -p fixbolt-w2w                                  # 23 passed on 6e716a5
cargo test -p fixbolt-codec --test bench_baselines         # 6 passed
cargo check -p fixbolt-w2w --target x86_64-apple-darwin
cargo bench -p fixbolt-engine --bench alloc                # 31 cases, each 0
cargo build -p fixbolt-w2w --release --features affinity,tls
scripts/check-no-kernel-sleep.sh && scripts/check-standard-gives-the-core-back.sh
W2W_EXTRA='--wire-timestamps --nic lo --observer-core 2' scripts/check-no-kernel-sleep.sh
W2W_EXTRA='--wire-timestamps --nic lo --observer-core 2' scripts/check-standard-gives-the-core-back.sh
scripts/check-machine-verdicts.sh                          # pass 27 fail 0
shellcheck -S info scripts/w2w-baseline.sh scripts/check-no-kernel-sleep.sh scripts/check-standard-gives-the-core-back.sh
python3 scripts/check-links.py
```

## Start here — 2026-09-14: the first TLS numbers, and kTLS was the slower one

Branch `plan/tls-numbers-on-s9`, off `main` at `1178f4d` (CI run
[`34767259852`](https://github.com/tmthang86/fixbolt/actions/runs/34767259852), push to `main`,
success). PR [#71](https://github.com/tmthang86/fixbolt/pull/71); CI run [`34797203780`](https://github.com/tmthang86/fixbolt/actions/runs/34797203780) on `00293a7`, success; closing commit `d022d9c`, CI run [`34799053966`](https://github.com/tmthang86/fixbolt/actions/runs/34799053966), success, 14 jobs of 14. **Docs only** — no line
of `crates/`, `scripts/`, `tools/` or `.github/` changed. Plan
[tls](docs/plans/2026-09-04-tls.md), steps **6-M** and **7b**, the last two. Gate for this PR:
`python3 scripts/check-links.py`.

### What closed

`[measured 2026-09-14]` on the §9 desktop — AMD Ryzen 7 3700X, Linux `7.0.0-31-generic`,
`isolcpus=6,7,14,15 rcu_nocbs=6,7,14,15 processor.max_cstate=1`, mitigations in force,
`fixbolt-machine on`, `check-machine.sh` `pass 12 fail 0 unknown 1` before and after each
procedure, engine `cpu6`, client `cpu7`, loopback, TLS at both ends of every TLS arm:

| What | Where it is now |
|---|---|
| `w2w --tls off\|ktls\|userspace`, eight arms × 20 runs × 20 000 round trips, **twice** in one boot | `DESIGN.md` §8 *The round trip under TLS, measured*; `docs/reference/measured-costs.md` *TLS on the wire*, both summary blocks verbatim |
| Both non-negotiable-4 scripts' kTLS arms, green, `hft` and `standard` | `docs/CONFORMANCE.md` §8 |
| ADR-0005 open question 2 — **answered at the level measured**: the kernel takes `TLS13_AES_128_GCM_SHA256` on `7.0.0-31-generic`; no other suite, no minimum kernel | `DESIGN.md` §9 TLS row, D11, ADR-0005 dated note |
| `userspace` allocates **80 000 per 20 000 round trips** (two single runs); `off` and `ktls` assert 0 in every procedure run | `measured-costs.md` |

| `hft`, p50 ns | off | kTLS | userspace |
|---|---|---|---|
| admin, procedure 1 / 2 | 16 065 / 15 670 | 25 298 / 24 657 | 20 774 / **17 473** |
| app, procedure 1 / 2 | 20 289 / 19 998 | 29 591 / 29 501 | 21 641 / 21 596 |

### The two surprises — both measured, neither explained

1. **kTLS was slower than userspace `rustls` in both `hft` paths, in both procedures, at p50, p99
   and p99.9.** Over `off`, kTLS added 9.0–9.5 µs at p50 in `hft` (9.6–9.7 µs in `standard`);
   userspace, run in `hft` only, added 1.8–4.7 µs (admin) and 1.4–1.6 µs (app). Loopback, so no
   NIC offload exists in either arm (`ethtool -k lo`: `tls-hw-tx-offload: off [fixed]`, read by
   the manager). The engine thread's most frequent syscalls under kTLS were the plain arm's
   (`recvfrom`, `accept4`, `sendto`) plus `getrandom` ×9 and `setsockopt` ×4, and no sleeper.
   **Not a cause, and not by itself a reversal of ADR-0005 decision 2**, whose reasons are D8,
   parse-in-place, AES-NI with NIC offload where the hardware supports it, and zero allocation —
   the kTLS arm still counts 0, and the NIC half is out of this measurement's reach. Item **84**,
   for the architect.
2. **`hft`/admin/`userspace` p50 moved 15.9% between two identical procedures** (runs
   20 639 .. 20 930, then 17 333 .. 17 813) while procedure 1's spread read 1.008; the other seven
   arms' p50 moved 0.2–2.5%. **At p99 it was not one arm**: `hft`/admin/kTLS moved −18.5% (procedure
   1's runs in two clusters) and `hft`/app/kTLS +7.2%. Recorded between or before the procedures,
   all candidates and none a cause: time since boot, the first procedure after the reboot, a
   quiet-row FAIL and the build before procedure 1, and before procedure 2 both mode scripts under
   `strace`, nine single runs and an active editor session. **A per-procedure spread bounds one
   procedure, not the next**, and the spread is max/median, which cannot register how far below
   the median a run falls.
   [a-tight-spread-inside-one-procedure-did-not-reproduce-across-two](docs/reference/a-tight-spread-inside-one-procedure-did-not-reproduce-across-two.md)
   `[to testing-skills]`. Item **85**.

### Not proven

- **A cause for either surprise.** Nothing was varied to isolate one.
- **Anything about a NIC, another kernel, or another suite.** The plan's optional suite step
  (§6.7 item 8) did not run. On the CI runner the `tls` job of run `34767259852` asserts the kernel
  took `TLS13_AES_128_GCM_SHA256` keys (`crates/engine/tests/tls.rs:40-45`, `:488-497`) and passed,
  but **its log prints no kernel version**: `6.17.0-1022-azure` is recorded on 2026-09-10
  (`docs/CONFORMANCE.md` §8) and by another job of that run, not by the job that took the keys.
- **That any TLS figure is reproduced.** Two procedures in one boot; one arm's p50 and two arms'
  p99 disagreed between them.
- **The commit, from the procedure's own output.** HEAD `1178f4d` and a clean tree were read by
  the manager's shell around the build; `w2w-baseline.sh` does not print them.
- **`hft` on an initiator under TLS.** `tools/w2w` is an acceptor; `connect_and_serve_tls` is
  `standard` only.
- **The new trap entry has no regression test — an unmet Definition-of-Done item** (`CLAUDE.md` §4,
  *every recorded trap gets a regression test*), routed to item 85. The procedure still publishes
  from one twenty-run median.
- `cargo test`, clippy, the acceptance definitions, `benches/alloc.rs` and Criterion were not
  run for this PR: no code changed.

### Next

- **Item 84 → architect**: does ADR-0005 decision 2 still hold for `hft`, and what would a NIC
  with offload change. A new ADR if it does not.
- **Item 85 → a plan**: how the §8 procedure reproduces a figure before publishing it.
- [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md): its B3 (this work) is
  done; B2's plain loopback arms compare against both procedures here, and item 85 bears on how
  B2 publishes.
- Machine: plan §6.7 item 10 is done. `sudo -n /usr/local/sbin/fixbolt-machine off` at
  2026-09-14T09:09:15+07:00 → governor `powersave`, boost 1, SMT on, THP `madvise`, `busy_poll` 0,
  `tls` still loaded, `nproc` 12. **The three boot parameters are left in place**, because
  the-second-linux-desk still needs this boot.

## Start here — 2026-09-13, later: what the review left open

PR [#70](https://github.com/tmthang86/fixbolt/pull/70), plan
[what-the-review-left-open](docs/plans/2026-09-13-what-the-review-left-open.md). **Last code commit
`e0ba30d`, CI run [`34766352240`](https://github.com/tmthang86/fixbolt/actions/runs/34766352240), 14 jobs of 14.**
The architect role ran on Opus for this plan, by the owner's call that the items were small.

| Item | Fix | Held by |
|---|---|---|
| 80 | probe 3 tries boolean spellings | R80-1, R80-2 red; residue R80-3 (`"always"`) green by design |
| 81 | `ReconnectInterval` cell `positive`; probe 7 needs `1` accepted before `0`'s refusal counts | R81-2, R81-3 red; review's `WrongRole` scenario red |
| 82 | check-links rule (d), autolinks, case-exact paths | R82-1..6 red; defect 1 URL reported again |
| 83 | code spans; rustdoc `-D warnings` ([ADR-0066](docs/decisions/ADR-0066-the-rustdoc-gate-denies-every-warning.md)) | R83-1, R83-2 |

**The senior review found one defect in this PR's own guard**: rule (d) had silenced rule (a), so
a URL `origin/main` reported passed. It and two gaps (autolinks never read; probe 7 accepting an
unrelated refusal) were reproduced by the manager, then fixed.

### Not proven

- Item 80's residue: an inline literal outside the boolean set passes probe 3.
- The `tls` build's probe counts (probe 3 `11 probed`) are not observable in CI: `cargo test` prints
  a passing test's output only with `--nocapture`, which no CI job passes. The floors still assert.
- `benches/alloc.rs`, Criterion, `tools/w2w`, invariant-4 scripts: not run, no hot path touched.

### Next

No open item from 80–83. Still waiting on the §9 machine: `tls` 6-M and 7b, and
[the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md).

## Start here — 2026-09-13, closed: what the residue review found

PR [#69](https://github.com/tmthang86/fixbolt/pull/69), branch `plan/what-the-residue-review-found`,
plan [what-the-residue-review-found](docs/plans/2026-09-13-what-the-residue-review-found.md).
**Last code commit `c6c907c`, CI run [`34761934969`](https://github.com/tmthang86/fixbolt/actions/runs/34761934969), 14 jobs of 14**;
the closing docs commit gets its own run on the pull request.

### What closed

| Item | How | Held by |
|---|---|---|
| 75 | validate before bind ([ADR-0064](docs/decisions/ADR-0064-a-door-acquires-nothing-before-it-has-validated.md)) | `serve_sharded_hft_refuses_the_plan_before_it_binds`; R75-1 red in a Linux container |
| 76 | doc fix (earlier commit) plus a regression test | probe 7, floor 4; R76-1 red |
| 77 | call-site leg in `doc_table` | `the_reader_table_matches_the_call_sites`; R77-1..3 red |
| 78, 79 | `feature-sets` CI job ([ADR-0065](docs/decisions/ADR-0065-every-feature-set-to-depth-two-is-built-linted-and-documented.md)): clippy `-D warnings` and rustdoc over every set to depth two, plus `--all-features` | the job; its first runs found 18 broken intra-doc links |

Also, asked for by the owner directly: `CLAUDE.md` rewritten to rules only (§11 removed, a new
rule that a review finding is verified before it is fixed), and `check-links.py` no longer
false-reds on another repository's `.github/workflows/ci.yml`.

### What the gate found that the desk could not

The Mac desk fixed 14 broken rustdoc links and read green; Linux CI found 4 more inside modules
macOS never compiles. The desk can type-check, lint and document for Linux with `ring`'s cross env,
and run Linux tests in a container —
[a-linux-only-module-is-invisible-to-a-mac-gate](docs/reference/a-linux-only-module-is-invisible-to-a-mac-gate.md).

### The review, verified before fixing

Eight findings, each reproduced by the manager first; all real. Fixed: `--all-features` was not in
the depth-2 powerset (the removed `docs` job had run it); probe 7's zero branch missed an
`ImpossiblePolicy` refusal; `check-links` rule (c) skipped this owner's mistyped repository names;
six stale doc lines; four rules the `CLAUDE.md` rewrite had weakened; R75-1 had never been seen red.
**Stated, not patched:** the call-site leg reads which reader a key reaches, not the value — item 80.

### Not proven

- `benches/alloc.rs`, Criterion, `tools/w2w` and the invariant-4 scripts were not run for this
  change: no step touched a hot path, a wait strategy or readiness.
- The `--features tls` sets were never built on the desk (ktls-core is Linux-only); CI's are the only
  observation.

### Next

Items **80–83** (none needs the §9 machine). Still waiting on the §9 machine: `tls` 6-M and 7b, and
[the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md).
...
PR [#68](https://github.com/tmthang86/fixbolt/pull/68), branch `plan/the-residue-of-an-obligation`, eight commits. **Closing commit `30b53cb`,
CI run [`34746295036`](https://github.com/tmthang86/fixbolt/actions/runs/34746295036), 14 jobs of 14.**
The first pull request built under the one-run-per-commit rule of PR #67: the draft was opened at
the first commit, and every commit got exactly one `pull_request` run.

### What closed

| Item | How |
|---|---|
| 71 | the `Link::Dropped` count (**23**) lives in one row of `prior-art.md`, and a test counts the source |
| 73 | every integer key read as written, through one function; `StartTime=+1:00:00` and `HeartBtInt=+7` parsed before |
| 21 | `serve_hft_pinned`: refuse the core, pin, read the mask back, then bind |
| 74 | probe 3 reads the parser's `match` arms beside its bounded search |
| 67 | probe 5 reads the `required…` cells; the rest of the table is prose, said where people read |
| 72 | `check-links.py`: own-repository URLs judged on any tail, foreign ones only on two segments or more |

### The finding that matters more than any item

**Every reversal the builders ran went red on its predicted sentence, and the senior review then
found five ways past the same guards.** `literals_of_match` read one quoted string per line, so
`"Y" | "yes"` passed both legs of probe 3 — `[measured 2026-09-13]` the manager broke the parser
the same way on the pre-review commit and on the reviewed one: `9 passed`, then `ResetOnLogon lists
["Y", "N"] but the parser's match arms read ["Y", "yes", "N"]`. Declaring `Reader::Numeric` switched
the leg off. Probe 5 did not read the condition in the cell, nor which key the refusal was about. The
`Link::Dropped` counter missed a return site in an unusual shape. **A set of red reversals proves
the guard catches what was tried** — [a-red-reversal-does-not-prove-the-assertion-you-wrote-it-for](docs/reference/a-red-reversal-does-not-prove-the-assertion-you-wrote-it-for.md)
is the nearest entry, and this is its wider case.

### Three traps paid for today, each written down

- **A restore by `git checkout` reverted the fix too**, because the fix was not yet committed —
  [a-restore-by-checkout-reverted-the-fix-too](docs/reference/a-restore-by-checkout-reverted-the-fix-too.md).
  The plan's own restore column said `checkout` on three rows; the step running in parallel was
  warned before it reached one.
- **An unpinned thread sat on the expected core 10 runs in 16**, so "running on core k" does not
  prove "pinned to core k"; the test asserts the mask too — a fourth instance in
  [a-reversal-needs-an-input-where-the-answers-differ](docs/reference/a-reversal-needs-an-input-where-the-answers-differ.md).
- **A bare filename is not evidence of a repository** —
  [its entry](docs/reference/a-bare-filename-is-not-evidence-of-a-repository.md).

### For the next manager, from how this one ran

- **Wait for the closing commit's run to finish before pushing the handoff commit.** A push
  cancels the run in progress for the same branch, and the closing commit would then have no CI
  result to name. Two intermediate commits here (`1a9c040`, `d44ea5a`) have no run of their own for
  that reason; the tip covers them.
- **Create worktrees yourself and run repo-wide scripts only when none exist.** `check-links.py` from
  the main checkout read 1485 files and nine false URLs while three worktrees were open.

### Not proven

- `benches/alloc.rs` was not re-run: nothing here touched a hot path, and no step claimed it did.
- Nothing about `serve_hft_pinned`'s hot path under `hft`: `check-no-kernel-sleep.sh` traces `w2w`.
- ~~Items 75–79 are measured once each by the review, on this desk, with no test guarding any of them.~~ `[struck 2026-09-13, PR #69]` 75, 77, 78, 79 each have a test or gate; 76 closed as a doc fix with probe 7 behind it.

### Next

Items **75–79** need a plan or an owner's decision (75 and 76 are behaviour). Still waiting on the
§9 machine: `tls` 6-M and 7b, and [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md).
...
Branch `plan/tls-initiator-and-numbers`, tip **`022ca36`**, PR
[#66](https://github.com/tmthang86/fixbolt/pull/66), 17 commits. **CI green on the commit being
closed**, run [`34739943517`](https://github.com/tmthang86/fixbolt/actions/runs/34739943517),
**14 jobs of 14** — `interop`, `bench` and `deny`, the three neither desk can run for itself, among
them. Steps **5a, 5b, 5c, 6a, 6b, 6c** (with 6c-2, 6c-3, 6c-4) and **7a** of
...
uninhabited marker types, and the acceptor path compiles to what it compiled to before.
`connect_and_serve_tls` dials a venue; `ClientTlsSettings` reads roots, identity and
`TlsRequireKernel` from the same `.cfg` shape an acceptor uses, and a settings file for the wrong
role now names a door that works **in both directions**. `tools/w2w --tls off|ktls|userspace`, with
the arm refusing before the first sample if the engine reads back a mode it did not ask for. Both
`scripts/check-no-kernel-sleep.sh` and `scripts/check-standard-gives-the-core-back.sh` gain a kTLS
arm, and a featureless build prints `TLS arm SKIPPED, NOT PASSED` and exits 2 rather than passing
quietly.

### The three findings, and none of them is what the step was for

1. **A live defect in code that was already merged.** `ktls-core`'s `tls13-key-update` feature was
   off, so a counterparty's TLS 1.3 `KeyUpdate` killed the session — `received fatal alert:
   InternalError` at **both** ends. It was found by writing the first test in this repository that
   ever sends that message, red first, fixed in `2d33d2a`. The test drives it from a **raw kTLS
   peer** writing its own 5-byte record, because rustls and ktls-core both always set
   `update_requested` and there is no other way to reach the other half of the mechanism.
2. **A reconnect storm the initiator had from the day it was written.** `up` was set after the
   turn, so a connection refused **in its first turn** never reached backoff: **879 dials in
   1.5 s**, 7 after the fix. The same step also found that the 4b fallback check was unreachable for
   the initiator — the check lives only in `add_with_prefix_config_and_journal` and `dial` called
   two other adds.
3. **A false green in a guard written the same day, caught by the senior review.** The kTLS arm's
   own `assert_eq!(allocs, 0)` fired before the script's `tls:` read-back, so the ordering claim in
   its comment — identity checked before allocations are counted — was false, and the first
   reversal written for it went red **in the wrong place** and looked like proof. Fixed in
   `71e4d30` with `Tls::wants()`.

### A third machine check for non-negotiable 4, and the hole it exists for

Both of rule 4's scripts trace `tools/w2w`, and **`w2w` is an acceptor**. The initiator's dial loop
runs on the caller's own thread, before any session exists, and **no script here can see it at
all**. `the_dial_loop_sleeps_rather_than_spins_while_the_handshake_waits`
(`crates/engine/tests/tls_initiator_wire.rs`) reads the dial thread's tid from `/proc/thread-self`
inside `connect_and_serve_tls`, parks a venue that completes the TCP connect and then says nothing,
and asserts the same four things the `standard` script does — including the last one, that closing
the venue's socket brings a second dial, because **asleep is not the same as unwakeable**.
`CLAUDE.md` §2 records it. **`hft` ~~under TLS~~ on the TLS initiator is still checked by nothing**:
`connect_and_serve_tls` is `standard` only. `[corrected 2026-09-14]` the acceptor's `hft` under
kTLS **is** traced — `scripts/check-no-kernel-sleep.sh`'s kTLS arm (step 6b) runs `tools/w2w --mode
hft --tls ktls`, green on the §9 desktop and in CI run `34767259852`.

### The allocation carve-out was a design question, and it went to the owner twice

[ADR-0063](docs/decisions/ADR-0063-a-peers-key-update-is-the-second-named-carve-out-and-a-ticket-is-not-read.md),
approved as the plan's *Sửa 7* and *Sửa 8*. A peer's `KeyUpdate` costs **2 boxes per rekeyed
direction** — 4 when the peer sets `update_requested`, 2 when it does not — and the test asserts
**that mechanism, not a number**. `RUSTLS_BOXES_PER_DIRECTION`, `RUSTLS_HKDF_EXPANDER_BOX = 184`
and two `DERIVED_FROM_*` constants are bound to `Cargo.lock`, and CI now runs `cargo metadata
--locked` plus `--locked` on the `tls` job's tests, so a manifest change cannot move the lock the
tests measure against. **A session ticket after the handover allocates nothing** — the
control-record buffer is pre-sized at `u16::MAX + 5`, which is what the ticket window actually
needs, and `Resumption::disabled()` alone removed **none** of the 16 allocations.

### Four plan facts that measurement refuted

Written down because each was read rather than run: the control-record buffer is **65 540 always**,
not 16 KiB + 5; `Resumption::disabled()` removes no ticket allocation on its own; `TlsTxRekeyOk` is
**+1, not unchanged**, because `/proc/net/tls_stat` counts per **network namespace** and the test's
own peer rekeys its send direction; and cargo 1.98.0 words its `--locked` refusal differently from
the sentence the plan quoted.

### Two existing test assertions were changed, which is the thing to look at hardest

`tls_keys_on_an_initiator_file_are_wrong_role` — forced by approved decision D1 of *Sửa 6*, because
an initiator file with TLS keys is now **valid**. And `assert_the_counter_is_live` moved from a
struct literal to a field comparison plus a **stronger** third assertion. Both are §10's named
failure mode — a fixture edited so new work can pass — so both are called out here rather than left
in a diff.

### Not proven

- **No TLS number is published by this pull request.** ~~`DESIGN.md` §8's TLS row is still empty.~~
  `[struck 2026-09-14]` filled by `tls` step 6-M on the §9 desktop — *Start here* 2026-09-14.
  Every figure here was measured on the **desktop** configuration — `scripts/check-machine.sh`
  reads `pass 5 fail 7 unknown 1` — so by non-negotiable 10 none of them is a §8 number.
- **`hft` mode under TLS is unmeasured and unchecked.** `connect_and_serve_tls` is `standard` only.
  `[narrowed 2026-09-14]` neither word holds for the acceptor: `hft` with kTLS through `tools/w2w`
  is measured in `DESIGN.md` §8, and traced by `scripts/check-no-kernel-sleep.sh`'s kTLS arm
  (step 6b). The initiator half stands.
- **Nothing here has spoken TLS to a counterparty that is not this engine.** `interop` runs
  plaintext; the kTLS peer in `tls_key_update.rs` is this repository's own.

### Next

**Session B is the §9 sitting**, and it is two steps: `tls` **6-M** and **7b**. This desk's §9
configuration costs **two reboots, not one command** — the three boot parameters
(`isolcpus=6,7,14,15 rcu_nocbs=6,7,14,15 processor.max_cstate=1`) were removed from
`/etc/default/grub` on 2026-09-05 — so it is merged into the §9 sitting of
[the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md) (its step B3) together with
items 49, 51 and 40. Machine-free and already approved:
[the-residue-of-an-obligation](docs/plans/2026-09-13-the-residue-of-an-obligation.md) (items 67,
71–74, 21).

## Start here — 2026-09-12, last: four items that were one family — an obligation nothing checks

**MERGED 2026-09-13 as `9346802`**, PR [#65](https://github.com/tmthang86/fixbolt/pull/65), no-ff,
closing commit `5f2bbaa`. **CI green on the merge commit itself**, run
[`34707356369`](https://github.com/tmthang86/fixbolt/actions/runs/34707356369), **14 jobs of 14** —
`interop`, `bench`, `deny` and `tls` among them, the jobs this desk cannot run for itself. `git diff
5f2bbaa 9346802` is **empty**, so the tested tip and the merge are the same tree rather than the same
by assumption. The branch tip was green too, run
[`34706835903`](https://github.com/tmthang86/fixbolt/actions/runs/34706835903), 14 of 14. Branch
deleted on both sides. Plan
[an-obligation-nothing-checks](docs/plans/2026-09-12-an-obligation-nothing-checks.md), all six steps.
**Closes items 57, 64, 69 and 70** — every open item that does not need the §9 measurement machine.
They were one shape: an obligation written down somewhere, with nothing standing guard over it.

**Three of the four decisions reversed something this repository asserted in writing**, and each
reversal came from a measurement rather than from an argument:

| Item | What this repository said | `[measured 2026-09-12]` |
|---|---|---|
| 57 | the licence gate's blind spot "could not be configured away" | `[licenses] include-dev = true`. The knob tried on 2026-09-08 was `[graph] exclude-dev` — the wrong table. **46 and 46** where the CI count step used to read 11 and 11 |
| 69 | the open question was regex versus a parser | `shellcheck -f json1` emits **diagnostics, not a parse tree**. On a file with all four bypasses it reports two unused variables. Option (b) is unreachable, not expensive — ADR-0061 |
| 70 | probe 3's reverse direction was a check | it was a **sample of three**. The universe is now ~4 422 short strings plus neighbours, and the first run was red: the parser accepted `+3`, `03`, `+6`, `06`, `+9`, `09` |
| 64 | the fix "is probably a debug assertion" | `clippy::panic = "deny"` **does not see `debug_assert!`** — a `panic` the lint guarding non-negotiable 7 is blind to. A `DropReason::NeverTicked` variant instead |

**The CI count step for the licence gate had been asking the right question with its own eyes
closed.** It asked whether cargo-deny judged every crate cargo resolves, while passing
`cargo tree -e normal,build` — which hides exactly the dev-dependencies cargo-deny was not judging.
Both sides agreed at 11 and 11 about a graph that was short at both ends.

**`DropReason::NeverTicked` names a fault on this side.** `Session::now_ms` starts at `0`, only
`tick_inner` writes it, and `received_with` takes no time argument, so every clock-dependent
judgement ran against whatever the last tick left behind. Item 63 is the instance already paid for:
a `Logon` judged before the first tick blamed the *counterparty's* clock for being two thousand
years out. **No existing test needed a tick added** — after this guard a session cannot reach
`LoggedOn` without ticking, so every test that logs on has ticked by construction.

**The senior review refuted a sentence this branch wrote the same day — the second pull request
running.** `docs/SESSION-BEHAVIOUR.md` claimed the two-thousand-year skew "can no longer be produced
at all". Measured on the branch:

```
the caller ticks with              last_skew_ms          the connection reads
nothing at all                     None                  NeverTicked
1                                  about -2 026 years    SendingTimeOutOfRange
Unix-epoch ms, 1789000000000       about -1 970 years    SendingTimeOutOfRange
FIXED_TIME_MILLIS (control)        Some(0)               the link stays up
```

The guard closes `now_ms == 0` and nothing else, and the third row is the mistake D13 and
`GUIDE.md` §5 exist to warn about. `CHANGELOG.md` said "on that path" and was already honest. The
table is now in the file, with which row has a test and which three are measurements.

**Eleven reversals, every one red on the sentence written for it before the code moved.** The review
also found the public rustdoc of `NeverTicked` wrong twice (a garbled `35=A` ends as
`LogonIncomplete`, and `Logon` is the only first message an acceptor can receive; `8=BeginString` is
read and judged before the guard), a script header bullet wider than the gate it described, and a
`docs/reference/` entry this branch invalidated and did not update — §4's highest-priority row.

**One finding was promoted from open item to must-fix, because this branch claims item 70 closed:**
a *Values* cell listing one literal twice (`` `acceptor` or `acceptor` ``) kept the probe green
while `initiator` left the document. Item 70's own shape in a different spelling. Now a third
assertion.

**Left open on purpose, and each says why**: items **40**, **49**, **51**, **52** need the §9 machine
and a reboot; **71** (a count stale in two accepted ADRs), **72** (`check-links.py` reads any
absolute URL ending in a filename this repository also has as a link home), **73** (`number()`
accepts `+30` and `030` across seven user-visible keys).

## Start here — 2026-09-12, latest: six ways past three gates, attacked by a fresh review — most closed, four left open on purpose

**MERGED 2026-09-12 as `064a48c`**, PR [#64](https://github.com/tmthang86/fixbolt/pull/64), no-ff,
closing commit `3571d1e`. **CI green on the merge commit itself**, run
[`34700635273`](https://github.com/tmthang86/fixbolt/actions/runs/34700635273), **14 jobs of 14** —
`interop`, `bench` and `deny` among them, the three neither desk can run for itself. `git diff
3571d1e 064a48c` is **empty**, so the branch's green transfers to the merge exactly rather than by
assumption. The branch head was green too, run
[`34700307871`](https://github.com/tmthang86/fixbolt/actions/runs/34700307871), 14 of 14.

**The lines worth reading rather than the tick**, from the merge run:

```
check-no-crate-root-allow: ok — 6 crate roots, 10 manifests, 55 inner attributes, 4 deny lints
check-scratch-fixtures: ok — 20 scripts, 1 enter a scratch dir, 1 pins
check-feature-gated-tests-ran: ok — fixbolt-engine --features tls: 38 binaries, 38 Running lines,
                               326 listed, 326 accounted for, 0 ignored (ceiling 0)
tls.rs repetitions failed: 0 of 3
```

**CI was red three times on this branch before it was green, each for a different reason, and this
desk was green for all three** — a merged stream's order, a `#[should_panic]` name spelled two ways,
and `CARGO_TERM_COLOR: always` on the runner. Written up in
[two-streams-through-one-pipe-have-no-guaranteed-order](docs/reference/two-streams-through-one-pipe-have-no-guaranteed-order.md)
and [a-test-s-name-is-spelled-differently-in-a-listing-and-in-a-run](docs/reference/a-test-s-name-is-spelled-differently-in-a-listing-and-in-a-run.md),
both `[to testing-skills]`.

Branch `plan/gates-that-match-a-meaning`, tip `3571d1e`. Plan
[gates-that-match-a-meaning](docs/plans/2026-09-12-gates-that-match-a-meaning.md), all 6 steps
plus one fix pass. This session is step 6 — the plan's own required independent senior review,
fresh context, given the plan's *Cách làm* and the three scripts, not this session's reasoning —
and the fix pass it produced. **Read `57b6b3a`'s commit message first**; it carries every
finding, every fix, every command and every number this section summarises.

**What step 6 found, and what happened to each finding:**

- **The crate-root gate's deny-list derivation was blind to a quoted TOML key.**
  `"unwrap_used" = "deny"` — valid TOML, cargo-honoured — did not match the extractor, so
  `DENY_LINTS` held strings `tools/attr-scan`'s output could never equal, and
  `#![warn(clippy::unwrap_used)]` at a crate root read `ok`. **Fixed**: quoted, dotted and
  inline-table keys are now recognised, a literal-string value is read, and an empty deny list
  is itself a `FAIL`. `check-no-crate-root-allow` now prints the deny-lint count.
- **The scratch-fixture gate was got past five more ways.** Two fixed: a `cp` reachable only
  through a trailing comment (comments are stripped before matching now), and a shell script
  with no `.sh` name (scope is now by shebang). **Four left open, by decision — item 69, for the
  architect**: a `cp` inside a heredoc body, a `cp` copying out of the scratch dir rather than
  into it, and two seeding misses, `read -r TMP < <(mktemp -d)` and a bare `TMP=/tmp`. Each is
  one more regex, and this plan's own section A already concluded that loop cannot be won.
- **Fixing the first finding surfaced a bug that had nothing to do with TLS or lint config.**
  `check-feature-gated-tests-ran.sh` — closed two steps earlier, item 68 — turned out to be
  wrong twice on its first delivery: it inferred which test binary owned which test from the
  relative order of two merged output streams, which CI's own scheduling does not guarantee
  (`[measured 2026-09-12]` run `34696713544` went red with a test line before any `Running`
  line); and it could not recognise a `#[should_panic]` test's run-line spelling, reading `325
  of 326` on a log where all 326 had run. Both fixed the same day; two new `docs/reference/`
  entries record the lessons —
  [two-streams-through-one-pipe-have-no-guaranteed-order](docs/reference/two-streams-through-one-pipe-have-no-guaranteed-order.md)
  and
  [a-test-s-name-is-spelled-differently-in-a-listing-and-in-a-run](docs/reference/a-test-s-name-is-spelled-differently-in-a-listing-and-in-a-run.md).
- **Also found and fixed**: probe 3 of item 67's `doc_table` samples its refusal side from
  three fixed candidates rather than searching, so a documented value narrower than the parser
  actually accepts can go unseen — measured on `TimestampPrecision`. Not fixed; too risky to
  widen without its own measurement (spurious reds) — item 70.

**Gates, re-run on this commit** (quote, do not summarise, before closing the branch):

```
$ bash scripts/check-no-crate-root-allow.sh
attr-scan: 6 files, 55 inner attributes
check-no-crate-root-allow: ok — 6 crate roots, 10 manifests, 55 inner attributes, 4 deny lints
$ bash scripts/check-scratch-fixtures.sh
check-scratch-fixtures: ok — 20 scripts, 1 enter a scratch dir, 1 pins
$ bash scripts/check-feature-gated-tests-ran.sh fixbolt-engine tls <real --tests --features tls log>
check-feature-gated-tests-ran: ok — fixbolt-engine --features tls: 38 binaries, 38 Running lines,
326 listed, 326 accounted for, 0 ignored (ceiling 0)
$ shellcheck -S info scripts/check-no-crate-root-allow.sh scripts/check-scratch-fixtures.sh scripts/check-feature-gated-tests-ran.sh
exit 0
$ cargo test --all
635 passed, 0 failed
$ python3 scripts/check-links.py
no dead internal links — 361 files, 1921 links today
$ grep -rn 'Two limits remain' CLAUDE.md docs/
[empty]
```

**Item status**: 66 does **not** fully close — gate A closes in full, gate B does not (four
spellings remain, item 69 asks the architect what to do about the gate's shape). 67 stays
narrowed-then-closed as before, with a second gap (item 70) added. 68 stays closed, its row now
naming the two reasons the closing gate was itself wrong. Two new open items, 69 and 70.

**Not proven, named rather than left implicit:**

- **The four gate-B spellings above are not fixed, and are not going to be under this plan** —
  item 69 is a design question, not a punch list.
- **`shellcheck` still covers three scripts of twenty**, at `-S info`. Unchanged by this
  session.
- **No CI run is named yet for `57b6b3a`.** The manager names one, on the commit that closes
  the branch, before merging — §9's last box.
- **Nothing here touches the engine thread under TLS**, same as every earlier entry in this
  plan — this plan never touched `codec`, `session`, `transport.rs` or `engine`'s non-test code.

**Next**: the manager re-runs every gate this section quotes on the commit that closes the
branch, names the CI run, and merges. Item 69 is a question for the architect and is not part of
this branch's exit criteria — the plan's own cut line closed item 66's gate-A half without it.

## Start here — 2026-09-12, later still: the two gates a senior review got past now read a meaning

**NOT MERGED.** Branch `plan/gates-that-match-a-meaning`, tip `7323299`. Plan
[gates-that-match-a-meaning](docs/plans/2026-09-12-gates-that-match-a-meaning.md), steps 1-5 of 6.
This is the direct continuation of PR #63's senior review below: that review got past both new
gates from the previous plan with spellings their authors never tried, and opened items 66, 67
and 68 rather than let the plan that built them reopen.

**What is built, one line per step:**

- **Step 1** (`6e6aa22`) — `tools/attr-scan` (package `fixbolt-attr-scan`, binary `attr-scan`,
  one dependency `proc-macro2` with `span-locations`) lexes a crate root with the same lexer
  `rustc` uses. `scripts/check-no-crate-root-allow.sh` A1/A2 are rewritten on its output, A0b is
  new (0 attributes read is a broken tool, not a clean tree). `$ scripts/check-no-crate-root-allow.sh`
  → `attr-scan: 6 files, 55 inner attributes` / `check-no-crate-root-allow: ok — 6 crate roots, 10
  manifests, 55 inner attributes`. All four spellings that got past the old regex go red again;
  the deliberate green control `#![doc = "#![allow(clippy::unwrap_used)]"]` reads **56**.
- **Step 2** (`6eb04f5`) — `scripts/check-scratch-fixtures.sh`: a `#`-first line is never read, `cp`
  counts only in command position, any assignment prefix seeds, and a `cd`/`pushd` naming no
  variable fails outright (B2b). `$ bash scripts/check-scratch-fixtures.sh` → `ok — 20 scripts, 1
  enter a scratch dir, 1 pins`.
- **Step 3** (`161744a`) — the `tls` CI job's `--test …` list is deleted; it runs `--tests
  --features tls` and `scripts/check-feature-gated-tests-ran.sh` takes the test list from the
  build itself and requires the job's own log to account for every one, by binary. Measured: **38
  binaries, 322 listed, 322 accounted for, 0 ignored.**
- **Step 4** (`7323299`) — four probes added to `mod doc_table` in
  `crates/engine/src/settings.rs`, driving the parser from cells that are already values (count
  sentence, backticked *Default*, enumerated *Values* of two-or-more literals, *Where*). Coverage
  **15/10/18 probed, 15/20/12 skipped** (16/11/18 with `tls`); **no §1 cell is wrong today.**
- **Sửa 1** (`3ad4ab9`) — CI shellchecks at `-S info`, not `-S warning`: `-S warning` cannot see
  `SC2086` (severity *info*). Two scoped `# shellcheck disable=SC2094` in
  `check-scratch-fixtures.sh`, each with its reason (both are reads, not a read-and-write).
- **Step 5** (this commit) — documentation sync: `CLAUDE.md` §2, `DESIGN.md` §6 (plus one new
  row, for `check-feature-gated-tests-ran.sh`), both `docs/reference/` entries, and `STATUS.md`
  are re-widened to say exactly what steps 1-4 measured and no further. Items 66 and 68 closed;
  item 67 narrowed then the narrowed part closed, with the *Meaning*/notes-cell gap named as what
  remains; item 62 carries one line pointing at what finally removed the list; the `shellcheck`
  *Not proven* bullets narrowed from "never run" to "three of twenty, at `-S info`".

**Gates, re-run on this commit** (quote, do not summarise, before closing the step):

```
$ scripts/check-no-crate-root-allow.sh
attr-scan: 6 files, 55 inner attributes
check-no-crate-root-allow: ok — 6 crate roots, 10 manifests, 55 inner attributes
$ bash scripts/check-scratch-fixtures.sh
check-scratch-fixtures: ok — 20 scripts, 1 enter a scratch dir, 1 pins
$ python3 scripts/check-links.py
[no dead internal links — 359 files, 1899 links today — 1893 was a transcription error against the commit beside it, which read 1899]
$ cargo test --all
[635 passed, 0 failed]
$ grep -rn 'ordinary spelling' CLAUDE.md docs/
[empty]
```

**Not proven, named rather than left implicit:**

- **Step 6 has not run.** An independent senior review, fresh context, given the plan's *Cách
  làm* and the three scripts — not this session's reasoning — still needs to attack all three
  gates with spellings not yet tried. Steps 1+2+5 are what the plan itself said closes item 66
  without step 6; step 6 is a further check, not a precondition, and it is still open.
- **`shellcheck` covers three scripts of twenty**, at `-S info`, in CI. Seventeen are not
  covered, and two of those need the §9 machine to run at all.
- **Nothing here touches the engine thread under TLS.** ~~`scripts/check-no-kernel-sleep.sh` still
  has no TLS arm, `DESIGN.md` §8's TLS row is still empty~~ — unchanged by this plan, which does not
  touch `codec`, `session`, `transport.rs` or `engine`'s non-test code. `[struck 2026-09-14]` the
  arm exists (`tls` step 6b) and ran green on the §9 desktop; the §8 TLS table is filled (step 6-M).
- **No CI run is named yet for `7323299` or the docs-only commit that follows it.** The manager
  names one, on the commit that closes the branch, before merging — §9's last box.

**Next**: step 6 (senior review, fresh context, opus, on the built gates), then the manager
re-runs every gate this section quotes on the commit it closes, names the CI run, and merges.

## Start here — 2026-09-12, later: TLS can be configured from a file, and a reversal was a no-op before it was a red test

**MERGED 2026-09-12 as `a8c9a3c`**, PR [#63](https://github.com/tmthang86/fixbolt/pull/63), no-ff,
10 commits, closing commit `c1d2925`. **CI green on the merge commit itself**, run
[`34691768411`](https://github.com/tmthang86/fixbolt/actions/runs/34691768411), **14 jobs of 14** —
`interop`, `bench` and `deny` among them, the three neither desk can run for itself. `git diff
c1d2925 a8c9a3c` is **empty**, so the branch's green transfers to the merge exactly rather than by
assumption. **The line worth reading is not the tick:** the merge run's `tls` job prints `TLS tests
that ran: 55` where this branch found it printing **14**, and before the branch **11** — the count
moved twice in one day because twice a `--features tls` test existed that no CI job named. And
`tls.rs repetitions failed: 0 of 3`.

**A senior review ran before the merge and changed the branch.** It was given the plan and the
rules, not this session's reasoning, and it found one thing that would have shipped: three
`#[cfg(feature = "tls")]` tests in `crates/engine/tests/settings.rs` that no job ran, one of them
the guard for `Problem::NeedsTlsDoor` — a file asking for TLS poured into plain `serve`. It also
**got past both new gates** with spellings their author never tried, which is why items **66**,
**67** and **68** are open and why four documents were narrowed from *any* / *every* to what was
measured. What it attacked and could **not** break: non-negotiable 6, hunted across 18
configurations × 2 feature sets, with no path found from a TLS-requesting file to a plaintext port.

Branch `plan/crate-root-allow-and-scratch-fixtures`, steps 4-7 of
[the 2026-09-12 plan](docs/plans/2026-09-12-crate-root-allow-scratch-fixture-and-tls-4c.md),
which is also step **4c** of [the `tls` plan](docs/plans/2026-09-04-tls.md). Items 58 and 23
closed earlier today on the same branch — see *Open items* below, not repeated here. `8fef1d7`
approved this plan and the `tls` plan's *Sửa 5* in the same sitting, and settled the key-name
question `Sửa 5` had left open: QuickFIX C++'s name where C++ has one. `348b9a8` is step 4, the
doc↔key two-way gate; `b39b52e` is step 5 (`tls` plan's 4c-1), the four keys refused cleanly
without the `tls` feature; `0a49778` is step 6 (4c-2), `tls::load_pem` and a real `.cfg` bringing
a session up over a socket. Step 7 — the documentation that turns the deliberate red from step 6
green — is this commit.

**Four keys, `[DEFAULT]`-only and acceptor-only**: `SocketUseSSL`, `ServerCertificateFile`,
`ServerCertificateKeyFile`, `TlsRequireKernel` — QuickFIX C++'s spelling where C++ has one,
QuickFIX/J's for the switch (C++ selects TLS by class, not by key), this repository's own for
the fourth (no engine surveyed has it; precedent `ReconnectCeiling`). `TlsCaFile` was proposed
and dropped: `tls.rs` calls `with_no_client_auth()`, and a key nothing reads is a promise in a
document, not a feature.

### Step 5 was red on purpose for two commits, and that was the plan working

Step 4's two-way gate (`docs/CONFIGURATION.md` §1 checked against `Key::name`, both directions)
was written *before* the four keys had a documentation row, so that step 5 arrived red:
`Key has no doc row: SocketUseSSL`. It stayed red through step 6, because the `tls` plan says in
its own words that 4c-1 does not merge without 4c-2. Step 7 turns it green by writing the four
rows — never by touching the test, which is exactly the failure mode this session was briefed to
watch for.

### Three reversals this branch ran into, all the same family: red at the wrong place

- **Step 1's matcher couldn't match `clippy::`.** Already written up:
  [a-matcher-excluded-the-separator-every-real-name-uses.md](docs/reference/a-matcher-excluded-the-separator-every-real-name-uses.md),
  `[to testing-skills]`.
- **A reversal that changed nothing, caught before it was trusted as one.** Step 5's
  featureless-build refusal is gated by exactly one `#[cfg(not(feature = "tls"))]`, on a
  statement rather than a block. The first attempt at proving it by reversal replaced the
  attribute with a comment — the guarded statement stayed compiled unconditionally, the refusal
  still fired, and the test stayed green. **That is a reversal that proves nothing, dressed as
  one that passed.** Deleting the refusal outright is what actually goes red, on the assertion
  the test was written for (`b39b52e`'s own message carries both attempts). Written up as
  [a-reversal-that-removed-the-guard-s-label-not-the-guard](docs/reference/a-reversal-that-removed-the-guard-s-label-not-the-guard.md),
  `[to testing-skills]`, and the general rule it carries is that **a guard which is a
  conditional has two things to break, and only one of them is the guard**: breaking the
  condition asks whether it is restricted to the right cases, breaking the body asks whether it
  does anything at all, and a condition-only reversal is a no-op by construction wherever every
  test sits on one side of that condition. The three entries cross-link each other, because
  reading any one of them alone reads as a one-off.
- **Step 6's path-swap never reached `load_pem`.** The plan's own reversal — swap the certificate
  and key paths in the `.cfg` — is caught by an earlier assertion before the loader ever runs;
  the reversal that actually reaches `load_pem` is deleting the certificate after the file names
  it. Same class as the matcher case above, and recorded in `tls_settings_wire.rs`'s own module
  doc rather than a new `docs/reference/` file.

Two of the three already have a durable write-up; the middle one does not, and that is said
...
This is the gate step 7 exists to turn green — it was `1 failed` (`Key has no doc row:
SocketUseSSL`) at the start of this step and is `0 failed` now, without a line of `settings.rs`
or its tests touched.

`cargo test --all`: every binary `0 failed` (exit read from `${PIPESTATUS[0]}`, not from a
`tail`, per the `[measured 2026-09-08]` lesson two entries below this one).
`cargo test --all --no-default-features`: same, every binary `0 failed`.
`bash scripts/check-no-optional-deps.sh`: `ok` on all nine checks (three crates × three
dependencies), including the one that was failing for the documentation gap alone.
`python3 scripts/check-links.py`: `356 markdown and rust files, 1879 internal links checked …
no dead internal links` — up from 1867 two commits ago, so the new cross-references this step
added (`ADR-0060`, `ADR-0005`, `tls.rs`, `settings.rs`) all resolve.
`bash scripts/check-no-crate-root-allow.sh` and `bash scripts/check-scratch-fixtures.sh`: both
still `ok`, unchanged — this step touched no code they watch.
`git status --porcelain`: five files, the ones this step was briefed to touch
(`CHANGELOG.md`, `STATUS.md`, `docs/CONFIGURATION.md`, `docs/DESIGN.md`, `docs/GUIDE.md`) plus
the two plan files' delivery logs.
...
- **`shellcheck` coverage is partial, not absent.** [gates-that-match-a-meaning](docs/plans/2026-09-12-gates-that-match-a-meaning.md) Sửa 1 put `shellcheck -S info` (not `-S warning`, which cannot see `SC2086`) into the `lint-config` CI job over three scripts: `check-no-crate-root-allow.sh`, `check-scratch-fixtures.sh` and `check-feature-gated-tests-ran.sh`. **Three scripts are covered; seventeen are not, and two of those need the §9 machine** to run at all, so shellcheck over them cannot be a CI job on a shared runner. This is item 66's own note, not struck by it: the two new scripts are proven by reversal to assert what they mean, and shellcheck is a separate, narrower kind of evidence about quoting and word-splitting on paths no reversal took.
- **Nothing here says anything about the engine thread under TLS.**
  ...
  empty.~~ `[struck 2026-09-14]` the arm exists (`tls` step 6b) and ran green on the §9 desktop;
  the §8 TLS table is filled (step 6-M).
- ADR-0005 open questions **2** (kernel/cipher floor) and **6** (a TLS 1.3 key update under
  kTLS) are still open. `[2026-09-14]` no longer: question 6 was answered at phase-1 level on
  2026-09-13, and question 2 at the level measured on 2026-09-14 — ADR-0005's dated notes. Question **3** (what asserts which mode is live) is answered as of this
  morning's `e728c16` — `DESIGN.md` D11 is corrected in this same commit to say so; it previously
  still listed question 3 as open after the code that answers it had merged.
...
**Wave B plan 4, step 4a, is done on `plan/tls`.** Before it, `TlsTransport` was built, tested
and **unreachable**: every `serve*` constructed `PendingSet<TcpTransport, presession::Table, RX>`
outright (`crates/engine/src/lib.rs:2207`), so the only way to get TLS into an engine was to
assemble the engine by hand — the same shape that made every published `hft` figure describe a
hand-built engine until `hft_wire.rs`.

**The plan had no step that owned this.** It appeared once, inside step 2's prose, and step 2
closed without doing it — which its own delivery log said plainly. Fixed as the plan's **Sửa 3**,
approved before any code was written: step 4 becomes 4a (TLS reaches the front door), 4b (the
mode event and `TlsRequireKernel`) and 4c (the five configuration keys).

**The change in `lib.rs` is one line of substance**: `pump` is generic over the transport, and
the `PendingSet` annotation names `T`. `Engine<T, …>` was already generic over its transport, so
what was missing was a *name* for the substitution — `AcceptorEngineOver<T, …>`, with
`TcpAcceptorEngine` now an alias for it. No existing signature changed.

### The reversal that was predicted green and came back red

`serve_tls` takes a certificate and a key rather than a `rustls::ServerConfig`, because
`enable_secret_extraction` and a TLS 1.3 / `AES-128-GCM` narrowing are load-bearing for the kernel
handover. **The reason written into the rustdoc for that was wrong**, and reversal 1 is what said
so. The prediction: flip `enable_secret_extraction` to `false` and the test **stays green**,
because the session falls back to userspace and serves correctly — the limitation this gate
cannot see.

`[measured 2026-09-10]` it went **red**: `ConnectionReset`. Only a `setup_ulp` refusal reaches the
fallback. `dangerous_into_kernel_connection` **consumes** the rustls connection, so a refusal
there leaves nothing to fall back to and the connection dies — with **no FIX-level explanation,
because there is no session yet to carry one**. The engine's own `hand_over` comment had said this
all along; the prediction was made from the shape of the feature rather than from the code.
Corrected in three places, and the true reason is a stronger argument for the API than the guess
was: it is not about picking the wrong mode, it is about the connection not working.

### A set of reversals can be all-red and still leave an assertion untried

Reversals 1, 2 and 3 all failed at the socket — `ConnectionReset`, `UnexpectedEof`, `WouldBlock` —
**before reaching the `LoggedOn` assertion**, the one that separates *it served* from *it did not
crash*. A fourth was needed to reach it, asking `wait_for_event` for `EndedWithoutReason`; it
reported `the stream held [LoggedOn]`. **Three red reversals said nothing about the assertion that
carries the test's meaning.** `[to testing-skills]`

### Not done, said plainly

**Nothing reads `TlsMode`.** ADR-0005 open question 3 is exactly as open as it was: a deployment
cannot tell whether its bytes went through the kernel or through userspace `rustls`. No
`TlsRequireKernel`, no event, no configuration keys, **no initiator**, and `w2w` does not know
what TLS is. **No §8 number and no §9 measurement** — `scripts/check-no-kernel-sleep.sh` has no
TLS arm, so this repository makes **no claim** about the engine thread under TLS.
...
[plan](docs/plans/2026-09-09-events-without-a-shared-lock.md). PR
[#58](https://github.com/tmthang86/fixbolt/pull/58) merged no-ff as **`aca9365`**, and **CI is
green on the merge commit itself**: run
[`34372299308`](https://github.com/tmthang86/fixbolt/actions/runs/34372299308), **13 jobs of 13**.
Read from the job log rather than off the check marks — the job's own `git log -1` prints
`aca9365bcdf667197e1fefba290a892f2a8e7ec0`, both new tests appear by name, **1242 passed / 0
failed**, and `FAILED` occurs zero times in the whole log.

**Reading the event stream no longer destroys it.** `EventRing::push` took `try_lock` on a mutex
every reader also took, so an operator polling attentively made the engine drop the events it was
pushing at that instant — permanently, counted only in `events_lost()`. Per-slot locking removes
the contention: the producer never blocks and never loses to a reader, readers serialise only
against each other, and **`events_lost() != 0` now means exactly one thing — the ring was full**.
No public API changed, and no `unsafe` was added.

### Every timing hypothesis was wrong, and the evidence was in the first CI log

`saw: [ one event ]`, not `saw: []`. **A test timing out on a slow engine sees nothing; seeing one
of two says the other was destroyed, not delayed.** That line was printed by the very first failing
run and went unexamined while three environmental hypotheses were built and killed:
`[measured 2026-09-09]` 32 spinners pinned to two cores left it at **0.12 s** against a 5 s budget;
the failing test passed **60 of 60** on two cores; the full suite **3 of 3**.

Reproducing it meant inverting the instinct — not adding load, but **removing the reader's
politeness**. With its 2 ms sleep deleted: **9 failures in 20 runs**, the exact CI signature. Then
**6 failures in 30** for the committed test against the old code, and **0 in 220** against the new
one. 220 is part of the evidence rather than thoroughness theatre: at 6-in-30, five green runs
would have proven almost nothing.

### A five-engine survey is why this became an ADR and not a test patch

QuickFIX C++, QuickFIX/J and quickfix-go never lose a session event — and all three pay by running
the operator's callback **on the session thread**, which non-negotiable 4 forbids here. nanofix has
counters only and cannot say why a connection ended. So the survey **split the behaviour in two**:
loss-on-overflow is a forced trade, and loss-on-read was an implementation detail wearing that
trade's clothes. Without the comparison both halves would have been defended together, or
abandoned together. `GUIDE.md` now warns that this stream can lose where no other engine's does.

### Two things went sideways, both recorded

The **indexing ratchet went red because the debt went DOWN** — 184 → 181, `slots[head]` became
`.get()` — and demanded the ceiling be lowered in the same commit. First time it has fired that
way. And the tests had to be **unit tests**: `Shared::emit` is `pub(crate)`, and adding a test-only
door to the public API to check a private contract is the wrong trade.

**Four test files that consume this stream now assert `events_lost() == 0` before asserting on its
contents. Three had never mentioned the counter.**

### Not done, said plainly

**No §9 measurement**, no before/after timing on that machine, `benches/turn.rs` not re-run. A
hand-written concurrent queue was rated a **High** risk in the plan and stays one.

## Start here — 2026-09-09, latest: seven widths, and four engines in one family disagree about them

**`STATUS.md` open item 59 is CLOSED**, [ADR-0058](docs/decisions/ADR-0058-a-timestamp-is-read-at-every-precision-and-written-at-three.md)
`Accepted`, [plan](docs/plans/2026-09-09-utc-timestamp-widths.md). `52=` and `122=` are read at
**17 bytes and at 19 through 30** — one to twelve fractional digits — on **both** readers, from
**one rule rather than two tables**. `scripts/interop.sh` gains scenario **§4i**, and it is the
first gate here that **no fixture in this repository could fake**.

**`cargo test --all` 613 → 619, 0 failed; `--no-default-features` 608 → 614; 59 / 59; `fmt`,
`clippy -D warnings`, `check-indexing-debt.sh` (184, ceiling 184), `check-no-optional-deps.sh`,
`check-links.py`, `benches/alloc.rs` (17 cases, all 0) all clean. `scripts/interop.sh`
7/7 + 8/8 + 6/6 + 6/6 + 6/6 + 9/9 + 5/5 + 3/3.**

### The owner asked for a survey before approving, and it changed the decision

The first draft of ADR-0058 proposed 17 and 19..=27, reasoning from QuickFIX C++ alone. The
survey — five engines, **each one's parser read rather than its documentation** — is in
[prior-art.md](docs/reference/prior-art.md) and returned three things:

| Engine | Widths its parser accepts | A bare `.` |
|---|---|---|
| QuickFIX **C++** | any length **17–27** | **accepted**, as `fraction = 0` |
| QuickFIX/**J** | exactly **17, 21, 24, 27, 30** | rejected |
| **quickfix-go** | exactly **17, 21, 24, 27** | rejected |
| QuickFIX/**n** | **≥ 17, any fraction length**, plus a timezone offset | rejected |
| **nanofix** | **none — it never reads one.** `SENDING_TIME: u32 = 52` is declared and used nowhere; there is no skew check | — |

**The family splits two against two**, so "match the oracle" was never one instruction — the
sharpest instance yet of this repository's own rule that a family name on a feature list is a
claim about whichever member you happened to read. **And the survey found a seventh refused
width**: QuickFIX/J takes 30 bytes, picoseconds, and `[measured 2026-09-09]` this engine returned
`None` for it as silently as for 19. Item 59 counted six. The bound moved from 9 digits to 12.

**What decided it is an asymmetry, not a preference.** Accepting a width can never break
interoperability with a stricter engine, because a stricter engine never *sends* one. Refusing one
does — and here it breaks it as a hang-up with no byte sent. The strict alternative is recorded as
ADR-0058 (d) and is not a straw man: half the family does it, and the EP wording enumerates rather
than ranges.

### The gate no fixture here could fake, and its reversal

§4i stands `libquickfix` up at **`TimestampPrecision=2`** — a width `settings.rs` **refuses on the
way out**, because this engine writes 3, 6 or 9 digits and nothing else. So the bytes under test
cannot be produced by this repository at all.

```
interop-odd: 20-byte 52= from libquickfix — 10
interop-odd: 35=3 naming tag 52 — 0 (must be 0)
interop-odd: PASS 3/3
```

**Reversal**: `parse_utc` back to its four widths, `dict` left widened so the reader under test is
isolated. It fails on **`the odd-precision session never logged on — a valid 52= was refused
before the Logon, in silence`**, and on that assertion alone — the other two stay green, so the
setup is proven valid at the moment the gate goes red. **The stamp count goes 10 → 97**, which is
the silence made visible: the C++ end reconnects over and over because nothing tells it why.

### Three existing tests were edited, and that is the thing to look at hardest

§10 names a fixture edited so new work can pass as the failure mode to watch for, so this is
stated rather than buried. Six assertions across `crates/dict/tests/field_types.rs` and
`clock.rs`'s own unit tests said widths like 22, 23, 25, 26, 28 and 29 bytes were **not**
timestamps. Each was a direct statement of the policy ADR-0058 reverses — not a fixture bent to
fit. Each edit carries an `[amended 2026-09-09, ADR-0058]` note saying what it used to assert and
why it changed, and **what those tests were really guarding — that widening did not remove the
check — is still guarded**, by boundary cases at 18 and 31 bytes and by a non-digit tail.

### A stale doc comment, found on the way and older than this work

`clock.rs` still said *"This engine still writes 21 bytes; the send half is ADR-0057 and is not
...
### §9's last box, closed on the merge commit itself

**`[2026-09-09, merged]`** PR [#56](https://github.com/tmthang86/fixbolt/pull/56) merged no-ff as
**`906c256`**, and **CI is green on the merge commit itself**: run
[`34366320153`](https://github.com/tmthang86/fixbolt/actions/runs/34366320153), **13 jobs of 13**.
The three neither desk can run for itself — `interop`, `bench` and `deny` — are among them.

**`interop` read from its log, first line to last, rather than off the check mark.** `git log -1`
inside the job prints `906c25624873b9ba4002385d966196a49f381a4e`, so it really is the merge that
was checked out and not a branch head. `7/7 + 7/7 + 6/6 + 6/6 + 6/6 + 9/9`, then
`interop-micros: 24-byte 52= — 12 from fixbolt, 10 from libquickfix`, `21-byte 52= — 0`,
`35=3 naming tag 52 — 0`, and the new scenario: **`interop-odd: 20-byte 52= from libquickfix — 10`,
`35=3 naming tag 52 — 0`, `PASS 3/3`** — the same numbers this desk saw, on a machine that is not
this desk. `the run added nothing git can see`, and **zero** shell-error lines anywhere in the run,
which is the class `reading-the-output-you-grepped-for.md` is about and which §4i added ~130 lines
of new shell to.

### The run named below is for the branch rather than the merge

**`[corrected 2026-09-09]` the first version of this paragraph named a commit that stopped being
the tip an hour later**, which is §4's staleness in the one section that exists to be exact. The
26/26 below is `588b350`, the commit the work was finished on; the merge commit gets its own run
named after it merges, the way PRs #51 and #54 did.
...
**And one job went red on a later push of the same branch, for a reason that is not this work.**
`crates/library`'s `an_operator_stops_the_front_door_and_serve_comes_back` — a shutdown test — got
an empty read where it wanted a `Logon`. **Two runs of that job on the same commit disagreed**,
one red and one green in the same minute, which is what classified it rather than any reasoning
about the diff. It is `STATUS.md` item **60**, open, with the evidence in
...
Recorded rather than re-run until green and forgotten.

### Not done, said plainly

**No §9 measurement**, and no `benches/baselines.tsv` row. ADR-0058's *Bad* section owes a
before/after reading of the receive path for the new branch, and this desk is two reboots from
§9 — so the plan's verification list carries it as owed rather than done. **`cmake` is not
installed on this desk**, so `scripts/interop.sh` was run against a cmake fetched into the
scratchpad, outside the repository; CI is still the authority for that job.

## Start here — 2026-09-09, later: a field with two readers, and only a stranger could see the second

**Wave B plan 3, [timestamp-micros](docs/plans/2026-09-04-timestamp-micros.md), is CLOSED — both halves.** [ADR-0057](docs/decisions/ADR-0057-sub-millisecond-time-arrives-beside-the-tick.md) was approved and is `Accepted`; half B shipped all five steps B1–B5 on the day. `TimestampPrecision` is the **26th** settings key, `52=SendingTime` goes out at **21, 24 or 27 bytes**, and `scripts/interop.sh` has a scenario that judges the **bytes** rather than a step line.

**`cargo test --all` 594 → 613, 0 failed; `--no-default-features` 589 → 608; 59 / 59; `fmt`, `clippy -D warnings`, `check-no-optional-deps.sh`, `check-links.py` and `check-indexing-debt.sh` (184, ceiling 184) all clean.**

### What this plan cost was not in the plan

**Step B4's first run logged on and then had every message rejected.** Both ends at `TimestampPrecision=6`; the session came up; then `35=3 … 371=52 373=6` — *Incorrect data format for value*, tag 52 — one Reject per Heartbeat, per SequenceReset, per Logout.

**`52=` has two readers in this codebase and half A widened one of them.** `session::clock::parse_utc` turns the stamp into a number so the skew can be measured; `dict::FieldType::UtcTimestamp` decides whether the value **is a `UTCTIMESTAMP` at all**, and its time helper still read `if v.len() != 8 && v.len() != 12 { return false }`. So a 24-byte stamp parsed, measured a sensible skew, and was then refused as malformed.

**Nothing here could see it, and each blind spot is ordinary on its own.** Half A's new test drives a `Logon` — the reject fires on messages *after* one. The 59 definitions carry no stamp wider than 21 bytes, which half A recorded honestly, so their green meant *nothing regressed* and could not mean *this works*. And `dict`'s own tests enumerate the widths `dict` already accepted — a test written from an implementation's set of accepted values can never find a value missing from that set. **What found it was somebody else's engine, configured for the feature, judged on the bytes.** [one-field-two-readers](docs/reference/one-field-two-readers.md), marked `[to testing-skills]`.

That case also records **two things that cost time on the way**: `tools/interop/initiator.cpp` printed its wire transcript only on **failure**, so a passing run had no evidence at all until `--dump-tape` was added; and the first §4h died **silently** — it globbed for a QuickFIX file log that is never written (the tool installs its own `RawLogFactory`, so `FileLogPath` is ignored), and under `set -o pipefail` a glob matching nothing ended the script with the scenario's header printed and nothing after it.

### The decision, and what the benchmark said about it

Option **(b)**: the sub-millisecond remainder travels **beside `now_ms`, in one call, from one clock reading**. `Session::tick_at` / `tick_at_with` are the new doors and `Engine::turn` uses them; `tick` / `tick_with` keep their signature and mean *a millisecond and nothing finer*. **D13 is not reversed** and no unit in the workspace changed.

**The configured width is a ceiling and the tick is the floor**, so a session configured for microseconds but driven through the millisecond door writes **21 bytes**, not `.xxx000`. The reversal of that guard prints the lie the ADR declined, in full: `20260828-12:00:00.000000`.

**ADR-0057 decision 5 left the runtime-field-versus-const-generic choice to the plan under one constraint — measure it.** `[measured 2026-09-09]` Intel Xeon @ 2.80GHz, a shared cloud VM and **not** the §9 desktop, three runs per arm, one variable at a time:

| | ns/op |
|---|---|
| before the change, same machine | **4.7** |
| slice return instead of `&[u8; 21]`, no precision branch | **4.8** |
| as shipped, default `Millis` | **5.4** |
| `Micros` | **10.5** |
| `Nanos` | **13.0** |

**+0.7 ns at the default: the runtime branch 0.6, the slice return 0.1.** The published **4.9 ns** (`benches/baselines.tsv:141`, AMD Ryzen 7 3700X) was **not** re-measured and no baseline row was added.

**Two regressions were caught mid-build rather than shipped.** Folding all three precisions into one loop cost **4.7 → 6.4 ns**, +36% at the default, for digits the default never writes — a straight-line millisecond arm brought it to 5.4. And `scripts/check-indexing-debt.sh` went **184 → 186** on the first version, from one computed index and one computed slice; rewriting with a slice pattern put it back. That also taught something about the ratchet itself: `clippy::indexing_slicing` **does not fire on a constant index into a fixed-size array**, so the number counts *unprovable* indexing, which is the kind that panics.

### The gate, and its reversal

`scripts/interop.sh` §4h, on QuickFIX's own transcript of the wire:
...
**The count of what is *not* there is the load-bearing half.** QuickFIX C++ accepts any `UTCTimestamp` from 17 to 27 bytes, so a run in which this engine silently stayed at milliseconds passes every step gate. Reversal: this end back to `TimestampPrecision=3` reads `0 from fixbolt` and `12` millisecond stamps, and the gate fails.

### Not done, said plainly

**No §9 measurement.** That machine is two reboots from §9 (`[2026-09-05]`), so every figure above carries the machine it came from and `benches/baselines.tsv` gained no row. **`SECONDS` precision on the way out** stays out of scope. **Open item 59 is still open**: `dict` now matches `parse_utc`'s four widths, but the oracle accepts 17–27, so six precisions it can be configured to emit are still refused here.

## Start here — 2026-09-09: the gate is written, and reading the oracle's source moved two of its answers

**[ADR-0057](docs/decisions/ADR-0057-sub-millisecond-time-arrives-beside-the-tick.md) is written and `Proposed`.** It is the gate half B of [timestamp-micros](docs/plans/2026-09-04-timestamp-micros.md) stands behind, and **no line of half B is written before it is `Accepted`** — that rule is the plan's own and this page does not relax it. **Nothing was built today: this is a decision and two documents, no code.**

**The decision.** Option **(b)** of the plan's three, in one specific shape: the sub-millisecond remainder reaches the session **beside `now_ms`, in the same call, from the same clock reading**. The plan's objection to (b) — *a second notion of time in a layer whose design rests on there being one* — is answered by the shape rather than argued away: two numbers that can be set independently can drift, and two numbers delivered in one call cannot. **D13 stands**; `[measured 2026-09-09]` an `_ms` identifier occurs **396 times across 42 files** in `crates/` and `tools/`, and this touches the formatter and leaves the other 395 alone.

**What writing it found, and neither was in the plan.**

**1 — The engine reads the clock once per turn, so `52=` already names the turn's start rather than the send.** `Engine::turn` takes one reading (`crates/engine/src/lib.rs:950`) and hands it to every connection; `Conn::turn` says so in a comment (`crates/engine/src/conn.rs:292`); and `Session::received` takes **no time at all** (`:2063`), so a reply is stamped by the tick that preceded it in the same turn. At milliseconds nobody had to care. At microseconds it becomes expressible, and six digits published without saying what they are the resolution *of* is a quieter version of the lie option (c) was rejected for. That is now decision 3, and it puts a sentence in `GUIDE.md` rather than leaving it to be discovered by a counterparty.

**2 — `scripts/interop.sh` can be an oracle for this feature in both directions, and this repository had the key's shape wrong.** `[researched 2026-09-09]` QuickFIX C++ read from source at `386ce46e`, the SHA `scripts/fetch-quickfix-assets.sh` pins — nothing fetched into `vendor/`, nothing committed. `TimestampPrecision` is a real key read with **`getInt`, 0–9**, default **3** (`Session.h:167–174`, `Session.cpp:68`); FIX **4.4** supports sub-second stamps (`Session.h:175–184`); the parser accepts **any length 17–27** (`FieldConvertors.h:492–596`). So the C++ end can be told `TimestampPrecision=6` and will both send and accept a 24-byte `52=` — unlike `369`, which ADR-0056 had to record could **never** be judged by that oracle.

**And the plan's *what is known for certain* table carried `TimestampPrecision=SECONDS|MILLIS|MICROS|NANOS`, cited to `reference/prior-art.md`, which has never contained that spelling.** It is QuickFIX/**J**'s. `prior-art.md` says only *"`TimestampPrecision` beyond milliseconds"* — and one line further down it already carries the lesson: *a family name on a feature list is a claim about whichever member you happened to read*. **The row survived the re-verification of 2026-09-08**, two days after that correction was written, because a re-verification pass re-checks facts and line numbers and **does not re-check provenance**. A citation is the kind of claim that looks strongest exactly when nobody opens it. Written into `prior-art.md` with the measured C++ table, marked `[to testing-skills]`.

**3 — A new open item, 59.** This engine accepts 17/21/24/27 bytes; the oracle accepts 17–27. So a QuickFIX C++ end configured `TimestampPrecision=1, 2, 4, 5, 7` or `8` sends a stamp this engine refuses — and before a Logon that refusal is a silent hang-up (`crates/session/src/lib.rs:2846–2856`), **the exact defect half A was written to close, still reachable through the oracle's own configuration file**. Receive-side, so half A's closure does not cover it and ADR-0057 does not decide it; it needs its own plan under Rule Zero.

**Also corrected, and it is the third time this cell has rotted:** *Where the work is* said `Branch` was `main` at **`6d3a191`**, which stopped being true when PRs [#52](https://github.com/tmthang86/fixbolt/pull/52) and [#53](https://github.com/tmthang86/fixbolt/pull/53) merged. `main` is **`974a14b`**, and **CI is green on it**, run [`34308659470`](https://github.com/tmthang86/fixbolt/actions/runs/34308659470), **13 jobs of 13** — the `interop`, `bench` and `deny` jobs among them.

## Start here — 2026-09-08, later: a valid field, refused for its width, answered with silence

**Half A of [timestamp-micros](docs/plans/2026-09-04-timestamp-micros.md) is closed**, wave B plan 3, approved by the owner on the day. All five steps A1–A5. `cargo test --all` **589 → 594**, 0 failed; `--no-default-features` 584 → 589; the 59 acceptance definitions **59 / 59**; `benches/alloc.rs` **0 on all 30 engine cases and all 6 codec cases**; `cargo fmt` and `cargo clippy --all-targets -D warnings` clean; `scripts/check-no-optional-deps.sh` ok on all three crates; `RUSTDOCFLAGS="-D rustdoc::broken_intra_doc_links" cargo doc --workspace` 0 warnings.

**`clock::parse_utc` reads 17, 21, 24 and 27 bytes now, and the fraction is truncated rather than rounded.** `Input::Tick` is milliseconds by D13, so a stamp finer than a millisecond loses at most 999 µs against a 120 000 ms skew bound; rounding would let a stamp that is exactly right arrive one millisecond in the future.

**The gap this closes is not a parsing inconvenience.** `[measured 2026-09-08]` a European venue under MiFID II RTS 25 stamps `52=` with six fractional digits, which is valid and ordinary. Through the old parser it was `None`, indistinguishable from a field that is absent or garbled — and before a `Logon` a `None` there is `Refusal::BadSendingTime`, which **hangs up without sending a byte**. That silence is right for a wrong clock, which is what `1c` and `1d` assert; applied to a right one it gives the counterparty no logon, no Reject, no `58=`, and no way to tell this from a firewall. `122=OrigSendingTime` failed differently through the same function: `373=1 RequiredTagMissing` naming tag 122 — this engine telling a venue a field is missing that the venue can see in its own log.

**Five gates were green over this for the length of the project, and the last one is the transferable finding.** 0 of the 59 definitions carry a stamp wider than 21 bytes; every fixture in 589 tests was built from that corpus; the `libquickfix` interop counterparty sends milliseconds by default, so two engines agreed about a case neither exercised; clippy sees correct Rust. And the parser's own negative-case list was **written from the implementation**, so it could only ever agree with it. **A suite derived from the specification you implemented cannot find the part of the world the specification left out.** What found it was a question asked from outside — *what does a real counterparty put on this wire in 2026?* Write-up: [a-valid-field-refused-for-its-width](docs/reference/a-valid-field-refused-for-its-width.md), **`[to testing-skills]`**.

**Two reversals, not one, and each was red at its own assertion.** Deleting `24` from the accepted widths turns four tests red — `Dropped` where `Up` was expected in `skew.rs`, and in `application.rs` the literal `8=FIX.4.4|…|58=Required tag missing|371=122|373=1|` printed by the failure message. Deleting `27` separately turns the two nanosecond assertions red and leaves everything else green. One reversal removing both widths could not have told a live nanosecond arm from a dead one.

**And the `122=` fixture was wrong in a second way, which is how the test got sharper.** `[measured 2026-09-08]` the first version stamped `122=` at `.123456` while leaving `52=` at `.000`, so `122=` named an instant **after** the message carrying it — `2f_PossDupOrigSendingTimeTooHigh`, `373=10`, Logout. The test went red on the link assertion rather than the one it was written for, which is the two-faults-at-once shape `14b` and `14d` are in the corpus to warn about. Rewritten to `.000456`, which truncates into the same millisecond as `52=` — so the test now **also** fails if the sub-millisecond part is ever kept or rounded up.

**Item 55's ceiling moved, 188 → 184.** `clock.rs` carried a file-wide `#![allow(clippy::indexing_slicing)]` for its four subscripts; `parse_utc` was being rewritten anyway, the new code asks with `get`, and the four went with it. The `allow` is gone from that file. `scripts/check-indexing-debt.sh` **was red on arrival** — *"the debt went DOWN, 188 -> 184, and the ceiling still says 188"* — which is the ratchet doing exactly what it was built for, and is the second time in two days it has caught something on a run it was not written for. `CLAUDE.md` §2 carried that ceiling in prose and now reads 184; **that number in that file was changed by this session and is said out loud here** rather than left to be noticed in a diff.

**Half B is NOT done and it is not blocked on effort.** This engine still **writes** 21 bytes at every precision, with no key to change it, so a venue that requires a microsecond `52=` *from* this engine is not served today. `TimestampCache` lives inside the pure session and is fed `Input::Tick` in milliseconds; there is no sub-millisecond time down there to format, and where one would come from is a decision with three options and no obvious winner — **`ADR-0057`, unwritten**. `docs/SESSION-BEHAVIOUR.md` §1a says so on the page a user reads rather than only here.

**Said rather than implied.** The 59 definitions ran 59/59 at every step and **prove nothing about this change** — they carry no stamp wider than 21 bytes, so they are evidence that nothing broke and no evidence that anything was added. There is **no capture of a real venue** sending a microsecond `52=` in this repository; the behaviour is built from the specification and from what MiFID II requires, which `CLAUDE.md` §7 rates below a real capture, and this sentence is what that costs. `scripts/interop.sh` was **not** run — this container has no `cmake` — and the QuickFIX fixture would have nothing to say here anyway: its default precision is milliseconds and the microsecond key belongs to the unbuilt send half.

**Sửa 1 to the plan, written before any code moved.** *Chia việc* named step A1 `a_microsecond_sending_time_is_refused_today` — a test asserting **today's** behaviour, so green from the start — and had A4 *"reverse"* it. But the plan's own *Cách kiểm chứng* demands *"A1 red first"* and *"A4 green after, the same test, no assertion edited"*, and its trap table says a green A1 means stop. The stricter half was taken: the test asserts the **desired** behaviour, was red at `Dropped` vs `Up`, and is green now with **not one assertion changed**. A test written to be green and later flipped is exactly the *"fixture bent to fit the code"* shape §10 says to watch for in one's own hand.

---

## Start here — 2026-09-08, latest: three gates borrowed by shape, and two of them were green about nothing

**All five steps of [three-gates-a-sibling-engine-has](docs/plans/2026-09-07-three-gates-a-sibling-engine-has.md) are done**, approved on the day and run on the Linux desk rather than a laptop, which is what makes step 4's dispatch figures Linux figures from the start. `cargo test --all` **585 → 589**, 0 failed; `--no-default-features` 580 → 584; the 59 acceptance definitions **59 / 59**; `benches/alloc.rs` **0 on all 30 cases**; `scripts/bench.sh` **16 of 16 targets measuring, 0 silent, 0 invariant failures, 0 over baseline, 0 under the band**, with `benches/dispatch.rs` at 7.7 / 271.4 / 503.9 ns. `[measured 2026-09-08]` on the owner's Linux desk, AMD Ryzen 7 3700X, Linux 7.0.0-31, `scripts/check-machine.sh` **10 pass / 2 fail / 1 unknown** — the two failures are `isolcpus` and C-states, both of which need a kernel command line and a reboot, so these are **not publishable latency figures** and are used here as A/B against the committed baselines, which is what the gate is for.

**None of the three came from a red run here.** They came from reading [TrueFix](https://github.com/truefix-labs/truefix), another Rust FIX engine, on 2026-09-07 — a survey whose original question (their conformance harness) turned up nothing worth borrowing, `crates/conformance` being stronger on all three axes the plan names. What it turned up instead was three holes on this side. **The shape was borrowed and every line rewritten**; TrueFix is Apache-2.0 OR MIT and copying would pull the `NOTICE` obligation ADR-0001 exists to avoid.

**One — `tools/interop` gave three socket endings one word.** `Ok(0) | Err(_) => return None`: the counterparty exited, the counterparty went quiet, and the socket broke were the same `None`, and the run printed *"the counterparty stopped answering"* for all three. `ReadOutcome` separates four endings plus a fifth that is not a socket ending at all. **No gate could have found this**, because the failure message is part of the test and nothing tests the failure message. Item 54.

**Two — `indexing_slicing` was not on, and it is the lint non-negotiable 7 was missing.** `[measured 2026-09-06]` a ring dispatch record gained a field, a slice went from four bytes to five in silence, and `copy_from_slice` panicked **in a library crate** — invisible to `unwrap_used`, `expect_used` and `panic`, because `a[i..j]` names none of them. 207 sites, now `deny` with a `--force-warn` ceiling, **188 after `journal.rs` and `dispatch.rs` were cleaned**. Item 55.

**Three — nothing looked at what the dependency tree is licensed under**, in a repository whose first paragraph says every commit is treated as already public. `deny.toml` and a blocking CI job. Item 56.

**And two of the three were green about nothing, each found by a reversal that first looked broken.**

**The licence gate could not see a dev-dependency.** `[measured 2026-09-08]` an `MPL-2.0` crate in `[dev-dependencies]` is resolved by `cargo metadata` and printed by `cargo tree`, and `cargo deny check` prints `advisories ok, bans ok, licenses ok, sources ok`. The same crate as a normal dependency is rejected at once. The first reversal used a dev-dependency and came back green; **three explanations were checked before the hypothesis "the tool never looked"**. Not fixable from here, so the CI job diffs the crate set the gate judged against the set cargo resolves — 11 and 11 today. Item 57.

**The indexing ratchet's enforcing half was inert and its counter read perfectly.** Three of the 21 debt annotations were `#![allow(...)]` at the top of a `lib.rs` — an *inner* attribute, which at a crate root silences the whole crate. `engine`, `session` and `dict`, most of the code, accepted new indexing without a word, while `check-indexing-debt.sh` reported the right number throughout because `--force-warn` overrides an allow. **The first attempt to reverse it was itself broken**: an unquoted backtick pair inside an `echo` made the shell run `deny` as a command, and the grep after it printed `0` — the same answer the real defect gives. Caught by reading the whole output, which is the same lesson as [reading-the-output-you-grepped-for](docs/reference/reading-the-output-you-grepped-for.md), two days later, in this session's own shell. Item 58.

**What the cleaning found, and it is not cosmetic.** `crates/engine/src/journal.rs` reads a file **another process wrote**, which can be torn, truncated or garbage. Every read of it goes through `get` now, with a `None` treated exactly as a torn tail — and `open_with` gained the `checked_add` its sibling `Reader::open` already had, so a corrupt length on a 32-bit target can no longer wrap into a range that is *in bounds and wrong*. In `dispatch.rs`, `&reply[r]` used a `Range` **the application returned**: a panic inside a library crate, raised by a caller's arithmetic. It is counted as a drop now.

**Three write-ups, all `[to testing-skills]`**: [three-outcomes-collapsed-into-one-none](docs/reference/three-outcomes-collapsed-into-one-none.md), [a-license-gate-that-cannot-see-dev-dependencies](docs/reference/a-license-gate-that-cannot-see-dev-dependencies.md), [an-allow-at-the-top-of-a-file-silenced-the-whole-crate](docs/reference/an-allow-at-the-top-of-a-file-silenced-the-whole-crate.md).

**`[measured 2026-09-08]` The first CI run was red on three jobs, and two of the three reds were the new gates failing at their own reading rather than at the code.** `CARGO_TERM_COLOR: always` sits at the top of `.github/workflows/ci.yml`; with colour forced on, `cargo --message-format short` wraps every line in ANSI escapes, so `check-indexing-debt.sh` matched **none of 188 sites and counted 0**, and the crate-list comparison in the `deny` job reported all eleven crates as both missing and added — two identical lists differing only in escape codes. **The zero-guard is what made the first of those a red job rather than a green one**; it was written on general principle with nothing to catch, and it caught something on the first real run. Both read `--color never` now, and both were re-checked under `CARGO_TERM_COLOR=always` locally before pushing. The third red was real: **three indexing sites in `crates/engine/src/affinity.rs`, behind `--features affinity`, off by default and Linux-only** — invisible to the clippy run the 21 annotations were enumerated from, so the ceiling had been measured under one feature set and believed for all of them. Cleaned rather than annotated, and the script now counts the union of both feature sets on Linux, proven by a site added to that file moving the count 188 → 189.

**`[2026-09-08]` §9's last box, closed on the commit it asks about.** PR [#50](https://github.com/tmthang86/fixbolt/pull/50) merged as **`93ae2b5`**; the commit CI judged is **`7bf7a71`**, runs [`34241853635`](https://github.com/tmthang86/fixbolt/actions/runs/34241853635) and [`34241859435`](https://github.com/tmthang86/fixbolt/actions/runs/34241859435), **13 jobs of 13**. The `interop` job's log was read top to bottom rather than off its `PASS` line: **`7 / 7 + 8 / 8 + 6 / 6 + 6 / 6 + 6 / 6 + 9 / 9`**, byte-identical to the scores before step 2 touched the read path, which is what that step asked for and what this desk could not check for itself. Grepped for the shell-error class [reading-the-output-you-grepped-for](docs/reference/reading-the-output-you-grepped-for.md) is about — `command not found`, `: line N:`, `No such file` — and there is none. The `deny` job printed **`cargo-deny judged 11 crates; cargo resolves 11`**, so its comparison ran rather than merely passing.

**Said rather than implied.** `scripts/interop.sh` was **not** run here — this desk has no `cmake` — so step 2's requirement that the six scenarios keep their scores is carried by the CI `interop` job on the closing commit and by nothing else. And the plan's file list said 21 files would take an `allow`; **81 did**, because `[workspace.lints]` reaches `tests/`, `benches/` and `tools/` too. That divergence is recorded here rather than left to be noticed in the diff.

---

## Start here — 2026-09-06, latest: two sequence-resync fields, and a seam that had to move to fit one of them

**All six steps of [seq-resync-789-369](docs/plans/2026-09-04-seq-resync-789-369.md) are done**,
approved by the owner on the day, with
[ADR-0056](docs/decisions/ADR-0056-the-application-is-told-what-the-session-owns.md) as step 1.
`cargo test --all` **570 → 585**, 0 failed; `--no-default-features` 565 → 580; the 59 acceptance
definitions **59 / 59** throughout; `scripts/interop.sh` **7 / 7 + 8 / 8 + 6 / 6 + 6 / 6 + 6 / 6
+ 9 / 9**, the last of those new; `benches/alloc.rs` 0 on every case with a new `next-expected`
one; all four CI lines clean.

**`789=NextExpectedMsgSeqNum` is read unconditionally and sent behind a knob**, and it has
**three branches, not the four the draft named**: a reset is applied before the comparison, so
`141=Y` with `789=1` lands in the *equal* branch on its own. `[measured 2026-09-06]` a real
`libquickfix` accepted this engine's `789=1` and answered `789=2` — which is the same
off-by-one this engine implements on its own acceptor side, arriving from the other direction
and confirming a decision that had been taken by reading `Session.cpp:713`.

**`369=LastMsgSeqNumProcessed` cost a public API change**, and the reason is structural rather
than incidental: an application *reply* is bytes the application wrote, which this session emits
untouched, so there was nowhere to add a field.
`Application::on_message` now takes a `Header<'_>` — `seq`, `stamp`, `last_processed` — and
**`library::Handler` did not change at all**, which is what makes `369` a guarantee above that
seam rather than something a handler remembers. Below it, it is offered and `GUIDE.md` says so.

**Four findings during the build, each of which changed the code.**

**One — the value is the message being answered, not `next_in - 1`.** The first version had this
engine's `Logon` reply read `369=0` while answering `34=1`. ADR-0056 had recorded this as an
open question and guessed wrong twice: `[verified 2026-09-06]` QuickFIX/J (`Session.java:2638`)
and QuickFIX/n (`Session.cs:1419`) **both** special-case their Logon reply to use the incoming
message's own number, because their generic path is stale for anything that answers something.
Two of the three reach it by special case, which is why reading only the generic path made it
look like one engine's quirk. One field, `last_in`, set before any reply is written.

**Two — a record layout must be `start .. start + LEN`, never `start .. next_field`.** The ring
dispatch record gained a field between two existing ones and `fixed[OUT_SEQ..OUT_LAST_PROCESSED]`
went from four bytes to five in silence. `copy_from_slice` **panicked, in a library crate** —
non-negotiable 7's subject, and invisible to the `unwrap_used`/`expect_used` lints that enforce
it. Four `dispatch` tests caught it; nothing else could have.

**Three — the ADR contradicted itself, and only the implementation could see it.**
`Header::last_processed` was a bare `u32` "supplied either way", which would have made every
reply through the `library` seam carry `369` regardless of the knob while the session's own
messages obeyed it. It is an `Option` now, and the ADR is revised in place with the revision
recorded (§5, it is still `Proposed`).

**Four — and this one is the most uncomfortable: a reversal did nothing.** Deleting the four
lines in `Reply::message` that write `369` left `cargo test --all` at **581 passed, 0 failed**.
ADR-0056 calls that guarantee *most of the point* of the whole change and it had no gate behind
it — it survived because it is the **easy** part, and the tests that existed covered what had
been hard to write rather than what would be expensive to lose. Three tests guard it now.
Written up with the interop false green in
[who-owns-the-outbound-header](docs/reference/who-owns-the-outbound-header.md), both marked
**`[to testing-skills]`**.

**The interop false green, measured rather than predicted.** Configured with QuickFIX/J's key
name — which this repository had recorded, and which QuickFIX C++ ignores in silence — the
scenario reads `interop: PASS 7/7` over a `789` that never crossed the wire. The correct key
gives the same seven greens plus the field. Only the added precondition tells them apart, and
...
repository judging its own work; `CONFORMANCE.md` says so where the interop table would
otherwise imply coverage by its silence.

---

## Before that — 2026-09-06: a draft re-verified, and the pair it would have reused is the wrong pair

**No code that day.** [seq-resync-789-369](docs/plans/2026-09-04-seq-resync-789-369.md) — wave B's second plan, `789=NextExpectedMsgSeqNum` and `369=LastMsgSeqNumProcessed` — was re-verified against `3f90561` and revised from *Draft* to *Chờ duyệt*, under item 45's standing rule that a draft is re-checked against the code of the day it is picked up.

**The rule paid for itself again, and this time the failure it prevented would have been silent.** The draft told a future builder to answer a low `789` by reusing `start_resend(begin, end)`. `[verified 2026-09-06]` **there is no such function.** The cursor that replays messages *out* is `owed: Option<Replay { next, end }>` (`crates/session/src/lib.rs:1096`), set when a `ResendRequest` arrives (`:2947`) and spent in batches by `continue_replay` (`:2434`). The pair whose names look right, `resend_from` / `resend_to` (`:1075`), tracks the opposite direction — *this end asked for a resend and is waiting on it* — and is closed by `advance_past`. Setting that pair in answer to `789` would leave the session believing it was owed something, **sending nothing**, while any test asserting session state passed.
...
## Before that — 2026-09-05, latest: wave B, plan 1 — the file a FIX desk expects

**All seven steps of [settings-for-both-roles](docs/plans/2026-09-04-settings-for-both-roles.md)
are done.** `engine::settings` recognised **eleven keys, every one of them the acceptor's**, so
the one role that *dials* was the one role that needed a rebuild to point somewhere else. It
recognises **twenty-three** now, against QuickFIX C++'s 113.

**The re-verification found seven wrong lines, and two halves of the work already delivered.**
Item 45 (c) held this plan back until items 46, 48 and 47 closed, and that was right:
`connect_and_serve` has seven parameters now rather than six and the entry-point count is
**twelve** rather than six (ADR-0047's `_with` twins, ADR-0054's `Handles`). Step 0's
`prior-art.md` correction had been made during the *first* re-verification on 2026-09-04, and
step 6's `const _: () = assert!(PRE == RX);` was made unnecessary by ADR-0047: both `PRE`
constants are gone, because the pre-session buffer **is** the engine's `RX` as a type parameter
rather than as a promise two comments made separately. **An assert checks a promise; a type
parameter removes the need to make one.**

**[ADR-0055](docs/decisions/ADR-0055-max-message-size-is-not-a-key-and-rx-is-the-answer.md) —
`MaxMessageSize` is not a configuration key in any engine, and this repository had said it was.**
`SessionSettings.h` carries **113 keys** and this is not one; `MaxMessageSize` is
`FixFieldNumbers.h:61`, **tag 383**, an optional `Logon` field by which the two ends tell each
other their limit. `[verified 2026-09-05]` the 113 was re-run from the vendor tree rather than
carried forward. `RX` is the answer and ADR-0047 already put it in the caller's hands; `RX = 4096`
is **not raised**, and the reason is that it has never been measured rather than that it is
expensive — wave C, on the §9 machine.

**What is in the file now.** `ConnectionType=acceptor|initiator` with `SocketConnectHost`,
`SocketConnectPort`, `ReconnectInterval` and `ReconnectCeiling`; `ResetOnLogon`, `ResetOnLogout`,
`ResetOnDisconnect`; `LogonTimeout`, `LogoutTimeout`; `AllowUnknownMsgFields`,
`ValidateUserDefinedFields`. **Six of the seven gaps `prior-art.md` listed against the
QuickFIX family closed in one day**; the seventh, `ValidateFieldsOutOfOrder`, came off the list
**rather than being built**, because this parser reads header-versus-body order out of a flat
index with one comparison inside the scan that checks everything else — there is no pass to
skip, and a missing feature and a feature that cannot exist in this shape are different facts.

**`Settings::into_table` returns `Result` now**, and refuses an initiator file by line. That is
the mistake worth a signature change: the table would be perfectly well formed, and the acceptor
would sit waiting for the venue it was told to dial, **with nothing on the wire to say so because
nothing would happen on the wire**. `serve_sharded_hft` takes a `Table` and nothing else, so the
sharded question the plan asked separately has the same answer — one mechanism, not a second
check to disagree with the first.

**`into_initiator` hands back the address as text, not a `SocketAddr`, against what the plan
wrote.** `TcpStream::connect` looks the name up on **every dial**, so a venue whose DNS fails
over keeps working; resolving at parse time would pin the engine to one address chosen at
startup and put a nameserver in the path of reading a file.

**`Registry::admit` sees the `Logon`.** ADR-0026 calls `lookup` the authentication hook, and
`[verified 2026-09-05]` it was handed an `Identity` — 49, 56, 50, 57 and nothing else.
`553`, `554` and `96` were on the message in front of it and **no implementation could reach
them**, so the hook the ADR named could not do the job the ADR named it for. The default
forwards to `lookup`, so `Table` and every registry written before today keep working.
**fixbolt still stores no credential and compares none.**

**A frame the pre-session stage can never read has its own number.** It was counted as *gone*,
beside a peer that simply left — a counterparty closing a socket and a counterparty this
acceptor **can never talk to** under one count. `Progress::unframeable` and
`Snapshot::unframeable_prelogon`, because a number nothing publishes is a number nobody has.

**Five findings this closing produced that the plan did not name.**

**One — a test was written here that could not fail, and only the reversal noticed.** It
asserted the 2 MiB resend ring is on the heap; reversing it did not turn it red, **it stopped
the crate compiling**, at a `const` assertion in `journal.rs` whose own comment reads *"a compile
error, not a test"*. The test had been green from the moment it was written and would have read
as coverage forever. **A reversal has three outcomes and not two** — red where aimed, green, or
*does not build*, which means something stronger got there first. Write-up:
[a-test-that-cannot-fail-reads-as-coverage](docs/reference/a-test-that-cannot-fail-reads-as-coverage.md),
**`[to testing-skills]`**.

**Two — a false green of my own, in a test guarding a goodbye.** `ResetOnLogout`'s test first
lived in `tests/logon.rs`, where the nearest thing to a counterparty `Logout` is swapping `35=A`
for `35=5` — which leaves `98=` and `108=` on the message and earns a `Reject`. The assertion
guarding *"the goodbye keeps the number it was owed"* read `|34=2|` and **passed on the Reject**,
because a Reject is also the second message that session sends. `tests/goodbye.rs` already had
the helper, with the trap in its own doc comment. **A number alone cannot tell a goodbye from a
refusal**, and both halves are asserted now.

**Three — a line whose reversal changed nothing, found and given a guard.** `connect` clears the
instant both new deadlines are measured from; removing those two lines broke no test, because
every test connected once. A deadline carried across a reconnect would expire the new connection
on the old one's clock. `a_reconnect_gets_a_whole_new_logon_deadline` says so now.

**Four — the plan predicted a trap and named the wrong fault code.** It expected
`AllowUnknownMsgFields=Y` to move `14a_BadField.def`. It cannot: `[verified 2026-09-05]` all
four of that file's cases are `373=0` and the knob governs `373=2`. What that file can see is
the *other* knob, on its `5000=HI` case — the one QuickFIX's own comment marks *"user defined is
not implemented yet"*.

**Five — memory figures three documents were doing arithmetic on were eight bytes stale.**
Published as 23 752 / 36 040 from a laptop on 2026-09-04; **23 760 / 36 048** here, wave A
accounting for the difference, and neither `CONFIGURATION.md` nor `GUIDE.md` had any way to
notice. `crates/engine/tests/connection_size.rs` asserts the part that matters — that raising
the receive buffer costs exactly the buffer and no padding.

`cargo test --all` **524 → 570**, 0 failed; `--no-default-features` 565; the **59 acceptance
definitions 59 / 59** through every step, mirror green; `scripts/interop.sh`
**7 / 7 + 8 / 8 + 6 / 6 + 6 / 6 + 6 / 6** unchanged; `benches/alloc.rs` 0 across every case in
`engine`, `session` and `library`, with a new `reject` case; clippy `-D warnings`, `fmt`,
`check-lint-config.sh` (by reversal), `check-no-optional-deps.sh`, `check-links.py` and rustdoc
under `-D rustdoc::broken_intra_doc_links` all clean.

**Thirteen reversals, each red only where it was aimed.** Among them: the receive buffer pinned
to a fixed size reads `left: 0, right: 12288`; each of the three `ResetOn*` terms dropped fails
its own test and no other; `into_table` without its role check fails one test; the pre-session
stage calling `lookup` again fails exactly the two credential tests; one `to_vec()` inside
...
**§9's last box, closed on the commit it asks about.** PR
[#46](https://github.com/tmthang86/fixbolt/pull/46) merged as **`0f9585d`**, run
[`33998809034`](https://github.com/tmthang86/fixbolt/actions/runs/33998809034), **11 jobs of
...
C++ initiator against this engine's acceptor, stopped by asking), three × `PASS 6/6` each with
`resumes: 1` and `two_sources ok`, and `==> the run added nothing git can see`.

The branch tip was green first — `8cfa3b2`, run
[`33980369287`](https://github.com/tmthang86/fixbolt/actions/runs/33980369287), also 11 of 11 —
and **that is not the same commit**. A pull request's own run checks out GitHub's merge ref; only
the run on `main` tests what actually landed. Items 48 and 47 needed a second pull request to
learn that, and this one did not.
...
the `ResetOnLogon` branch when its session was **resumed**, so the scenario needs the recovery
entry point in the interop fixture — a fixture change larger than the knob, and one that would
be judged by a gate nobody had written yet. What is proven instead is each layer and the seam
between them: the session behaviour by six unit tests with three neutral twins, the key reaching
`Config` by the settings tests, and **file → live session over a real socket** by
`settings_wire.rs::a_validation_knob_written_in_a_file_reaches_a_session_over_a_real_socket`,
whose reversal is a key that stops before the `Config`. The `ResetOn*` end-to-end scenario is
**open item 53**.
...
| 21 | ~~`serve_hft` pins nothing; the refusals D8 promises do not exist for it~~ — **closed 2026-09-13 without the §9 machine**: `serve_hft_pinned`, PR [#68](https://github.com/tmthang86/fixbolt/pull/68); the pinning tests run on GitHub's runner | `sched_setaffinity` |
...
| | |
|---|---|
| Branch | `[2026-09-13, merged]` **`main`**, at **`9346802`** — PR [#65](https://github.com/tmthang86/fixbolt/pull/65) merged `plan/an-obligation-nothing-checks` no-ff, 8 commits: items **57**, **64**, **69** and **70** closed, items **71**–**74** opened. **CI green on the merge commit itself**, run [`34707356369`](https://github.com/tmthang86/fixbolt/actions/runs/34707356369), **14 jobs of 14**; `git diff 5f2bbaa 9346802` is empty. **No pull request is open.** **Every open item that does not need the §9 machine is now closed** — what is left is items 40, 49, 51 and 52, which need `isolcpus`/`nohz_full`/`rcu_nocbs` restored and a reboot, plus the four opened here and the `tls` plan's steps 5-7. Before that: `[2026-09-12, later still, merged]` **`main`**, at **`064a48c`** — PR [#64](https://github.com/tmthang86/fixbolt/pull/64) merged `plan/gates-that-match-a-meaning` no-ff, 10 commits: item **68** closed, **67** narrowed and the narrowed part closed, **66** closed in its gate-A half only, and items **69** and **70** opened. **CI green on the merge commit itself**, run [`34700635273`](https://github.com/tmthang86/fixbolt/actions/runs/34700635273), **14 jobs of 14**; `git diff 3571d1e 064a48c` is empty. **No pull request is open.** **Item 69 is for the architect** — four ways past `check-scratch-fixtures.sh` are known, measured and deliberately unpatched, because each is one more regex. Before that: `[2026-09-12, merged]` **`main`**, at **`a8c9a3c`** — PR [#63](https://github.com/tmthang86/fixbolt/pull/63) merged `plan/crate-root-allow-and-scratch-fixtures` no-ff, 10 commits: items **58** and **23** closed, and step **4c** of the `tls` plan built in full (4c-0 the two-way documentation gate, 4c-1 the four keys, 4c-2 `load_pem` and the wire test, 4c-3 the documentation). **CI green on the merge commit itself**, run [`34691768411`](https://github.com/tmthang86/fixbolt/actions/runs/34691768411), **14 jobs of 14**; `git diff c1d2925 a8c9a3c` is empty. **No pull request is open.** Three new open items came out of the senior review — **66**, **67**, **68** — and none of them is a defect in shipped behaviour; all three are gates that match a spelling rather than a meaning. Before that: `[2026-09-09, merged]` **`main`**, at **`aca9365`** — PR [#58](https://github.com/tmthang86/fixbolt/pull/58) merged no-ff, item 60 closed, **CI green on the merge commit itself**, run [`34372299308`](https://github.com/tmthang86/fixbolt/actions/runs/34372299308), 13 of 13. **One pull request stays open on purpose**: [#57](https://github.com/tmthang86/fixbolt/pull/57), wave B plan 4, whose exit criteria are **not** met — §8 says merge only when they are. Before that: `main` at **`906c256`** — PR [#56](https://github.com/tmthang86/fixbolt/pull/56) merged no-ff, item 59 closed, **CI green on the merge commit itself**, run [`34366320153`](https://github.com/tmthang86/fixbolt/actions/runs/34366320153), 13 of 13. **One pull request is open**: [#57](https://github.com/tmthang86/fixbolt/pull/57), wave B plan 4's first four steps. Before that: `[2026-09-09, latest]` `plan/utc-timestamp-widths`, off `main` at **`28465e8`** — which is where `main` actually was, two commits past what this cell said for the fifth time running (PR [#55](https://github.com/tmthang86/fixbolt/pull/55) merged `043a4a2`). Before that: `[2026-09-09, merged]` **`main`**, at **`94b325d`** — PR [#54](https://github.com/tmthang86/fixbolt/pull/54) merged `claude/focused-brahmagupta-7ep6ok` no-ff, wave B plan 3 in full. **CI green on the merge commit itself**, run [`34340173659`](https://github.com/tmthang86/fixbolt/actions/runs/34340173659), **13 jobs of 13**; `git diff 301cd2e 94b325d` is empty. **No pull request is open** except the one carrying this line. **This cell has now gone stale four times, and the fourth was this session's own merge** — it is written the moment the merge lands, in the same pass that reads the merge commit's CI, because the three earlier times it was noticed days later by somebody re-reading the table. Before that: `[2026-09-09]` **`main`**, at **`974a14b`** — PRs [#52](https://github.com/tmthang86/fixbolt/pull/52) and [#53](https://github.com/tmthang86/fixbolt/pull/53) merged since this cell last read, and **it named `6d3a191` again**, which is the third time this one cell has gone stale. **CI green on the tip itself**, run [`34308659470`](https://github.com/tmthang86/fixbolt/actions/runs/34308659470), **13 jobs of 13**, including `interop`, `bench` and `deny`. **No pull request is open.** Before that: `[2026-09-08, later]` **`main`**, at **`6d3a191`** — PR [#51](https://github.com/tmthang86/fixbolt/pull/51) merged `claude/status-next-item-4677n1` (docs only: wave B plan 3 re-verified, two false cells in this table, one false green). **CI green on the merge commit itself**, run [`34252988486`](https://github.com/tmthang86/fixbolt/actions/runs/34252988486), 13 jobs of 13. Before that: `[2026-09-08]` **`main`**, at **`071bc79`** — three plans merged onto it since this cell last read, and **it said `16ad9b6` for five days**: `seq-resync-789-369` (PR [#48](https://github.com/tmthang86/fixbolt/pull/48)), the interop read fix (PR [#49](https://github.com/tmthang86/fixbolt/pull/49)) and `three-gates-a-sibling-engine-has` (PR [#50](https://github.com/tmthang86/fixbolt/pull/50), merge `93ae2b5`). **CI green on the tip itself, `071bc79`, run [`34244122687`](https://github.com/tmthang86/fixbolt/actions/runs/34244122687)**; the commit that closed the plan, `7bf7a71`, is runs [`34241853635`](https://github.com/tmthang86/fixbolt/actions/runs/34241853635) and [`34241859435`](https://github.com/tmthang86/fixbolt/actions/runs/34241859435), 13 jobs of 13. **No pull request is open.** Before that: `[2026-09-03]` **`main`**, at **`16ad9b6`** — PR [#32](https://github.com/tmthang86/fixbolt/pull/32) merged `plan/the-doc-set` no-ff and the branch is deleted on both sides. **CI green on the tip that was merged, `86c3913`, run [`33742995012`](https://github.com/tmthang86/fixbolt/actions/runs/33742995012), 11 jobs of 11.** Before that: `[2026-09-01]` **`claude/linux-desktop-testing-review-pgpaw1`**, and it **is** `origin/main` — the last five plans were merged onto it through PRs [#11](https://github.com/tmthang86/fixbolt/pull/11)–[#15](https://github.com/tmthang86/fixbolt/pull/15) and the remote's default branch now points at the same commit, `73e48c6`. **CI green there, run [`33473213210`](https://github.com/tmthang86/fixbolt/actions/runs/33473213210).** A local `main` ref left over from before that will read 83 commits behind; `git fetch origin main` settles it. **This is the branch-convention question below, answered by practice rather than by decision, and it should still be written down.** Before that: **`ktls-spike-steps-2-5`** — PR [#10](https://github.com/tmthang86/fixbolt/pull/10), `ktls-spike` closed. `[measured 2026-08-31]` **CI green on the commit being closed, `1b9b356`, run [`33386125577`](https://github.com/tmthang86/fixbolt/actions/runs/33386125577), 9 / 9 jobs.** Before that: **`main`.** PR #2 (`claude/project-status-hdx7k1`) merged 2026-08-30 as **`6d35b75`**, no-ff, 12 commits — `standard-mode` closed. `[measured 2026-08-30]` **CI green on the merge commit itself, run [`33326803468`](https://github.com/tmthang86/fixbolt/actions/runs/33326803468)**, and on the merged head `ae3d78a` before it, run `33325530208`, 9 / 9 jobs. `git diff ae3d78a 6d35b75` is **empty**, so the branch's green transfers to the merge exactly rather than by assumption. Before that: PR #1 merged as `76d6989`, run `33307963879` |
...
| Plan in flight | `[2026-09-14, later]` **[the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md), window A (A1–A8), built**, branch `plan/the-second-linux-desk-a` off `25e54dc`, draft PR [#72](https://github.com/tmthang86/fixbolt/pull/72); CI run [`34842390918`](https://github.com/tmthang86/fixbolt/actions/runs/34842390918) green on `f9abc1a`, the closing commit is `0861edf`, run [`34849545025`](https://github.com/tmthang86/fixbolt/actions/runs/34849545025), 14 jobs of 14 — after run [`34848285886`](https://github.com/tmthang86/fixbolt/actions/runs/34848285886) on `2ad9bb0` failed the rustdoc feature-set job on a doc link the F3 fix added, which no per-step gate had run. **Boot B is next** and is a pull request of its own; B6 needs the cable and the Mac. Before that: `[2026-09-14]` **[tls](docs/plans/2026-09-04-tls.md), its last two steps, 6-M and 7b**, branch `plan/tls-numbers-on-s9` off `1178f4d`, PR [#71](https://github.com/tmthang86/fixbolt/pull/71) — docs only, measured on the §9 desktop in a boot of their own. Approved and not started: [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md), whose B3 this was. Before that: `[2026-09-13, last]` **None in flight.** [the-residue-of-an-obligation](docs/plans/2026-09-13-the-residue-of-an-obligation.md) closed, PR [#68](https://github.com/tmthang86/fixbolt/pull/68), closing commit `30b53cb`, CI run [`34746295036`](https://github.com/tmthang86/fixbolt/actions/runs/34746295036). Approved and not started: [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md) (needs the §9 machine, a cable and an adapter), which also carries `tls` 6-M and 7b. Before that: `[2026-09-13]` **[tls](docs/plans/2026-09-04-tls.md), branch `plan/tls-initiator-and-numbers`, tip `022ca36`, PR [#66](https://github.com/tmthang86/fixbolt/pull/66).** Steps 5a–5c, 6a–6c (+6c-2/3/4) and 7a built; **6-M and 7b are the only two left and both need the §9 machine.** CI green on the closing commit itself, run [`34739943517`](https://github.com/tmthang86/fixbolt/actions/runs/34739943517), **14 of 14**. Three revisions written and approved during the build, all three before code moved: *Sửa 6* (steps 5–7 re-verified against the code of the day and split into brief-sized pieces, with the §9 half named as its own session), *Sửa 7* (a peer's `KeyUpdate` is a named carve-out of non-negotiable 1 — a design question, so it went to the architect and then to the owner rather than being decided by a developer), *Sửa 8* (the carve-out is `2 × rekeyed directions`, not a flat 4, and the constants bind to `Cargo.lock`). **Not proven:** no §8 TLS number exists, `hft` under TLS is checked by nothing, and nothing here has spoken TLS to another engine. **This cell is written when the branch is created, and it has now been stale six times** — it is being written at merge instead, which is the only change that has ever helped. Before that: `[2026-09-12, last]` **[an-obligation-nothing-checks](docs/plans/2026-09-12-an-obligation-nothing-checks.md)** — six steps, all built. The four open items that need no §9 machine, closed in one pull request; the architect chose every decision in it and three of the four reversed something this repository had in writing. Before that: `[2026-09-12, later still]` **[gates-that-match-a-meaning](docs/plans/2026-09-12-gates-that-match-a-meaning.md), branch `plan/gates-that-match-a-meaning`, six steps, all built.** Items **68** closed, **66** closed only in its gate-A half, **67** narrowed and the narrowed part closed; items **69** and **70** opened. Two plan revisions, both forced by a reversal: *Sửa 1* `shellcheck -S info` rather than `-S warning` (SC2086 is severity info, so the threshold the plan chose could never see the defect class section E named), *Sửa 2* a *Values* cell is an enumeration only at two or more literals. **Step 6, the senior review, changed the branch** — it got past all three new gates, six ways were fixed and four were left open by decision as item 69. **And CI was red twice before that review, for a reason the desk could not see**: `cargo test` prints `Running <path>` to stderr and `--list` names to stdout, and the gate attributed tests to binaries by their relative order through one pipe. **This cell has now been stale five times, and the sentence two entries down already said so** — it is written when the branch is created, not when it merges. Before that: `[2026-09-12, later]` **[tls](docs/plans/2026-09-04-tls.md) step 4c is built.** Four configuration keys (`SocketUseSSL`, `ServerCertificateFile`, `ServerCertificateKeyFile`, `TlsRequireKernel`) landed through [the 2026-09-12 plan](docs/plans/2026-09-12-crate-root-allow-scratch-fixture-and-tls-4c.md)'s steps 4-7, on branch `plan/crate-root-allow-and-scratch-fixtures`: step 4 the doc↔key two-way gate (`348b9a8`), step 5 = 4c-1, the four keys refused cleanly without the `tls` feature (`b39b52e`), step 6 = 4c-2, `tls::load_pem` and a real `.cfg` bringing up a TLS session over a socket (`0a49778`), step 7 the documentation that closes the deliberate red left at the end of step 6 (this commit). `cargo test -p fixbolt-engine --lib settings` is green now; it was the whole point of step 7. **No CI run is named yet for any commit after `41bdd99`** (14 of 14, run [`34682470085`](https://github.com/tmthang86/fixbolt/actions/runs/34682470085)) — the manager names one before this branch merges. **Not proven:** `shellcheck` has never run over any script in this repository; nothing here says anything about the engine thread under TLS (`scripts/check-no-kernel-sleep.sh` still has no TLS arm, `DESIGN.md` §8's TLS row is still empty). Steps 5, 6 and 7 of the `tls` plan itself — the initiator side, `w2w --tls`, and that same TLS arm — are still not started. Before that: `[2026-09-09, merged]` **[tls](docs/plans/2026-09-04-tls.md)**, wave B plan 4, on branch `plan/tls` — PR [#57](https://github.com/tmthang86/fixbolt/pull/57), **open and deliberately unmerged**. `[note 2026-09-09]` after item 60 landed, the indexing ceiling is **181**, not 184; that branch adds `tls.rs` and may need a pass over its indexing before it can rebase clean. **Steps 1-3 and the transport half of 4 are built and tested; 4b, 5, 6 and 7 are not**, and nothing in `presession.rs`, `lib.rs`, `settings.rs` or any `serve*` signature is touched, so **no deployment can reach TLS yet**. That is deliberate: settings keys nothing reads are a promise in a document. Before that: **[utc-timestamp-widths](docs/plans/2026-09-09-utc-timestamp-widths.md)** closed and merged as `906c256`, item 59. **Wave B plan 4, [tls](docs/plans/2026-09-04-tls.md), is APPROVED and next**: re-verified 2026-09-09, moved *Draft* → *Chờ duyệt* → approved, and its Sửa 1 records that `PendingSet` carries one transport type so the handshake belongs inside `TlsTransport::recv`/`send` rather than in a new pre-session stage — a simpler shape than the draft's, and `presession.rs` needs no change. `scripts/check-ktls-on-a-plain-socket.sh` is **green on kernel 7.0.0-31-generic**, spike 15 pass / 0 fail. Before that: `[2026-09-09, merged]` **None, and wave B plan 3 is on `main`.** [timestamp-micros](docs/plans/2026-09-04-timestamp-micros.md) closed both halves and merged as `94b325d`. **Nothing is approved-and-not-started.** Next in item 45's order is **wave B plan 4, [tls](docs/plans/2026-09-04-tls.md)** — a *Draft*, and the largest of them: 3–5 days, Linux from step 2, and `scripts/check-ktls-on-a-plain-socket.sh` re-run on the kernel of the day before it becomes a plan. Before that: `[2026-09-09, later]` **None.** [timestamp-micros](docs/plans/2026-09-04-timestamp-micros.md) closed today, both halves, with ADR-0057 `Accepted`. **Nothing is approved-and-not-started.** Next in item 45's order is **wave B plan 4, [tls](docs/plans/2026-09-04-tls.md)** — a *Draft*, to be re-verified against the code of the day before it becomes a plan. Before that: `[2026-09-09]` **None, and half B is now blocked on a signature rather than on an unwritten page.** [ADR-0057](docs/decisions/ADR-0057-sub-millisecond-time-arrives-beside-the-tick.md) is written and `Proposed`; [timestamp-micros](docs/plans/2026-09-04-timestamp-micros.md) half B starts when it is `Accepted` and not before. **Nothing is approved-and-not-started.** Before that: `[2026-09-08, later]` **None.** [timestamp-micros](docs/plans/2026-09-04-timestamp-micros.md)'s **half A** closed today, all five steps A1–A5; its **half B is not started and is blocked on `ADR-0057`, which is unwritten** — a plan half-closed on purpose, at a gate the plan itself put there, and not a plan abandoned. **Nothing is approved-and-not-started.** Next in item 45's order is either **ADR-0057** (which unblocks half B) or **wave B plan 4, [tls](docs/plans/2026-09-04-tls.md)** — a *Draft*, to be re-verified against the code of the day before it becomes a plan. Before that: `[2026-09-08]` **None.** [three-gates-a-sibling-engine-has](docs/plans/2026-09-07-three-gates-a-sibling-engine-has.md) closed today, all five steps, on the Linux desk. Before it, [seq-resync-789-369](docs/plans/2026-09-04-seq-resync-789-369.md) closed 2026-09-06 — wave B plan 2, [ADR-0056](docs/decisions/ADR-0056-the-application-is-told-what-the-session-owns.md). **Nothing is approved-and-not-started.** Next in item 45's order is **wave B plan 3, [timestamp-micros](docs/plans/2026-09-04-timestamp-micros.md)** — draft re-verified against the code of 2026-09-08 and moved to *Chờ duyệt*; it found that **the send half needs an ADR before any code**, because `TimestampCache` lives inside `Session` (`crates/session/src/lib.rs:1179`) and is fed `self.now_ms`, which only ever comes from `Input::Tick` in milliseconds (`:1921`, D13) — a pure session has no sub-millisecond time to format, and the draft had assumed the engine could pass one down. **This cell itself read `[2026-09-05] None` for three days and three merged plans**, which is the same rot it already records two sentences later about `counterparty-registry`. Before that: `[2026-09-05]` **None.** [what-is-left-and-what-a-message-touches](docs/plans/2026-09-05-what-is-left-and-what-a-message-touches.md) closed and merged as **`44c57ec`**, PR [#43](https://github.com/tmthang86/fixbolt/pull/43); **CI green on the merge commit itself, 11 jobs of 11, run [`33959509459`](https://github.com/tmthang86/fixbolt/actions/runs/33959509459)** — the tip that was merged, `4e3fb1c`, was 22 of 22. **The machine has been returned to desktop settings in full, and §9 is no longer one command away.** `fixbolt-machine off` + `smton` restored the runtime knobs (governor `powersave`, boost on, SMT on, 16 logical cores, THP `madvise`, `busy_poll` 0). `[2026-09-05]` the three boot parameters were then **removed from `/etc/default/grub`** as well — `isolcpus=6,7,14,15`, `rcu_nocbs=6,7,14,15` and `processor.max_cstate=1` — because `fixbolt-machine off` cannot touch them and they are not free on a desktop: four of sixteen logical CPUs were held back from the scheduler, and `max_cstate=1` left only `POLL` and `C1`, so the box never idled deeply. `update-grub` run, `/boot/grub/grub.cfg` clean, a timestamped backup of `/etc/default/grub` beside it. **They take effect at the next boot and are not in force yet.** So the §9 recipe for this box is now: put those three back on the kernel command line, reboot, then `fixbolt-machine on` + `smtoff` — **two reboots, not one command**. Items 49, 51, 40 and wave C all pay that. [what-is-left-and-what-a-message-touches](docs/plans/2026-09-05-what-is-left-and-what-a-message-touches.md) closed today — item 14's open half closed, item 49 narrowed from ~2 804 to ~2 770 ns with two candidates retired, and three new open items came out of it (51, 52, and the framing candidate that item 49 now holds alone). **Four revisions were written into that plan before or during the build and every one of them changed what got built**: the payload sizes were wrong, the struct size was a week stale, `Loopback` could not be used, and N=128 cost more machine time than it was worth. **Nothing is approved-and-not-started** from this session; two plans written in parallel by another session for items 47 and 48 are drafts awaiting review. Before that — `[2026-09-05]` **None.** [the-baselines-and-the-pass-nobody-timed](docs/plans/2026-09-02-the-baselines-and-the-pass-nobody-timed.md) closed today, all three steps — items 41, 39 and 34. **Nothing is approved-and-not-started**: the seven wave B/C/D files are drafts, and item 45 holds their order. Before that — `[2026-09-05]` the same plan, steps 1 and 2 closed, step 3 open. Before that — `[2026-09-05]` **all three plans written 2026-09-03 are closed** (acceptor-interop 09-04, resend-from-the-journal 09-04, message-log 09-04), and so are reconnect-interop, buffers-the-caller-can-size and an-engine-that-can-speak-first, all 09-05. **This cell said *None started, three Chờ duyệt* for two days after the last of them closed** — the same failure it already records one sentence later about counterparty-registry, which is why the plan table above now carries the in-flight plan as its topmost row rather than leaving it to this cell alone. Before that — **None, and nothing is approved-and-not-started either.** `[2026-09-03]` **[the-doc-set](docs/plans/2026-09-02-the-doc-set.md) closed and merged** (A, E, B, D; C deferred to phase 2) — PR [#32](https://github.com/tmthang86/fixbolt/pull/32). **[counterparty-registry](docs/plans/2026-09-01-counterparty-registry.md) is CLOSED, not pending** — this cell called it *approved and not started* for two days after item 28 had been struck; it closed 2026-09-01 with ADR-0029 and ADR-0030. **So the next unit of work starts with writing a plan, not with picking one up**, and the candidates are open items **34** (the library's reply path, machine-independent to fix), **39** (the dictionary pass has never been timed), **36 / 38** (the initiator, still paused on ADR-0006) and **phase 2's first ADR**, which is also what un-defers `docs/internals/`. Before that: `[2026-09-01]` three closed that day — `pre-session-routing`, `what-mitigations-cost`, `release-profile`; `[2026-08-31]` six before them — `threads-and-affinity`, `standard-mode`, `serialise-and-the-60ns-target`, `session-recovery`, `ring-full-policy`, `ktls-spike`; `[2026-08-30]` `gates-that-can-be-trusted` and `data-fields`. `[2026-09-02]` **`w2w-and-linux-numbers` is now closed in full** — half B ran at the desktop and closed items 6, 11, 13 and the decision on 12, which is what closed phase 1. **One plan is neither closed nor in flight and says why**: the initiator plan is **paused** on ADR-0006. **So the next unit of work starts with writing a plan, not with picking one up** |
| Item 45's order `[2026-09-18]` | Moved here from the open-items row, struck 2026-09-18: 1. boot C (this plan's measurement package); 2. boot D; 3. wave D ([const-templates-in-dict](docs/plans/2026-09-04-const-templates-in-dict.md), [deterministic-simulation](docs/plans/2026-09-04-deterministic-simulation.md)); 4. phase 2 opens with [ADR-0078](docs/decisions/ADR-0078-sbe-enters-as-an-encoding-without-a-session-and-fixp-is-its-own-phase.md) / [ADR-0079](docs/decisions/ADR-0079-one-view-per-encoding-and-one-trait-over-them.md); 5. the BPF `standard` stamp ([ADR-0073](docs/decisions/ADR-0073-the-standard-wire-figure-goes-through-bpf-sock-ops-and-not-before-phase-2.md)) at phase 2's first §9 boot |
...
`[measured 2026-09-14]` **both mode gates' kTLS arms on the §9 desktop, and the TLS round trip.**
Linux `7.0.0-31-generic`, `check-machine.sh` `pass 12 fail 0 unknown 1`, commit `1178f4d` (CI run
[`34767259852`](https://github.com/tmthang86/fixbolt/actions/runs/34767259852)).
`check-no-kernel-sleep.sh`:
`GREEN ok — --tls ktls: no blocking call, socket calls present, tls: kernel`, exit 0. `check-standard-gives-the-core-back.sh`: `standard + ktls` at **.31% CPU, sleeping
20/20**, `tls reported kernel`, exit 0. `scripts/w2w-baseline.sh`, eight arms, run twice, both
`baseline exit=0` — `DESIGN.md` §8 *The round trip under TLS, measured*; `docs/CONFORMANCE.md` §8;
`docs/reference/measured-costs.md` *TLS on the wire*.
...
`[2026-09-14]` **Walked for the `tls` 6-M/7b pull request, [#71](https://github.com/tmthang86/fixbolt/pull/71).** `grep` over this file for
*TLS row*, *TLS number*, *no TLS arm*, *hft under TLS* and *question 2* found two bullets here and
six in older *Start here* sections. Five claims the 2026-09-14 measurement disproves are struck
with that date (one here, four there); the two `hft`-under-TLS bullets (one here, one there) are
narrowed rather than struck, because their initiator half still stands; and one bullet naming
questions 2 and 6 as open is annotated. What this pull request adds to the list is in its *Start
here*: no cause for either finding, and items 84 and 85.

`[2026-09-13, last]` **Walked for PR [#68](https://github.com/tmthang86/fixbolt/pull/68), nothing struck.** `grep` over this section for `serve_hft`,
*pins nothing*, *eighteen*, `check-links`, `number()`, `doc_table`, *probe* and *required* found two
bullets, both about `serve_sharded_hft`'s recovery and stop — neither is about an item this pull
request closed. What it adds here is in its *Start here*: `serve_hft_pinned`'s hot path under `hft`
is unmeasured, and items 75–79 are single measurements with no test behind them.

`[2026-09-13]` **Walked for PR [#66](https://github.com/tmthang86/fixbolt/pull/66). One bullet was
read as a candidate to strike and was left standing**, correctly: the interop bullet's *"one
counterparty, no TLS"* is still true, because `scripts/interop.sh` runs plaintext and the kTLS peer
in `crates/engine/tests/tls_key_update.rs` is this repository's own code, not another engine. What
that pull request **adds** here is three lines, and the first is the one a reader should weigh
hardest:

- ~~**No §8 TLS number exists.** `DESIGN.md` §8's TLS row is still empty.~~ — **FILLED 2026-09-14**
  by `tls` steps 6-M and 7b on the §9 desktop, `pass 12 fail 0 unknown 1`: `DESIGN.md` §8 *The
  round trip under TLS, measured*, and *Start here* 2026-09-14. What it left unproven is written
  there — no cause for either surprise, no NIC, one kernel, one suite, two procedures that disagree
  on one arm (items 84, 85). Every figure produced on
  this branch was measured with the machine in **desktop** configuration —
  `scripts/check-machine.sh` reads `pass 5 fail 7 unknown 1`, against the `pass 12 fail 0 unknown 1`
  §9 asks for — so by non-negotiable 10 none of them is a published number, and none is quoted as
  one. Steps 6-M and 7b of the `tls` plan are exactly this gap.
- **`hft` mode under TLS is unmeasured and unchecked by anything.** `connect_and_serve_tls` is
  `standard` only, and both of non-negotiable 4's scripts trace `tools/w2w`, an acceptor. The new
  dial-loop guard covers `standard` on the initiator and says so in its own *What this file does NOT
  prove* header. `[narrowed 2026-09-14]` neither word holds for the acceptor: `hft` with kTLS
  through `tools/w2w` is measured in `DESIGN.md` §8, and traced by `scripts/check-no-kernel-sleep.sh`'s
  kTLS arm (step 6b). The initiator half stands.
- **Nothing in this repository has spoken TLS to a counterparty it did not write.** The
  `KeyUpdate` behaviour, the session-ticket count and the kTLS handover are all measured against
  rustls, ktls-core and a raw kTLS peer built here. `libquickfix` is never reached over TLS.

`[2026-09-12, last]` **Walked for PR [#65](https://github.com/tmthang86/fixbolt/pull/65), and nothing
here had to be struck** — no bullet in this section claimed anything about items 57, 64, 69 or 70,
which is itself the §4 row that exists because this section rots in the direction of claiming the
project is *less* proven than it is. Recorded rather than left silent. **What that pull request adds
to this section is one line**, and it is about the thing it built rather than the thing it closed:
`DropReason::NeverTicked` is proven for `now_ms == 0` by test and by reversal, and **the three other
rows of its table in `docs/SESSION-BEHAVIOUR.md` — a tick of `1`, a tick in Unix-epoch milliseconds,
and the control — are measurements taken once on this desk, with no test guarding any of them.** They
are there so a reader can re-run them, not so a reader can trust them.
...
- **`[2026-09-08]` Nothing here has seen a real venue send a microsecond `52=`.** Half A of
  wave B plan 3 reads 24- and 27-byte timestamps because FIX 5.0 SP2 EP allows them and
  MiFID II RTS 25 requires the clock behind them — **built from a specification and a
  regulation, not from a capture.** 0 of the 59 definitions carry a stamp wider than 21
  bytes, and the `libquickfix` interop fixture sends milliseconds by default, so neither
  gate is an outside opinion about this. `CLAUDE.md` §7 rates a real capture above an
  invented message; there is no capture. **`[amended 2026-09-09]` half of that is now false and
  is struck rather than left to read as modesty**: `scripts/interop.sh` §4h stands a real
  `libquickfix` up at `TimestampPrecision=6`, and it both sends and accepts a 24-byte `52=` —
  10 of them from the oracle, 12 from this engine, on QuickFIX's own transcript. So there **is**
  an outside opinion, from another implementation over a real socket. `[amended 2026-09-09]` **§4i strengthens it again**: the oracle is stood up at `TimestampPrecision=2`, a width this engine **cannot be configured to send**, so those bytes cannot have come from any fixture here. What is still missing is a
  capture from a **venue**, which is a different claim and a weaker gap than this bullet used to
  describe. **What is
  not in doubt is the old behaviour**: the silent hang-up and the `373=1` naming tag 122
  were both reproduced by reversal, output quoted in
  [reference/a-valid-field-refused-for-its-width.md](docs/reference/a-valid-field-refused-for-its-width.md).
- **`[2026-09-09]` The cost of `TimestampPrecision` has never been measured on the §9 machine.**
  Half B's figures — 4.7 → 5.4 ns at the default, +0.6 for the runtime branch and +0.1 for the
  slice return, and 10.5 / 13.0 ns at `Micros` / `Nanos` — were taken on an **Intel Xeon @
  2.80GHz shared cloud VM**, three runs per arm, one variable at a time. They are internally
  comparable and say nothing about the §9 desktop. The published **4.9 ns** row
  (`benches/baselines.tsv:141`, AMD Ryzen 7 3700X, n = 20) was **not** re-measured and no
  baseline row was added for the two new arms, so `scripts/bench.sh --strict` reports them as
  *no baseline* rather than as a pass. The machine is two reboots from §9 (`[2026-09-05]`).
  ADR-0057 decision 5 named that row as the constraint; it has been answered on one machine and
  not on the one that counts.
...
- ~~**`[2026-09-12]` Neither new script has been through `shellcheck`.**~~ **Narrowed
  2026-09-12** by [gates-that-match-a-meaning](docs/plans/2026-09-12-gates-that-match-a-meaning.md)
  Sửa 1: `shellcheck -S info` now runs in the `lint-config` CI job over
  `scripts/check-no-crate-root-allow.sh`, `scripts/check-scratch-fixtures.sh` and
  `scripts/check-feature-gated-tests-ran.sh` — **three scripts, not zero** — and `-S info` is the
  level that matters, since the step's own reversal found `-S warning` cannot see `SC2086`
  (severity *info*, not *warning*, `[measured 2026-09-12]` on shellcheck 0.11.0). `[measured
  2026-09-12]` `check-scratch-fixtures.sh` reported two `SC2094` at `-S info`, both false
  positives (both uses of `$f` there are reads, not a read-and-write of one file), disabled at the
  two loops with a reason each rather than file-wide. **Still true, narrower**: `shellcheck` was
  proven by seven reversals against **what these scripts assert**, and that is still no evidence
  about quoting, word splitting, or an unset variable on a path no reversal took. **Seventeen
  scripts are still not covered, and two of those need the §9 machine** to run at all, so a CI job
  cannot reach them either. The risk is not theoretical in this repository: item 58's first
  reversal was defeated by an unquoted backtick pair inside an `echo`, and it produced the same
  number the real defect produces
  ([an-allow-at-the-top-of-a-file-silenced-the-whole-crate](docs/reference/an-allow-at-the-top-of-a-file-silenced-the-whole-crate.md)).

`[2026-09-18]` **PRD 7's wakeup number (C-PRD7)** — `crates/engine/benches/wakeup.rs` is built by
plan step 4.2, but no run has produced p50/p99/p99.9, so [ADR-0025](docs/decisions/ADR-0025-hft-has-a-hard-session-ceiling-and-the-engine-advises-rather-than-applies.md)'s
figure of 4 stays unconfirmed until boot C's C-PRD7 records it.

`[2026-09-18]` **Item 49's in-situ kernel number (C-49)** — the session's own `Heartbeat` serialise
gets a bench case at plan step 4.1, but the remainder of ~2 770 ns that a kernel `strace`/`perf`
delta would attribute is not measured until boot C's C-49, per
[docs/decisions/ADR-0074-kernel-bypass-io-uring-and-the-logon-hop-stay-unmeasured-by-decision.md](docs/decisions/ADR-0074-kernel-bypass-io-uring-and-the-logon-hop-stay-unmeasured-by-decision.md).
...
| ~~[the-tls-tests-have-no-ci](docs/plans/2026-09-10-the-tls-tests-have-no-ci.md)~~ | 62 and 65 — **CLOSED 2026-09-12**, Sửa 1, steps 1-8. A `tls` CI job, the kernel requirement asserted in its own words, and — added by Sửa 1 because the job's own first run found two red tests no step owned — the flake diagnosed by measurement and fixed by construction. **Touches no line of `crates/*/src`**, and the measurement is what kept it that way |
| ~~[the-residue-of-an-obligation](docs/plans/2026-09-13-the-residue-of-an-obligation.md)~~ | 21, 67, 71, 72, 73, 74 — **CLOSED 2026-09-13**, PR [#68](https://github.com/tmthang86/fixbolt/pull/68), all seven steps; 55 kept as a standing ratchet (Q5). Closing commit `30b53cb`, CI run [`34746295036`](https://github.com/tmthang86/fixbolt/actions/runs/34746295036). **The senior review found five ways past guards whose reversals had all gone red**, fixed twelve findings in all, and opened items 75–79 from what it measured outside the PR |
| ~~[handles-through-the-front-door](docs/plans/2026-09-05-handles-through-the-front-door.md)~~ | 47 — **CLOSED 2026-09-05, all five steps**, [ADR-0054](docs/decisions/ADR-0054-the-handles-are-made-before-the-engine-and-the-engine-adopts-them.md) `Accepted`. `observe::Handles` is built before the engine and adopted by it, the ten `serve*` / `connect_and_serve*` signatures each gained the parameter, `dial` stopped draining the user's event ring, and `serve` returns through `Admin::shutdown` — which `GETTING-STARTED.md:186` had promised since 2026-09-03. **This row read `Chờ duyệt 2026-09-05, not started` for eight days and through six merged pull requests**, while item 47 four screens below it read `CLOSED 2026-09-05` and named this same plan. `[measured 2026-09-13, last]` the contradiction was settled by reading the code rather than either row: `crates/engine/src/lib.rs:1717` takes `handles: crate::observe::Handles` as `serve`'s last parameter, `observe.rs:440` declares the type, `lib.rs:648` is `Engine::adopt`, and the plan's *Nhật ký giao hàng* reads *Đóng 2026-09-05, cả năm bước* with `cargo test --all` 519 → 524. **The staleness ran the other way from §4's warning**: *Not proven* rots by claiming the project is less proven than it is, and so, it turns out, does this table — a plan table and an item table can disagree about one plan, and nothing reads them against each other |
...
| ~~[settings-for-both-roles](docs/plans/2026-09-04-settings-for-both-roles.md)~~ | 45, wave B, plan 1 — **CLOSED 2026-09-05**, all seven steps, [ADR-0055](docs/decisions/ADR-0055-max-message-size-is-not-a-key-and-rx-is-the-answer.md). Eleven configuration keys become twenty-three; a file describes either role; `Registry::admit` sees the `Logon`; `Reply::business_reject`; an unframeable pre-session socket has its own number. Two revisions written **before** code moved (*Sửa 2*, *Sửa 3*) and both changed what got built |
| ~~[seq-resync-789-369](docs/plans/2026-09-04-seq-resync-789-369.md)~~ | 45, wave B, plan 2 — **CLOSED 2026-09-06**, all six steps, [ADR-0056](docs/decisions/ADR-0056-the-application-is-told-what-the-session-owns.md). `789` read unconditionally and sent behind `SendNextExpectedMsgSeqNum`; `369` behind `EnableLastMsgSeqNumProcessed`, which cost a public API change because a reply is bytes the application wrote. `cargo test --all` 570 → 585; `scripts/interop.sh` gains a sixth scenario, **9 / 9**. Four findings changed the code and **one reversal did nothing**, which is the one worth reading. Re-verified before approval: Re-verified against `3f90561` and revised: **one line of its *Những gì đã biết chắc* was simply wrong** — `start_resend(begin, end)` does not exist, the outbound replay cursor is `owed: Option<Replay>` (`lib.rs:1096`) spent by `continue_replay`, while `resend_from`/`resend_to` are the **inbound** gap tracker, so the draft's instruction would have set the wrong pair and left the session silent behind green state assertions. Two more were right about the code and wrong about where it lives, and `EnableNextExpectedMsgSeqNum` **cannot be verified from `vendor/` at all** — `SessionSettings.h` is not in the sparse checkout, so it stays `[documented]` and the interop step is the only oracle. **And the re-verification found what the draft could not**: an application *reply* never passes through `rebuild()` — the application writes the whole message and the session emits it untouched (`lib.rs:2982`) — so `369` on every outbound message is impossible to patch in and the plan now opens with an ADR about the `Application` seam rather than a line of code |
| ~~[timestamp-micros](docs/plans/2026-09-04-timestamp-micros.md)~~ | 45, wave B, plan 3 — **CLOSED 2026-09-09, both halves**, [ADR-0057](docs/decisions/ADR-0057-sub-millisecond-time-arrives-beside-the-tick.md) `Accepted`. `TimestampPrecision` the 26th key; `52=` out at 21/24/27; `Session::tick_at` carries the remainder beside `now_ms` in one call and D13 is unreversed; the configured width is a **ceiling** and a millisecond tick writes 21 bytes rather than `.xxx000`. `cargo test --all` 594 → 613. **Step B4 found a defect in the half that closed the day before**: `dict::FieldType::UtcTimestamp` accepted 8 or 12 bytes of time, so a `libquickfix` counterparty at `TimestampPrecision=6` logged on and had every message after it answered `373=6` — one field, two readers, and only somebody else's engine could see it ([reference](docs/reference/one-field-two-readers.md)). `[measured 2026-09-09]` +0.7 ns at the default on a **non-§9** machine, branch 0.6 and slice return 0.1, one variable at a time; the 4.9 ns §9 row was not re-measured. **Before that: HALF A CLOSED 2026-09-08**, all five steps A1–A5; `parse_utc` reads 17/21/24/27 bytes and truncates the fraction, `52=` and `122=` both. `cargo test --all` 589 → 594. `[2026-09-09]` **Half B is still not started, and its gate is now written rather than missing**: [ADR-0057](docs/decisions/ADR-0057-sub-millisecond-time-arrives-beside-the-tick.md) is `Proposed` and awaits the owner — option (b), the remainder beside `now_ms` in one call, D13 unreversed. The engine still writes 21 bytes and there is no key to change it. Writing the ADR also found **two things the re-verification had not**: the clock is read **once per turn** (`engine/src/lib.rs:950`), so `52=` names the turn's start and not the send; and the plan's own `TimestampPrecision=SECONDS|MILLIS|MICROS|NANOS` row was **cited to a document that has never said it** — the oracle's key is an int 0–9, which also means `scripts/interop.sh` can judge this feature in both directions. Sửa 1 written before the code moved — see the plan's delivery log. Re-verified against the code of the day as item 45 requires, and **five facts had moved**. The one that changed the plan's shape: a `52=` of 24 bytes is `None` from `parse_utc`, which is `time_ok = false`, which in `AwaitingLogon` is `Refusal::BadSendingTime` — **a silent hang-up, no Reject, for a timestamp FIX 5.0 SP2 EP and MiFID II RTS 25 both call valid**. And the send half **cannot be built as drafted**: `TimestampCache` sits inside `Session` (`lib.rs:1179`) fed by `self.now_ms`, assigned only from `Input::Tick` in milliseconds (`lib.rs:1921`, D13), so a pure session has no sub-millisecond time to format and `SystemClock::now_ms` truncates with `as_millis()` (`engine/src/clock.rs:35`) before it could. Split in two with a gate between: the receive half needs no ADR and no §9 machine; the send half needs **ADR-0057** to say where sub-millisecond time enters, and the cheapest answer — padding `.123000` — is a lie about resolution to a venue that measures it |
| [tls](docs/plans/2026-09-04-tls.md) | 45, wave B, plan 4 — `[2026-09-14]` **EVERY STEP BUILT: 6-M and 7b on branch `plan/tls-numbers-on-s9`, PR [#71](https://github.com/tmthang86/fixbolt/pull/71), measured in their own §9 boot rather than inside the-second-linux-desk's; the plan closes when that pull request merges.** First TLS figures in `DESIGN.md` §8; kTLS read slower than userspace `rustls` in both `hft` paths, and one arm's p50 moved 15.9% (two kTLS arms' p99 7–19%) between two identical procedures — items **84** and **85**, no cause for either. Before that: **STEPS 1–3, 4a, 4b, 4c, 5a–5c, 6a–6c, 7a CLOSED; 6-M and 7b OPEN and both need the §9 machine.** Session A merged as PR [#66](https://github.com/tmthang86/fixbolt/pull/66), CI green on the closing commit `022ca36`, run [`34739943517`](https://github.com/tmthang86/fixbolt/actions/runs/34739943517), 14 of 14. [ADR-0063](docs/decisions/ADR-0063-a-peers-key-update-is-the-second-named-carve-out-and-a-ticket-is-not-read.md) `Accepted`: a peer's `KeyUpdate` allocates `2 × rekeyed directions` boxes of 184 bytes each and a session ticket allocates nothing, both bound to `Cargo.lock` and to `--locked` in CI. **The pull request's three most valuable findings were none of the ones it was written to produce**: `ktls-core`'s `tls13-key-update` was off, so a counterparty's `KeyUpdate` killed the session in merged code; the initiator dialled 879 times in 1.5 s because `up` was set after the first turn; and a kTLS guard written the same day was a false green whose reversal went red in the wrong place. A third machine check for non-negotiable 4 came out of it — `the_dial_loop_sleeps_rather_than_spins_while_the_handshake_waits`, because both of rule 4's scripts trace `tools/w2w` and `w2w` is an acceptor. **~~No §8 TLS number is published~~ `[struck 2026-09-14]`, `hft` ~~under TLS~~ on the TLS initiator `[corrected 2026-09-14]` is checked by nothing, and nothing here has spoken TLS to another engine.** ~~6-M and 7b run inside the §9 sitting of [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md)~~ `[2026-09-14]` they ran in a §9 boot of their own |
| [closing-the-open-items](docs/plans/2026-09-18-closing-the-open-items.md) | every open item that needs no §9 measurement, plus the boot C bundle — `[2026-09-19]` **BUILT to its desk-free end and measured at boot C except C-40**, branch `plan/the-second-linux-desk-c`, PR [#78](https://github.com/tmthang86/fixbolt/pull/78); open: C-91b (bisect), C-40 (Mac). Approved by the manager 2026-09-18 under the owner's mandate |
| [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md) | 45, wave C, and items 40, 49, 51, 52 — `[2026-09-19]` **boot C in flight on PR #78**: item 89 and the bundle measured; the `mitigations=off` arm and boot D wait for the owner's reboot. Before that: `[2026-09-14, later]` **WINDOW A BUILT (A1–A8), branch `plan/the-second-linux-desk-a`, PR [#72](https://github.com/tmthang86/fixbolt/pull/72); boot B next**, then boot C (Q1) and D. Approved 2026-09-13 (Sửa 1), Sửa 2 approved 2026-09-14; B3 done by PR #71. B6 needs the cable and the Mac. Before that: **Draft 2026-09-04**; needs the §9 desktop and a second machine with a NIC |
...
| 23 | ~~**A gate can be green in CI and red on the machine doing the work.**~~ — **CLOSED 2026-09-12, the class and not only the instance.** `scripts/check-scratch-fixtures.sh` holds it, keyed on the trigger rustup actually uses: *entering* a directory outside the tree, not calling `mktemp`. Every such line must carry, in the same script, a `cp` of **each** pinning artefact that exists at the repository root — a set derived from what is there rather than hard-coded, with an empty set an error, because a deleted `rust-toolchain.toml` must be heard. 19 scripts, 1 entering a scratch dir, 1 pin; in the `lint-config` CI job. **There is no allow-list, on purpose** — a named exemption is permanent, and the day an exempt script grows a `cargo build` in its scratch dir nobody would check; the three audited-clean scripts below pass by never leaving the tree instead. Three reversals, each red on its predicted sentence, including the one the manager re-ran: `check-scratch-fixtures: FAIL — scripts/check-no-kernel-sleep.sh:102 enters a scratch dir ($TMP, from mktemp at :31) and never copies rust-toolchain.toml into it` — a script no list named, which is the point. **What is now held, precisely**: a `cd`/`pushd`/`--manifest-path`/`-C` into a scratch path inside `scripts/*.sh`, with the variable chased to a fixed point (`TMP` → `CRATE`), plus a refusal of `Command::new("cargo"|"rustc")` anywhere in `crates/` or `tools/` because this check cannot read Rust. **What is still not held, and the script's header says so in the same words**: a `cd` reached through a function or `eval`; `cd -- "$(dirname "$X")"` and other indirection; a fixture built in a `ci.yml` `run:` step under `$RUNNER_TEMP`; a fixture written in Python or any other language; and an artefact pinned at the **machine** level (`$CARGO_HOME/config.toml`), which no repository file can pin and therefore no repository check can see. Each gets the same answer when it appears for real: extend the script in the commit that finds it. The original finding: `[measured 2026-08-31]` `scripts/check-lint-config.sh` built its throwaway crate in `mktemp -d`, where `rust-toolchain.toml` does not reach, and on a desktop with no `rustup default` it exited 1 saying *"the workspace lints do not deny: unwrap_used expect_used panic"* — while `cargo clippy` had not run at all. A **false red about the system under test**, and the same construction had a quieter twin: on any machine that *did* have a default, the gate was checking the workspace's lint config against a different clippy from the one the workspace pins, which `rust-toolchain.toml`'s own comment calls load-bearing. Fixed by copying `rust-toolchain.toml` into the scratch crate; proven by reversal — commenting out `unwrap_used = "deny"` names that one lint and exits 1, restoring it exits 0. **Found only because §9's checklist requires every gate to be run here and its output read.** Nothing in CI could have shown it: CI is the environment where it passes. Write-up: [reference/a-scratch-fixture-inherits-the-machine.md](docs/reference/a-scratch-fixture-inherits-the-machine.md), marked `[to testing-skills]`. **The other scripts were then audited and are clean**: three more use `mktemp -d`, but only for output files — `check-no-kernel-sleep.sh` and `check-standard-gives-the-core-back.sh` run a binary built in the tree, and `check-ktls-on-a-plain-socket.sh` runs `cargo build` inside `spikes/ktls`, where rustup still walks up to the repository's `rust-toolchain.toml`. One script was affected and it is fixed. | closed — the instance 2026-08-31, the class 2026-09-12 |
...
| 32 | ~~**Sharded deployments still cannot resume.**~~ — **(a) CLOSED 2026-09-20**, [ADR-0088](docs/decisions/ADR-0088-recovery-reaches-the-sharded-runtime-and-the-journal-crosses-the-channel-with-the-connection.md), `bd6be07`. `serve_sharded_hft_with_recovery` and `serve_sharded_hft_with_recovery_with` ask a `Recovery` on the acceptor thread (which may block) and carry the answer as `Start<J>` down the existing `Pending` channel to the shard thread; `Shardable::add_started` is the required method, `add` keeps every old call site compiling through `Start::Fresh(J::default())`. Proven by `crates/engine/tests/shard_recovery.rs::a_sharded_acceptor_resumes_the_numbers_recovery_hands_it` — a `Recovery` answering `Resumed { next_out: 5, … }` gets a Logon reply carrying `34=5`, not `34=1` — and `serve_sharded_hft_serves_a_session` in `tests/shard_hft.rs` stays green **unmodified**, proving the fresh (`NoRecovery`) path did not move (`tests/shard.rs` is a different file and *did* change — `Counter` implements `Shardable`, so the new required method is an `E0046` for it). `[2026-09-02]` **(b) and (c) are closed** — [ADR-0039](docs/decisions/ADR-0039-a-fresh-journal-is-the-deployments-to-build.md), [plan](docs/plans/2026-09-02-recovery-reaches-the-disk.md). `Recovery::fresh` moved the `J: Default` bound off the serving loop and onto the implementations that want it, so a `FileJournal` per counterparty now runs through `serve_with_recovery`; and `Journal::mark_active` / `last_active` put the instant a session was last alive on disk, as a record whose sequence number is zero — the format did not change. **The reversal for (b) is a compile error rather than a red test**, which proves the bound is absent and not merely unexercised. **Still open, and this row keeps it**: `serve_sharded_hft*` still cannot be **stopped** — ADR-0088 decision 5 named this "not decided here": an ordered shutdown across shard threads needs a design of its own, the same gap ADR-0034 and ADR-0038 left for the single-engine door before it had one. `Recovery::recover`/`fresh` also remain unbounded on the acceptor thread, a slow implementation delays every connection behind it, with ADR-0020's pending deadline as the only backstop | A sharded deployment that must be taken out of service in order — the resume half is no longer a reason to avoid sharding |
...
| 40 | ~~**`DESIGN.md` §6's wire-to-wire row asks for NIC to NIC and item 6 closed over loopback.**~~ — **CLOSED 2026-09-18: the row is MET at p50 and p99, `hft`, back to back.** `[measured 2026-09-18]` boot C step C-40 of [closing-the-open-items](docs/plans/2026-09-18-closing-the-open-items.md), §9 desk `pass 16 fail 0 unknown 0`, commit `ecfdd81`'s code (Mac `w2w` `2f93af5c` from `main` `ece17e7`), `enp9s0` (I211) ↔ a Mac mini over the direct cable, EEE off, IRQs on `cpu4`, hardware RX and TX stamps at the acceptor, one PHC; interval 0, 20 000 × 10 runs per arm, two procedures ([ADR-0068](docs/decisions/ADR-0068-a-published-figure-is-two-procedures-shown-side-by-side.md)), `--dump` join `PASS` on every qualifying run. **Wire p50 / p99 / p99.9, p1 ‖ p2: admin 27 050 / 31 982 / 35 374 (6 of 10) ‖ 27 114 / 32 254 / 49 146 (8 of 10); application 28 894 / 34 058 / 62 306 (6) ‖ 28 878 / 34 110 / 88 630 (8).** p50 and p99 reproduced (≤ 0.9%); **p99.9 did not** (27–42%) and is published marked, meeting nothing. `standard` is not wire-stamped ([ADR-0073](docs/decisions/ADR-0073-the-standard-wire-figure-goes-through-bpf-sock-ops-and-not-before-phase-2.md), phase 2). What closed it was a decision, as this row said it would be: [ADR-0071](docs/decisions/ADR-0071-a-skipped-tx-stamp-is-a-missing-sample-not-a-failed-run.md) — a skipped TX stamp is a missing sample; a run over 0.1% TX-missing is disqualified and the procedure continues (decision 1, revised the same morning after the first attempt aborted at run 5 with four good runs thrown away); the dropped samples are shown not to move the counterparty's distribution by a per-sample join (decision 3). **The sweep** (decision 5) found that pacing 0–50 µs does **not** stop the I211 skipping — `hw-tx-missing` 0–109 of 20 000 with no trend, a third of runs over the line at every interval, `tx_hwtstamp_skipped` 413 → 2 381 — so the pacing at which skips stop is between 50 µs and 1 s (boot B's 1 s: 0), and a NIC with more slots (`igc`, four) is the next purchase, named and not bought. Back to back is ~18 µs faster than boot B's 1 s-paced figure (45 146 ‖ 42 918 admin): the hot figure beside the cold one. `DESIGN.md` §6 NIC row, §8 *Boot C, over the cable*; [measured-costs](docs/reference/measured-costs.md) *Boot C*, C-40; `PRD.md` §2's *what the ticks do not buy* bullet. Instrument finding on the way: `compare-w2w-procedures.sh` compared the counterparty's p50 rather than the wire rows — fixed with a test. The original finding: | §6's second wire-to-wire row — met at p50/p99; p99.9 and `standard` are what it does not buy |
...
| 49 | ~~**~2 770 ns of the application round trip is unattributed, two of the four candidates are dead with numbers, and the survivor holds almost all of it.**~~ — **CLOSED 2026-09-18, by decision, after the third probe retired the last named candidate.** `[measured 2026-09-18]` boot C step C-49, `perf trace -s` on the engine tid over whole `hft` runs of both paths (`pass 16 fail 0 unknown 0`; the tracer inflates every syscall ~2×, p50 under trace 35.2–35.8 µs on **both** paths, so only differences count): `sendto`, 22 001 per run, admin 186.608 / 186.508 ms total (avg 8.48 µs) against app 187.663 / 183.876 ms (avg 8.53 / 8.36 µs) — **≤ 50 ns apart, of both signs**; `recvfrom` 2.47 vs 2.45 µs. **The kernel does not charge the application path for its bigger payload** at this resolution, which the +24.5 ns slope row predicted. So the ~3 130 ns outside one engine turn (D_in 765.5 ns, boot B) is not payload-proportional kernel work, not the client's loop (identical bytes-for-bytes on both paths, `tools/w2w/src/main.rs` `measure`), not `Journal::put` (8.9 ns), not the dictionary pass beyond its 679 ns. `DESIGN.md` §8 publishes the remainder as **unattributed — outside the engine turn and not the kernel's payload work**, and stops: three probes across three boots each retired a candidate, and the positioning no longer rests on the number (ADR-0077). **The one instrument that would split it is named and not scheduled**: software `SO_TIMESTAMPING` stamps on both sockets (loopback carries software TX/RX stamps), four segments per round trip — client stack out, engine-side dwell socket-in → socket-out, engine stack out, client stack in — a `--stamp software` arm of `w2w --wire-timestamps`, whose cmsg parsing already reads `ts[0]`. Side finding: one `accept4` per spin turn in these runs, because `w2w`'s `--listener-every` defaulted to 1 in its own parser and did not follow `Limits`' 16 (ADR-0069 *Measured*; fixed in the next commit). Was: | `DESIGN.md` §8's bottom line — now a stated unattributed row, not an open question |
...
| 51 | ~~**A TCP loopback write costs thirty-two bare syscalls on this box, and nothing here explains it.**~~ — **CLOSED 2026-09-23, boot F, at the mitigation tier** ([ADR-0096](docs/decisions/ADR-0096-a-figure-measured-elsewhere-meets-the-same-band-the-baseline-file-stays-out-of-the-heap-and-a-boot-may-rebuild-on-its-housekeeping-cores.md) decision 5): 55–58 % of the loopback round trip is the Zen 2 mitigations (boot C), 3.3 % conntrack (boot B), and **the flush arm ran** (F9, `target/boot-f-evidence/f9b.txt`): `TCP loopback, 8 in 8 out`, `taskset -c 6`, 5 runs per phase, medians **A 12 828.7 → B 9 935.4 (tailscaled stopped, `nft flush ruleset`; −22.6 %, −2 893 ns) → A′ 12 818.7 ns/op**. The ruleset was entirely tailscaled's (iptables-nft `filter`/`nat`/`mangle`, `ts-*` chains), rebuilt identical modulo counters by restarting tailscaled. The three arms ran on three boots and are not additive. One machine, one boot, one case: an A/B, not a figure. `DESIGN.md` §8 now says the floor carries both terms; splitting the mitigation term per mitigation is **decided, not measured** (a plan if the owner asks). [measured-costs](docs/reference/measured-costs.md) *Boot F, item 51*; [a-saved-iptables-nft-ruleset-does-not-load-back-through-nft](docs/reference/a-saved-iptables-nft-ruleset-does-not-load-back-through-nft.md). **History:** `[measured 2026-09-19]` **Suspect 2 tested — it is most of it, A/B not published:** boot C with `mitigations=off` (kernel 7.0.0-31, commit 0fe50c0, `check-machine.sh` `pass 15 fail 1`, the FAIL being ADR-0023's row), same two procedures as B7: `TCP loopback, 8 in 8 out` **12 594.4 → 5 241.1 ns/op (−58.4%)**, 5 runs spread 0.9%; `w2w` `hft` admin p50 **18 249 / 18 179 → 8045 / 8086 (−55.9 ‖ −55.5%)**, 10 + 9 qualifying runs (procedure 2's run 9 disqualified at 6% busy), all nineteen in one mode (8025..8125 / 7985..8115) — B7's bimodality did not appear, cause still unknown. Cross-boot, cross-day, one machine, all-off only; the flush arm is still unrun; the mitigated §9 figures do not move. [measured-costs](docs/reference/measured-costs.md), *Item 51's last arm*. `[measured 2026-09-15]` **Suspect 1 tested: conntrack is ~420 ns (3.3%) of it.** Boot B step B7, A–B–A with a temporary `notrack` table on `lo`: `TCP loopback, 8 in 8 out` medians 12 594.4 → 12 175.0 → 12 597.8 ns/op (`payload.rs`, 5 runs each). **And a candidate nobody predicted**: `w2w` `hft` admin read p50 **18 249 → 15 364 → 18 179** — both notrack-off phases bimodal (most runs ~18.2 µs, some ~15.7), never slow under notrack, and a run with no payload bench before it read 15 133 — so conntrack state the payload bench leaves behind may send later connections down a ~3 µs slower path. Not a cause. The flush arm was skipped (owner not at the desk, Q2); `mitigations=off` is boot C. [a-loopback-write-costs-thirty-two-syscalls](docs/reference/a-loopback-write-costs-thirty-two-syscalls.md). `[measured 2026-09-05]` `getppid` **170.5 ns**; a `pipe` write-and-read of 8 bytes **778.9 ns**; a UNIX `socketpair` **1 924.9 ns**; TCP `127.0.0.1` **10 228.6 ns**, of which the write alone is **5 450 ns**. **Not the read waiting** — a non-blocking version reads **0.00 `EAGAIN` per operation** and costs the same. **Not scheduling** — `taskset` agrees to 0.1%. **Not this project's code** — thirty lines of `std::net` under `rustc -O`. **Candidates, neither tested and neither claimed:** `nf_tables`, `nf_conntrack`, `nft_compat`, `xt_MASQUERADE` are all loaded and `tailscale0` is up, so every loopback packet traverses chains Tailscale and Docker installed; and Zen 2 carries retpolines, `IBPB: conditional`, `Safe RET` and `IBPB before exit to userspace`. **Why it matters:** every wire-to-wire figure this project has published went over `127.0.0.1`, `DESIGN.md` §8's floor is 10–20 µs, and if four syscalls are ten of them then the budget's largest term is one nothing here owns — and item 40's NIC figures will not be comparable to what they are about to be compared against. **Testing either candidate changes the owner's machine** (an `iptables -t raw -j NOTRACK` rule, or a boot with `mitigations=off`), which is their call and not a benchmark's. [a-loopback-write-costs-thirty-two-syscalls.md](docs/reference/a-loopback-write-costs-thirty-two-syscalls.md) `[to testing-skills]` | `DESIGN.md` §8's floor, and item 40 |
| ~~52~~ | **CLOSED `[2026-09-21, boot D step D3]`.** The remaining half — *whether the margin ladder can mean anything for a case this small* — is answered by measurement: twenty runs of the pre-built `w1s` journal binary on the §9 desk (`pass 16 fail 0 unknown 0`, run 1 discarded, 8 s apart, a quiet row per run) read **median 7.4, max/median 1.0946** — 7.4 seventeen times, then 7.5, 8.0, 8.1. The ladder's smallest step at or above that is **1.10**, the floor every neighbour carries, so `benches/baselines.tsv` now holds `journal put, 191 bytes, one slot  7.4 / 1.10 / n = 20` and the deliberate disagreement ADR-0052 recorded is retired. It can be measured this way only because [ADR-0067](docs/decisions/ADR-0067-the-baselines-are-read-at-run-time-not-compiled-in.md) made the table read at run time: 8.2 (compiled-in, before the append), 6.3 (compiled-in, after) and **7.4** (run-time) are the same case on the same desk, differing only in which binary was asked. Same step recorded the first-ever baseline for `serialize Heartbeat (session)` — **847.0 / 1.10 / n = 20**, max/median 1.0122 — a case that until now printed `NO BASELINE for 'AMD Ryzen 7 3700X 8-Core Processor'`, which is neither red nor green. [recording-a-baseline-changed-the-baseline.md](docs/reference/recording-a-baseline-changed-the-baseline.md) *n = 20 on the run-time table*. The layout hole (ADR-0049) is untouched and is not this item. **History:** **Recording a baseline changed the baseline, because the table is compiled into the binary.** `[measured 2026-09-15]` **The three `--strict` runs the A1 binary owed are green**: boot B step B1, `FIXBOLT_NIC=enp9s0 scripts/bench.sh --strict` three times, each `pass 15 fail 0 unknown 0`, `targets measuring 16 of 16`, `timing over baseline 0`, `cases w/o a baseline 0`, `cases under the band 0` — after five lines were recorded at n = 20 (three new, two moved: `SendingTime from the cache` 4.9 → 5.8 and `validate NewOrderSingle, w2w bytes` 897.3 → 994.5, no cause claimed; `benches/baselines.tsv` tail). Every other case in `serialize`, `density` and `validate` is in its band; `density` and `validate` read 3.0–8.4% slower than their 2026-09-05 line except `encode ExecutionReport (template)`, which read 3.5% faster (229.3 against 237.6) — none re-recorded (plan Q6). **Still open:** the margin ladder for cases this small, and the `6.3 / 1.35 / n = 8` line. `[2026-09-14, later]` **HALF CLOSED — the table is no longer compiled in.** [ADR-0067](docs/decisions/ADR-0067-the-baselines-are-read-at-run-time-not-compiled-in.md), `bdd673f`, PR [#72](https://github.com/tmthang86/fixbolt/pull/72): `harness.rs` reads `benches/baselines.tsv` at run time, so appending a line changes no bench binary (the `parse` bench's sha256 read the same before and after, and cargo did not rebuild); a missing or malformed file exits non-zero in every mode. Since `970b612` it also refuses `nan`, `inf` and non-positive values, a margin below 1.0 and a non-positive n, naming the line — `crates/codec/tests/bench_baselines.rs`, 6 tests, red before the check. **Still open:** whether the margin ladder can mean anything for cases this small, and the `6.3 / 1.35 / n = 8` line; boot B step B1 owes three `bench.sh --strict` runs on the new binary. The layout hole (ADR-0049) is untouched. `[measured 2026-09-05]` `journal put, 191 bytes, one slot` read **8.2 ns** over 20 clean runs — eighteen of them between 8.1 and 8.3. The seventeen new lines were appended to `benches/baselines.tsv`, and the very next `bench.sh --strict` went **red on that case at 6.4**. On the binary that now exists it reads **6.3, eight times, never anything else**, while its two siblings in the same binary do not move (8.9 and 5.3–5.4). The file is `include!`d into `harness.rs`, so **appending the results changed the binary the results came from**, and one small case moved **23%**. **Alignment was pinned throughout** — ADR-0049's flag was in force, read back off all sixteen binaries, and did not prevent it. Correcting the value converges immediately (8.2 → 6.3, rebuild, reads 6.3), so it is a fixed point and not a chase. **What is open:** whether the ladder rule can mean anything for cases this small, and whether the table should stop being compiled in — a build script writing it to `OUT_DIR` would move the problem rather than remove it, since that file is an input too. For now one line carries **6.3 / 1.35 / n = 8** against neighbours that all say n = 20, and the disagreement is deliberate. Third appearance of the layout hole, and the first that is self-referential. [recording-a-baseline-changed-the-baseline.md](docs/reference/recording-a-baseline-changed-the-baseline.md) `[to testing-skills]`, [ADR-0052](docs/decisions/ADR-0052-two-candidates-are-retired-with-numbers-and-a-baseline-disagrees-with-its-neighbours.md) | ADR-0016, ADR-0031, ADR-0049 |
...
| 57 | **CLOSED 2026-09-12.** The premise was wrong rather than the tool. `[measured 2026-09-12]` `cargo-deny` 0.20.2 **does** judge dev-dependencies: `[licenses] include-dev = true`. The knob tried on 2026-09-08 was `[graph] exclude-dev`, which configures the graph the *bans* check walks and not what `check licenses` reads — so "could not be configured away" was false in four documents for four days. Proven by reversal: `option-ext` (`MPL-2.0`) into the existing `[dev-dependencies]` of `tools/jrnl` gives `error[rejected] … option-ext v0.2.0 … licenses FAILED`, and the same tree with the knob off reads `licenses ok` — so the knob is what closes the hole, not a no-op. **The CI count step had been asking the right question with its own eyes closed**: `cargo tree -e normal,build` hid exactly the dev-dependencies cargo-deny was not judging, so both sides agreed at **11 and 11** about a graph short at both ends. Now **46 and 46**. `scripts/check-every-crate-is-licensed.sh` stays — not a workaround for a closed hole but a second route from a source that is not cargo-deny. Commit `d7e8608`, plan [an-obligation-nothing-checks](docs/plans/2026-09-12-an-obligation-nothing-checks.md) §A. **The text this item was opened with, unedited:** **The licence gate cannot see a dev-dependency, and it says `ok` rather than saying so.** `[measured 2026-09-08]` `cargo-deny` 0.20.2 does not judge the dev-dependencies of this workspace's members. An `MPL-2.0` crate in `[dev-dependencies]` of `tools/jrnl`, and separately of `crates/codec`, is resolved by `cargo metadata` (12 packages) and printed by `cargo tree`, while `cargo deny list` shows 11 crates and `cargo deny check` prints all four `ok` lines. `exclude-dev = false` changes nothing; stripping the config to two lines changes nothing; **the same crate as a normal dependency is rejected immediately**, so the checker works and the graph it is handed is short. **It nearly hid itself**: the first reversal used a dev-dependency, came back green, and read as a broken reversal — three explanations were checked before *the tool never looked*. Not fixable from here, so it is made loud: the CI job diffs the crate set `cargo deny list` judged against the set `cargo tree` resolves, 11 and 11 today, and fails naming the crate when they differ. **Still open** because the hole itself is still there | [a-license-gate-that-cannot-see-dev-dependencies](docs/reference/a-license-gate-that-cannot-see-dev-dependencies.md) **`[to testing-skills]`** |
| 58 | ~~**A ratchet's counter can read correctly while its enforcing half is switched off.**~~ — **CLOSED 2026-09-12.** `scripts/check-no-crate-root-allow.sh` now refuses the ordinary spelling of an inner `allow`/`expect` at a crate root, and any `warn` naming a lint the workspace denies, over every `lib`/`bin` target under `crates/` read from `cargo metadata` — 6 crate roots, 9 manifests, and 0 of either is an error rather than a pass. In the `lint-config` CI job. Reversal R-A1 red on the predicted sentence, `crate-root allow at crates/dict/src/lib.rs:1`. **R-A4 is the one that matters, because it ties the textual check to the compiler effect this item is actually about**: `[measured 2026-09-12]` with `#![allow(clippy::indexing_slicing)]` at line 1 of `crates/dict/src/lib.rs`, a fresh `pub fn probe(v: &[u8]) -> u8 { v[0] }` in a clean submodule compiled at **exit 0 with no diagnostic** — item 58's defect, reproduced on demand — and with that one line removed the same code is **exit 101, `error: indexing may panic` at `probe`**. **And building the check produced a finding one turn past this item**: `[measured 2026-09-12]` A2's first implementation went *green* on its own reversal, because its matcher excluded `:` and so could never match a lint written after `clippy::`; the guard itself was off, and only a reversal whose FAIL sentence was written down in advance could see it ([a-matcher-excluded-the-separator-every-real-name-uses](docs/reference/a-matcher-excluded-the-separator-every-real-name-uses.md), `[to testing-skills]`). **What is guarded is the shape already met, not every shape**: an outer `#[allow]` on `mod foo;`, `RUSTFLAGS=-A` from the environment, `--cap-lints` from outside the repository and `tools/` are all out of scope on purpose, and the script's header names each. The original finding: `[measured 2026-09-08]` item 55's `deny` was inert across `engine`, `session` and `dict` — most of the code — for the length of one build. Three of the 21 debt annotations were written `#![allow(clippy::indexing_slicing)]` at the top of a `lib.rs`; `#!` is an **inner** attribute and at a crate root it silences the whole crate, submodules included. `scripts/check-indexing-debt.sh` was unaffected, because `--force-warn` overrides an allow, so **the number everybody looks at was right the whole time**. Found by pasting a fresh `v[0]` into a clean module and getting exit 0. **The first attempt at that reversal was itself broken** — an unquoted backtick pair inside an `echo` made the shell run `deny` as a command, and the grep after it counted `0`, the same answer the real defect gives; caught only by reading the whole output. Fixed by scoping the allows to 15 functions, and the reversal is now run once per crate. Write-up: [an-allow-at-the-top-of-a-file-silenced-the-whole-crate](docs/reference/an-allow-at-the-top-of-a-file-silenced-the-whole-crate.md), marked **`[to testing-skills]`**. `[2026-09-12, later]` **this row describes the gate as it was on the day this item closed, and item 66 replaced it the same day**: the regex is gone, `tools/attr-scan` lexes each crate root, and the `ok` line reads 10 manifests rather than 9. The sentence *"refuses the ordinary spelling"* was accurate here and is no longer the limit | closed — the instance 2026-09-08, the class 2026-09-12 |
| ~~59~~ | ~~**Six of the ten `52=` precisions the interop oracle can be configured to emit are refused here, and before a Logon that refusal is silence — the exact defect half A was written to close.**~~ — **CLOSED 2026-09-09**, [ADR-0058](docs/decisions/ADR-0058-a-timestamp-is-read-at-every-precision-and-written-at-three.md), [plan](docs/plans/2026-09-09-utc-timestamp-widths.md). Both readers take 17 and 19..=30 from **one rule instead of two tables**; `scripts/interop.sh` §4i judges it with the oracle at a width this engine cannot send, and its reversal fails on *never logged on* alone. **The item counted six and it was seven**: the survey the owner asked for before approving found QuickFIX/J accepts 30 bytes, picoseconds, refused here just as silently. **It also found the family disagrees with itself** — C++ and .NET take a range, J and Go an enumeration — so the decision rests on an asymmetry rather than on copying anybody: accepting a width cannot break interop with a stricter engine, refusing one can. 18 bytes stays refused, a chosen divergence from C++. **Original text follows.**  `[amended 2026-09-09]` **still open, and the count is unchanged, but the shape moved**: half B found that `52=` had a *second* reader, `dict::FieldType::UtcTimestamp`, which accepted only 8 or 12 bytes of time and rejected the very stamps `parse_utc` had been widened to read. That is fixed — both readers now take the same four widths — and it is **not** this item: this item is about the six widths *neither* reader takes. `[researched 2026-09-09]` QuickFIX C++ at `386ce46e`, the SHA `scripts/fetch-quickfix-assets.sh` pins, read from source: `TimestampPrecision` is an **int 0–9**, default 3 (`Session.h:167–174`, `Session.cpp:68`), its serialiser writes `17 + 1 + precision` bytes (`FieldConvertors.h:465–489`) and its parser accepts **any length 17–27** (`:492–596`). `parse_utc` here accepts **17, 21, 24 and 27 only** (`crates/session/src/clock.rs:37–40`). So `TimestampPrecision=1, 2, 4, 5, 7` or `8` — lengths 19, 20, 22, 23, 25, 26 — reads as `None`, becomes `time_ok = false`, and in `AwaitingLogon` is a hang-up with **no byte sent** (`crates/session/src/lib.rs:2846–2856`). **Half A's closure does not cover this**: it widened the accepted set from two widths to four, and the widths outside the set behave exactly as before. **Whether the answer is to accept 1–9 digits or to keep refusing them more loudly is not decided** — the FIX 5.0 SP2 EP wording sanctions 3, 6 and 9, so strictness is defensible and silence is not. Receive-side; needs its own plan under Rule Zero. Found while writing [ADR-0057](docs/decisions/ADR-0057-sub-millisecond-time-arrives-beside-the-tick.md), which records it and does not decide it | ADR-0057, and [prior-art.md](docs/reference/prior-art.md) |
| ~~60~~ | ~~**A test fails under CI load and passes everywhere else.**~~ — **CLOSED 2026-09-09**, [ADR-0059](docs/decisions/ADR-0059-an-event-is-lost-only-when-the-ring-is-full.md), [plan](docs/plans/2026-09-09-events-without-a-shared-lock.md). **It was not a timing problem and not a test defect: reading the event stream destroyed it.** `EventRing::push` took `try_lock` on a mutex every reader also took, so a reader polling attentively made the engine drop the very events it was pushing — permanently, counted only in `events_lost()`. Per-slot locking removes the contention; the producer never blocks and never loses to a reader, readers serialise only against each other, and **`events_lost() != 0` now means exactly one thing: the ring was full**. No public API changed. **Every timing hypothesis was wrong, and the evidence that settled it was in the first CI log**: `saw: [ one event ]`, not `saw: []` — a test timing out on a slow engine sees nothing; seeing one of two says the other was destroyed. `[measured 2026-09-09]` 32 spinners on two cores left it at 0.12 s against a 5 s budget, 60/60 green, full suite 3/3 green; but deleting the reader's 2 ms sleep took it to **9 failures in 20**. After the fix: **0 in 220**. **A five-engine survey is why this became an ADR rather than a test patch**: QuickFIX C++, QuickFIX/J and quickfix-go never lose an event and all three pay by running the operator's callback on the session thread, which non-negotiable 4 forbids here; nanofix has counters only. So the loss-on-overflow is a forced trade — and the loss-on-read was an implementation detail wearing that trade's clothes. `GUIDE.md` now warns that this stream can lose where no other engine's does. **Original text follows.** **A test in `crates/library` fails under CI load and passes everywhere else, and it is a defect in the test rather than in the engine.** `[measured 2026-09-09]` `end_to_end.rs::an_operator_stops_the_front_door_and_serve_comes_back` asserts a `Logon` comes back and got an **empty read** — not the timeout, which has its own message, so the acceptor closed the socket or returned end-of-file. **Two runs of the same job on the same commit disagreed**, one red and one green within the same minute, which is what classified it: [`34362977166`](https://github.com/tmthang86/fixbolt/actions/runs/34362977166) failed and [`34362986989`](https://github.com/tmthang86/fixbolt/actions/runs/34362986989) passed. Ruled out as a consequence of item 59's change two ways first — the test stamps its `Logon` at 17 bytes, a width whose code path is provably identical before and after, and 8 consecutive local runs were green. **Neither of those could have distinguished a flake from a real race that needs a slower host**; only the second sample of the same job could. Likely a readiness race in the test's own helper: `connect()` retries until `TcpStream::connect` succeeds, and a listening socket accepts connections before the accept loop is running. **Not fixed**, and this row is the record that it is open rather than explained away. [reference](docs/reference/the-same-commit-went-red-and-green-in-the-same-minute.md) | a test whose readiness check is the engine's readiness, not the kernel's |
| 66 | **HALF CLOSED 2026-09-12, and the open half is now item 69.** [gates-that-match-a-meaning](docs/plans/2026-09-12-gates-that-match-a-meaning.md) steps 1, 2 and 6. **Gate A closes in full.** `scripts/check-no-crate-root-allow.sh` no longer matches a regex against source: `tools/attr-scan` (package `fixbolt-attr-scan`) lexes each crate root with the same lexer `rustc` uses. `[measured 2026-09-12]` `ok — 6 crate roots, 10 manifests, 55 inner attributes, 4 deny lints`; all four spellings this item found go red again, and the deliberate green control `#![doc = "#![allow(…)]"]` reads `ok` at **56**, which is what shows the attribute was read and judged rather than skipped. The step-6 review then attacked it with eight further spellings — `cfg_attr` nested, `reason =`, a raw string, the `doc = "/*"` … `doc = "*/"` sandwich, `expect`, a generated bin target — and broke none of them. **It did break the half of gate A that was still a regex, and that is fixed here too**: `"unwrap_used" = "deny"` is valid TOML that cargo honours, the deny-list extractor did not match a quoted key, and A2 then passed silently on a crate-root `warn`. Quoted, dotted and inline-table keys are accepted now, an empty deny list is a FAIL, and the count is printed. The honest limit, in the header: CI's clippy job still caught the `unwrap` itself, so that was a gate measuring nothing rather than non-negotiable 7 switched off. **Gate B does NOT close.** `scripts/check-scratch-fixtures.sh` got two real fixes — a pin name in a trailing comment no longer counts, and scripts are selected by shebang rather than by a `.sh` name — but the step-6 review found **five** false greens and **four remain open by decision**: a `cp` inside a heredoc body, a `cp` in the wrong direction (`cp "$CRATE/x" "$ROOT/x.copy"`), and two seeding misses, `read -r TMP < <(mktemp -d)` and `TMP=/tmp`. None of the four is an `eval`, a function call or a `#` inside a string — **so the *"two limits remain"* sentence this repository wrote on 2026-09-12 was false the day it was written**, and it is corrected in `CLAUDE.md` §2, `DESIGN.md` §6 and the reference entry. They were not patched because each is one more regex, which is the loop section A of the plan concluded cannot be won; the question of the gate's *shape* is **item 69**, for the architect. `[measured 2026-09-12]` `ok — 20 scripts, 1 enter a scratch dir, 1 pins`. **Original text follows.** **Two new gates match a spelling, and the senior review got past both.** `[measured 2026-09-12]` PR [#63](https://github.com/tmthang86/fixbolt/pull/63)'s reviewer attacked the checks that closed items 58 and 23 with shapes their authors never ran, and won four times against one and three against the other. `scripts/check-no-crate-root-allow.sh`: **`#![/*x*/allow(clippy::unwrap_used, …)]` in `crates/session/src/lib.rs` passes the check, passes `cargo fmt --all --check`, passes `check-lint-config.sh`, and a real `x.unwrap()` in that library crate then compiles clean** — `cargo clippy -p fixbolt-session --lib -- -D warnings` exits 0, and deleting that one line makes it `error: used unwrap() on an Option value` at `clock.rs:208`. That is non-negotiable 7 switched off in one line, which is exactly the class item 58 opened. Three more spellings — `# ! [ allow ( … ) ]`, a `/* c */` prefix, `#!` then a newline — also pass the check but are rewritten by `cargo fmt`, so CI catches them; **only the comment form survives the whole pipeline.** `scripts/check-scratch-fixtures.sh`: `cd "$(mktemp -d)"` seeds no variable, `readonly TMP=…` is not in the `local\|declare\|export` seed set, and **a `cp` that is commented out satisfies B2** — `# TODO: cp rust-toolchain.toml "$TMP"` counts as proof the copy happens, and the check even increments its `enter` count while accepting it. The third is the one that will actually happen, because it is what a person leaves behind while debugging. **Both scripts' headers now carry the bypasses verbatim**, and the prose that said they refuse *any* inner allow, or that *every* entering line carries a `cp`, is narrowed in `CLAUDE.md` §2, `DESIGN.md` §6 and both `docs/reference/` entries. **Not fixed**: hardening either matcher is a change to a gate under a closed plan, so it needs its own. Strip comments before matching, accept any assignment prefix, and require the `cp` to be a live line — **and this is what step 6 of the same plan did, the same day, and it was not enough.** **Correction, 2026-09-12, same day — this item's `CLOSED` verdict above was written before step 6, the plan's own required independent review, had run.** Step 6 attacked gate A ([check-no-crate-root-allow.sh](scripts/check-no-crate-root-allow.sh)) again and found a second blind spot, distinct from the first: the deny-list derivation matched only a bare identifier key, so `"unwrap_used" = "deny"` — quoted TOML, cargo-honoured — passed through unmatched and `#![warn(clippy::unwrap_used)]` read `ok`. **Fixed the same day**: quoted, dotted (`clippy.unwrap_used = "deny"`) and inline-table (`unwrap_used = { level = "deny" }`) keys are recognised, a literal-string value (`'deny'`) is read, and an empty deny list is itself a `FAIL`. `[measured 2026-09-12]` `check-no-crate-root-allow: ok — 6 crate roots, 10 manifests, 55 inner attributes, 4 deny lints`. **Gate A closes in full.** Step 6 also attacked gate B ([check-scratch-fixtures.sh](scripts/check-scratch-fixtures.sh)) and got past it five more ways. Two are fixed: a `cp` whose pin name lived only in a trailing comment counted as proof the copy happens (trailing `#`-comments are stripped before the pin is matched now), and a shell script with no `.sh` name was never scanned (scope is now by shebang, not extension). **Four remain open, by decision, not by oversight**: a `cp` inside a heredoc **body** — text that looks like a command but is data fed to another one; a `cp` that copies **out of** the scratch dir rather than into it (`cp "$CRATE/x" "$ROOT/x.copy"` names both the artefact and the scratch variable and still satisfies the check); and two ways of seeding the scratch variable this script's regex does not recognise, `read -r TMP < <(mktemp -d)` (seeded by `read`, not an assignment) and a bare `TMP=/tmp` (an assignment the regex does see, to a value its own literal-matching does not accept as the seed it means). `[measured 2026-09-12]` `check-scratch-fixtures: ok — 20 scripts, 1 enter a scratch dir, 1 pins` — unchanged by the two fixes above, and unable to see the four that remain. **Gate B does not close.** Each of the four is patchable with one more regex, which is the loop section A of the plan concluded cannot be won; item 69 is that question, for the architect | items 58 and 23, the reviewer of PR #63, and a second review the same day |
| 67 | **CLOSED 2026-09-13**, PR [#68](https://github.com/tmthang86/fixbolt/pull/68), plan [the-residue-of-an-obligation](docs/plans/2026-09-13-the-residue-of-an-obligation.md), as a decision (owner's Q3). Probe 5 checks the seven `required…` Default cells, reading the condition in each cell and which key the refusal names; with probes 2, 3, 4 and 6 that is six probes, and the rest of the table — *Meaning* and notes — is prose read by a person, said in `mod doc_table`'s docstring, a new `DESIGN.md` §6 row and `CONFIGURATION.md` §1. Probe 5 found no `required` cell wrong. Was: **The two-way documentation gate checks that a row exists, never that it is true.** `[measured 2026-09-12]` `doc_table` in `crates/engine/src/settings.rs` compares the `Key` enum with the **first cell** of each `CONFIGURATION.md` §1 row. The reviewer of PR [#63](https://github.com/tmthang86/fixbolt/pull/63) rewrote the meaning, the valid values and the default of two TLS rows to say the opposite of the code — `SocketUseSSL` as `` `1` or `0` `` defaulting to `Y`, `TlsRequireKernel` documented as *turning TLS off* — and changed the count sentence to **"Four hundred keys"**, and the suite still read `3 passed; 0 failed`. **This already cost a real error inside the work that built the gate**: the `TlsRequireKernel` notes cell shipped as *"does nothing without `SocketUseSSL=Y`"* when `settle` refuses it, and only a hand-check caught it before the commit. The module docstring now says which question the gate answers and which it does not. **What would close this** is not obvious and should not be guessed: parsing the *Values* and *Default* cells and driving the parser with them is one option, and it makes the document a test fixture, with everything that implies. **Narrowed 2026-09-12** by [gates-that-match-a-meaning](docs/plans/2026-09-12-gates-that-match-a-meaning.md) step 4, and the narrowed part is **closed**: `doc_table` now drives the parser from four kinds of cell that are already *values*, each with its own probe — a *count sentence* checked against the number of `Key` arms in words; a *Default* cell beginning with a backticked literal, parsed both from the minimal sample and from the sample plus `KEY=<default>`, requiring equal `configs()`/`role`/`dial`/`tls()`/`log()`; an enumerated *Values* cell of **two or more** literals (Sửa 2: one literal is a shape, not an enumeration), each listed literal accepted and a foreign one refused; and a *Where* cell naming `[DEFAULT]` only or allowing `[SESSION]`, checked against `settle`'s own refusal. `doc_rows` reads cells by column name rather than position, because §1's four tables have two column shapes, and an exhaustive `match` on `Key` with no `_` forces a new key to declare its sample group or fail to compile. `[measured 2026-09-12]` coverage, each a floor that may only rise: Default cells 15 probed / 15 skipped (16/16 with `tls`); enumerated Values cells 10 probed / 20 skipped (11/11 with `tls`); Where cells 18 probed / 12 skipped. **No cell of §1 is wrong today** by these four probes, in either feature set — a negative result, reported as one. **What remains open, and is not smaller than it sounds**: the *Meaning* cell and every notes cell are still prose, checked only by hand — nobody has taught the gate what a cell *means*, only what it parses as. The skip counts above are exactly the measure of how much of §1 still sits outside any probe, and they are a floor precisely so that adding a fifth probe (or a sixth) is the only way to lower them, never a rewrite of one of the four that exist. **A second, narrower gap in the same four probes, found by the same review**: probe 3 (enumerated *Values*) is exhaustive over the literals a cell lists — every one is written and must parse — but it samples the *refusal* side from three fixed foreign literals (`1`, `true`, `nope`), so it can only ever say something about those three. `[measured 2026-09-12]` narrowing `TimestampPrecision`'s documented values from `` `3`, `6` or `9` `` to `` `3` or `6` `` left the suite green — the parser still accepts `9`, and none of the three foreign candidates happens to expose that a listed value went missing. See item 70 | non-negotiable 10's spirit, applied to prose |
| ~~68~~ | ~~**Three `tls`-gated tests ran on one desk and no CI job, and a list of test FILES cannot catch that class.**~~ — **CLOSED 2026-09-12**, [gates-that-match-a-meaning](docs/plans/2026-09-12-gates-that-match-a-meaning.md) step 3. The `tls` job's `--test …` list is deleted rather than extended: it now runs `cargo test -p fixbolt-engine --tests --features tls --no-fail-fast`, and `scripts/check-feature-gated-tests-ran.sh` asks the build itself what exists (`-- --list --format terse`), sections the job's own log by its `Running <path>` lines, and requires every listed test to appear in its own binary's section. `[measured at `161744a`, step 3's own commit]` `ok — fixbolt-engine --features tls: 38 binaries, 322 listed, 322 accounted for (0 ignored)`; **326 at this file's own tip, `57b6b3a`** — step 4 added four `doc_table` lib tests to `crates/engine/src/settings.rs` afterwards, and both numbers are named so the drift is explained rather than silently overwritten. A canary test added to `tests/admin.rs` and left off a synthetic log reproduces the FAIL sentence this item asked for (`… is in the tls build and did not run`), and a log of zero lines reads `0 accounted for` rather than a pass. **This closes the class this item named, not only the instance**: there is no longer a list to fall behind, because there is no list. `DESIGN.md` §6 carries the new row. **The gate that closed this item was itself wrong on first delivery, for two reasons, both found and fixed the same day the item closed** ([two-streams-through-one-pipe-have-no-guaranteed-order](docs/reference/two-streams-through-one-pipe-have-no-guaranteed-order.md), [a-test-s-name-is-spelled-differently-in-a-listing-and-in-a-run](docs/reference/a-test-s-name-is-spelled-differently-in-a-listing-and-in-a-run.md)): **(1)** the listing half read cargo's `Running <path>` (stderr) and each binary's own test lines (stdout) merged with `2>&1`, and inferred which binary owned which test from their relative order — CI run `34696713544` went red with a test line appearing before any `Running` line, because order between two streams through one pipe is not guaranteed and the desk's green had been green by luck; the fix asks `cargo test --no-run --message-format=json` for the binaries and runs each one's own `--list`, so attribution is exact by construction and the reconcile half now asserts only three things that do not depend on order. **(2)** libtest spells a `#[should_panic]` test's run line differently from its listing (`NAME: test` listed, `test NAME - should panic ... ok` run), so the first version of the count read **325 of 326** on a log where all 326 had genuinely run — a red that said nothing about the thing under test; the fix counts run-status occurrences of a name rather than pattern-matching one line shape. **Original text follows.** **Three `tls`-gated tests ran on one desk and no CI job, and a list of test FILES cannot catch that class.** `[measured 2026-09-12]` `crates/engine/tests/settings.rs` holds three `#[cfg(feature = "tls")]` tests. `cargo test --all` compiles them away because `tls` is not a default feature, and the `tls` job names its binaries explicitly — `--test tls --test tls_wire --test tls_mode --test tls_settings_wire` — and had no reason to name `settings`, most of which is not about TLS. So `TLS tests that ran: 14` was a true count of a list that was missing three. One of the three, `into_table_refuses_a_file_that_asks_for_tls`, guards the expensive mistake `Problem::NeedsTlsDoor` exists for: a file asking for TLS poured into plain `serve`, serving plaintext on a port an operator believes is encrypted. **Fixed in PR #63 by adding `--test settings`**, which takes the count 14 → 55, and the job's comment now says the rule is *a `tls`-gated test anywhere in the crate must be named here*, not *add the new file*. **Still open as a class**: nothing enumerates `#[cfg(feature = "tls")]` tests and compares that set with the job's list, so the fourth time will look like the first two. **This is item 62 for the third time in one day** | the `tls` CI job, item 62 |
| 69 | **CLOSED 2026-09-12.** Closed **as a decision**, by [ADR-0061](docs/decisions/ADR-0061-the-scratch-fixture-gate-is-a-regex-and-says-so.md): the gate stays a regex and now says what it cannot see. **The fork was settled by measurement, not preference** — `[measured 2026-09-12]` `shellcheck -f json1`, the only parser already on CI, emits diagnostics and not a parse tree; on a file containing all four bypasses it reports two unused variables and nothing else. Option (b) through shellcheck is **unreachable**, not expensive; every other parser is a new binary on the runner and the desk and still cannot see `eval`, `source`, a `cd` behind a function or a `$(…)`. Deliberately the opposite conclusion to the sibling gate one pull request earlier, and the difference is the adversary: that one faced legal Rust that ordinary code writes; this one faces a heredoc line that looks like a command, and no script here writes any of the four shapes. **One notch weaker than it reads, recorded in the ADR's *Consequences* by the senior review**: `scripts/check-bench-alignment.sh:102` already writes `read -r … < <(…)`, the third bypass's idiom, by this repository's own author for an unrelated reason — so "the adversary is only an accidental fixture" has a counterexample in the tree. Commit `f085f43`, and no line of gate logic changed. **The text this item was opened with, unedited:** **`scripts/check-scratch-fixtures.sh` is a regex over `bash`, and a regex is a list of spellings — item 66's own gate-B half is now where the crate-root gate stood before step 1 of this plan, and this item is the question, not another patch.** `[measured 2026-09-12]` four ways past it, each verified as a false green (the script reads `ok` while the pinning artefact is not really copied into the scratch dir): a `cp` inside a heredoc **body** — the line looks like a command but is data being fed to another one, and the script's line-by-line scan cannot tell heredoc data from a live line; a `cp` that copies **out of** the scratch dir rather than into it, `cp "$CRATE/x" "$ROOT/x.copy"`, which names both a pinning artefact and the scratch variable and satisfies B2 in the wrong direction; and two ways of seeding the scratch variable the seeding regex does not recognise — `read -r TMP < <(mktemp -d)`, seeded by a `read` builtin rather than an `=` assignment, and a bare `TMP=/tmp`, an assignment the regex does see but whose value its own literal-matching does not accept as the seed it means. **None of the four is what the item that opened this gate predicted** (an `eval`, a function call, a `#` hidden inside a string) — the class the second review actually tried was a different one, which is the pattern item 58 already set: a gate built to a predicted list of bypasses gets past on a spelling the plan did not think of. **Why not fixed**: each is patchable with one more regex, and section A of [gates-that-match-a-meaning](docs/plans/2026-09-12-gates-that-match-a-meaning.md) already concluded, before step 1 was built, that this particular loop — enumerate a bypass, patch it, find the next one — cannot be won that way; that conclusion is why step 1 replaced a regex with a real lexer (`tools/attr-scan`) for the *other* gate this plan built, and gate B never got the same treatment. **For the architect**: either (a) keep the regex and say the limit honestly in its own header and in `DESIGN.md` §6 — which is where it stands today — or (b) read `bash` with something that actually parses it. `shellcheck -f json1` carries token positions and is already a dependency of this repository's CI (`0.9.0` on the runner, `0.11.0` on this desk); `shfmt --to-json` was considered and rejected in this same plan's *Cách làm* §E as one more binary to install for a job `shellcheck` can already reach. Not started; no code change should be made under this item without a new plan, per `CLAUDE.md` §1 | item 66 (gate B), the second senior review |
| 70 | **CLOSED 2026-09-12.** The reverse direction is now a **bounded search**, plan [an-obligation-nothing-checks](docs/plans/2026-09-12-an-obligation-nothing-checks.md) §C, commit `95ab19a`: every string of length 1 and 2 over `0-9 A-Z a-z + - . _` (4 422) plus nine neighbours of each listed literal, minus what is listed, with INI syntax characters excluded **by design** because a spurious red is the worse failure direction. Three outcomes, three sentences, and the third says **the probe is wrong, not the document** — which is what makes a wider universe safe to write. Red before green, and the red was a real defect: the parser accepted `+3`, `03`, `+6`, `06`, `+9`, `09`, because `"+3".parse::<u32>()` is `Ok(3)`, so `TimestampPrecision=+3` configured a precision the document never offered. 49 000 parses cost **0.32 s** against a 10 s ceiling written into the plan before the code. **A second hole, found by the senior review and fixed in the same pull request**: a *Values* cell listing one literal twice kept the probe green while the other value left the document — item 70's own shape in a different spelling. **Still open as item 74**: a literal of three characters or more that the parser accepts while the document omits it is still invisible, measured with a fake `"acc"` alias for `ConnectionType`. **The text this item was opened with, unedited:** **Probe 3 of item 67's `doc_table` samples its refusal side from three fixed candidates, and a value the parser accepts while the document omits it is invisible unless it happens to be one of those three.** `[measured 2026-09-12]` narrowing `TimestampPrecision`'s documented *Values* cell from `` `3`, `6` or `9` `` to `` `3` or `6` `` left `cargo test -p fixbolt-engine --lib doc_table` green: the parser still accepts `9` on the wire, the cell no longer lists it, and none of the three foreign literals probe 3 writes (`1`, `true`, `nope`) exposes that gap, because the probe only ever writes what *is* listed and one arbitrary thing that is not. The forward direction (every listed literal must parse) is exhaustive and not in question; this is the reverse direction (nothing unlisted should parse) alone, and it is a sample of three, not a search. **Widening the candidate set is not free**: a foreign literal chosen badly risks a *spurious* red — a value that happens to parse for an unrelated reason (a different key's value colliding lexically, a prefix match) would fail the probe for a cell that is actually honest, which is the opposite failure direction from the one measured here. **Not started**: widening needs its own measurement, run before it is trusted, of which candidates are safe to add and why; it is not a same-day fix the way the four-spelling fixes above were | item 67 |
| 71 | **CLOSED 2026-09-13**, PR [#68](https://github.com/tmthang86/fixbolt/pull/68), plan [the-residue-of-an-obligation](docs/plans/2026-09-13-the-residue-of-an-obligation.md). The count lives in one place — `docs/reference/prior-art.md`'s `fixbolt` row, `[measured 2026-09-13]` **23** return sites with the `grep` that produces it — and `crates/session/tests/drop_reason.rs::the_site_count_prior_art_quotes_is_the_one_in_the_source` re-derives it from the source and fails when the row disagrees; the senior review added that every non-comment line naming `Link::Dropped` must be counted or be a comparison, so a return site in an unusual shape cannot hide. The other documents drop the number; ADR-0035 and ADR-0059 each carry one dated erratum line, bodies unchanged. Was: **"`Link::Dropped` from eighteen places" is stale in two accepted ADRs and in `prior-art.md`, and `grep` reads 29.** `[measured 2026-09-12]` `grep -c 'Link::Dropped' crates/session/src/lib.rs` is **29**; [ADR-0035](docs/decisions/), [ADR-0059](docs/decisions/), `docs/reference/prior-art.md` and the header of `crates/session/tests/drop_reason.rs` all carry `[verified 2026-09-02]` and the word *eighteen*. **Found by the senior developer while building item 64, and correctly not fixed there**: adding a `DropReason` variant adds a *reason*, not a site — every refusal flows through one place in `received_inner` — so this branch did not make it worse, and `CLAUDE.md` §5 forbids editing an accepted ADR's substance. **The question is what to do about a number that two accepted ADRs assert and the code refutes**: a superseding note, a corrected `prior-art.md` with the ADRs left as the record of what was believed on 2026-09-02, or a machine check that counts them. **No gate can see this class** — a count quoted in prose, true when written, with nothing reading it since. For the architect | the senior developer's report on step 4 |
| 72 | **CLOSED 2026-09-13**, PR [#68](https://github.com/tmthang86/fixbolt/pull/68), plan [the-residue-of-an-obligation](docs/plans/2026-09-13-the-residue-of-an-obligation.md). Two rules replace one in `scripts/check-links.py`: a URL into this repository's own GitHub path (compared by segment, case-insensitively, without `www.`) is judged on any matching tail; any other host only on a tail of two or more segments. The class no longer judged is counted in the summary line. The stated limit — this repository's root file under the wrong organisation passes, counted — is in the docstring, `DESIGN.md` §6 and [a-bare-filename-is-not-evidence-of-a-repository](docs/reference/a-bare-filename-is-not-evidence-of-a-repository.md). Was: **`scripts/check-links.py` reads any absolute URL whose last path segment is a filename this repository also has as a link home.** `[measured 2026-09-12]` a citation of *cargo-deny's* `CHANGELOG.md` — `https://github.com/EmbarkStudios/cargo-deny/blob/main/CHANGELOG.md` — was reported as `this repository has CHANGELOG.md; link it by relative path`. **The host-agnostic rule is deliberate and has earned its place**: it is what caught two links to a `fixbolt` GitHub organisation that does not exist, and its docstring says so. But **a bare filename is not evidence of which repository a file belongs to**, and `CHANGELOG.md`, `README.md` and `LICENSE` are names nearly every project has. The two real catches were both multi-segment paths (`docs/decisions/…`), so requiring the matched tail to contain a separator would keep both and drop this class — at the cost of missing an absolute link to a root-level file of *this* repository, which is a limit that can be stated. **Worked around rather than fixed**: the citation was re-pointed at the releases page, and the gate was left alone because changing a gate's meaning needs a plan (§1). For the architect | this branch's own CI |
...
| 77 | **CLOSED 2026-09-13**, PR [#69](https://github.com/tmthang86/fixbolt/pull/69), plan [what-the-residue-review-found](docs/plans/2026-09-13-what-the-residue-review-found.md). `the_reader_table_matches_the_call_sites` reads every `<fn>(…, Key::X` call above `mod doc_table` and requires the reader to agree with `reader(key)` (floor 25). **Its limit is item 80.** Was: **`doc_table`'s `reader(key)` trusts its own declaration.** A key declared as read by one `match` but in fact read by another carrying the same literals plus an alias would not be seen: the arm-reading leg compares the table with the arms of the `match` the declaration names. Found by the senior review of PR #68; stated, not closed |
| 78 | **CLOSED 2026-09-13**, PR [#69](https://github.com/tmthang86/fixbolt/pull/69), plan [what-the-residue-review-found](docs/plans/2026-09-13-what-the-residue-review-found.md). The import is gated on `standard`, and the `feature-sets` CI job runs `clippy -D warnings` over every feature set to depth two plus `--all-features` ([ADR-0065](docs/decisions/ADR-0065-every-feature-set-to-depth-two-is-built-linted-and-documented.md)). **Correction to the row below**: CI *did* test `--no-default-features --features affinity` (`ci.yml`, the affinity job) — it never ran clippy `-D warnings` on it. Was: **An unused import under a feature combination no gate builds.** `[measured 2026-09-13]` `cargo check -p fixbolt-engine --no-default-features --features affinity` → `warning: unused import: crate::msglog::MaybeLog` at `crates/engine/src/shard.rs:43`. CI builds `--no-default-features` and `--features affinity` separately, never the two together. Pre-existing |
| 79 | **CLOSED 2026-09-13**, PR [#69](https://github.com/tmthang86/fixbolt/pull/69), plan [what-the-residue-review-found](docs/plans/2026-09-13-what-the-residue-review-found.md). The `docs` job is replaced by `feature-sets`, which runs rustdoc over the same sets; its first runs found **18** intra-doc links broken without `standard` (14 seen on macOS, 4 only on Linux), all fixed — [a-linux-only-module-is-invisible-to-a-mac-gate](docs/reference/a-linux-only-module-is-invisible-to-a-mac-gate.md). Was: **The `docs` CI job builds only the default feature set and `--all-features`.** `[measured 2026-09-13]` in PR #68 an intra-doc link to `Self::Tls` in a new variant's rustdoc was valid under `--all-features` and broken under `--features affinity` alone; neither CI build could see it, and the developer caught it by hand. Every narrower set is unchecked |
| 80 | **CLOSED 2026-09-13, with a stated residue**, PR [#70](https://github.com/tmthang86/fixbolt/pull/70), plan [what-the-review-left-open](docs/plans/2026-09-13-what-the-review-left-open.md). Probe 3's behavioural candidates include the 22 YAML 1.1 boolean spellings, so a parser that also accepts `yes` or `true` is red whatever the source looks like (R80-1, R80-2). **Residue**: an inline literal outside that set (`"always"`) still passes (R80-3, green by design), written in the test's doc. Was: **`doc_table`'s call-site leg checks which reader a key reaches, not what value reaches it.** `[measured 2026-09-13]` by the senior review of PR [#69](https://github.com/tmthang86/fixbolt/pull/69): `&& (v.1 == "yes" \|\| flag(v, Key::ResetOnLogon)?)` — the parser accepting an extra literal inline — leaves `the_reader_table_matches_the_call_sites` **ok**; a wrapped value (`flag(normalise(v), Key::X)`) is the same hole. Not patched with another pattern, per ADR-0061's conclusion; stated in the test's doc. Closing it means reading behaviour, not source — the neighbourhood of item 74's candidate-set question | item 77 |
| 81 | **CLOSED 2026-09-13**, PR [#70](https://github.com/tmthang86/fixbolt/pull/70), plan [what-the-review-left-open](docs/plans/2026-09-13-what-the-review-left-open.md). The cell says `positive`; probe 7's `positive` branch uses `refused_for_its_value` and first requires `1` accepted, so an unrelated refusal cannot pass for a value refusal (review gap 3). Floor 5. Was: **`ReconnectInterval=0` is refused and `docs/CONFIGURATION.md` §1 does not say so.** `[verified 2026-09-13]` `reconnect::Policy::new` → `FirstIsZero` → `Problem::ImpossiblePolicy`; the cell reads `integer, **seconds**`. The cell says less than the parser, item 76 in reverse. Writing `positive` into it today would make probe 7 go red *falsely*, because its `positive` branch requires a refusal `is_about_the_value` and that helper (shared with probe 3) does not list `ImpossiblePolicy` — so the fix is the cell and that predicate together | item 76, probe 7 |
| 82 | **CLOSED 2026-09-13**, PR [#70](https://github.com/tmthang86/fixbolt/pull/70), plan [what-the-review-left-open](docs/plans/2026-09-13-what-the-review-left-open.md). `check-links.py` rule (d): an own-repository `blob|tree|raw|blame` or `raw.githubusercontent.com` URL is judged by whether its path exists, case-exact, `..` refused, in every file including `crates/library/README.md`; `<url>` autolinks are read. Floor 3. Stated limits: `%2F`, `fixbolt.git/…`, `edit/`, `commits/`. Was: **`scripts/check-links.py` does not report an own-repository absolute URL to a path that does not exist** (`github.com/tmthang86/fixbolt/blob/main/DESIGN.md`). The tail search only matches files that exist. Pre-existing; found while proving rule (c) in PR #69 | item 72 |
| 83 | **CLOSED 2026-09-13**, PR [#70](https://github.com/tmthang86/fixbolt/pull/70), plan [what-the-review-left-open](docs/plans/2026-09-13-what-the-review-left-open.md). The two links are code spans, and `feature-sets` runs rustdoc with `RUSTDOCFLAGS="-D warnings"` ([ADR-0066](docs/decisions/ADR-0066-the-rustdoc-gate-denies-every-warning.md)). Was: **`fixbolt-session` rustdoc warns that public docs link to private items** — `parse_utc` → `LEN_SECONDS`, `LEN_MAX` (`crates/session/src/clock.rs:57-58`). A warning, so no gate fails; seen in every `feature-sets` doc run | item 79 |
| 84 | ~~**kTLS was slower than userspace `rustls` on loopback, in both `hft` paths and both procedures — and ADR-0005 decision 2 prefers kTLS for `hft`.** `[measured 2026-09-14]` on the §9 desktop, `pass 12 fail 0 unknown 1` (`DESIGN.md` §8 *The round trip under TLS, measured*; `docs/reference/measured-costs.md` *TLS on the wire*). p50 added over `off`, procedure 1 / 2: kTLS **+9 233 / +8 987** (`hft` admin), **+9 302 / +9 503** (`hft` app), **+9 613 / +9 689** ns (`standard` admin); userspace **+4 709 / +1 803** (admin) and **+1 352 / +1 598** ns (app). kTLS minus userspace: admin 4 524 / 7 184, app 7 950 / 7 905 ns, and positive at p99 and p99.9 as well. TLS 1.3 `TLS13_AES_128_GCM_SHA256` at **both** ends of the socket. **No cause is claimed** — nothing was isolated (`CLAUDE.md` §10). **What it does not show:** a NIC — over loopback neither arm had hardware offload to use (`ethtool -k lo` reads `tls-hw-tx-offload: off [fixed]` and `tls-hw-rx-offload: off [fixed]`, read by the manager 2026-09-14) — another kernel, another suite, or one end's cost. **What it does not change by itself:** decision 2's stated reasons are D8, parse-in-place, AES-NI with NIC offload where the hardware supports it, and the hot-path guarantee; the kTLS arm still counts `allocs 0` against userspace's 80 000 per 20 000 round trips, and the NIC half of the third reason is out of this measurement's reach. **The design question, for the architect:** does decision 2 still hold for `hft`; what would a NIC with TLS offload change; and does ADR-0005's *Alternatives* row, which prices a sidecar against "a sub-microsecond crypto cost", survive measured round-trip deltas, with TLS at both ends, of 1.4–4.7 µs (userspace, `hft`), 9.0–9.5 µs (kTLS, `hft`) and 9.6–9.7 µs (kTLS, `standard`). A reversed decision is a new ADR (`CLAUDE.md` §5). **Not a defect, and not a cause.** [ADR-0070](docs/decisions/ADR-0070-ktls-stays-the-hft-steady-state-for-the-guarantee-not-for-latency.md); two mixed arms owed at C-84.~~ | ADR-0005 decision 2 for `hft`; `docs/best-practices-hft.md` §9's recommendation rests on it |
| 85 | ~~**An arm's p50 moved 15.9% between two identical procedures, two kTLS arms' p99 moved 7–19%, and the per-procedure spread could not see either.**~~ — **CLOSED 2026-09-23, boot F, as *cause unnamed, comparator built*.** The comparator: `scripts/compare-w2w-procedures.sh` reads two `w2w-baseline.sh` summaries per percentile against ADR-0068's 5 %, and its reversal `scripts/check-w2w-compare.sh` (6 % must FAIL, 4 % must PASS, at p50, p99, p99.9) runs in the CI `gates` job (`6b2833b`). The probe C-85, one variable, `hft`/admin/`off`, 20 runs each, `w2w` sha256 `1cbe3f0…`: **P1** (24 min after boot, nothing built) p50 **16 226**, **P2** (6 s after the F6 build) **16 256**, **P3** (≥ 10 min idle) **16 240** ns; every pair `reproduced` at every percentile (p50 diffs 0.19 / 0.09 / 0.10 %, largest 1.161 % at p99.9). The "after a build" candidate is refuted a second time (first: boot C). P3's own header read `pass 16 fail 1` (quiet row, 4 % busy); its runs all qualified. [measured-costs](docs/reference/measured-costs.md) *Boot F, item 85*; `target/boot-f-evidence/f8.txt`. **History:** `[measured 2026-09-15]` **The guard half is built, and the effect came back larger.** Step S2 (`d5be4d6`): `w2w-baseline.sh` prints commit, tree, uptime and binary sha256, keeps every run, and reads dispersion from both sides (`scripts/check-w2w-baseline-summary.sh` in CI); [ADR-0068](docs/decisions/ADR-0068-a-published-figure-is-two-procedures-shown-side-by-side.md) makes a figure two procedures. Boot B then ran every loopback arm twice: **all ten zero-interval arms of B2, B5 and B8 were 4.7–6.7% faster in procedure 2**, each with its p50 runs inside −0.5%/+1.1% of its own median (min/median ≥ 0.992, max/median ≤ 1.013); nine of the ten did not reproduce, and `standard` app `--journal file-async` did (4.7/4.1/3.3%); the paced arms (0.4–3.9% at every percentile) and nanofix's own arms (1.8–3.1%) reproduced, but B8's fixbolt arms did not (p50 5.6% and 5.2%); a third B2 run matched procedure 2 in `hft`. Candidate, not a cause: procedure 1 began ~7 minutes after a build slot. **Still open**: the cause; nothing runs the two procedures or compares them (ADR-0068's rule is applied by hand). `[measured 2026-09-14]` `hft`/admin/`userspace` p50 read **20 774** ns (runs 20 639 .. 20 930, spread 1.008) in procedure 1 and **17 473** (runs 17 333 .. 17 813, spread 1.019) in procedure 2 — same command, same binary, same boot, half an hour apart; the other seven arms' p50 moved 0.2–2.5%, all faster. **At p99:** `hft`/admin/kTLS 37 345 → 30 447 (−18.5%; procedure 1's per-run p99 in two clusters, 8 runs 30 928–32 141 and 12 runs 37 220–37 841), `hft`/app/kTLS 36 409 → 39 029 (+7.2%). Inside procedure 1, `standard`/admin/kTLS shifted level at run 11 (p50 29 305–29 506, then 28 704–28 965). Inside procedure 2, `hft`/app/`off` printed spread **1.008** with five runs at p50 17 323–19 136 against a median of 19 998: the spread is max/median (`scripts/w2w-baseline.sh:222-225`), which cannot register how far below the median a run falls. **Recorded between or before the procedures, all candidates and none a cause:** the quiet-row FAIL at 07:24:03, the release build (07:24:19) and the discard run (07:25:18) before procedure 1, about 7 minutes after boot; both mode scripts under `strace` (07:50:59, 07:51:01), two single runs at 07:53 and seven at 07:54:28–07:55:19 with the manager's editor session active before procedure 2, 38 minutes after boot. **Why it matters:** the §8 procedure publishes from **one** twenty-run median and prints neither HEAD nor tree state; run once, this arm's p50 and two arms' p99 would have depended on which half hour it ran in. **No guard yet** — no test and no gate, an unmet `CLAUDE.md` §4 obligation for the new trap entry; `DESIGN.md` §8 shows both procedures side by side in the meantime. Needs a plan: how a figure is reproduced before it is published, at every published percentile; a dispersion that reads both sides; and HEAD and tree state printed by the procedure itself. [a-tight-spread-inside-one-procedure-did-not-reproduce-across-two.md](docs/reference/a-tight-spread-inside-one-procedure-did-not-reproduce-across-two.md) `[to testing-skills]`. Comparator 2.3; cause probe C-85. | how every §8 figure is published, B2 of [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md) included |
| 86 | ~~**Non-negotiable 4's `hft` gate needs a tracer, and a tracer strips file capabilities.** `[2026-09-14]` Option R5 of [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md) Sửa 2, Điều 1, not built. `scripts/check-no-kernel-sleep.sh` reads the engine thread's syscalls under `strace`; a program exec'd under an unprivileged tracer gets no file capabilities, so the `--wire-timestamps` arm runs on `lo` inside `unshare -Urn` and cannot see a real NIC ([a-traced-process-gets-no-file-capabilities](docs/reference/a-traced-process-gets-no-file-capabilities.md)). **The idea:** a second, tracer-free gate — `w2w` reads the engine thread's `voluntary_ctxt_switches` from `/proc/self/task/<tid>/status` before and after the timed window, and `hft` must read 0. It needs no capability and runs on the real NIC at B6. **A new gate with a new way of measuring**, so a plan of its own, with a reversal (a `standard` run must read non-zero). After PR 2: [ADR-0072](docs/decisions/ADR-0072-a-tracer-free-check-that-the-hft-engine-thread-never-sleeps.md), `scripts/check-no-kernel-sleep-by-ctxt.sh`. **Closed `[2026-09-18]`** by [ADR-0072](docs/decisions/ADR-0072-a-tracer-free-check-that-the-hft-engine-thread-never-sleeps.md) and `scripts/check-no-kernel-sleep-by-ctxt.sh`: the tracer-free gate reads `voluntary_ctxt_switches` for the engine thread around the timed window and the idle hold, `hft` asserts 0 inside `tools/w2w` itself, and the reversal is the same assertion under `--mode standard`, which must go red. Commit id to follow.~~ | non-negotiable 4 on a hardware NIC; `CLAUDE.md` §2 *Machine checks* row 4 |
| 87 | ~~**`standard` cannot be wire-stamped without the instrument making it spin.** `[2026-09-14]` Option S3 of [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md) Sửa 2, Điều 3, not built. A TX timestamp requested on the engine's socket lands in its error queue, `poll(2)` always reports `POLLERR`, and a `standard` engine loops `poll → EAGAIN` until the observer drains it ([a-transmit-timestamp-wakes-a-blocking-engine](docs/reference/a-transmit-timestamp-wakes-a-blocking-engine.md)); so by the owner's Q10 `w2w` refuses `--mode standard --wire-timestamps` on a real NIC and item 40's wire row is `hft` only. **The idea:** BPF sock_ops TX timestamping (in the kernel since 6.15) — a cgroup program enables `SK_BPF_CB_TX_TIMESTAMPING` and receives the hardware stamp in `BPF_SOCK_OPS_TSTAMP_SND_HW_CB`, leaving the engine's socket untouched. **Read `__skb_tstamp_tx` first**: that a BPF-only request queues nothing on the error queue is inferred from how the flags are split (`SKBTX_HW_TSTAMP = SKBTX_HW_TSTAMP_NOBPF \| SKBTX_BPF`), not verified. Needs a BPF loader (a dependency and an ADR), `CAP_BPF` and `CAP_NET_ADMIN`; ~2–3 days, a plan of its own. [ADR-0073](docs/decisions/ADR-0073-the-standard-wire-figure-goes-through-bpf-sock-ops-and-not-before-phase-2.md): deferred to phase 2's first §9 boot, precondition verified.~~ | a `standard` wire figure on a NIC |
| 88 | ~~**`--journal file-async` changes the journal ring size together with the journal kind.**~~ — **CLOSED 2026-09-15**: B5 ran on it, both procedures (`DESIGN.md` §8 *Boot B*). `[2026-09-15]` **FIXED.** Step S1 of [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md) *Sửa 3* Điều 1 (owner: Q12): `tools/w2w` builds `FileStore` = `FileJournal<{ SLOTS }, { SLOT_LEN }>`, the same 4 096 × 512 as `Store`, and `crates/engine/benches/alloc.rs` case `journal-async-busy` opens the same shape. Gates re-run by the manager: `journal-async-busy 0` (reversal by the developer: a `to_vec()` in the Async push read 1, restored 0); `w2w --mode hft|standard --path app --journal file-async --messages 2000 --warmup 200` → `journal: file-async`, `allocs 0`, exit 0 both; the no-flag engine type is unchanged (`nm -C`: every `MemJournal<4096, 512>` instantiation identical, every `FileJournal<64, 512>` now `<4096, 512>`). `[2026-09-14]` Finding F13 of PR [#72](https://github.com/tmthang86/fixbolt/pull/72)'s review, **verified and not fixed**. `tools/w2w/src/main.rs` builds `FileJournal<64, 512>` for `--journal file-async` (`OneFile`), while the default `--journal mem` uses `fixbolt_engine::journal::Store` = `MemJournal<4096, 512>` (`crates/engine/src/journal.rs`, `SLOTS = 4096`). So boot B step B5's A/B varies two things at once — mem → file **and** 4 096 → 64 slots — and a difference cannot be put on either (`CLAUDE.md` §10, one variable at a time). **Resolve before B5**: match the ring sizes, or add the arm that isolates the ring | B5 of [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md), `DESIGN.md` §8's journal/log row |
| 89 | ~~**`[2026-09-22, boot D step D5]` the cadence's cost is profiled, and it is smaller here than at boot C.**~~ — **CLOSED 2026-09-23, boot F step F4, *accept, unnamed*** (`8c66123`): ADR-0095 decision 3 over the D5 records named `native_queued_spin_lock_slowpath` at −399…−435 ns in all four cross pairs — **against** the gap's sign (+61…+130 ns); on the gap's side no symbol clears the same-arm bar (≤ +58 ns vs +42 ns noise). IPC N = 1 → 16: 0.7688 / 0.7689 / 0.7726 → 0.7902 / 0.7947. [ADR-0096](docs/decisions/ADR-0096-a-figure-measured-elsewhere-meets-the-same-band-the-baseline-file-stays-out-of-the-heap-and-a-boot-may-rebuild-on-its-housekeeping-cores.md) decision 6 adds the missing sign clause to ADR-0095 decision 3. [measured-costs](docs/reference/measured-costs.md) *Boot D, D5 — the admin gap, diffed*. **History:** One `w2w` process, `hft`/`admin`, engine cpu6, client cpu7, 400 000 messages, `perf` attached to the engine tid the tool prints; 10 of 12 runs qualified. **N = 1 → p50 median 16 250 ns; N = 16 → 16 351 ns — a gap of 101 ns, 0.62%**, where boot C read 16 070 / 16 381 and ~1.9%. `engine-ctxt voluntary 0` and `allocs 0` on every run (non-negotiables 4 and 1, read per run). **The binary is not manifest-pinned**: `w1s`'s pre-built `w2w` lacks `--features affinity` and refuses to run unpinned, so D5 used the pre-boot-C `target/release/w2w` (sha256 `1cbe3f0…`, built 2026-09-19 09:18) — nothing was compiled, and the two boots are compared as tiers rather than differenced. The `perf diff` over the kernel symbols is post-boot work on files that live only on this desk. **History:** ~~**The `hft` engine thread spends 49.9% (admin) / 36.7% (app) of its samples asking an empty listener for a connection.**~~ — **Built 2026-09-18** (`Limits::listener_every`, `DEFAULT_LISTENER_EVERY = 16` at `crates/engine/src/presession.rs:442`, [ADR-0069](docs/decisions/ADR-0069-the-listener-is-polled-on-a-cadence-in-hft.md) Accepted). **What stays open is a mechanism question — the unexplained +1.7 % / +1.9 % on the admin path at N = 16 — and it now belongs to [boot D](docs/plans/2026-09-20-boot-d.md) step D5, not to this plan.** `[measured 2026-09-15]` boot B step B9, **diagnostic**: `perf record -F 4999 -g -t <engine-tid>` for 4 s of a 400 000-round-trip loopback `hft` run. `fixbolt_engine::Acceptor::accept` holds **49.9%** of engine-thread samples on the administrative path and **36.7%** on the application path; `__x64_sys_accept4` 27.7% / 18.3%, and under `do_accept` the kernel allocates a socket file and inode (`sock_alloc_file` 10.9% / 7.5%, `inode_init_always_gfp`, then `__fput`, `evict`) before answering `EAGAIN`. The thread spins, so these are shares of the spin loop, **not** a per-message cost: what a listener poll adds to the time between a request landing and the engine reading it is not measured. Non-negotiable 4's `hft` gate already lists `accept4` as allowed. For the architect: whether the listener should be polled less often than the sessions, and what that measures as. `docs/reference/measured-costs.md` *Boot B* — **NARROWED TO BOOT C 2026-09-18.** [polling-the-listener-less-often-than-the-sessions](docs/plans/2026-09-18-polling-the-listener-less-often-than-the-sessions.md) built the cadence: `Limits::listener_every` / `ListenerEveryTurns` (default 1, `presession.rs`, `settings.rs`), `pump`'s `ListenerCadence` counting down in the spin half and resetting on every wake (`lib.rs`), `tools/w2w --listener-every` mirroring it with a `logon-rtt` line, and [ADR-0069](docs/decisions/ADR-0069-the-listener-is-polled-on-a-cadence-in-hft.md) **Proposed**. Default stays 1, so nothing on the hot path changed by shipping this. **Not measured — boot C**: the A/B (N ∈ {1, 16, 256}, ADR-0068) that would set a different default and move ADR-0069 to Accepted needs the §9 machine, which the plan's step 5 could not reach on macOS. | `DESIGN.md` §8's floor; D8; ADR-0069 |
| 90 | ~~**A paced `w2w` run longer than the counterparty's `MaxLatency` is rejected, and nothing refuses to start one.**~~ — **CLOSED 2026-09-18**, branch `plan/w2w-maxlatency-guard`, commit `fc1fa54`. `[measured 2026-09-15]` boot B step B4: `--interval 1000000` with 125 messages failed at sequence 122 on `35=3 373=10` *SendingTime accuracy problem* — `w2w` renders `52=` before the clock starts, and the session's `MaxLatency` is 120 s. The engine was right; the run was re-done with 105 messages. **FIXED.** `paced_run_fits(interval_us, warmup, n, max_skew_ms)` in `tools/w2w/src/main.rs` refuses such a run at startup, before any engine or socket; `--max-skew-ms` declares the counterparty's `MaxLatency`, default `fixbolt_session::DEFAULT_MAX_SKEW_MS`. Tests: `a_paced_run_past_maxlatency_is_refused`, `a_paced_run_inside_maxlatency_is_allowed`, `an_unpaced_run_is_never_refused`, `a_declared_maxlatency_moves_the_bound`. [a-paced-run-outlived-the-sessions-maxlatency](docs/reference/a-paced-run-outlived-the-sessions-maxlatency.md) | `tools/w2w`; every paced §8 figure |
| 91 | ~~**The timing suite reads 3.0–8.4% slower than its 2026-09-05 lines, in band, everywhere B1 looked … Not isolated: code merged since 2026-09-05, the machine, or both.**~~ — **CLOSED 2026-09-18: it is code, in five steps, and the first one over the line is named.** `[measured 2026-09-18]` boot C step C-91, one A/B in one boot (`pass 16 fail 0 unknown 0`): the commit that recorded the 2026-09-05 lines, `0149b26`, built beside `85460c1`, 20 rounds alternating order — `engine turn, 1 busy sessions` 1 660.2 → 1 817.2 ns (**+9.5%**), every engine-turn case +6.7–9.8%, `validate NewOrderSingle` +5.2%, `validate TestRequest, w2w bytes` +6.1%, `encode ExecutionReport (template)` −3.4%. **The old commit in today's boot reads its own 2026-09-05 numbers** (1 660 vs the 1 657.7 line), so the kernel `-30 → -31` candidate is dead; the toolchain was `1.98.0` throughout. **C-91b bisected it** (judge `engine turn, 1 busy sessions`, median of 3, bad above 1 743 ns): `git` names **`588b350`** — `feat(session)!: read 52= at every precision` — as the first bad commit, worth **~+33 ns** (1 719.6 → 1 752.2); it touches `crates/session/src/clock.rs` (`parse_utc`, two widths → seven) and `crates/dict/src/field_type.rs`, and the candidate mechanism is **the width dispatch now on the hot path of every inbound `SendingTime`** — the same commit the validate cases pointed at, so one commit explains both families' first step. **Not one step, though: five.** ~+40 ns by `792c2e7` (code of 09-05 → 09-06), ~+20 by `28465e8`, ~+33 at `588b350`, ~+40 between `1c36406` and `f085f43` (TLS transport, settings, 09-09 → 09-12), ~+25 from `f085f43` to HEAD — each 2–3%, inside a 10% band, 9.5% together. The remaining four segments are **item 93**. Baselines stay at their 2026-09-05 values until each step has a name and a decision ([measured-costs](docs/reference/measured-costs.md) *Boot C*, C-91 and C-91b; `DESIGN.md` §8). The original finding: | ADR-0016, ADR-0031; the `density` and `validate` baselines, deliberately not re-recorded |
| 92 | ~~**Four `Reachable` mirrored definitions are red, and on today's evidence the harness is the suspect, not the session.**~~ — **CLOSED 2026-09-18, `09effd1`, as a harness hole.** `[measured 2026-09-18]` `make_receivable` substituted the corpus's placeholder timestamp only in `52=`; the session parses `122=OrigSendingTime` and `60=TransactTime` as `UTCTimestamp` too (FIX 4.4) and refused `00000000-00:00:00` with `371=122/373=1` and `371=60/373=6` — the engine was right and the harness had never had to make those two real, because QuickFIX's comparator matches by shape ([a-corpus-placeholder-timestamp-lives-in-three-tags](docs/reference/a-corpus-placeholder-timestamp-lives-in-three-tags.md)). Three tags substituted in `crates/conformance/src/script.rs`, three unit tests, reversal quoted; `Adapter::at` resumes at the first `I` line's `34=`. **Mirrored 14 / 14, no line of `crates/session/src` changed, 59 / 59 both ways.** The composition of the 14 moved by two each way: `8_AdminAndApplicationMessages` and `8_OnlyApplicationMessages` green; `19a`/`19b` (`97=Y`) pass and were misclassified — `Reachable` now; `6_SendTestRequest` and `1a_ValidLogonMsgSeqNumTooHigh` still red because **the corpus's script side falls silent where a correct initiator answers** (6: we answer the `TestRequest`, no `I` line claims it; 1a: the script's `ResendRequest` is never answered, we gap-fill, the file expects our `Logout`) — not `Reachable`, the two rows of the new class `NeedsAScriptThatAnswers` ([ADR-0076](docs/decisions/ADR-0076-the-mirrored-ceiling-is-a-classified-count-not-an-estimate.md) decision 2, *Measured*). Hypothesis B of the original row (`Session::resume(cfg, 5, 1)`) was built and was not the reason 1a is red. Ceiling 14, score 14 / 14, `docs/CONFORMANCE.md` §1; CI run id on the closing commit. **Owed in code**: the four rows' class and `why` in `mirror.rs`. The original row: | ADR-0076's ceiling of 14 being met; `docs/CONFORMANCE.md` |
| 93 | ~~**The engine turn gained ~9% in five steps since 2026-09-05.**~~ — **CLOSED 2026-09-23, boot E (S1, S4).** Five verdicts (row history below), the segment-(3) fix measured on `main`: `engine turn, 1 busy sessions` **1747.8 ns** on `m` (`badc144`, n = 21) against 1799.1 on `w1` (`76e53cb`, boot D) — **−51 ns**, and every engine-turn case on `main` reads inside the **2026-09-05 band** in `bench.sh --strict` (`target/boot-e-evidence/s3-strict.txt`, no engine-turn `OVER`). Baselines re-recorded per ADR-0090 decision 4 from `ms` n = 21 with the ADR-0095 drift ledger ([measured-costs](docs/reference/measured-costs.md) *Boot E*): `engine turn, 1 busy` 1659.8 → 1745.8 (+5.2 %), `ring 4096` 1657.7 → 1775.9 (+7.1 %) — what remains after the fix is segments (1), (2), (4), (5), each accepted by name or by IPC. **History:** `[2026-09-22, later]` all five segments have a verdict; (3) merged as PR [#94](https://github.com/tmthang86/fixbolt/pull/94) (`badc144`). P2 of the 2026-09-22 closing plan ran `perf diff` over the boot D `d2-<sha>-{1,2}.data` pairs (four cross pairs, two same-binary pairs as the noise floor, ADR-0095 decision 3): **(1)** `0149b26`→`792c2e7` *accept, unnamed* — no symbol clears twice the floor, IPC 2.856→2.983; **(2)** `792c2e7`→`28465e8` *accept, unnamed* — turn moved +0.7 ns, nothing to name; **(3)** fixed, `parse_utc` 21-byte straight-line path, 33.1 ns at boot D on the arm; **(4)** `1c36406`→`f085f43` *accept, named* `scan_fields::<64>` +13.1 ns (4.4× floor) with `Engine::turn` −7.1 ns on the same commit — inlining moved between two symbols, not an algorithm; **(5)** `f085f43`→`85460c1` *accept, named* `__memcmp_avx2_movbe` +13.4 ns (9× floor), IPC 3.023→2.998, caller unreadable because the D2 records carry no call-graph. Tables: `target/boot-d-evidence/d2-analysis/seg{1,2,4,5}.txt`, [measured-costs](docs/reference/measured-costs.md) *Item 93 — five segments named*. **Closes at phase 2 S4**: the fix measured on `main` (S1, `m`/`ms` arms), then the ADR-0090 decision-4 re-record. **History:** `[2026-09-22, boot D]` segment (3) has a cause and a fix; the other four are recorded but not yet analysed. Step D4, `w1` `76e53cb` → `w2` `1825d8c` (`ab/parse-utc-fast-path`), n = 20 each: `engine turn, 1 busy sessions` 1 799.1 → 1 766.0 ns, **33.1 ns faster**, against a rule declared before the boot that ≥ 25 ns confirms the mechanism and ≤ 10 ns refutes it. The two control cases moved 1.1% and 0.1%, and the arm touches only the parse path — so the segment's cost is the UTC parse, and the arm becomes a fix PR. Step D2 recorded both ends of every segment: seven worktrees `e1`…`e7`, 21 runs, ~65 MB and ~1.7 M samples each, **no lost samples**, `engine turn` walking 1 657.9 → 1 726.0 ns (**+4.1%, +68.1 ns**) across the bisect — which is **not** the slope C-91b found: that read 1 660.2 → 1 817.2, **+9.5% / +157 ns**, more than twice this, and C-91b put `588b350` at +33 ns where `e4` reads 16 ns *faster* than `e3`. Two days, two procedures (3-run medians under `bench.sh` there; one run apiece under `perf` here), neither refutes the other, **no cause claimed** — but the D2 column cannot be quoted as confirming C-91b's ordering, which is what it was recorded to do. **The `perf diff`/`perf annotate` work has not been done and the files live only on this desk** (`target/boot-d-evidence/d2-*.data`). [measured-costs.md](docs/reference/measured-costs.md) *Boot D … Item 93*. **History:** **The engine turn gained ~9% in five steps since 2026-09-05, and each step is a perf task with its bisect segment named.** `[measured 2026-09-18]` boot C steps C-91 and C-91b ([measured-costs](docs/reference/measured-costs.md) *Boot C*): `engine turn, 1 busy sessions` 1 660.2 → 1 817.2 ns between `0149b26` and `85460c1`, `git bisect` medians of 3 runs, 5% judge line at 1 743 ns, log `target/boot-c-evidence/c91b-log.txt`. The five segments, each a task of its own — profile the two endpoints on the §9 desk (`perf record` on `density`'s `engine turn, 1 busy sessions`, n ≥ 5 each side, one boot), name the function that grew, decide *fix* or *accept with a re-recorded baseline*: **(1)** `0149b26` → `792c2e7`, ~+40 ns, code merged 2026-09-05 → 09-06 (wave B plan 1/2: settings for both roles, `789`/`369` — the session's `Reply` seam moved); **(2)** `792c2e7` → `28465e8`, ~+20 ns, PR #55; **(3)** `28465e8` → `588b350`, **~+33 ns, the named commit** — `parse_utc` two widths → seven, `dict/field_type.rs`; candidate mechanism the width dispatch on every inbound `SendingTime`, checkable by reverting the dispatch to a length switch with the 17/21 fast path first and re-running `validate Heartbeat` (+3.3%) and the turn; **(4)** `1c36406` → `f085f43`, ~+40 ns, the TLS transport generic over its side, `TlsMode` read-back and the settings keys of 09-09 → 09-12 — a transport generic that stopped inlining a `recv` is the first suspect; **(5)** `f085f43` → `85460c1`, ~+25 ns, 09-12 → 09-18, `pump`'s `ListenerCadence` and the `--assert-no-voluntary-switches` sample among them (the cadence should make a turn *cheaper*; `density.rs` drives `Engine::turn` directly, so if it shows here the cost is in `turn`, not `pump`). No segment tripped `bench.sh` alone (each 2–3%, band 10%) — [ADR-0031](docs/decisions/ADR-0031-a-baseline-is-a-band.md)'s band is wider than one step and the ratchet is per case, not cumulative; whether the band should also read a cumulative drift is a question for the architect once the five have names. **The baselines are not moved** until then. Each task is briefable as: two commits, one bench case, one profile each side, one named function, one decision | `DESIGN.md` §8 `N × 449 ns` and the engine-turn rows; ADR-0016/0031's band |
| 94 | ~~**The alloc bench's message fails its own checksum, and the 2026-09-05 correction reached one file of four.**~~ — **CLOSED by [ADR-0089](docs/decisions/ADR-0089-a-shared-bench-fixture-has-one-source-included-by-path-and-a-test-that-parses-it.md), commit `68d442e`**: one source, `crates/codec/benches/fixture.rs`, included by `#[path]` into all four benches; `crates/codec/tests/bench_fixture.rs` runs on every commit and holds two tests — one parses the fixture under `Validation::ALL`, one walks every `.rs` file recursively under `crates/`, `tools/`, and `benches/` for the `167=BOO\x0110=` marker and fails naming any bench that kept its own copy. **What is new this time**: the fourth recurrence is closed by a gate, not by a corrected byte — the 2026-09-05 and 2026-09-19 corrections each reached one file by hand and the other copies stayed wrong. The two tests carry their own anti-vacuous asserts (red if the walk found no files under `crates/` or `tools/`, or fewer than 100 in total, red if `fixture.rs` itself loses the marker), so the singleness test cannot pass by looking at nothing. `[2026-09-19]` `crates/codec/benches/alloc.rs` carried `10=098` where the checksum is 097, so the `parse` and `parse via Encoding` cases measured a walk that ends in `Err(BadCheckSum)` — the whole message is walked either way, so no allocation count moved, and the liveness asserts added in `6f82cf3` stated the outcome rather than depended on it. `engine/benches/dispatch.rs` and `engine/benches/ring_full.rs` held the same literal and never parsed it. [a-bench-message-that-fails-its-own-checksum](docs/reference/a-bench-message-that-fails-its-own-checksum.md), [a-benchmark-parsed-a-message-the-parser-rejects](docs/reference/a-benchmark-parsed-a-message-the-parser-rejects.md). |
| 95 | ~~**`validate Heartbeat` is 12.3% slower on `main` than it was before phase-2 PR A, and three more session-dictionary cases moved with it.**~~ — **CLOSED 2026-09-23, boot E S1, as a bisect at merge granularity: the whole step is inside PR B.** `target/boot-e-evidence/s1m/summary.txt`, n = 21 per arm, CONTROL `ms`: `validate Heartbeat` `wa` 6fbe851 **166.8** → `b1` e673e8f (PR B merge) **190.8** → `b2` 188.1 → `b3` 188.0 → `w1` 189.0 → `m` 188.7; `validate NewOrderSingle` 950.3 → 1012.9 → 1008.1 → 1010.1 → 1009.2 → 1010.0; `validate TestRequest, w2w bytes` 220.6 → 239.5 → … → 242.5. One step of +14 % / +6.6 % / +8.6 % between `wa` and `b1`, then flat within `max/median` ≤ 1.04. **The commit inside PR B is not named** (PR B is one merge of FIXT 1.1 / FIX 5.0 SP2 dictionary work); the baselines now carry the new level with the ledger, and whether PR B's step is bought back is a plan of its own, not this item. The original finding: **`validate Heartbeat` is 12.3% slower …** `[measured 2026-09-22, boot D step D4]` `wa` `6fbe851` → `w1` `76e53cb`, n = 20 each, same features, one rotation against a shared control on the §9 desk (`pass 16 fail 0 unknown 0`): `validate Heartbeat` 167.2 → 187.8 ns (**+12.32%, outside the 1.10 band**), `validate TestRequest, w2w bytes` +9.39%, `validate NewOrderSingle` +5.75%, `validate NewOrderSingle, w2w bytes` +4.97%. **The engine turn did not move**: −0.6% at 1 busy session and +0.3…+0.9% at 2–64, so this is the session's dictionary pass and not the turn. The commits in that span are PRs B, C, #85 and #86 — none of which promised this band beforehand, which is why the plan's rule opens an item rather than stopping the boot. **No bisect has been run**, and no cause is claimed; ADR-0086's descent is the obvious suspect and is *not* evidence (item 96 is why). **Seen against the recorded baselines rather than against `wa`, this is worse than +12.32% looks** — `validate NewOrderSingle` is only +5.75% on the `wa`→`w1` delta but **×1.160 over its own line** (item 97), so the span merely finished a breach that was already building. [measured-costs.md](docs/reference/measured-costs.md) *Boot D … The band across PRs B, C, #85 and #86*. | ADR-0031, ADR-0085, ADR-0086 |
| 96 | ~~**`[2026-09-23, boot E S2]` ADR-0094's in-binary experiment ran: the descent's own code is 2.94 % of the run — 2 455.6 ns, 3.0 % of `validate TradeCaptureReport (33 groups)` — and that is a lower bound, because the `-g` callchains of a frame-pointer-less release build are garbage and the inclusive share was never measured.**~~ — **CLOSED 2026-09-23, boot F step F7: the descent's inclusive price is ≥ 7 021.6 ns** (children 8.49 %, self 2.86 %, n = 3, range 6 844.8–8 023.2 ns; 8.87 % of the case divided by its own 95.72 % share of the run). Three `perf record -e cycles -F 999 --call-graph dwarf,32768` of the manifest-pinned `ms` `validate` binary (sha256 `e34e60f9…`, cpu6): children 8.49 / 9.68 / 8.28 %, self 2.86 / 3.31 / 2.85 %, case 82 666.9 / 82 704.7 / 82 884.6 ns/op, Total Lost Samples 0 / 0 / 0; C1–C5 hold, every chain reaches `_start`. Boot E's 2.94 % was the self share only; boot D's −10.83 % sits ~2 points above, which is layout or second-order cost. The read needed a workaround: perf 7.0.14's libdw unwinder fails on the LLD-linked PIE, and children ≡ self was the tell ([perf-dwarf-unwind-fails-on-an-lld-linked-pie](docs/reference/perf-dwarf-unwind-fails-on-an-lld-linked-pie.md)). The manager did not re-run the read; the agent's independent `perf script` re-aggregation reproduced the Children figures. **Follow-up for the architect (a note, not an edit of ADR-0094's substance):** ADR-0094 gives "the first level is inlined" as the reason for "≥"; the records show one outer call site (`c7046` → `c9680`), so no level of the descent is inlined — what is inlined into `validate_with` is `bad_group_count`'s loop that drives it. [measured-costs](docs/reference/measured-costs.md) *Boot F, Item 96*; `target/boot-f-evidence/d96f-analysis.txt`. **History:** **`[2026-09-23, boot E S2]` ADR-0094's in-binary experiment ran: the descent's own code is 2.94 % of the run — 2 455.6 ns, 3.0 % of `validate TradeCaptureReport (33 groups)` — and that is a lower bound, because the `-g` callchains of a frame-pointer-less release build are garbage and the inclusive share was never measured.** Five records, 0 lost samples, C1 and C3 hold, C2 trivially. Its callees (`group_members` 31.3 %, `region_end`, `open`) are shared with `defers` (ADR-0085) and cannot be divided without a real call-graph; the inclusive cost lies between ~3 % and ~48 % of the case, consistent with boot D's −10.83 % and closing nothing. **For the architect:** one more §9 record of the same pinned binary with `--call-graph dwarf` after `debug = 1` in a bench profile. [measured-costs](docs/reference/measured-costs.md) *Boot E, Item 96*; `target/boot-e-evidence/d96-analysis.txt`. **History:** **The experiment that would price the ADR-0086 descent cannot isolate it, and the reason is layout.** `[measured 2026-09-22, boot D step D4]` `w1s` `76e53cb` → `w3` `d6f79dc` (`ab/validate-no-descent`), both `fix50sp2`, n = 20: `validate TradeCaptureReport (33 groups)` 82 071.1 → 73 182.8 ns, **−8 888.3 ns (−10.83%)**. The design required four group-free control cases — messages that never enter the descent — to agree within 2% before that difference could be attributed; **three moved 2.9–5.8%** (`validate Heartbeat` +5.78%, `validate TestRequest, w2w bytes` +4.35%, `validate NewOrderSingle, w2w bytes` +2.86%). So the magnitude on this desk is known and the **attribution is not**: some of the 10.83% is the descent and some is a different binary's code layout, the same hole ADR-0049's flag narrows but does not close ([a-benchmark-that-measures-where-the-compiler-put-it](docs/reference/a-benchmark-that-measures-where-the-compiler-put-it.md)). **For the architect**: what experiment prices a descent without building a second binary? ADR-0086 carries a dated note; the cost stays in *Not proven*. **Experiment designed: ADR-0094** — attribution inside one binary, C1 (`nm` shows `bad_nested_count`) checked pre-boot. Waits for the next §9 boot; `ab/validate-no-descent` retired. | ADR-0049, ADR-0086, ADR-0090, ADR-0094 |
| 97 | ~~**Eight medians over five recorded baseline lines are breached on `main` on the §9 desk.**~~ — **CLOSED 2026-09-23, boot E S3/S4.** `bench.sh --strict` on the old lines read **red as observed** (`target/boot-e-evidence/s3-strict.txt`: three `OVER BASELINE`, `validate NewOrderSingle` 1024.2 vs 882.1, `Heartbeat` 193.5 vs 169.5, `TestRequest, w2w bytes` 247.3 vs 218.4; the engine-turn cases were **not** over — PR #94 had already pulled them back), then `benches/baselines.tsv` was re-recorded from `ms` n = 21 (15 lines moved, 2 FIXT and 4 SBE lines added) and the second run (`s4-strict.txt`) reads every re-recorded case in band. **`--strict` is still not green**, for two reasons that are their own items: the two `wakeup` p50 cases have no baseline mechanism at all (item 100) and `journal put, 191 bytes, one slot` moved 7.4 → 12.4 ns between the two runs (item 99). **History:** `[2026-09-22, later]` 7a read: the fixed detector names the eight breached medians by machine — `d4`+`d4b` re-extracted and merged (2080 rows, n = 20), `CONTROL=w1s --summary`: `w1s validate NewOrderSingle 1023.5 20/20`, `Heartbeat 195.6 20/20`, `TestRequest, w2w bytes 248.1 20/20`; `w1 validate NewOrderSingle 1008.9 20/20`, `engine turn, 1 busy, ring 64 1822.2 20/20`, `ring 4096 1860.3 20/20`, `validate Heartbeat 187.8 15/20`, `TestRequest, w2w bytes 242.4 15/20`; no `w0` row over; footer `over baseline: 30`. Phase 2 S3 runs `bench.sh --strict` on the desk expecting **red** (observed, not inferred), S4 re-records. **Detector record fixed 2026-09-22 by ADR-0092** (`ab_extract` anchors on the row, exit status read, `OVER` is its own state, `--summary` has an `over` column and `--reextract`). **The slowdown is untouched and the baselines are not moved** — ADR-0090 decision 4 still holds. Open until 7a re-reads `d4m/` on the desk and item 93 concludes. **Eight medians over five recorded baseline lines are breached on `main` on the §9 desk, so `scripts/bench.sh --strict` would be red there — the band check that fired is the one inside each bench binary, which `bench.sh` drives.** `[measured 2026-09-22, boot D step D4]` The bench harness panics on an over-band case, and it did so on **every qualifying round** of both arms carrying `main` `76e53cb` — `w1s` (`fix50sp2`) `validate` 20 of 20, `w1` (no feature) `validate` 21 of 21 and `density` 21 of 21. Medians over n = 20 against `benches/baselines.tsv`: `validate NewOrderSingle` 1 023.5 vs 882.1 (**×1.160**, ceiling 970.3), `validate Heartbeat` 195.65 vs 169.5 (**×1.154**), `validate TestRequest, w2w bytes` 248.1 vs 218.4 (**×1.136**) on `w1s`; on `w1` the same three at ×1.144 / ×1.108 / ×1.110 plus `engine turn, 1 busy, ring 4096` 1 860.3 vs 1 657.7 (**×1.122**) and `ring 64` 1 822.2 vs 1 635.5 (**×1.114**). `wa` breached 3 of 21 `density` rounds and 1 of 21 `validate`; `w0` never; **nothing read under its floor**. **Why no gate caught it**: the CI `bench` job runs on GitHub's runner, which has no line in this table, and the §9 desk only runs `bench.sh` when someone runs it, and the last recorded `--strict` runs here are boot B step B1, `[measured 2026-09-15]`. Boot D did not run `bench.sh` either; it ran the pre-built bench binaries directly, and **their own band check is what panicked**. **Do not re-record the baselines to make it green**: ADR-0090 decision 4 reserves re-recording for when item 93 concludes, and moving the line now writes the slowdown into the detector. Items 93, 95 and this one are three views of one slowdown; this is the one that says it has crossed a line the project already drew. [measured-costs.md](docs/reference/measured-costs.md) *Boot D … `main` is over its recorded baselines*. | ADR-0016, ADR-0031, ADR-0090, ADR-0092 |
| 98 | **CLOSED by ADR-0093.** Both recorded traps now have a gate. **(a) decision 2** — `scripts/check-sudo-names-what-root-can-find.sh`, red on boot D's exact line in `check-sudo-verdicts.sh` (`FAIL R2 cargo`). Two more rounds ran after the plan's own text was written: **G5 closed** — the gate's own verdict-test file was excluded from scope by name; it is now scanned like every other file, with a trailing `# check-sudo: fixture` marker (honoured only inside `check-sudo-verdicts.sh`) skipping the lines that are deliberately bad fixtures, and the count of skipped lines prints on the `ok` line (`9e33aa9`). **A tokenisation gap closed in both directions** — `unfuse_quotes` turns a quote-fused word into one R2 can read (`sudo sh -c 'cargo bench -q'` now reads `FAIL R2 cargo`, not `ok`), and fixed a false positive the same cause produced (`sudo 'tee' /sys/x` no longer reads `FAIL R1`) (`67e2898`). **What stays open, by decision, not by omission**: a word fused to a shell metacharacter — `&`, `;` or `|` — is still missed (`sudo sh -c 'true;cargo bench'` reads `ok`), because unfusing `|` would make `sudo grep -E "cargo|rustc" /etc/x` tokenise as a toolchain invocation and flag a legitimate line. On the real tree: `ok — 32 scripts scanned, 32 sudo lines read, 19 fixture lines skipped by marker, 0 findings`. **(b) decision 3** — `check-machine.sh`'s `no timer due` row, `systemctl list-timers --all --output=json` through `timers_verdict`, `FIXBOLT_TIMER_WINDOW` default 12 h, `FAIL` on any timer due inside it; `ab-rotation.sh` refuses to start on that `FAIL`. **Drivers: decision 1** — a campaign driver belongs in `scripts/`, on the plan's branch, before the reboot; the three dead files (`run-d2.sh`, `run-d3.sh`, `run-d5.sh`) are not rebuilt, by decision, not lost twice. | CLAUDE.md §4, §10, ADR-0093 |
| 99 | ~~**`journal put, 191 bytes, one slot` read 7.4 ns in one `--strict` run and 12.4 ns twenty-seven minutes later in the same boot, same pinned binary, and stayed at 12.4 on four re-runs.**~~ — **CLOSED 2026-09-23, boot F, cause named: the bench's heap layout, set by the length of `benches/baselines.tsv`** — the second instance of the ADR-0067 class, at run time. Old binary (F1, before any `perf record`, `target/boot-f-evidence/f1-table.txt`): **12.4** on the 24 897-byte file, **7.4** on the 24 226-byte `9374820` file, 12.4 again after restore; a 16-byte-step sweep `k = 0…1024` read **6.3 … 19.0 ns**, `walking` flat at 8.7 … 8.9. F1c (`f7.txt`): after an S2-shaped `perf record -g`, still 12.4 — **`perf` is not a cause**. Fix `6b2833b`: `load_baselines`/`cpu_model` read into a fixed `String::with_capacity(1 << 20)` (ADR-0096 decision 2, revised in place: a fixed-size brk block, not `mmap`), test `a_loaded_file_reads_into_a_fixed_one_mib_buffer`. The sweep on the new binary (`f8.txt`) reads **7.4 … 8.3 — max/min 1.12, NOT flat by the plan's own 1.10 bound**, two single points (8.2, 8.3) over the 8.1 ceiling; the ×1.68 step is gone. The `one slot` line was **not** re-recorded (7.4 / 7.6 / 7.4 against 7.4, band [6.7, 8.1], in the three boot F `--strict` runs). The residual 1.12 is in *Not proven*; the fix moved another case — item 101. [measured-costs](docs/reference/measured-costs.md) *Boot F, item 99*; [recording-a-baseline-changed-the-baseline](docs/reference/recording-a-baseline-changed-the-baseline.md) *The second time, at run time*. **History:** `[measured 2026-09-23, boot E]` `target/boot-e-evidence/s3-strict.txt` (21:07 UTC) 7.4, in band; `s4-strict.txt` (21:44 UTC) 12.4 `OVER BASELINE` (band [6.7, 8.1]); then three bare runs of the `main-tree` `journal` binary from `../fb-s9e/MANIFEST.txt` and one under `taskset -c 6`, all 12.4. The two `walking` cases did **not** move (8.8 / 5.3 ns), `check-machine.sh` still `pass 17 fail 0 unknown 0`, THP `never`. Between the two runs the desk ran S2 (`perf record -g` ×5 of the `validate` bench) and S1c (24 rounds of `sbe`). The case is pure L1 (one slot, eight lines) so the suspects are CPU-side state a `perf record` could leave (PMU / NMI-watchdog counter re-armed), not the code. **The baseline line is not moved.** First check next boot: run the journal binary **before** any `perf record`, then after one, and read the case both times. | ADR-0031; `bench.sh --strict` on the desk |
| 100 | ~~**`bench.sh --strict` cannot be green on the §9 desk while the `wakeup` bench prints its two p50 cases as `NO BASELINE` by construction.**~~ — **CLOSED 2026-09-23, boot F.** `Suite::figure` (ADR-0096 decision 1, `6b2833b`) judges the two p50s against `benches/baselines.tsv` like any case; lines `wakeup epoll p50` **5 089.0** and `wakeup poll p50` **4 939.0** ns, unpinned, margin 1.10, **n = 19** (20 runs, run 1 discarded — one short of the plan's 20), `target/boot-f-evidence/f23.txt`. `bench.sh --strict` run 1 on the desk (`f8-strict-1.txt`): **exit 0, `cases w/o a baseline 0`**, over 0, under 0 — the first green `--strict` on the desk since boot B B1 (2026-09-15). Run 2 was red on one other case (item 101), run 3 after its re-record green again (`f8-strict-3.txt`: exit 0, over 0, under 0, no-baseline 0). **History:** `[2026-09-23, boot E S3/S4]` `crates/engine/benches/wakeup.rs` lines 59–66 and 153–159: the p50 arms are printed as `NO BASELINE` cases "until boot C records one" and there is no `benches/baselines.tsv` lookup for them, so `bench.sh --strict` counts `2 case(s) had no baseline` on every run — it has been structurally red since item 89 merged (2026-09-18); the last green was boot B step B1 (2026-09-15) and nobody ran `--strict` on the desk in between (item 97 said so). Design question for the architect: give the p50 arms a tsv line (which quantile, which margin, what n — one run is n = 20 000 samples, not 20 runs) or exclude them from `--strict`'s count by decision. Until then a desk `--strict` is read as "red only on `wakeup`" by hand. | ADR-0016, ADR-0031, item 89; `scripts/bench.sh` |
| 101 | ~~**The fix for item 99 moved a different case +10 %: `fixbolt-sbe`'s `walk nested group + varData` read 153.6 … 160.3 ns on the pre-`6b2833b` binary and 174.2 … 176.1 on the post-`6b2833b` one, same boot, interleaved, pinned `cpu6`, nothing in `crates/sbe` changed.** `[measured 2026-09-23, boot F]` `target/boot-f-evidence/f8-sbe-ab.txt`, n = 5 per arm. **Not** glibc's raised mmap threshold: `MALLOC_MMAP_THRESHOLD_=131072` read 174.7 … 177.4. The mechanism — the heap position after the harness's fixed 1 MiB block, or the harness's own code layout — is **not isolated**. It turned `--strict` run 2 red (177.9 over 175.5); the line was re-recorded **with this cause** (`5576694`, ADR-0095 decision 2): 159.5 → **176.5**, median of 20, max/median 1.038, margin 1.10; run 3 green. **For the architect:** make a bench's data placement independent of the harness's own allocations (e.g. allocate the timed working set aligned and first, or from a fixed arena), or accept layout sensitivity for sub-200 ns cases by decision. Every fix to one layout term so far has been a new layout for the others (items 52, 99, 101). [measured-costs](docs/reference/measured-costs.md) *Boot F, `--strict` three times*.~~ **CLOSED 2026-09-23** by [ADR-0102](docs/decisions/ADR-0102-a-line-that-moves-while-its-instruction-count-does-not-is-a-layout-move-and-the-count-is-read-off-the-desk.md) *Outcome*, PR [#101](https://github.com/tmthang86/fixbolt/pull/101): the binary's own layout, instructions:u −0.0317 %. | ADR-0067, ADR-0095, ADR-0096 decision 2; `benches/baselines.tsv` |
| 102 | **A panic that unwinds past `after_serving` skips the wait for retired journal writers**, so records a retired writer had not flushed can be lost at process exit. `[2026-09-24]` review of PR #103; ADR-0154 names it; no test. | ADR-0153, ADR-0154 |
| 103 | **A blocking `connect()` in the dial loop hears `Admin::shutdown` only when the kernel's SYN timeout ends it (~2 min)**; a reconnect wait now hears it in ~100 ms (D2). `[2026-09-24]` review of PR #103, probe `connect-in-progress shutdown NOT heard within 8 s`. | `crates/engine/src/lib.rs` dial connect |
| 104 | **In `pump`, `Recovery::recover` reads the journal file on the engine thread mid-serving** — a filesystem syscall on the `hft` hot path, older than phase 3; `ready()` no longer does (ADR-0155). | non-negotiable 4; ADR-0155 |
| 105 | **Narrowed 2026-09-24: emptying `after_serving` turns the shard test red only 10 runs of 20**, because the one writer often finishes inside the join anyway. ~~`Shards` has no join~~ — **fixed**, branch `fix/after-serving-race`: `impl Drop for Shards` disconnects then joins every shard thread, so the drop returns after each shard's `after_serving`. Without the join the test's own wait read zero writers before the shard had retired one and returned at once — main's CI run 35918095562, *"found 1990"* of 2000. Guarded by `shard_serve_returns_after_its_writers_finished` (`Drop` emptied: 10 of 10 red on *"drop(shards) returned while a retired writer was still writing"*). [a-drop-that-only-signals-is-not-a-shutdown](docs/reference/a-drop-that-only-signals-is-not-a-shutdown.md) | ADR-0088 decision 5; ADR-0153 |
| 106 | **`sbe-gen`: a field's own `presence` overrides a constant `<type>`'s `valueRef`** — a `required` field on a constant type silently gets 1 wire byte. Pre-existing; found by the review of PR #102; no test. | `docs/reference/sbe-valueref-on-a-composite-member.md` |
| 107 | **Applying rustls's `discard` before `refuse()` has no test that bites**: rustls 0.23.45 drains the queued alert before reading input, so skipping `discard` stays green. | ADR-0151; `docs/reference/rustls-unbuffered-hands-out-a-queued-alert-only-on-the-next-call.md` |
...
| 114 | **Kernel bypass waits for a machine: a later-phase candidate, not phase 4.** Dropped at G1 on the desk's I211 (ADR-0203); the owner kept it for later (2026-09-26). Reopens only on a named acceptor host, NIC, kernel and peer with G0 (`ethtool -x <nic>` prints an RSS key) quoted; which NICs pass (`ixgbe`, `i40e`, `ice`, `mlx5`; not AWS `ena`) and what a rented VM or bare-metal server can do: [kernel-bypass-needs-a-machine-this-project-does-not-have](docs/reference/kernel-bypass-needs-a-machine-this-project-does-not-have.md). | ADR-0204 (Proposed); `PRD.md` §2 *Later phases* |
...
| ~~62~~ | ~~**No CI job has ever run a TLS test.**~~ — **CLOSED 2026-09-10**, [plan](docs/plans/2026-09-10-the-tls-tests-have-no-ci.md). A `tls` job runs `clippy --features tls` and the three TLS test files, in **its own job** rather than as a step on `fmt · clippy · test`: one of those tests asserts `/proc/net/tls_stat` moved, which is a claim about the **runner**, and folded in with six other commands a kernel that lost its `tls` module would read as *"the test suite is broken"*. **Its first step asserts the environment and fails in the environment's own words.** `[measured 2026-09-10]` **step 1 was a measurement and it deleted the hardest step of the plan**: GitHub's runner is `Linux 6.17.0-1022-azure`, `CONFIG_TLS=m`, module loaded, `setsockopt(TCP_ULP, "tls") ACCEPTED`, verdict `READY` — so nothing had to be `#[ignore]`d and the plan's whole *a skipped test looks exactly like a passing one* problem never arose. Deciding first would have built an ignore mechanism and then learned it was unnecessary. **The reversal worth keeping**: `[measured 2026-09-10]` dropping `--features tls` from the command leaves **cargo at exit 0 with 0 tests run**, against **11** with it — so the job counts the tests that ran and fails at zero, because a green `cargo test` that compiled nothing looks identical to a real one. **Still open, and named rather than folded in**: `check-no-kernel-sleep.sh` still has no TLS arm, so nothing here says anything about the engine thread under TLS. **Original text follows.** **No CI job has ever run a TLS test.** `[2026-09-10]` **plan written and awaiting approval**: [the-tls-tests-have-no-ci](docs/plans/2026-09-10-the-tls-tests-have-no-ci.md). Its step 1 is a **measurement, not a fix** — whether GitHub's runner carries kTLS is not known here and the plan refuses to assume it. Its governing rule is that **an environment that cannot run the test must fail differently from an engine that is broken**: today both produce `tls_stat TlsTxSw did not move`, which blames the engine for a missing kernel module. `[measured 2026-09-10]` `.github/workflows/ci.yml` passes `--features affinity` for the shard tests and `--all-features` to `cargo deny` and (since item 61) to `cargo doc`, but **never `--features tls` to `cargo test`**. So `crates/engine/tests/tls.rs` — six tests, merged in PR [#57](https://github.com/tmthang86/fixbolt/pull/57) — and `crates/engine/tests/tls_wire.rs` have **only ever run on the owner's desktop**. Read off the green run for `20a3282`: `grep` for any of their test names across the whole job log returns **0**, while `interop`, `bench` and `deny` all print their own lines. **This is item 61's family, one layer out**: the rustdoc gate could not see a feature-gated *file*; here the whole test suite for a feature is outside every gate, and 26 of 26 checks are green about it. **Why it is not fixed in the same commit that found it**: `tls.rs::after_the_handover_the_kernel_holds_the_keys_and_read_returns_plaintext` asserts `/proc/net/tls_stat` moved, which **fails rather than skips** on a kernel without `CONFIG_TLS`, and whether GitHub's runner has it is unknown here. `scripts/check-ktls-available.sh` already answers that question in one command and `check-ktls-classify.sh` already runs in CI, so the shape of the fix exists; **choosing between "run them and require kTLS on the runner" and "run them and skip the kernel half" is a decision, not a typo**, and CLAUDE.md §1 says a decision gets a plan. `tls_wire.rs` alone needs no kTLS — it serves through the userspace fallback — so a partial fix is available and is deliberately not taken piecemeal. **This item's shape recurred twice more in one day** (item 68's "`TLS tests that ran: 14` was a true count of a list that was missing three") **and item 68 is what finally removed the list**: `scripts/check-feature-gated-tests-ran.sh`, closed 2026-09-12, asks the build what tests exist rather than naming files, so there is no list left for a fourth instance to fall behind | the TLS feature has no gate but one desktop | closed |
| 61 | ~~**The rustdoc gate never opened the files it was guarding.**~~ — **CLOSED 2026-09-10, by running the same command twice.** `[measured 2026-09-10]` the `docs` CI job ran `cargo doc --workspace --no-deps` under the **default** feature set, so it never compiled `crates/engine/src/tls.rs` — whose **very first line** carried a broken intra-doc link for as long as the file had existed — and it printed green about it. Adding `--all-features` surfaced **two more** in `shard.rs`, one of them pointing at a type that **had been deleted**. **Found as a by-product**: a different broken link, in a file the default build does compile, turned the job red on `fc2cb6a`; fixing it meant running the command locally, and running it locally meant running it a second time with the feature on. Nothing was scheduled to ever look. **Both invocations are kept, and that is the decision worth reading**: neither is a superset of the other. The link that turned CI red was on an **ungated** type and pointed at `serve_tls`, which exists only with `--features tls` — so the *default* run failed and the all-features run was fine, the exact opposite direction from the three the wider run found. **Proven by reversal, which is the step this class of gate keeps skipping**: a broken link pasted inside the gated module leaves the default run at **exit 0** and takes the all-features run to **exit 101**. [a-doc-gate-never-opened-the-file-it-was-guarding](docs/reference/a-doc-gate-never-opened-the-file-it-was-guarding.md) **`[to testing-skills]`** | closed |
| 45 | ~~**The order of everything after wave A, so it does not live only in a chat.** `[amended 2026-09-14, later]` **Wave C's window A is built, and (b)'s counterparty is ready to be measured against — no figure yet.** [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md) steps A1–A8, PR [#72](https://github.com/tmthang86/fixbolt/pull/72); its B3 was done by PR #71. For (b), step A7 (`a7d5943`): `matthart1983/nanofix` cloned into gitignored `vendor/nanofix`, **pinned at `0f79bae`** (MIT), the commit `measured-costs.md` §1 already read; two local patches recorded **in words only** (`build.rs` made a no-op because it never reads `CARGO_FEATURE_AERON`; an empty `[workspace]` table); an example acceptor built from nanofix's public pieces that **primes the expected sequence**, because `FixServer` never runs the inbound Logon through `validate_inbound_seq` and every later message reads as a gap — a defect in nanofix, not patched. `w2w --connect` on `--path admin` and on `--path app` both answer, exit 0. The head-to-head itself is boot B step B8, `standard`. `[amended 2026-09-10, later]` **Step 4a of plan 4 is done: TLS reaches the front door.** `serve_tls` / `serve_tls_with` build `PendingSet<TlsTransport, …>` and the new `AcceptorEngineOver<TlsTransport, …>`; `crates/engine/tests/tls_wire.rs` brings a session up through a real TLS handshake with an ordinary `rustls` client. **The plan had no step that owned this** — building the engine over `TlsTransport` appeared only inside step 2's prose, and step 2 closed without it — so step 4 split into 4a / 4b / 4c, recorded as the plan's Sửa 3 and approved before any code. **4b and 4c are open**: nothing reads `TlsMode`, so a deployment still cannot tell which of ADR-0005's three modes carried its bytes, and there are no configuration keys. `[amended 2026-09-10]` **Wave B plan 4, `tls`, is neither a draft nor unstarted — it was approved on 2026-09-09 and steps 1-3 plus the transport half of step 4 are built**, on `plan/tls`, PR [#57](https://github.com/tmthang86/fixbolt/pull/57), **26 of 26 checks green**. `crates/engine/src/tls.rs` (787 lines) and `crates/engine/tests/tls.rs` (467) exist; **no `serve*` reaches TLS yet** and that is deliberate, not abandoned — a config key nothing reads is a promise in a document. Steps 4b (`TlsRequireKernel`, the event), 5 (initiator), 6 (`w2w --tls`, the three §8 numbers, a TLS arm on `check-no-kernel-sleep.sh`) and 7 (docs) are open. **The sentence below saying it is *still a Draft, still owing this row's re-verification* was true at 2026-09-09 morning and false by that afternoon**; two pull requests merged after it and neither reader corrected it, which is this section's rot in its usual direction — claiming less is built than is. `[amended 2026-09-09, later]` **Wave B plan 3 is CLOSED, both halves.** ADR-0057 `Accepted`, half B's five steps shipped the same day: `TimestampPrecision` as the 26th key, `52=` out at 21/24/27 bytes, and an interop scenario judged on bytes. **So wave B's next unit of work is plan 4, [tls](docs/plans/2026-09-04-tls.md)** — still a *Draft*, still owing this row's re-verification, and the largest of the drafts (3–5 days, Linux from step 2, and `scripts/check-ktls-on-a-plain-socket.sh` re-run on the kernel of the day before it becomes a plan). **The re-verification rule earned its keep a fifth time, and in the worst way yet**: this plan's own *what is known for certain* table was checked line by line on 2026-09-08 and still missed that `52=` has **two** readers — `dict::FieldType::UtcTimestamp` was in no row of it. Re-verifying the facts a plan lists does not find the fact it never listed. `[amended 2026-09-09]` **The gate is written: [ADR-0057](docs/decisions/ADR-0057-sub-millisecond-time-arrives-beside-the-tick.md), `Proposed`, awaiting the owner.** It decides option (b) — the sub-millisecond remainder travels beside `now_ms`, in one call, from one clock reading — and **D13 is not reversed**. Half B stays not-started until the ADR is `Accepted`; on approval, **wave B's next unit of work is half B (B1–B5), then plan 4, [tls](docs/plans/2026-09-04-tls.md)**, which is still a *Draft* and still owes this row's re-verification. Writing the ADR earned that rule its keep a **fourth** time, in a new way: the re-verification of 2026-09-08 checked every line number and every code fact and **passed a row whose citation was the wrong document** — `TimestampPrecision=SECONDS|MILLIS|MICROS|NANOS` cited to `prior-art.md`, which has never said it. Reading the oracle's source instead found an **int 0–9**, default 3, and that `scripts/interop.sh` can judge this feature **in both directions** — which `369` never could. `[amended 2026-09-08, later]` **Wave B plan 3's receive half is CLOSED and its send half is not started.** Half A shipped the four widths, `52=` and `122=`; half B is behind **ADR-0057**, which nobody has written, so **wave B's next unit of work is either that ADR or plan 4, [tls](docs/plans/2026-09-04-tls.md)** — and `tls` is still a *Draft* that this row's own rule says must be re-verified against the code of the day before it becomes a plan. That rule has now earned its keep **three times running**. `[amended 2026-09-08]` **Wave B plan 3, `timestamp-micros`, is *Chờ duyệt*** — the draft was re-verified against that day's code exactly as the rule below asks, and the re-verification is what earned its keep: **the plan as drafted could not have been built**. It assumed the engine would hand microseconds down to `TimestampCache`; the cache is inside the pure session, fed by `Input::Tick`, which is milliseconds by D13. So the send half is now behind **ADR-0057** and the receive half — which closes a real product gap on its own, a valid `52=` answered with a silent disconnect — goes first, needing no ADR and no §9 machine. **That is the second draft in a row whose *Những gì đã biết chắc* was wrong by the time its turn came**, after the 09-06 draft, and it is the argument for the rule rather than against it. `[decided 2026-09-03]` **B** — `settings-for-both-roles` (initiator keys, `ResetOnLogon/Logout/Disconnect`, `LogonTimeout`/`LogoutTimeout`, per-session validation knobs, `MaxMessageSize`, a registry that sees the `Logon` for 553/554/96, a `35=j` helper on `Reply`), `seq-resync-789-369`, `timestamp-micros`; then TLS (ADR-0018 unblocked it). **C** — `the-second-linux-desk`: item 40, `SO_BUSY_POLL`/`SO_PREFER_BUSY_POLL`/`SO_INCOMING_NAPI_ID`, `w2w --interval` for cold-cache figures, `mlockall` read back from `/proc/self/status`, alloc cases for `FileJournal` and `FileLog`, items 39 and 41. **D** — `const-templates-in-dict` (item 34) and `deterministic-simulation` (fault-injecting harness plus a session fuzz target), both before phase 2 because two encodings pay for them twice. **Then phase 2**, opened by one ADR: the encoding trait, a dictionary chosen at `Logon` by the registry, `docs/internals` — **and the consolidation pass of `DESIGN.md`, deliberately not before.** `[decided 2026-09-04]` The review asked whether the design needs rewriting; it does not: no decision in D1–D13 is reversed, the findings are defaults, evidence and missing features, and each plan amends the section it touches. What the page does need is editorial — 1 258 lines that read as layered `[measured]` paragraphs, so D7 takes four *As built* passes to say what it is today. Doing that now would be redone by waves A–D; the encoding ADR is the one moment the architecture genuinely changes (D2, D3, D9 presuppose tags on the wire), so that is when §4 is restated to hold only the current decision, and the history moves to `measured-costs.md` and the ADRs. **Deliberately not planned**: `io_uring` (needs the NIC-to-NIC number first; ADR-0012 open question 3), kernel bypass (item 14), `SendRedundantResendRequests`, `RefreshOnLogon`, a database store, `DefaultApplVerID` before FIXT. `[2026-09-04]` **Drafts exist for all of B, C and D, at the owner's request, and every one carries the same rule in its header: it is not a plan until its turn comes.** When a wave is reached, the draft's *Những gì đã biết chắc* is re-verified against the code of that day (waves A and B change `journal.rs`, `observe`, `settings`, the resend loop), the plan is revised, and only then does it move to *Chờ duyệt* — a draft is a starting point, not an approval. B: [settings-for-both-roles](docs/plans/2026-09-04-settings-for-both-roles.md), [seq-resync-789-369](docs/plans/2026-09-04-seq-resync-789-369.md), [timestamp-micros](docs/plans/2026-09-04-timestamp-micros.md), then [tls](docs/plans/2026-09-04-tls.md). C: [the-second-linux-desk](docs/plans/2026-09-04-the-second-linux-desk.md). D: [const-templates-in-dict](docs/plans/2026-09-04-const-templates-in-dict.md), [deterministic-simulation](docs/plans/2026-09-04-deterministic-simulation.md). **One fact surfaced while drafting and is not yet a test**: `clock::parse_utc` accepts 17 and 21 bytes only (`crates/session/src/clock.rs:40`), so a counterparty's microsecond `52=` reads as `None` — what the session does next is the first step of the micros draft. **`[amended 2026-09-05]` Two changes, both recorded here rather than in a chat.** (a) **Item 46 moves ahead of wave B and becomes its own plan**, [an-engine-that-can-speak-first](docs/plans/2026-09-05-an-engine-that-can-speak-first.md), *Chờ duyệt* — the draft order put it *inside* wave B beside `settings-for-both-roles`, and that is the wrong way round: an acceptor that cannot originate is a product gap while the rest of wave B is knobs, and `settings-for-both-roles`'s own `35=j` helper is one of the four things item 46 says cannot be sent, so building the helper first delivers it with no door. Wave B is otherwise unchanged and follows. (b) **Wave C grows step 7**, a head-to-head against `matthart1983/nanofix` on the §9 desktop — same harness, same machine, same §9 rows, measured against `standard` because that engine is thread-per-connection with a blocking `recv` (`server.rs:117`, `transport_tcp.rs:23–37`) and pairing it with `hft` is the mode mixing non-negotiable 4 refuses. The result goes to `measured-costs.md` as this repository's own measurement; `prior-art.md`'s row stays a claim and points at it. **`[amended 2026-09-05]` (c) Items 48 and 47 move ahead of wave B, as two plans, in that order** — [a-journal-that-knows-the-numbering](docs/plans/2026-09-05-a-journal-that-knows-the-numbering.md) then [handles-through-the-front-door](docs/plans/2026-09-05-handles-through-the-front-door.md), both *Chờ duyệt*. Same argument as (a): an initiator that cannot survive a clean logout and a front door that cannot be stopped are product gaps, and wave B is knobs. **And 48 does not wait for 47 after all**: item 48's own text said the right number was unreachable without an `Observer`, but an `Observer` is the wrong source — it knows the number only when asked, so a Heartbeat between the last question and the crash still leaves it short. The durable source that is present at `recover` time is the journal, which already records the inbound count per message (ADR-0017) and not the outbound one; the fix is a `mark_out`, and it needs nothing from item 47. `settings-for-both-roles` is re-verified only after both close: its `ResetOn*` keys are about numbering, which 48 makes durable, and its entry points are the ten signatures 47 changes — re-verifying it before would be done twice. **`[2026-09-05]` items 48 and 47 are both closed** (ADR-0053, ADR-0054), so **wave B is next**, opening with `settings-for-both-roles` — whose re-verification was deliberately held until both landed: its `ResetOn*` keys are about numbering, which 48 made durable, and its entry points are the ten signatures 47 changed. **`[2026-09-05]` wave B's first plan is CLOSED**, all seven steps — see *Start here*. Its re-verification found **seven wrong lines**, and **two halves of its own work already delivered** (step 0's `prior-art.md` fix during the first re-verification, step 6's `PRE == RX` assert by ADR-0047), which is the strongest argument yet for the rule this row states: a draft re-verified on the day it is picked up is a different document from the one that was written. **`[2026-09-06]` plan 2 is CLOSED**, all six steps, ADR-0056 — see *Start here*. **Next in wave B**: [timestamp-micros](docs/plans/2026-09-04-timestamp-micros.md), then [tls](docs/plans/2026-09-04-tls.md), each still a draft and each re-verified before it becomes a plan. **`[2026-09-06]` plan 2's re-verification earned the rule its keep a second time**: the draft told a future builder to reuse a function that does not exist, and to reuse the pair `resend_from`/`resend_to` — which track a resend this end **asked for**, not one it **owes**. Following it would have produced a session that answers `789` by going quiet, with every state assertion green. The other two drafts of wave B stay drafts. Order moved to *Where the work is*, 2026-09-18.~~ | nothing; it is the map |
