...
item 14 unchanged. The mirrored corpus (36) stays open and blocks nothing. Plans are written one
wave at a time, on this repository's own finding that a case is only worth publishing once it
has survived the work that came after it.
...
- **The acceptor has never been driven by another implementation.** `[2026-09-03]` Every
  acceptor result on this page — 59 / 59 in process, over a socket, in `standard`, through two
  shards — is this repository's runner reading QuickFIX's definitions. `scripts/interop.sh` runs
  the *initiator* against `libquickfix`; no `libquickfix` initiator has ever logged on to this
  acceptor. Open item 42, [plan](docs/plans/2026-09-03-acceptor-interop.md).
- **A resend that reaches past the journal ring is silent.** `[2026-09-03]` Read off
  `crates/session/src/lib.rs:2097–2130` and `crates/engine/src/journal.rs:40`: eight slots by
  default, gap fill for everything older, no counter. Not a measurement — a reading, and item
  43's plan turns it into a test that is red today.
...
| Plan | Closes |
|---|---|
| **[acceptor-interop](docs/plans/2026-09-03-acceptor-interop.md)** | 42 — **written 2026-09-03, *Chờ duyệt***. `libquickfix` initiator → this engine's acceptor, 7 steps judged on the raw wire, in the same script and CI job as the existing direction. Optional step 5 closes 38 |
| **[resend-from-the-journal](docs/plans/2026-09-03-resend-from-the-journal.md)** | 43 — **written 2026-09-03, *Chờ duyệt***. ADR-0046 to be written first: the ring is the whole resend store, `SLOTS` 8 → 4096, O(1) addressing, replay in batches of `resend_batch` so a resend cannot trip D10, and two events — `ResendBeyondJournal`, `JournalRefused` |
...
| 42 | **The acceptor has no second opinion.** `[2026-09-03]` `scripts/interop.sh` and the `interop` CI job drive this engine's **initiator** into a `libquickfix` acceptor (`tools/interop/src/main.rs:1`). The acceptor — the product this repository is positioned on — has never had another implementation log on to it; its whole evidence is 59 `.def` files read by this repository's own runner, and [a-role-can-be-wrong-in-a-direction-no-gate-runs](docs/reference/a-role-can-be-wrong-in-a-direction-no-gate-runs.md) is the record of what that shape misses. **Plan: [acceptor-interop](docs/plans/2026-09-03-acceptor-interop.md)**, *Chờ duyệt*. Its optional step 5 is item 38 | the honesty of *fastest acceptor* as a claim |
| 43 | **A resend gap-fills every application message older than the journal ring, silently, and a large resend ends the session.** `[2026-09-03]` `Store = MemJournal<8, 512>` (`crates/engine/src/journal.rs:40`); `FileJournal::get` reads its ring only (`journal.rs:440–470`); the resend loop fills over whatever `get` lacks (`crates/session/src/lib.rs:2097–2130`); `Journal::put` refuses over-long messages without a return value (`journal.rs:99`); the replay loop emits everything in one call, so more than `TX` = 8 KiB trips D10's `Disconnect` (`conn.rs:581`); `MemJournal::get` is a linear scan (`journal.rs:112`). Legal on the wire, and a counterparty's fills are gone with nothing on any counter. **Plan: [resend-from-the-journal](docs/plans/2026-09-03-resend-from-the-journal.md)**, *Chờ duyệt*, with ADR-0046 as its step 0 | deployability of the acceptor for order flow |
