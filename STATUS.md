...
- **Tuning a machine to §9 moves every bench median by under 2%.** `[measured 2026-08-30]`
  ...
  with the ceiling at the median. Run `dispatch` alone and a **0.5%** median shift takes
  `ring, one way` from failing 14/15 to failing 1/15 — the ceiling is measuring the governor,
  not the code. A repeatable **323.8 ns** second mode is recorded as **unexplained**, and the
  Zen-2 L3 hypothesis for it was **tested and refuted** — no L3 effect, ~259 ns in all three
  arms. [reference/measured-costs.md](docs/reference/measured-costs.md).
