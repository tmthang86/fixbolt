...
Last updated: **2026-09-02, this session** — **PHASE 1 IS COMPLETE. Exit criterion 6 is met and `[2026-09-02]` **Open item 33 is new, and it is about this repository's documentation rather than its code**: `README.md` has **0 `rust` blocks**, there is **no *Getting Started* heading anywhere** in `README.md` or `docs/`, `engine::settings` accepts **10 keys** of which **`EndDay` and `MaxSkewMillis` are named in no document at all**, no page publishes the conformance results, and `engine`'s **20 modules** have no map. Planned by [the-doc-set](docs/plans/2026-09-02-the-doc-set.md); **phase A is closed**.
...
**[the-doc-set](docs/plans/2026-09-02-the-doc-set.md) — phase A closed, B–E open.** Open item 33.
`[measured 2026-09-02, on `ce14417`]` `README.md` has **0 `rust` blocks**; **no *Getting Started*,
*Installation*, *Usage* or *Quickstart* heading exists** in `README.md` or anywhere under `docs/`;
`engine::settings` accepts **10 keys** and **`EndDay` and `MaxSkewMillis` appear in no document at
all**; there is **no `Setting / Valid values / Default` table**; **no page publishes the conformance
results**, though every number is already under **Proven** here; and `engine`'s **20 modules** have no
map, against `DESIGN.md`, which records what was decided rather than how the code works today.
**Three `hft` entry points — `serve_hft`, `serve_hft_with_recovery`, `serve_sharded_hft` — are still
called by no test, no bench and no tool**, so the mode this project makes its claim about has no gate
running through its own front doors; `serve` and `serve_with_recovery` gained one with `library`.
It is the first plan here whose gates are all `scripts/check-links.py`, `cargo doc` and reading —
**no benchmark, and no new figure**.
...
| **[the-doc-set](docs/plans/2026-09-02-the-doc-set.md)** | 33 — **approved 2026-09-02**, five phases. **PHASE A CLOSED 2026-09-02**: item 33 registered, `README.md:23` corrected, `GUIDE.md:331`'s fence closed, and a **Where to start** section written — the bootstrap was not recorded anywhere a newcomer would look. CI green on the commit closed, `37cb3c5`, run [`33608929249`](https://github.com/tmthang86/fixbolt/actions/runs/33608929249), **9 jobs of 9**. B–D not started; **C and D do not wait on `library`**, only E does. **Written twice**: the first draft was measured against `73e48c6`, and 73 commits had landed by the time it was written down — `GUIDE.md` had already grown 501 → 1054 lines and absorbed operations, so `OPERATIONS.md` was dropped before it was written. Sửa 1 in the plan records it |
