...
* **Phase-2 PR A closes on its own rule — the A-desk band ADR-0082 left empty is measured.**
  ...
  now on the record rather than absent from it: **the engine turn is 4.1–5.1% slower** at every
  session count and ring size, `parse`/`serialize` move **≤ 0.2%** (what ADR-0079 decision 5 asks
  for), `validate` is **0.5–5.9% faster**. In band is not free; it is in band. `DESIGN.md` D16
...
**Eight recorded baselines are breached on `main` on the §9 desk** — open item **97**, and `scripts/bench.sh --strict` would be red there. This is not an
...
Branch `plan/boot-d-numbers`, PR [#90](https://github.com/tmthang86/fixbolt/pull/90); the closing
commit's CI run id is named in the delivery note at the end of this entry. Evidence is
...
* ~~**A-desk**, PR A's ADR-0031 band — still `**NOT RUN**`, unchanged by this PR.~~ — **RUN 2026-09-22**, boot D step D4: **22 cases of 22 in band**, n = 20 per arm; the engine turn is 4.1–5.1% slower and `parse`/`serialize` move ≤ 0.2%. PR A closes on its own rule.
