...
* **`is_admin` was never built.** The plan's traps table expects B1 to generate it from `msgcat`
  with `is_admin(b"n")` true. Measured: 0 occurrences in the generated table, 0 reads of `msgcat`
  in `build.rs`, and `ADMIN` is a hand-written 7-entry const at `crates/session/src/lib.rs:291`
  **lacking `b"n"`**. Whether that is B1 debt or was dropped silently is unresolved, and is
