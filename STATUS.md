...
| 80 | **`doc_table`'s call-site leg checks which reader a key reaches, not what value reaches it.** `[measured 2026-09-13]` by the senior review of PR [#69](https://github.com/tmthang86/fixbolt/pull/69): `&& (v.1 == "yes" \|\| flag(v, Key::ResetOnLogon)?)` — the parser accepting an extra literal inline — leaves `the_reader_table_matches_the_call_sites` **ok**; a wrapped value (`flag(normalise(v), Key::X)`) is the same hole. Not patched with another pattern, per ADR-0061's conclusion; stated in the test's doc. Closing it means reading behaviour, not source — the neighbourhood of item 74's candidate-set question | item 77 |
| 81 | **`ReconnectInterval=0` is refused and `docs/CONFIGURATION.md` §1 does not say so.** `[verified 2026-09-13]` `reconnect::Policy::new` → `FirstIsZero` → `Problem::ImpossiblePolicy`; the cell reads `integer, **seconds**`. The cell says less than the parser, item 76 in reverse. Writing `positive` into it today would make probe 7 go red *falsely*, because its `positive` branch requires a refusal `is_about_the_value` and that helper (shared with probe 3) does not list `ImpossiblePolicy` — so the fix is the cell and that predicate together | item 76, probe 7 |
| 82 | **`scripts/check-links.py` does not report an own-repository absolute URL to a path that does not exist** (`github.com/tmthang86/fixbolt/blob/main/DESIGN.md`). The tail search only matches files that exist. Pre-existing; found while proving rule (c) in PR #69 | item 72 |
| 83 | **`fixbolt-session` rustdoc warns that public docs link to private items** — `parse_utc` → `LEN_SECONDS`, `LEN_MAX` (`crates/session/src/clock.rs:57-58`). A warning, so no gate fails; seen in every `feature-sets` doc run | item 79 |
