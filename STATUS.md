...
Branch `plan/boot-d-numbers`, PR [#90](https://github.com/tmthang86/fixbolt/pull/90). Evidence is
...
* Do not read the `--summary` rows whose case name ends in `:` as measurements. They are the
  bench's own band-warning lines, which `scripts/ab-rotation.sh` parses as phantom cases —
  a defect in the driver, not data.
