...
* **Item 99** — named: the `journal` bench's heap layout, set by the length of `benches/baselines.tsv`
  ...
  max/min 1.12 — not flat by the plan's 1.10 bound**; the 3× step is gone and the line was not moved.
...
* Do not move `journal put, 191 bytes, one slot` on the strength of the 1.12 sweep: it reads 7.4
  against 7.4, and the residue is item 101's question.
...
| 99 | ~~**`journal put, 191 bytes, one slot` read 7.4 ns in one `--strict` run and 12.4 ns twenty-seven minutes later in the same boot, same pinned binary, and stayed at 12.4 on four re-runs.**~~ — **CLOSED 2026-09-23, boot F, cause named: the bench's heap layout, set by the length of `benches/baselines.tsv`** — the second instance of the ADR-0067 class, at run time. Old binary (F1, before any `perf record`, `target/boot-f-evidence/f1-table.txt`): **12.4** on the 24 897-byte file, **7.4** on the 24 226-byte `9374820` file, 12.4 again after restore; a 16-byte-step sweep `k = 0…1024` read **6.3 … 19.0 ns**, `walking` flat at 8.7 … 8.9. F1c (`f7.txt`): after an S2-shaped `perf record -g`, still 12.4 — **`perf` is not a cause**. Fix `6b2833b`: `load_baselines`/`cpu_model` read into a fixed `String::with_capacity(1 << 20)` (ADR-0096 decision 2, revised in place: a fixed-size brk block, not `mmap`), test `a_loaded_file_reads_into_a_fixed_one_mib_buffer`. The sweep on the new binary (`f8.txt`) reads **7.4 … 8.3 — max/min 1.12, NOT flat by the plan's own 1.10 bound**, two single points (8.2, 8.3) over the 8.1 ceiling; the ×1.68 step is gone. The `one slot` line was **not** re-recorded (7.4 vs 7.4 in all three boot F `--strict` runs). The residual 1.12 is in *Not proven*; the fix moved another case — item 101. [measured-costs](docs/reference/measured-costs.md) *Boot F, item 99*; [recording-a-baseline-changed-the-baseline](docs/reference/recording-a-baseline-changed-the-baseline.md) *The second time, at run time*. **History:** `[measured 2026-09-23, boot E]` `target/boot-e-evidence/s3-strict.txt` (21:07 UTC) 7.4, in band; `s4-strict.txt` (21:44 UTC) 12.4 `OVER BASELINE` (band [6.7, 8.1]); then three bare runs of the `main-tree` `journal` binary from `../fb-s9e/MANIFEST.txt` and one under `taskset -c 6`, all 12.4. The two `walking` cases did **not** move (8.8 / 5.3 ns), `check-machine.sh` still `pass 17 fail 0 unknown 0`, THP `never`. Between the two runs the desk ran S2 (`perf record -g` ×5 of the `validate` bench) and S1c (24 rounds of `sbe`). The case is pure L1 (one slot, eight lines) so the suspects are CPU-side state a `perf record` could leave (PMU / NMI-watchdog counter re-armed), not the code. **The baseline line is not moved.** First check next boot: run the journal binary **before** any `perf record`, then after one, and read the case both times. | ADR-0031; `bench.sh --strict` on the desk |
