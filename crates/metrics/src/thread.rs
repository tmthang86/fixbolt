...
/// The caller's handler for events, run on the exporter thread.
pub(crate) type OnEvent = Box<dyn FnMut(&Event) + Send + 'static>;
...
impl Loop {
    ...
    fn answer(&mut self, mut s: TcpStream) {
        let timeout = Some(self.timings.read_timeout);
        ...
        if s.set_nonblocking(false).is_err()
            || s.set_read_timeout(timeout).is_err()
            || s.set_write_timeout(timeout).is_err()
        {
            self.bad_requests += 1;
            return;
        }
        let n = match http::read_request(&mut s, &mut self.request) {
            Received::Complete(n) => n,
            Received::TooLarge => {
                self.refuse(&mut s, Status::BadRequest);
                return;
            }
            Received::Gone => {
                self.bad_requests += 1;
        ...
        match request {
            Request::Serve {
                ...
                let result = match encode::encode(&mut self.body, &self.engines, exporter) {
                    Ok(()) => http::respond(&mut s, Status::Ok, true, &self.body, head),
                    Err(encode::Overflow) => {
                        let st = Status::InternalError;
                        http::respond(&mut s, st, false, st.body(), head)
                    }
                };
                drop(result);
            }
            Request::Serve {
                ...
                drop(http::respond(&mut s, st, false, st.body(), head));
            }
            Request::NotFound => self.refuse(&mut s, Status::NotFound),
            Request::MethodNotAllowed => self.refuse(&mut s, Status::MethodNotAllowed),
            Request::Bad => self.refuse(&mut s, Status::BadRequest),
        }
    ...
    fn refuse(&mut self, s: &mut TcpStream, st: Status) {
        self.bad_requests += 1;
        drop(http::respond(s, st, false, st.body(), false));
        drop(s.shutdown(Shutdown::Write));
