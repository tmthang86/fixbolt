...
//!
//! **This file is the fast path's safety net, not a specification.**
//! `clock.rs::parse_utc` has a straight-line branch for the two widths the wire
//! actually carries — 17 and 21 bytes — and leaves ADR-0058's rule in charge of
//! every other width. A fast path that is fast and *wrong* would have bought
//! nothing at all, let alone the 33 ns measured for `STATUS.md` item 93's
//! bisect segment (3), so the question this file asks is not "is the reader
//! correct" — `timestamp_widths.rs` and `scripts/interop.sh` §4i ask that — but
//! **"does it still answer what the one rule alone answered, byte for byte,
//! value for value"**. It is the reversal named in `clock.rs`'s rustdoc: delete
//! the 21-byte arm and this file must stay green, change what it computes and
//! `the_reader_answers_what_the_one_rule_reader_answered` goes red.
//!
