...
| `LogonTimeout` | How long a connection may sit without completing its `Logon`. **The initiator's** — an acceptor has `Limits.logon_ms` in front of it. Also how long an acceptor holds a connection **parked** because its `Recovery::ready` said *not yet* (a journal file still being written, ADR-0154); `0` there falls back to `Limits.logon_ms` | integer, **seconds**; `0` is off | `0` |
...
| `SLOT_LEN` | Largest message the journal ring can keep. **A message longer than 65 535 bytes is never kept**, whatever this is set to: a slot records its length as a `u16`, and `put` refuses it (counted as `JournalRefused`) — [ADR-0150](decisions/ADR-0150-the-journal-writer-holds-the-largest-record-the-slot-allows-and-stops-only-on-a-record-no-message-can-be.md) decision 3 | bytes | `512` | `MemJournal<SLOTS, SLOT_LEN>` / `Store` | [`journal.rs:56`](../crates/engine/src/journal.rs#L56) |
...
| `logon_timeout_ms` | `[added 2026-09-05]` How long a connection may sit without completing its `Logon`. **The initiator's** — an acceptor has `Limits.logon_ms` in front of it, before a `Session` exists. `[2026-09-24]` Also bounds a connection **parked** because `Recovery::ready` said *not yet*: the acceptor's serving loops drop it after this long (after `Limits.logon_ms` when this is zero), and `connect_and_serve*` counts it from the dial, as the handshake ([ADR-0154](decisions/ADR-0154-a-journal-file-has-one-appender-a-reconnect-waits-for-it-by-parking-and-what-the-file-missed-is-counted.md) decision 3) | milliseconds; **zero is off** | `0` | `Config::with_logon_timeout_ms` | [`session/src/lib.rs`](../crates/session/src/lib.rs), [SESSION-BEHAVIOUR §1](SESSION-BEHAVIOUR.md) |
...
| `read_timeout` | `[added 2026-09-24]` How long a scraping client has, **from the moment it is accepted**, to send its request and take the answer before the exporter drops it — one deadline per connection, not a timeout per read, so a client trickling bytes is dropped on time (`a_client_that_trickles_bytes_is_dropped_at_the_deadline`); below `MIN_DURATION` it is raised, not rejected | `Duration`, floor `MIN_DURATION` (1 ms) | `1 s` | `.read_timeout(d)` | [`crates/metrics/src/lib.rs:83`](../crates/metrics/src/lib.rs#L83) |
...
| Parameter | Meaning | Values | Default | Where set | Source |
|---|---|---|---|---|---|
| `UringConfig::new(buffers, buffer_len, connections)` | The provided-buffer ring's shape and how many connections may share it | `buffers`: power of two, `1..=32768` (`MAX_BUFFERS`); `buffer_len`: bytes, `> 0`; `connections`: `> 0`, and at least the engine's capacity plus `presession::Limits::pending()` (checked by the entry points, `UringRefused::TooSmall` otherwise) | **none** — refused by `UringConfigError` naming the first field out of range | `UringConfig::new`, passed to `serve_uring` / `serve_hft_uring` | [`transport/uring.rs`](../crates/engine/src/transport/uring.rs) |
...
checks each crate independently. **`fix50sp2` gates tests, which needs its own CI invocation** —
a feature-gated test compiles to nothing under `cargo test --all`, so the `gates` job runs the
...
`DESIGN.md`](DESIGN.md) has the full split by mode. The seven `SocketUseSSL`-family settings keys
