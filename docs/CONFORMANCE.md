...
### The mirrored corpus — the initiator side of the same 59
...
corpus's script side to answer where it stays silent. Score history: 0 → 2 → 10 (2026-09-02, two
session defects) → 10 / 14 at `a2aa9ce` → 14 / 14 at `09effd1`, the last step a harness hole
...
with no line of `crates/session/src` changed. Command: `cargo test -p fixbolt-conformance`.
