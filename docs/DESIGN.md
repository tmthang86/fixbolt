...
`[measured 2026-08-30]` moving the tick left the wire gate at 59 / 59 on the machine it was
measured on — see the gate's own row in §6 for what that figure does and does not reproduce.
...
| Session conformance, acceptor, **through a real socket** | **59 / 59, on every machine** | `cargo test -p nanofix-engine --test wire`. The same files over TCP: kernel sockets, the real framer, the real session, the real application. The only injected part is the clock, because every `I` line in the corpus carries a fixed instant. `[measured 2026-08-30]` **59 / 59** on the M5 and **39 / 59** on Linux — the harness settles by counting spins, so the score follows the bound rather than the protocol. **Not met until it is reproducible**: `STATUS.md` open item 17 |
