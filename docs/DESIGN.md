...
| `library` | L4 | The application-facing API, package **`fixbolt`**: `Handler`, `Incoming`, `Reply`, `App`, and a curated re-export of what an application needs (`serve`, `Config`, `Table`, `Limits`, `Settings`, `Observer`, `Admin`, `Recovery`, `FileJournal`, `FileLog`, …). `Engine`, `Dispatch`, `Transport`, `wait`, `shard`, `affinity`, `frame` and `ring` are deliberately absent; reaching for one means naming `fixbolt-engine` yourself | `engine` |
...
| The reconnect loop, against a real `libquickfix` that dies and comes back | **5 / 5** after a `SIGKILL`: nobody said goodbye · it came back unprompted · **at one past the number it last sent** · the venue's next messages were *delivered to the application*, not gap-filled · and no `35=2`, `141=Y` or `MsgSeqNum too low` anywhere. **3 / 3** after a `SIGTERM`: the goodbye was answered · it redialled anyway (ADR-0043 decision 5, confirmed by an engine that never heard of it) · and was refused by **exactly one**, which is the known gap item 48 names | `scripts/interop.sh` §4d/4e and the blocking `interop` CI job. `STATUS.md` item 38 — until 2026-09-05 every test of `connect_and_serve` was this repository's own reading, which ADR-0043's own *Consequences* said out loud. The counterparty's `FileStore` is what remembers the numbering, and its three `ResetOn*` are `N`: under the `Y` the other two directions use, both ends restart at 1 and a broken engine passes. `[measured 2026-09-05]` the first run refused the resumed session and named the cause in English ([a-journal-holds-messages-not-numbering](reference/a-journal-holds-messages-not-numbering.md)), and the assertion that read *"nothing arrived"* about a message that had arrived is its own lesson ([a-refused-message-never-reaches-the-callback](reference/a-refused-message-never-reaches-the-callback.md)) |
