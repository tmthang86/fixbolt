...
| `dict` | build | Code generation from the FIX XML: tag constants, message shapes, required-field tables, **field ordering**, group delimiters and members, and the validation tables (defined tags, message types, per-message tag sets, field types, enum values). The three QuickFIX XML files it generates from ship **inside the crate**, at `spec/`, byte-identical to a pinned commit, under `NOTICE` — the sole exception to "no QuickFIX source is copied" (ADR-0104), made so this crate, and everything built on it, needs nothing but crates.io. `tables`: `Tables`, the nine functions the session calls, implemented for `Fix44`; the alias `Fix44TagValue` (D16). `[2026-09-19]` behind the off-by-default `fix50sp2` feature it emits a **second table**, `Fixt11Fix50Sp2Tables`, built from two XML files at once — transport from `FIXT11.xml`, application from `FIX50SP2.xml` (ADR-0080). The eighth function, `is_defined_tag_for`, exists because an admin message's body is checked against the transport file alone (ADR-0084). `[2026-09-20]` the ninth, `is_admin(msg_type)`, is generated from each `<message>`'s `msgcat`, no default method, and answers ADR-0080 decision 3's `1128` exemption rather than a hand-written list beside the call site (ADR-0086 decision 2). `[2026-09-26]` the generator is a library in `src/codegen/`, loaded twice ([ADR-0207](decisions/ADR-0207-a-custom-dictionary-is-an-overlay-generated-in-the-users-build-into-the-users-own-type.md) decision 1, `sbe-gen`'s shape): `build.rs` includes it by `#[path]` to write the tables above — byte-identical to what `build.rs` wrote when it held the generator itself, pinned by their SHA-256 in `tests/generated_is_pinned.rs` — and the off-by-default `codegen` feature declares `pub mod codegen` for a user's own `build.rs`. Lint-clean like any `src/`: a refusal is a `GenError`, never a `panic` or an exit | `codec`; it implements `codec::Dictionary`. `roxmltree` is a build-dependency always and a normal dependency only under `codegen` |
...
| `transport::uring` | `[2026-09-24]` `Uring`, `UringConfig`, `UringTransport`, `UringSpin` (`hft`), `UringBlock` (`standard`), `HftArm`, `UringRefused`, `UringReport` — a second `Transport` reaped by the idle strategy, behind `#[cfg(all(feature = "io-uring", target_os = "linux"))]` | D5, [ADR-0190](decisions/ADR-0190-the-io-uring-transport-is-reaped-by-the-idle-strategy-and-an-hft-turn-enters-the-kernel-once-without-waiting.md), [ADR-0191](decisions/ADR-0191-the-hft-sleeper-list-reads-io-uring-enter-by-its-min-complete.md) |
...
carries `#[inline]`, and `crates/codec/src/lib.rs` carries
`const _: () = assert!(core::mem::size_of::<MessageView<'static, 64>>() == 24);` so that growing
it fails to compile rather than silently costing a spill.
...
**As built, a second transport: `io_uring` (Linux, kernel ≥ 6.1) `[2026-09-24]`.**
`transport::uring`, behind `#[cfg(all(feature = "io-uring", target_os = "linux"))]`, follows
both rules above: the feature gates the `mod uring` declaration itself, and the crate's `build.rs`
is untouched — the dependency (`io-uring`, pinned `>= 0.7.15`) is pure Rust, so nothing runs an
external toolchain. `UringTransport` receives from a multishot `recv` into a provided buffer
ring that an idle strategy (`UringSpin` for `hft`, `UringBlock` for `standard`) reaps; `recv`
itself makes no system call. Blocked — a seccomp filter (Docker ≥ 25 / containerd
`RuntimeDefault`), a sysctl, or a kernel without `CONFIG_IO_URING` — is refused at startup with
a named `UringRefused` cause, **never a silent fallback to `read(2)`**, the same rule ADR-0060
already applies to a deployment that requires the kernel TLS path
...
decision 7). Full mechanism and cost: ADR-0190; the gate: §6 below.
...
**As built, the `io_uring` arms `[2026-09-24]`.** `UringSpin` (`hft`) and `UringBlock`
(`standard`) are the idle strategy **and** the reaper for `transport::uring::UringTransport`
(D5, ADR-0190): reaping happens in `idle` because `idle` is already the one call the engine
makes exactly when a turn found nothing to do, so the next turn's `recv` calls find staged bytes
without a system call. `UringSpin::idle` is one `io_uring_enter(to_submit, min_complete = 0,
GETEVENTS)` — the engine thread enters the kernel to run the ring's deferred completion work
(`IORING_SETUP_DEFER_TASKRUN`) and **never waits there** — which is what keeps it inside rule
4's `hft` half even though it is, syscall for syscall, an entry into the kernel every idle turn
(ADR-0191 teaches the gate to read that syscall's third argument rather than its name).
`UringBlock::idle` is the `standard` equivalent of `Block`'s `poll(2)` wait: one
`io_uring_enter(…, min_complete = 1, GETEVENTS | EXT_ARG, timeout)`, arming a one-shot `POLL_ADD`
per extra source (the listener, the waker pipe) before the wait and cancelling whichever did not
fire right after it, with the cancels' own completions reaped in the same turn that queued them
— getting that submit-and-reap ordering wrong is what
[the trap](reference/an-io-uring-cancel-completion-satisfies-the-next-wait.md) records (ADR-0190
Revision 2). **SQPOLL (`HftArm::Sqpoll`) exists only as an `hft` arm, pinned by a `CorePin`, and
is never a default** (the owner's Q8): it burns a second core for the kernel's own submission-
queue polling thread, is not reachable from `standard` by the type system (`UringBlock`'s ring
configuration carries no SQPOLL field), and every figure taken from it says so beside the
number. Row 7 (out of this plan's scope) is what decides whether `io_uring` is kept at all.
...
| The engine thread never sleeps in the kernel (`hft`), **`io_uring` arm** `[2026-09-24]` | `hft` over the ring enters the kernel only with `io_uring_enter(…, min_complete = 0, GETEVENTS)`; `standard` over the ring must trip the same check | Both scripts above, phase 4 row 5 ([ADR-0191](decisions/ADR-0191-the-hft-sleeper-list-reads-io-uring-enter-by-its-min-complete.md)). **`io_uring_enter` is judged by its third argument, not its name**: the strace check names it `io_uring_enter_nowait` (`min_complete` 0), `io_uring_enter_wait` (anything else) or `io_uring_enter_unparsed` (unreadable), and the last two are sleepers — a trace it cannot read fails. Runs: `--mode hft --transport uring` must pass with `io_uring_enter_nowait` > 0 and a `transport: uring … cqes=` line above zero; `--mode standard --transport uring` must trip it with `io_uring_enter_wait`; the ctxt check asserts `hft` + uring 0 voluntary switches and requires `standard` + uring to go red. The SQPOLL arm runs only where `FIXBOLT_SQPOLL_CORE` names a core (`FIXBOLT_SQPOLL_ALLOW_UNISOLATED=1` off the §9 line, printed as *not a §9 run*), otherwise *SKIPPED, NOT PASSED*; a build without `--features io-uring` is *SKIPPED, NOT PASSED*, exit 2. `[measured 2026-09-24]` the `standard` red line's count is **read, not asserted**: 7 385 `_wait` against ~345 kernel `poll` was a spin ([the trap](reference/an-io-uring-cancel-completion-satisfies-the-next-wait.md)); 356 after the fix |
...
| A `standard` engine gives the core back, **`io_uring` arm** `[2026-09-24]` | the same four assertions on `standard --transport uring`, plus `transport: uring arm=block … cqes=` above zero and `unarmed=0` | `scripts/check-standard-gives-the-core-back.sh`'s io_uring block, phase 4 row 5 ([ADR-0190](decisions/ADR-0190-the-io-uring-transport-is-reaped-by-the-idle-strategy-and-an-hft-turn-enters-the-kernel-once-without-waiting.md) decision 5): `unarmed` > 0 is a source woken only by the timeout, so it fails the arm. **Red halves**: `hft` + uring must trip it, and the SQPOLL arm must trip it where `FIXBOLT_SQPOLL_CORE` is set (else *SKIPPED, NOT PASSED*) |
| An `io_uring` engine refuses to start when `io_uring` is blocked `[2026-09-24]` | a blocked `io_uring_setup` ends startup with a named cause, before a socket is bound; never a fallback to `read(2)` | Seccomp half in CI: `crates/engine/tests/uring.rs` installs a real filter answering `EPERM` on the test's own thread (Docker ≥ 25's mechanism) and requires `UringRefused::Blocked`, and that `serve_hft_uring` binds no socket. Sysctl half **on the desk only**: `scripts/check-uring-refused-under-sysctl.sh` sets `kernel.io_uring_disabled=2` with `sudo -n`, requires `tools/w2w --transport uring` to exit non-zero with `UringRefused::Disabled`'s sentence, and always restores and re-reads the old value; without `sudo -n`, the sysctl, or an `io-uring` build it is *SKIPPED, NOT PASSED*, exit 2. Not run in CI. Cannot see value 1 (`kernel.io_uring_group`), covered only by the pure classification table |
...
| Every `codegen`-gated test the build lists actually ran `[2026-09-26]` | red when a test the `--features codegen` or `--features codegen,fix50sp2` build of `fixbolt-dict` lists cannot be accounted for in the job's own log | `scripts/check-feature-gated-tests-ran.sh`, in the `gates` job's `codegen` step, which also lints both sets. `[measured 2026-09-26]` on Linux: `53 listed, 53 accounted for` and `68 listed, 68 accounted for`. Reversal: the same check pointed at a log from a run without the feature reads `FAIL R1 — generated_fix44_equals_the_build_output is in the fixbolt-dict --features codegen build … and the log records it running 0×` |
| A check script that needs bash 4 refuses to run under bash 3.2 `[2026-09-26]` | every `scripts/check-*.sh` using `declare -A`, `mapfile` or `readarray` carries the guard, and exits 2 with its sentence when run under a real bash 3.2 | `scripts/check-old-bash-is-refused.sh`, in the `lint-config` job, running each script in the `bash:3.2` image. `[measured 2026-09-26]` before the guard, `check-dict-spec-pin.sh` and `check-sudo-names-what-root-can-find.sh` exited 0 with `ok` having checked nothing ([a-check-script-under-bash-3-2-can-print-ok-having-checked-nothing](reference/a-check-script-under-bash-3-2-can-print-ok-having-checked-nothing.md)). Reversals: the guard deleted reads `has no bash 4 guard`; moved below the first `mapfile`, the script exits on something else first and reads `exit 2, expected 2 and "needs bash 4 or newer"` |
...
| An optional dependency is really optional | absent from the crate's graph with no features on, and the crate still builds and tests that way | `scripts/check-no-optional-deps.sh`, per crate. Reversal: removing `optional = true` from `libc` turns it red with the graph printed. `[measured 2026-09-26]` case `fixbolt-dict:roxmltree` (optional under `codegen`, a build-dependency always): making it a plain normal dependency reads `FAIL: roxmltree is in fixbolt-dict's normal dependency graph with no features on`, with the graph printed |
...
| What a `.crate` actually contains, for each of the six | `fixbolt-dict` ships `NOTICE` + its three `spec/*.xml` + every tracked file under `src/codegen/` (read from `git ls-files`, so a new generator file is required without editing the script); every crate ships `README.md`, `LICENSE-MIT`, `LICENSE-APACHE`; none ships `tests/`, `benches/`, `vendor` or a `.def` fixture | `scripts/check-package-contents.sh`, reading `cargo package --list -p <crate>` per crate — packages nothing to disk. `[measured 2026-09-23]` reversal: dropping `NOTICE` from `fixbolt-dict`'s `include` reads `FAIL fixbolt-dict: NOTICE missing from package`; adding `tests/**` to `fixbolt-codec`'s reads `FAIL fixbolt-codec: tests/ shipped (tests/<name>.rs)` once per file. `[measured 2026-09-26]` narrowing `fixbolt-dict`'s `include` so `src/codegen/merge.rs` is left out reads `FAIL fixbolt-dict: src/codegen/merge.rs missing from package` |
| The packaged sources build, on the pinned toolchain and on the declared `rust-version`, over the feature sets a user can pick | **15 cases**: 11 on the pinned default toolchain (one feature at a time beside each crate's default, plus `fixbolt-codec` alone, plus each of `fixbolt`/`fixbolt-engine`/`fixbolt-sbe` with `default-features = false`, plus `fixbolt-dict` with `codegen` alone and beside `fix50sp2`), 4 on `+1.89.0` (the combined-everything build for `fixbolt`, `fixbolt-engine` and `fixbolt-dict`, plus `fixbolt --no-default-features`) — declared `rust-version` is now `1.89` (ADR-0154 decision 1, `File::try_lock`), superseding the `1.88` this row measured against | `scripts/check-packaged-build.sh`: a scratch crate per case, outside the workspace (its own `[workspace]`), depending on the crate under test through an ordinary version requirement that `[patch.crates-io]` redirects to `target/package/<name>-<version>/` — the directory the dry run above leaves behind, byte-identical to a `.crate` upload. `[measured 2026-09-23]` all 12 `Finished` on the desk, `cargo` 1.98.0 + `1.88.0` (the then-declared MSRV). Reversal: reverting the declared `rust-version` to `1.85` while the source still uses `1.88`-only syntax (post 6a', let chains) and building on `+1.88.0`'s sibling `+1.85.0` reads `error[E0658]: 'let' expressions in this position are unstable` in `fixbolt-codec`'s `src/template.rs` — the exact trap [publishing-a-workspace-to-crates-io](reference/publishing-a-workspace-to-crates-io.md) trap 3 found by hand. `[measured 2026-09-26]` on Linux (container, aarch64): all 15 `Finished`; reversal: a `#[cfg(feature = "codegen")] compile_error!` in `fixbolt-dict`'s `lib.rs` turns exactly the three `codegen` cases red and leaves the other twelve green |
