...
| Allocations on the hot path — engine | **0**, counted separately on **twenty** paths: idle, send, recv, frame, turn, shard-turn, busy, ring, interests, pending-idle, pending-busy, pending-cycle, registry-lookup, observe-idle, observe-asked, events-idle, events-busy, admin-idle, admin-busy, shutdown | `crates/engine/benches/alloc.rs`, counting allocator. `busy` is a whole turn carrying a message in and a reply out, and it asserts the session is still logged on at the end of the count — `[cost]` an earlier version measured a connection that had been dropped at message two and reported the test double's queue growth as the engine's |
...
| Session conformance, initiator, mirrored corpus | **2 / 50** `[measured 2026-09-02]`, ceiling **45**, **and the harness's own drive count asserted beside it** | `cargo test -p fixbolt-session --test mirror`. **The secondary gate, and it cannot check its own reading** — mirroring is this project's interpretation of a suite written for the other direction, and its ceiling moved 51 → 50 → 45 across two readings. It asserted `passed == 0` for three days, and a gate pinned at a constant reports nothing about the code under it: an initiator that answered a `Logon` with a `Logon` went through it. It now reads 2 and **can fall**. The score is asserted **by file name**, and next to it `Report::driven` — every time the harness played the operator, by `MsgType`, as exact numbers, because a score a harness can raise by driving harder is not a score. Reversals: `make_receivable` neutered takes it to 0 / 50 and the drives from 179 to 1; the origination removed takes it to 1; and letting the **acceptor** corpus be driven makes `tests/score.rs` panic, which is the guard that keeps a harness from making a broken session look correct |
