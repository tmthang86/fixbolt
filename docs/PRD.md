...
| Operator visibility — session state, sequence numbers, counters | Session state via the `Application` callbacks and a log backend `[documented]` | **`Engine::observer()` → a `Snapshot`, on request, readable from another thread.** `[2026-09-01]` per session: logged-on state, `next_out`, `next_in`, whether output is backed up, and **the measured clock skew** ([ADR-0032](decisions/ADR-0032-observation-is-a-snapshot-taken-on-request.md)). `Snapshot::healthy()` is the health probe. **Still missing:** ring depth, pending-set occupancy, ordered shutdown, live sequence-number administration, and an event stream carrying the *reason* a connection ended — `STATUS.md` item 30 (a), (c), (d), (e) | P1, narrowed |
