...
| Session schedules — start/end time, weekday | Yes `[documented]` | **`Schedule` in `session`, done 2026-09-02** — daily, weekly, weekday filter, a window that crosses midnight, and the sequence-number reset decided by `same_session` ([ADR-0033](decisions/ADR-0033-a-schedule-is-utc-arithmetic-and-the-calendar-stays-outside.md)). **Two things are deliberately not here:** timezone names, because an IANA database cannot live in a pure layer — the caller resolves the offset and rebuilds; and **persistence of the instant itself** — `[2026-09-02]` `serve_with_recovery` asks a `Recovery` per counterparty and carries the numbers, the journal and `last_active_ms` ([ADR-0034](decisions/ADR-0034-recovery-is-asked-once-the-counterparty-is-known.md)), but nothing saves `Session::last_active_ms()` for you, and the sharded runtime has no variant (`STATUS.md` item 32) | P1, narrowed |
...
| Operator visibility — session state, sequence numbers, counters | Session state via the `Application` callbacks and a log backend `[documented]` | **`Engine::observer()` → a `Snapshot`, on request, readable from another thread.** `[2026-09-01]` per session: logged-on state, `next_out`, `next_in`, whether output is backed up, and **the measured clock skew** ([ADR-0032](decisions/ADR-0032-observation-is-a-snapshot-taken-on-request.md)). `Snapshot::healthy()` is the health probe. `[2026-09-02]` `Observer::events()` adds the endings the snapshot cannot hold — `EventKind::Ended(DropReason)`, **pushed** when a session's state changes, with losses counted rather than swallowed ([ADR-0035](decisions/ADR-0035-an-event-is-pushed-and-a-loss-is-counted.md)). On the wire a bad clock and a shut venue are both silence; this is what tells them apart. `[2026-09-02]` `Engine::admin()` is the second handle over that same mechanism — `Observer` looks, `Admin` changes — and carries the 3 a.m. operation: `SetNextOut`, `SetNextIn`, `SendSequenceReset`, applied before a turn numbers anything ([ADR-0036](decisions/ADR-0036-one-mechanism-two-capabilities.md)). `[2026-09-02]` `Admin::shutdown` stops the engine without lying to the counterparty, and `Shutdown` says whether anyone failed to answer ([ADR-0038](decisions/ADR-0038-an-ordered-shutdown-is-a-state-not-a-flag.md)). `[2026-09-02]` `tools/jrnl` and `journal::Reader` answer the other question — *"we sent order X, did you receive it?"* — from **outside the process**, over the whole file rather than the recovery ring ([ADR-0037](decisions/ADR-0037-reading-a-journal-is-not-recovering-from-one.md)). **Still missing:** ring depth and pending-set occupancy — item 30 is otherwise **closed** | P1, mostly done |
...
- **Record retention, immutability, tamper evidence and search.** `[2026-09-01]`
  [ADR-0027](decisions/ADR-0027-the-engine-owes-a-byte-stream-not-an-archive.md): this engine
  ...
  archive; every one of those is a storage-system property, and an engine that grew them would be
  a worse engine and a worse archive. **Drop copy is a deployment topology**, not an engine
  feature — once the registry exists it is an ordinary entry in it.
...
| 8 | ~~**Where does the counterparty registry live — `presession`, `engine`, or `library`?** `[2026-09-01]` `presession` already reads the identity off the `Logon` and is the only layer that sees a socket before a session exists, which argues for it; but a registry owns a `Config`, a journal and a credential per counterparty, which is `library`-shaped. **The choice decides whether a counterparty can be added without a restart**, so it is not a placement detail. First step of the plan that closes open item 28, and it wants an ADR before any code.~~ **ANSWERED 2026-09-01 by [ADR-0026](decisions/ADR-0026-a-counterparty-registry-in-the-pre-session-stage.md): `presession`, and as a *trait*, not a table.** `[documented]` all three engines surveyed — QuickFIX, QuickFIX/J dynamic acceptors, Artio — decide at the `Logon` in the accepting stage and do it through a **provider or callback** rather than a fixed map; ADR-0020 put this project there already, from a conformance failure rather than an operational need. Authentication is `lookup` returning `None`, not a second hook. **Parts from Artio deliberately**: `lookup` is synchronous, because an accept path that awaits a network call is a denial-of-service surface no logon deadline closes. **And the survey found a real defect**: `Identity` is `(49, 56)` only, so a counterparty disambiguating by `50=`/`57=` cannot be served — Artio's `SessionIdStrategy` and QuickFIX's `SessionQualifier` both exist for that | Every entry point's signature; `GUIDE.md` §1a; hot reload, for ever |
| 9 | ~~**How much does this engine owe an auditor?**~~ **ANSWERED 2026-09-01 by [ADR-0027](decisions/ADR-0027-the-engine-owes-a-byte-stream-not-an-archive.md): a faithful, ordered, timestamped copy of both directions at a boundary, off the hot path — and nothing beyond it.** Retention, immutability, tamper evidence and search are **permanent non-goals**; `[documented]` MiFID II wants 5–7 years in a tamper-evident archive, which is a storage system, not an engine. The journal stays what D7 made it — `SLOTS = 8` `[verified]` — and the tap is a **separate** feature that does not share its store, because merging them is how an audit requirement lands on the hot path. Drop copy is a second FIX session, i.e. an ordinary registry entry. Original question: A FIX acceptor is usually required to answer *"what exactly did we send that counterparty, and when"* years later. The journal keeps what a **resend** needs — [D7](DESIGN.md) — which is not the same question. **An audit tap (a byte-for-byte copy of both directions, off the hot path via the ring) is a different feature from a message store**, and conflating them is how one ends up on the hot path. Decide the scope before either is built | The journal's format; open item 30's offline reader |
| 10 | ~~**Does the application get bytes, or typed values?**~~ **ANSWERED 2026-09-01 by [ADR-0028](decisions/ADR-0028-a-decimal-is-a-copy-value-parsed-on-demand.md), and it overturned this row's own guess.** `codec` gains `Decimal { value: i64, scale: u8 }` — `Copy`, `no_std`, parsed **on demand**, scale preserved exactly as it appeared on the wire, and **no `f64` in the public API on either path**. `[documented]` Artio, whose constraints are this project's, ships exactly that shape as `DecimalFloat` (value + scale). **The suspicion below was wrong**: what D2 forbids is the 8 224-byte owned `MessageView` that cost 5.9×, not 16 bytes of `Copy` produced when a caller asks. The gap is real. Original guess: `PRD` §3 lists *decimal / price types* as a phase-1 gap, but [ADR-0003](decisions/ADR-0003-message-representation.md) hands the application a borrowed view on purpose and a typed decimal is exactly the owned, per-message object D2 exists to avoid. **This may already be answered and mislabelled as a gap.** It needs a decision, not an implementation | `codec`'s public API; the phase-1 gap list's honesty |
