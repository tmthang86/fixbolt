...
| # | Criterion | Gate |
|---|---|---|
| 1 | Session conformance | **59 / 59** — `cargo test -p nanofix-session --test score`, in-process, no socket. **Met** `[measured 2026-08-29]`, and re-run 59 / 59 on a second machine 2026-08-30. Through a real socket, `cargo test -p nanofix-engine --test wire`: `[measured 2026-08-30]` **59 / 59** on the M5, **39 / 59** on Linux — that one is **not met until it reproduces**, `STATUS.md` open item 17 |
