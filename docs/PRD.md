...
| Session schedules — start/end time, weekday | Yes `[documented]` | **`Schedule` in `session`, done 2026-09-02** — daily, weekly, weekday filter, a window that crosses midnight, and the sequence-number reset decided by `same_session` ([ADR-0033](decisions/ADR-0033-a-schedule-is-utc-arithmetic-and-the-calendar-stays-outside.md)). **Two things are deliberately not here:** timezone names, because an IANA database cannot live in a pure layer — the caller resolves the offset and rebuilds; and **any engine-level recovery at all** — `[verified 2026-09-02]` `Engine` only ever builds `Session::new`, so a restart resets every session whatever the journal holds, and `resume_at` is reachable only by driving a `Session` directly (`STATUS.md` item 31) | P1, narrowed |
