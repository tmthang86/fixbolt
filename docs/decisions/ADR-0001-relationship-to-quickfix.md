...
nanofix is a FIX 4.4 engine in Rust targeting HFT-grade latency. QuickFIX C++ is the
reference open-source FIX engine: 20+ years old, deployed widely, with complete spec
coverage. The question put to this ADR is whether nanofix should be a **port** of it.
...
1. **Vendor `spec/*.xml`** as the input to nanofix's code generator. Fetched by script into
...
4. **Codec design follows `hffix`** (FreeBSD licence), not QuickFIX: parse and serialise in
   ...
   allocation on the hot path. `hffix` deliberately omits the session layer; nanofix
   supplies it.
5. If any QuickFIX-derived text or data ever ships inside a nanofix artifact, the
   attribution clause above is honoured in `NOTICE`, and the name "QuickFIX" is never used
   in nanofix's own name or marketing.

## Consequences

**Good**

- Latency is bounded by nanofix's own design, not inherited from a port.
...
- No attribution or naming obligations attach to nanofix's own source, because none of it
...
- **Spec coverage will lag QuickFIX for a long time.** QuickFIX handles FIX 4.0–5.0 SP2 and
  every edge case accumulated over two decades. nanofix will handle FIX 4.4 and whatever
...
- **No production track record.** QuickFIX's real value is that thousands of counterparties
  have already found its bugs. nanofix starts with that value at zero, and no amount of
