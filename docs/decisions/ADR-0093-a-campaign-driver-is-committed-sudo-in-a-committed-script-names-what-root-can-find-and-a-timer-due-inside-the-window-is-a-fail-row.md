...
- **`FAIL`** when any timer's `next` is `≤ now + window` — including a `next` in the past
  ...
  over-reading is the safe direction). The value names every such unit with its local time
  and `left`; the fix line is `sudo -n systemctl stop <unit>` per unit — *stop, not
