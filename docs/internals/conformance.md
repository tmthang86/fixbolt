...
| `script.rs` | Turning a QuickFIX `.def` file into a runnable script: `<TIME>` substitution, `9=`/`10=` computed at send time, not read off the file |
...
| `echo.rs` | The application the corpus assumes: echoes application messages back, rebuilt through `Template` rather than copied, so re-ordered input still gets ordered output |
