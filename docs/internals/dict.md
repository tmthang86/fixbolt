...
- `scripts/check-dict-refuses-a-message-without-msgcat.sh` — the only gate that observes
  `build.rs`'s three `die` arms actually fire (`emit`, `build.rs:792-808`, and `build.rs:1208`).
  ...
  already reads (`build.rs:106-112`, `cargo:rerun-if-env-changed`). Arm 0 (untouched) must build
  ...
  `msgcat='admin'` at all (the third reaches a second `die`, `admin_types.is_empty()` at
  `build.rs:1206`, rather than the first). Runs in the `gates` CI job on every commit
