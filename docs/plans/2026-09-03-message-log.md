...
> **Loại:** Plan · **Ngày:** 2026-09-03 · **Trạng thái:** **ĐÓNG** `[2026-09-04]` — cả 5 bước,
> CI xanh 12/12 trên `0e0b5f3`, run [`33861215461`](https://github.com/tmthang86/fixbolt/actions/runs/33861215461).
...
| 2 ✅ | `[2026-09-04]` **Xong.** Hook hai chiều ở `conn.rs` (8 chỗ dựng `Out`), `Engine<…, L>` + `with_log`, **5 entry point đều nhận `log`**, key `FileLogPath` (`[DEFAULT]`-only), `ServeError::LogPath`, `Snapshot::log_lost`, `EventKind::MessageLogLost`, `library` re-export. `cargo test --all` 478 → **483**; `--no-default-features` 479; fmt/clippy sạch; `check-no-optional-deps.sh` 6/6; 21+16 case alloc vẫn **0**. Reversal: `serve` nhận log rồi `drop` → `a_file_log_path_in_the_config_reaches_serve_and_the_file_fills` đỏ |
...
| 3 ✅ | `[2026-09-04]` **Xong.** `Connection::unsent_bytes()` đếm byte còn trong `tx` khi socket chết; `EventKind::MessageLogUnsent { bytes }` phát ở nhánh `Turn::Gone`, sau `L::LOGS`. Hai reversal, hai lần đỏ. **CI xanh trên `0e0b5f3`, run [`33861215461`](https://github.com/tmthang86/fixbolt/actions/runs/33861215461), 12 job / 12** |
| 3b ✅ | `[2026-09-04]` **Xong.** `msglog::shard_path` (`messages.log` → `.0`, `.1`), `MaybeLog`, `serve_sharded_hft` nhận `log_path: Option<&Path>` và mở **một file mỗi shard trước khi thread khởi động** — mở trên shard thread sẽ đánh dấu đuôi rách hai lần. `Shardable` mang `L`. `cargo test --all` 483 → **487**; `--no-default-features` 483 |
| 3c ✅ | `[2026-09-04]` **Xong.** Ba case alloc — `log-record` (0 trên 1000 record), `log-idle`, `log-busy` — nên `benches/alloc.rs` là **24 case**, không phải 21. `library` re-export. Tài liệu: `DESIGN.md` D14 + §3 + §6 + §8, `GUIDE.md` §6c, `CONFIGURATION.md`, `SESSION-BEHAVIOUR.md`, cả hai `best-practices-*`, `CHANGELOG.md`, `STATUS.md` item 44. `cargo test --all` **487** |
| 4 ✅ | `[2026-09-04]` **Xong.** Header `FXBJ\x01`, CRC32 (bảng 256 entry bằng `const fn`, **zero dependency**) sau mỗi record; CRC sai xử lý **y như đuôi rách** — dừng ở đó, phần trước vẫn dùng, `corrupt_records()` trên cả `FileJournal` lẫn `Reader`, `jrnl` exit 2. File không có header là **v0, đọc y như cũ, và ở lại v0 khi được append**. `Async` tính CRC trên writer thread, `Fsync` trên engine thread **qua các mảnh, không ghép buffer** (ghép là cấp phát). Ghi chú `[sửa 2026-09-04]` cuối ADR-0008. Fixture v0 là **bytes commit sẵn từ `df94c08`**. `cargo test --all` 487 → **492**. Hai reversal, hai lần đỏ |
