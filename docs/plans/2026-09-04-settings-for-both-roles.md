...
**ĐÓNG trên chính commit merge.** PR [#46](https://github.com/tmthang86/fixbolt/pull/46) merge
thành **`0f9585d`**, run
[`33998809034`](https://github.com/tmthang86/fixbolt/actions/runs/33998809034), **11 job trên
11**. Log job `interop` được **đọc từng dòng** chứ không đọc kết luận: `PASS 7/7`, `PASS 7/7` +
`shutdown ok Shutdown { … }`, ba lần `PASS 6/6` mỗi lần kèm `resumes: 1` và `two_sources ok`, và
`==> the run added nothing git can see`.

Đầu nhánh xanh trước đó — `8cfa3b2`, run
[`33980369287`](https://github.com/tmthang86/fixbolt/actions/runs/33980369287), cũng 11 trên 11 —
và **đó không phải cùng một commit**: run của PR checkout merge ref của GitHub, chỉ run trên
`main` mới kiểm cái thật sự đã đáp xuống. Items 48 và 47 phải mở thêm một PR nữa mới học được
điều đó; lần này thì không.
