...
| **2.2c** | 40 | Kiểm tra "bỏ mẫu không lệch phân phối" theo ADR-0071 quyết định 3 (đã sửa): `w2w --connect --dump <file>` ghi mỗi request trong cửa sổ một dòng `index, rtt_ns` (thứ tự gửi sau warmup); `w2w --listen --wire-timestamps --dump <file>` ghi `index, stamped|missing, wire_ns` (index = hạng của offset byte TCP sau `--warmup` của acceptor — hai bên khớp nhau vì cùng một `WARMUP` từ script); **ghi sau khi `ARMED` tắt**, từ dữ liệu đã có (`samples` và danh sách cặp), không thêm gì vào cửa sổ đo. `w2w-baseline.sh`: `scp` dump của generator khi có `GENERATOR_SSH`, join theo index (hàm thuần trong script), in p50/p99/p99.9 của generator trên **tất cả** và trên **các dòng acceptor có stamp**; PASS khi lệch ≤ 1 % p50 và ≤ 5 % p99/p99.9, ngược lại run *marked*. Đọc trước: ADR-0071 quyết định 3; `tools/w2w/src/main.rs` `generator_half` (`:1751-1810`, chỗ `samples` còn sống sau `measure`) và mã ghép cặp theo offset trong `mod wire`; `scripts/w2w-baseline.sh` phần `WIRE_NIC`/`GENERATOR_SSH` (`:41-90`, `:416-450`). Bẫy: `--dump` không có `--wire-timestamps` ở `--listen` bị từ chối (không có gì để ghi); hai `--warmup` khác nhau → header dump ghi warmup và script từ chối join khi lệch | `tools/w2w/src/main.rs` (cờ `--dump` cả hai nửa, hàm ghi), `scripts/w2w-baseline.sh` (fetch + join), `scripts/check-w2w-baseline-summary.sh` (test join: dump tổng hợp 20 000 dòng, bỏ 5 dòng ngẫu nhiên → PASS; bỏ 20 dòng chậm nhất → FAIL ở p99.9; warmup lệch → refuse) | `scripts/check-w2w-baseline-summary.sh` xanh với ba case mới; `cargo test -p fixbolt-w2w` (test đơn vị: dump có đúng `n` dòng và header warmup); `allocs 0` của cả hai nửa không đổi trong cửa sổ (quote); clippy | sonnet | 2.2, ADR-0071 (đã sửa) duyệt |
...
| **C-91b** | 91 | Bisect trên bàn §9, cùng boot với C-91: `git bisect start 85460c1 0149b26` (good = `0149b26`, bad = `85460c1`), mỗi commit: build release trong worktree riêng, chạy `scripts/bench.sh` cho `density` **n = 5**, lấy trung vị `engine turn, 1 busy sessions`; script phán `bad` khi trung vị **> 1 660.2 × 1.05 = 1 743 ns** (ngưỡng 5 % trên mốc `0149b26` đo trong C-91), `good` khi dưới, `skip` khi commit không build. ~log2(số commit) ≈ 7–8 vòng × 5 run. Đọc trước: `measured-costs.md` *Boot C* C-91 (bảng và hai manh mối: `588b350` cho validate, nhánh TLS cho engine turn); nếu commit tìm ra thuộc nhóm TLS/`pump`, chạy thêm `validate` n = 5 ở hai bên nó để biết validate có cùng thủ phạm không | `target/boot-c-evidence/c91b-bisect.sh` (script phán, ngoài repo), `target/boot-c-evidence/c91b-log.txt` | `git bisect log` quote trọn; commit thủ phạm nêu tên kèm 5 số hai bên | manager + haiku runner | C-91 |
...
- **Bước 3.1 — xong 2026-09-18, `a2aa9ce`** trên `plan/the-second-linux-desk-c`, senior developer (opus). `crates/conformance/src/mirror.rs` phân loại **cả 50 file**; enum cần **tám** lớp chứ không phải năm như ADR-0076 viết ban đầu — thêm `NeedsMalformedOutput` (ADR-0042 d1; đúng 5 file "Logon cố ý sai" của ADR-0006 **cộng 6 file nữa**), `NeedsBodyOrderTheTableRefuses` (bất biến 5 / D3), `NeedsAdminMessageTheApiDoesNotOriginate` (bất biến 2); `NeedsHarnessDirective` **rỗng**. Đếm: Reachable **14**, Header 11, Malformed 11, BodyOrder 6, UnsequencedReset 3, GapFill 3, AdminMsg 2. **Trần là 14, không phải 45.** ADR-0076 sửa tại chỗ (còn *Proposed*) với tám lớp và mục *Measured*; CI run id manager điền. Điểm hôm nay **10 / 14**: bốn file `Reachable` đỏ — `6_SendTestRequest`, `8_AdminAndApplicationMessages`, `8_OnlyApplicationMessages` (engine trả `Reject 45=2 371=122` vì `make_receivable` của harness biến dòng E thành `PossDup` **không có `122=OrigSendingTime`** — nghi lỗ ở harness, không phải session; senior dev đã thăm dò `send_application` hai lần → đủ 14 field, seq 2 rồi 3) và `1a_ValidLogonMsgSeqNumTooHigh` (harness không gọi `Session::resume(cfg, 5, 1)`). Ghi thành **item 92** trong `STATUS.md`, với hai giả thuyết và test quyết định từng cái; không sửa `crates/session`.
- **Bước 3.2 — xong 2026-09-18, `09effd1`**, senior developer. **Item 92 đóng: lỗ ở harness, không phải session.** `make_receivable` chỉ thay placeholder `00000000-00:00:00` ở `52=`; session đọc `122=` và `60=` cũng là `UTCTimestamp` (FIX 4.4, nguồn onixs tag 122, b2bits tag 43/122) và từ chối bằng `371=122/373=1`, `371=60/373=6`. Sửa ở `crates/conformance/src/script.rs`: ba tag được thay, ba unit test, đảo chiều đã quote; `Adapter::at` resume theo `34=` của dòng `I` đầu (không dời số nào). **Mirrored 14 / 14**, nhưng thành phần đổi: `8_AdminAndApplicationMessages`, `8_OnlyApplicationMessages` xanh; `6_SendTestRequest`, `1a_ValidLogonMsgSeqNumTooHigh` vẫn đỏ vì **phía script của corpus im lặng ở chỗ một initiator đúng phải trả lời** (6: ta trả lời `TestRequest`, không dòng `I` nào nhận; 1a: `ResendRequest` của script không bao giờ được script trả lời, ta gap-fill, file chờ ta `Logout`) → không `Reachable`, thành lớp thứ chín `NeedsAScriptThatAnswers` (ADR-0076 quyết định 2; `NeedsHarnessDirective` không hợp vì lớp đó nói về dòng directive, không về hành vi script); `19a`/`19b` (`97=Y`) **pass** → `Reachable`, lý do cũ của hàng sai. Trần 14 − 2 + 2 = **14**, điểm **14 / 14**; ADR-0076 *Measured* ghi hai commit, CI run id manager điền. **Không sửa `crates/session/src`; 59/59 hai chiều.** Đề nghị `KNOWN_RED` ở dòng trên **không cần nữa**: test điểm giữ `score == Reachable`. Bẫy ghi ở `docs/reference/a-corpus-placeholder-timestamp-lives-in-three-tags.md`; `docs/CONFORMANCE.md` §1 thêm dòng mirrored. Còn nợ trong code: hàng 6/1a/19a/19b của `mirror.rs` đổi lớp và `why` — commit đóng 3.2 hoặc commit kế tiếp của senior developer.
- **C-PRD7 — xong 2026-09-18, boot C**, bàn §9, `pass 16 fail 0 unknown 0`, code của `85460c1`, `benches/wakeup.rs` `WAKEUP_CORES=6,7`, 20 run × 20 000: **`epoll_wait` p50 4 960 ns** (4 949..4 980, max/median 1,004), **`poll` 4 819 ns** (4 809..4 829), p99 ≈ 7,8–7,9 µs, p99.9 ≈ 8,9 µs, max ≈ 10,3 µs. Nằm ở **đỉnh** khoảng 2–5 µs mà ADR-0025 giả định → ADR-0025 **giữ 4, chuyển Accepted** với con số này; điểm giao số học với turn rảnh 448,9 ns là N ≈ 11, nhưng turn *bận* ở N > 1 (câu hỏi mở 1) chưa đo nên không nâng trần. Dòng `baselines.tsv`: `wakeup epoll_wait` 4 960, **margin 1.10** (sàn thang; max/median đo 1,004), n = 20, manager thêm. `DESIGN.md` §8 hàng *Cost of a wakeup* có số; `measured-costs.md` *Boot C* mục C-PRD7. PRD §6 hàng 7 đóng.
- **C-85 — xong 2026-09-18, boot C**, một biến: procedure 1 bắt đầu < 2 phút sau `cargo build --release`, procedure 2 sau 10 phút máy rảnh; `hft` admin `off` (w2w ở N = **1** — default riêng của `w2w` không theo `Limits`, bẫy ghi ở ADR-0069), 10 × 20 000, `85460c1`: p50 16 005 ‖ 15 990 (0,09 %), p99 21 000 ‖ 20 794 (0,99 %), p99.9 22 978 ‖ 22 312 (2,99 %) — **tái lập mọi percentile → ứng viên build-slot/nhiệt bị bác**. Header mới đọc `thermal iwlwifi_1 65000 cpu6-freq n/a`: bàn này **không lộ** thermal zone CPU lẫn `cpuinfo_cur_freq`, hai cột ghi `n/a` chứ không in nhầm cảm biến. Ghi ở `a-tight-spread-…md` mục *Candidate tested*; nguyên nhân dịch 4,7–6,7 % của boot B **vẫn chưa rõ, hết ứng viên**; guard (ADR-0068 + `compare-w2w-procedures.sh`) là thứ giữ. Item 85 đóng ở mức: guard có, ứng viên duy nhất đã thử và chết.
- **C-84 — xong 2026-09-18, boot C**, bốn arm `hft` admin loopback, `--tls`/`--client-tls`, 10 × 20 000, hai procedure ngược thứ tự, `85460c1`, mọi arm tái lập (lệch tối đa 2,9 % ở p99.9); `w2w` chạy ở N = **1** (default riêng, không theo `Limits` — bẫy, ADR-0069) nên mốc `off` = arm N = 1 của boot C (16 070 ‖ 16 080). p50 p1 ‖ p2: kTLS/kTLS 25 503 ‖ 25 473 (+9,4 µs); userspace/userspace 20 824 ‖ 20 719 (+4,7 µs); **engine kTLS + client userspace 21 175 ‖ 21 250 (+5,1 µs)**; engine userspace + client kTLS 22 177 ‖ 22 147 (+6,1 µs). Đọc: **kTLS của engine chỉ đắt hơn userspace của engine 351 ‖ 531 ns (~0,4 µs, ~2 %)**; kTLS của *client* đắt ~1,4 µs; hai đầu cùng kTLS **siêu cộng** thêm ~2,8–3,0 µs so với tổng hai phần (ứng viên: hai lớp record kernel giải mã trong `recvmsg` chỉ khi đủ record, nối tiếp thay vì gối đầu — chưa cô lập). Arm lệch **in** `allocs` (4 001 trên thread client, là `rustls` của client) và **không assert**; arm đối xứng vẫn assert. Ghi vào ADR-0070 quyết định 5 + *Consequences*, `best-practices-hft.md` §9, `DESIGN.md` §8 bảng *Boot C, mixed TLS*, `measured-costs.md` *Boot C* mục C-84. Item 84 đóng với phần của engine tách riêng; quyết định 2 của ADR-0005 giữ nguyên.
- **C-91 — xong 2026-09-18, boot C: thủ phạm là CODE, không phải máy.** Worktree `../fb-0905` ở `0149b26` (commit ghi các dòng 2026-09-05) so với cây hôm nay (`85460c1`), 20 vòng đảo thứ tự, ba suite mỗi vòng, trung vị 20: **mọi case `engine turn` chậm 7–10 %** (`1 busy` 1 660,2 → 1 817,2, +9,5 %; `64 busy` +7,5 %), validate +3–6 % (`NewOrderSingle` 909,7 → 956,7), `encode ExecutionReport (template)` **−3,4 %**, `SendingTime from the cache` 4,9 → 5,8. Commit cũ chạy trong boot hôm nay đọc đúng số 2026-09-05 của nó (1 660 so với dòng 1 657,7) → ứng viên kernel `-30 → -31` **chết**; toolchain ghim `1.98.0` suốt. Con số lớn hơn +3 % boot B thấy. Việc kế: **C-91b** (bisect, hàng mới trong *Chia việc*, ngưỡng 1 743 ns, n = 5/commit). Log `target/boot-c-evidence/c91-runs.txt`, `c91-timeline.txt`; `measured-costs.md` *Boot C* mục C-91. Item 91 thu hẹp về "commit nào" — hàng STATUS ở scratchpad.
- **C-49 — xong 2026-09-18, boot C**, `perf trace -s` trên engine tid cả run `hft` hai path (tracer làm mọi syscall chậm ~2×, p50 dưới trace 35,2–35,8 µs **cả hai path** — chỉ hiệu số là số đo): `sendto` 22 001 lần/run, admin 186,608/186,508 ms (tb 8,48 µs), app 187,663/183,876 ms (tb 8,53/8,36 µs) — **lệch ≤ 50 ns, hai dấu**; `recvfrom` 2,47 so 2,45 µs. **Kernel không tính thêm cho payload lớn hơn** ở độ phân giải này → ~3 130 ns ngoài một lượt engine **không phải** việc kernel tỷ lệ theo payload. Đọc phụ: `accept4` 40,6–45,7 k/run ≈ `recvfrom` → một `accept4` mỗi vòng spin dù default thư viện là 16 → phát hiện bẫy `w2w` default riêng = 1 (đã trả lời, không mở). **Item 49 đóng bằng quyết định**: ba probe đã loại hết ứng viên có tên; phần còn lại công bố ở §8 là *unattributed, ngoài engine turn, không phải kernel*; dụng cụ duy nhất còn tách được là stamp **phần mềm** `SO_TIMESTAMPING` hai socket (bốn đoạn/round trip), đặt tên `--stamp software`, **không xếp lịch**. `DESIGN.md` §8 hàng *kernel, in situ* có số; `measured-costs.md` *Boot C* mục C-49; hàng STATUS ở scratchpad.
- **C-91b — xong 2026-09-18, boot C**, `git bisect start 85460c1 0149b26`, phán bằng `density` `engine turn, 1 busy sessions`, trung vị 3 run/commit (commit đầu 5), bad > 1 743 ns; log `target/boot-c-evidence/c91b-log.txt`. Trung vị theo thứ tự lịch sử: `0149b26` 1 660 · `792c2e7` 1 700,0 · `28465e8` 1 719,6 · **`588b350` 1 752,2** · `043a4a2` 1 740,2 · `627637e` 1 742,2 · `906c256` 1 752,9 · `1c36406` 1 753,8 · `f085f43` 1 792,2 · `85460c1` 1 817. `git` gọi tên **`588b350`** (`52=` đọc mọi độ chính xác; đụng `clock.rs parse_utc` bảy bề rộng và `dict/field_type.rs`) — điểm vượt 5 %, đáng ~+33 ns, trùng manh mối validate; ứng viên cơ chế: dispatch theo bề rộng trên đường nóng của mọi `SendingTime` vào. **Nhưng profile là cộng dồn**: ~+40 ns tới `792c2e7`, ~+20 tới `28465e8`, ~+33 ở `588b350`, ~+40 giữa `1c36406`→`f085f43` (TLS transport + settings), ~+25 `f085f43`→HEAD — năm bậc 2–3 %, không bậc nào tự làm `bench.sh` đỏ. Câu hỏi "máy hay code" **đóng** (item 91, hàng STATUS ở scratchpad); phần còn lại thành **item 93** — năm task perf, mỗi task một đoạn bisect có tên hai đầu. Baselines **không** dời tới khi từng bậc có tên và quyết định (`DESIGN.md` §8 một đoạn nói vì sao). `measured-costs.md` *Boot C* mục C-91b.
- **C-40 — lần thử đầu, 2026-09-18, boot C: bị chính luật của mình cắt ngang; số của bốn run ghi lại đây.** Cáp thẳng, interval 0, 20 000 request/run, `w2w` hai đầu từ `ece17e7` (Mac giờ là checkout git thật của `main`), `hw-rx-missing 0` mọi run; `hw-tx-missing` run 2–5: **17, 15, 3, 30** (run 1 DISQUALIFIED, busy 4 %); wire p50 **27 098 / 27 282 / 27 010 / 26 794 ns**, p99 ≈ 32,5 µs; phía Mac (không pin) thấy p50 ≈ 233 µs. Run 5 thiếu 30/20 000 = 0,15 % vượt ngưỡng 0,1 % của ADR-0071 quyết định 1 — và luật viết thành FAIL **cả script**, nên procedure 1 dừng ở run 5, bốn run đạt chuẩn bị bỏ; sẽ lặp lại ở procedure 2 và ở sweep (quyết định 5) — chính phép đo cho biết 0,1 % có đúng chỗ không. Tỷ lệ thiếu ở interval 0 trên I211 **nằm vắt qua ngưỡng** (boot B 1–4/20 000, hôm nay 3–30). **Quyết định của manager theo uỷ quyền**: ADR-0071 quyết định 1 sửa bằng ghi chú có ngày (ADR đã Accepted, câu cũ gạch, kiểu ADR-0002): run vượt 0,1 % bị **loại (disqualified)**, in số trên dòng của nó, giữ trên đĩa, procedure **chạy tiếp**; procedure chỉ FAIL khi **dưới một nửa `RUNS` đạt chuẩn** hoặc bất kỳ `hw-rx-missing > 0` (giữ nguyên). Script sửa theo (bước 2.2 mở lại một phần, sonnet), rồi C-40 chạy lại đủ hai procedure + sweep. Bốn số trên là **chẩn đoán, chưa phải figure** (một procedure dở, ADR-0068). Bẫy gặp: `w2w` build lại **mất file capability** — `docs/hft-playbook.md` §6 mục 4 đã nói ("the capability is lost at every rebuild", dòng 168, `sudo -n setcap cap_net_raw,cap_net_admin+ep target/release/w2w`); không ghi thêm nơi khác.
- **C-40 — xong 2026-09-18, boot C, chạy lại dưới luật đã sửa.** Bàn §9 `pass 16 fail 0 unknown 0`, code `ecfdd81` (Mac `w2w` `2f93af5c` từ `main` `ece17e7`), `enp9s0` ↔ Mac mini cáp thẳng, EEE off, IRQ cpu4, observer cpu7, stamp phần cứng hai chiều, `hft`, interval 0, 20 000 × 10 run/arm, hai procedure, `--dump` join mọi run đạt chuẩn = `join: PASS` (12/12 ở p1). **Wire (acceptor) p50/p99/p99.9, p1 ‖ p2**: admin **27 050 / 31 982 / 35 374 (6/10) ‖ 27 114 / 32 254 / 49 146 (8/10)**; app **28 894 / 34 058 / 62 306 (6/10) ‖ 28 878 / 34 110 / 88 630 (8/10)**. p50 và p99 tái lập (≤ 0,9 %) → **hàng §6 NIC-to-NIC MET ở p50/p99, `hft`, đúng tốc độ hàng đòi (back to back)**; p99.9 lệch 27–42 % → công bố cặp, đánh dấu *not reproduced*, không đạt gì (ADR-0068 q3). Mac (không pin) thấy p50 ≈ 232 µs, không phải số của engine. **Sweep** admin 0/10/20/30/50 µs: p50 27 018–27 138 phẳng, 7–8/10 đạt chuẩn ở mọi mức, `hw-tx-missing` 0–109/20 000 **không theo interval**, `tx_hwtstamp_skipped` 413 → 2 381 — **pacing tới 50 µs không làm I211 hết bỏ stamp**; điểm hết bỏ nằm giữa 50 µs và 1 s (boot B 1 s: 0). So với boot B 1 s: back-to-back nhanh hơn ~18 µs (nóng bên cạnh lạnh, ADR-0071 q2). Phát hiện dụng cụ: `compare-w2w-procedures.sh` so dòng p50 của **Mac** thay vì hàng `wire p50` — phải ưu tiên hàng wire khi có; developer sửa kèm test bằng hai summary của run này. Docs: `DESIGN.md` §6 hàng NIC (MET p50/p99), §8 khối *Boot C, over the cable*; `PRD.md` §2 bullet "Criterion 6 is loopback"; `measured-costs.md` *Boot C* mục C-40; ADR-0071 q5 ghi kết quả sweep. Item 40 đóng — hàng STATUS ở scratchpad; CI run id manager điền.
