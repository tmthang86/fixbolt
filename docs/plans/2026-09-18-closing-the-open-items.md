...
- **Bước 3.2 — đề nghị cho test điểm**: **đồng ý với manager, có một ràng buộc thêm.** Test điểm đọc trần từ bảng (`Reachable` = 14) và giữ **hai** assertion: (a) `score >= 10` — con số hôm nay, và (b) **bốn file đỏ được gọi tên trong một mảng `KNOWN_RED`**, test khẳng định *đúng* tập đỏ là tập đó — không hơn, không kém. Như vậy một file thứ năm đỏ là hồi quy (đỏ), và một file trong `KNOWN_RED` chuyển xanh cũng **đỏ** — để ai sửa harness phải xoá tên nó khỏi mảng trong cùng commit, đúng kiểu ratchet hai chiều của `check-indexing-debt.sh`. `score == 10` một mình không phân biệt được "file A xanh, file B đỏ" với hôm nay; tập tên mới phân biệt được. Khi item 92 đóng, `KNOWN_RED` rỗng và assertion (a) thành `score == 14`. `docs/CONFORMANCE.md` công bố *10 / 14, trần 14 (ADR-0076), bốn file item 92* kèm CI run id.
