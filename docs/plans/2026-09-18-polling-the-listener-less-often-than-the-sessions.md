...
- **Bước 5 — xong 2026-09-18, boot C, bàn §9**, code của `55a1549` trên nhánh `plan/the-second-linux-desk-c`, `FIXBOLT_NIC=enp9s0 scripts/check-machine.sh` → `pass 16 fail 0 unknown 0` trước mỗi run đạt chuẩn, loopback, `hft`, 10 run × 20 000 mỗi arm, hai procedure (thứ tự N = 1, 16, 256 rồi 256, 16, 1), driver `target/boot-c-evidence/c89.sh`, kết quả `target/w2w-baseline/boot-c-p{1,2}-n{1,16,256}/summary.txt`. **Cả sáu arm tái lập** (ADR-0068). p50 admin ‖ app, procedure 1 ‖ 2: N = 1 **16 070 ‖ 16 080 / 20 209 ‖ 20 228**; N = 16 **16 381 ‖ 16 361 / 17 644 ‖ 17 603**; N = 256 16 376 ‖ 16 361 / 17 608 ‖ 17 633 ns. App **−12,7 ‖ −13,0 %** (≈ một `accept4`, đúng như B9 gợi ý); admin **+1,9 ‖ +1,7 %** — tái lập, **chưa giải thích được** (ứng viên: khoá socket / backlog, ghi ở ADR-0069 *Measured*, chưa thử); 16 ≡ 256 trong 0,3 % nên N nhỏ hơn thắng vì trần trễ accept nhỏ hơn. `connect-rtt` 68–85 µs, `logon-rtt` 1,07–1,12 ms, không đổi theo N. `check-no-kernel-sleep.sh` xanh có và không `--listener-every 256` (`affinity,tls`); `strace -c` engine tid: 7 901/7 900/351 → 7 970/8 320/351 (`accept4`/`recvfrom`/`sendto`) — số `accept4` **trong cửa sổ có tải là suy ra** từ hiệu `recvfrom − sendto`, không đếm riêng. **Quyết định của manager (theo uỷ quyền của chủ sở hữu): mặc định N = 16**; ADR-0069 sửa tại chỗ và chuyển *Accepted*; `DESIGN.md` §8 thêm khối *Boot C*; `measured-costs.md` thêm mục *Boot C*. Hằng số và `docs/CONFIGURATION.md` do developer sửa song song, không trong commit này. Q3 trả lời: **không** mở plan cho tuỳ chọn (d).
