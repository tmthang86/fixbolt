...
- [ ] *Bẫy mới, review F9* → `docs/reference/a-shell-error-inside-an-if-drops-the-row-and-the-report-exits-zero.md`
      (architect, lượt cuối), canh bởi `scripts/check-machine-verdicts.sh` mục `=== timer_window_sec`
      (12 assert) từ `83b53e3`.
...
| Review senior, phần chữ | `7e815e6` | Năm finding xác nhận + ba nhỏ, toàn trong file của architect, manager tái hiện từng cái trước khi gửi: **F1** ADR-0093 mất bullet R3 + vết `\1`/`\n` (`re.sub` với replacement là chuỗi thô) — **ba gate và một CI run 14/14 xanh trên file có `\1`**, không gate nào đọc Markdown tìm vết `sed`; cái bắt được là reviewer chạy `grep` (§10). Manager tự kiểm "không mất gì khác": vùng R1..R3 so với `b93bbf7` → identical; grep `\1` trên ba ADR, hai trang reference, `STATUS.md`, `DESIGN.md` → rỗng. **F3** `STATUS.md` *Do not* thì hiện tại sau khi parser đã sửa — re-tense, chỉ `--reextract`; **F5** trang `perf-record…` chỉ vào `run-d2.sh` đã chết — thay bằng ADR-0093 quyết định 1; **F6** ADR-0092 "hai dòng tạm" → ba, nêu case in-band; **F7** hai parenthetical `G2b` xoá. Ba nhỏ: trích `:101,121` → `:216,236`; `DESIGN.md` §6 `FAILED` đủ ba trường hợp; hàng 5 bảng này điền. Gate: `check-links.py`, `check-adr-numbers.sh` xanh | **F2 còn mở** lúc đó — comment kết thúc bằng `\` nuốt dòng sống kế tiếp, dòng boot D đọc `ok — 0 sudo lines read` |
| Review senior, phần script | `83b53e3` | Sáu finding, manager tái hiện từng cái: **F2** liveness thử **trước** khi nối dòng `\` (như shell: comment hết ở newline) — probe đọc `FAIL — probe.sh:2 … (R2)`, đúng số dòng; **F8** "never calls `date`" nay **đúng như viết**: `utc_stamp` thuần bằng `awk` (Hinnant `civil_from_days`), so với GNU `date` trên 3015 giá trị không lệch, manager thử epoch 0, `2000-02-29`, `2038-01-19`, `2100-03-01` — người xây tự tìm ra fixture `2100-01-01` không chạm số hạng thế kỷ và đổi sang `2100-03-01`; **F9** `timer_window_sec` thuần: chấp nhận số thập phân không âm, còn lại **một hàng UNKNOWN nêu tên knob** — `' '`, `-1` trước đó được nhận **lặng lẽ** (0 s, −3600 s); hai câu trong brief của manager sai, đo lại: rỗng → mặc định 12; `$(( ))` hỏng trong `if` **không** kết thúc script mà bỏ block, hàng biến mất, exit 0 — preflight của driver grep hụt → chiến dịch từng có thể chạy **không có kiểm tra timer, không một lời** (trang reference mới); **F10** R3 đọc `--` của chính `sudo` → nay tìm sau từ lệnh; **G7** thêm, fixture hai chiều, ALLOW không nới; **F4** header `ab-rotation.sh` khớp code (5 cột, dòng suite, cột `over`, `--reextract`); **F11b** hai script `ab-rotation` vào step shellcheck (sạch ở `-S info` cả trên `main`); **F12b** test độ rộng 7/8/9 ký tự sinh từ chính format string của `harness.rs`, ghi rõ là tổng hợp. Gate: `check-sudo-verdicts.sh` `pass 44 fail 0`; gate sudo `ok — 32 scripts, 32 sudo lines, 29 fixture lines skipped, 0 findings`; `check-machine-verdicts.sh` `pass 63 fail 0`; `check-ab-rotation.sh` `pass 35 fail 0`; `check-scratch-fixtures.sh` ok; `shellcheck` bảy file sạch; YAML OK; `check-links.py`, `check-adr-numbers.sh` xanh; `baselines.tsv` sạch. `cargo test` không chạy lại — `git diff --stat 7e815e6 -- crates/ Cargo.toml Cargo.lock` rỗng nên 839/834 không thể đổi | **Còn nợ, và của ai**: (1) đảo ngược macOS trên Mac mini — ADR-0091 quyết định 2, **nay thêm** hình `date` BSD của F8 (chủ dự án / boot sau); (2) thấy hình hàng của F9 **một lần trên máy systemd thật** (bàn đo, dòng grub bất kỳ); (3) bước 7a `--reextract target/boot-d-evidence/d4m/` (bàn đo). Hai câu ADR-0093 → architect, đã làm trong lượt này |
| Chấp nhận ADR | *(commit merge của manager)* | ADR-0092, 0093, 0094: `Proposed` → **`Accepted — 2026-09-22`**, theo tiền lệ mọi ADR đã merge của repo (0088, 0090, 0091 `Accepted` ngay tại commit merge). **Cơ sở ghi đúng cái đã xảy ra, không phóng đại**: chủ dự án duyệt kế hoạch này **một chữ** ngày 2026-09-22 (`bb71859`), *Cách làm* nêu từng quyết định và link cả ba ADR; **không có uỷ quyền riêng cho kế hoạch này** (architect tìm trong kế hoạch, `STATUS.md`, và message của mọi commit trên nhánh: chỉ có uỷ quyền chung 2026-08-30 — lên plan, duyệt plan, merge — được nhắc như tiền lệ, không nhận làm cơ sở); chủ dự án chưa đọc ba văn bản; một chữ của chủ dự án đảo ngược trạng thái. ADR-0093 ghi bốn lượt sửa trong ngày thành một câu chuyện: ba quyết định không đổi, chỉ hiểu biết về điểm mù của gate đổi; ADR-0092 ghi câu "hai dòng tạm" là **bất khả**, không phải thiếu chính xác. `check-links.py`, `check-adr-numbers.sh` — xem báo cáo của architect | Manager nói với chủ dự án đúng cơ sở đã ghi |
...
5. **Review senior tìm ra một xanh giả trong chính gate mà nhánh này thêm** (F2: một comment kết
   thúc bằng `\` nuốt dòng sống kế tiếp, dòng boot D đọc `ok — 0 sudo lines read`) **và một tai
   nạn `sed` đã lọt qua ba gate và một CI run 14/14** (F1: `\1` nguyên văn thay cho bullet R3 của
   ADR-0093). **Mười bốn commit kỷ luật bằng chứng không bắt được cái nào**; một reviewer chạy
   `grep` trên file và một reviewer viết một file probe thì bắt được. §10 nói đúng thế: review
   diff hầu như không bắt gì; lỗi bị bắt khi có ai đó **chạy** một thứ và **đọc** output. Và
   trong cùng lượt, hai câu manager viết chắc nịch về hành vi cũ của `$(( ))` bị đo lại là sai —
   cái mà mọi người "biết" về shell là thứ phải chạy thử trước khi ghi vào ADR.
