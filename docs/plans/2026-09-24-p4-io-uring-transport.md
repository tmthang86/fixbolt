...
- **8 — `unsafe` cần plan và tên của thứ chứng minh nó.** Mỗi khối có
  ...
  | U2 | `register_buf_ring_with_flags` (mỗi khe một ring, ADR-0192) | bộ nhớ sống đến khi gỡ đăng ký hoặc ring bị huỷ | **cấu trúc**: mọi `UringTransport` giữ `Rc` sở hữu `Inner`, nên `Inner` (và bộ nhớ vùng đệm) chỉ bị huỷ sau kết nối cuối cùng — và mỗi kết nối, trước khi đóng fd, đã `shutdown(2)` socket (dừng `recv`), huỷ `recv` của nó và flush lệnh huỷ; không yêu cầu nào còn chọn được vùng đệm sau đó. Gỡ đăng ký trước khi giải phóng (`Drop` của `Inner`) là tuyến thứ hai, không test nào phân biệt được (review PR #110, L1: đảo thứ tự trường và xoá lệnh gỡ đăng ký, canary vẫn xanh); `unregistered_buffers_are_not_written_after_the_ring_is_dropped` chỉ kiểm trạng thái cuối |
