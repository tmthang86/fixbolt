...
| 3 | **Code store (ADR-0180).** `src/journal.rs` (`SqliteJournal`, cài `Journal`, `open`, `released`, `progress`, `close`, `Drop`), `src/writer.rs` (vòng ghi, gom lô, mark gộp, bí mật, lỗi lô, dừng/đóng/release/finish), `src/schema.rs` (pragma, tạo/kiểm schema và định danh, nạp lại). Struct `SqliteJournal` không chứa kiểu `rusqlite`. **Cho nghỉ luồng ghi theo đúng *The rule for a journal* của ADR-0181 quyết định 1 (Revision 1)**: trong `retire` trên luồng engine, đẩy bản ghi `STOP` một byte **một lần** (`Producer::push`, không lặp), rồi gọi `ticket.retire(pushed)` với kết quả lần đẩy đó, rồi thả luồng ghi; không đẩy gì sau đó. Luồng ghi: gặp `STOP` là dừng; `pop` thấy ring rỗng thì đọc `ticket.state()`, nếu là `RetiredStopWhenDry` thì **`pop` thêm một lần** và chỉ dừng khi lần đó cũng rỗng (đọc trạng thái trước, thấy rỗng sau); khi dừng: commit, checkpoint, đóng `Connection`, `releaser.release()`, rồi `ticket.finish()`. Không dùng (và không cần) `Producer::fits`. *Điều kiện trước bước này*: vòng sửa của bước 1 theo ADR-0181 Revision 1 đã xong và được manager commit — `finish` trên vé *running* chuyển sang *finished* không đụng biến đếm, `FileJournal` theo cùng quy tắc đẩy-rồi-`retire`, `writer_hooks.rs` thêm test `a_retire_after_the_writer_finished_counts_nothing` (đẩy `STOP`, luồng ghi `finish`, rồi `retire(true)` trả `false`, `wait_for_retired_writers` trả `true` ngay). Đảo ngược, mỗi cái viết câu FAIL trước: R1 bỏ nhánh bí mật trong writer → đỏ ở `a_message_carrying_a_secret…` và `secrets_through_a_real_logon…`; R2 `busy_timeout` 5000 → đỏ ở `a_second_open…` (thời gian); R3 bỏ kiểm định danh → đỏ ở `a_database_of_another_session…`; R4 `INSERT` thay `INSERT OR REPLACE` → đỏ ở `a_reused_number…`; R5 `retire` không lấy vé → đỏ ở `a_retired_writer_commits_everything…`; R5b bỏ lần `pop` thêm sau khi đọc `RetiredStopWhenDry` → đỏ ở một unit test `#[cfg(test)]` trong `src/writer.rs` chạy **một luồng, từng bước** (như unit test cuối `crates/engine/src/journal.rs` lái vòng ghi bằng một `AtomicU8` trần): `pop` thấy rỗng → đẩy một bản ghi → đặt vé `RetiredStopWhenDry` → hỏi hàm quyết định dừng của vòng ghi; bản ghi phải được ghi trước khi dừng. Race thật quá hẹp để test tích hợp bắt được, nên không dựa vào nó | senior developer (opus), cùng agent | Tạo/sửa: `crates/store-sqlite/src/{lib.rs, journal.rs, writer.rs, schema.rs}`. **Không** sửa test của bước 2 (sửa test cho xanh là thất bại), `crates/engine/`, `crates/session/` | `cargo test -p fixbolt-store-sqlite` xanh, chạy 3 lần liền (crash và serve không được chập chờn); `cargo test --all`; `cargo test --no-default-features`; `cargo clippy --all-targets -- -D warnings`; `cargo clippy -p fixbolt-store-sqlite --no-default-features --all-targets -- -D warnings`; `cargo fmt --check`; `scripts/check-indexing-debt.sh`; `scripts/check-no-crate-root-allow.sh`; `scripts/check-lint-config.sh`; `scripts/check-no-optional-deps.sh`; R1–R5 đỏ đúng chỗ rồi xanh lại | 2 |
| 4 | **Case alloc.** `crates/store-sqlite/benches/alloc.rs` (`harness = false`): bộ cấp phát đếm **theo luồng** (cờ `thread_local!` khởi tạo `const`, chỉ luồng đang đo được đếm) — tự kiểm ngay trong bench: cấp phát trên luồng khác trong cửa sổ → 0, trên luồng này → 1. Năm case: `sqlite-put`, `sqlite-mark-in`, `sqlite-mark-out`, `sqlite-mark-active`, `sqlite-retire`; mỗi case có khẳng định "đường có chạy" (dòng đã vào database / `writers_retired()` tăng đúng một). Riêng `sqlite-retire`: chứng minh bằng `writers_retired()` tăng đúng một quanh đúng một `Journal::retire`, **không bao giờ** bằng giá trị trả về của `WriterTicket::retire` — theo ADR-0181 Revision 2 bộ đếm này tăng dù luồng ghi đã `finish` trước hay chưa, nên kiểm tra không phụ thuộc race. In một dòng `allocations: sqlite-put 0 …`. Đảo ngược R6: `std::hint::black_box(Vec::<u8>::with_capacity(1))` trong `put` → `sqlite-put` > 0 | senior developer (opus), cùng agent | Tạo: `crates/store-sqlite/benches/alloc.rs`; sửa: `crates/store-sqlite/Cargo.toml` (`[[bench]]`). Không sửa gì khác | `cargo bench -p fixbolt-store-sqlite --bench alloc` (năm số 0, trích dòng `allocations:`); `scripts/bench.sh` liệt kê và chạy target mới (trích dòng của nó); R6 đỏ rồi xanh | 3 |
