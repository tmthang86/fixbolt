...
`cargo add nanofix`, a web search, or a GitHub search will surface both. The name is
already committed to at `github.com/tmthang86/nanofix`; changing it later costs more the
...
the gap nanofix exists to fill, and also the reason nobody has filled it cheaply.
...
nanofix benchmark produces its own.
