...
if [ "${BASH_VERSINFO[0]:-0}" -lt 4 ]; then
  echo "check-scratch-fixtures: FAIL — needs bash 4 or newer (declare -A); this is bash ${BASH_VERSION:-unknown}. Put a newer bash first on PATH; under macOS's /bin/bash 3.2 this script can report ok having checked nothing." >&2
  exit 2
fi
